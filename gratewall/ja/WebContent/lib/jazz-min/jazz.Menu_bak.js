(function($, factory) {
    ...
    $.widget("jazz.basemenu", $.jazz.boxComponent, {
         ...
         *  尚不了解为什么不能重新设定，所以注掉了该段代码----2015.11.30 @赵永生
         */
//        _width: function() {
//
//        },
        ...
        _bindMenuEvent: function() {
            ...
            el.on('contextmenu.jazz-menu-contextmenu', function(e) {
                ...
                var nw_left = parseInt(nextmenu.css("left").replace("px", ""));
                if(nw_left > 0){
                	//下级菜单在左边
                	...
                	//下级菜单在右边
                	if (x >= Math.floor(lileft + liw -1) || y <= litop || y >= Math.floor(litop + lih - 1)) {
                		nextmenu.hide();
                		$(this).removeClass("jazz-menu-label-active");
                	}
                }
            ...
            el.find(".jazz-menuitem-content").each(function() {
                var id = $(this).attr("id");
                var ispropagation = $(this).attr("ispropagation");
                var clickobj = $(this);
                $.each(itemEvents, function(i, temp) {
                    ...
                    if (auid == id) {
                        clickobj.on('click', function(e) {
                            ...
                            if (ispropagation == "true" || ispropagation == true) {
                                el.hide();
                            }
                        });
                    }
        ...
        _getWidgetMemuDom: function(items, isSubmenu) {
            ...
            if (items && items.length > 0) {
                var item, nextItems, name, vtype, html, aid, subMenuHtml, ispropagation,
                value;
                for (var i = 0; i < items.length; i++) {
                    item = items[i];
                    name = item["name"];
                    ...
                    html = item["html"];
                    htmlid = item["htmlid"];
                    ispropagation = item["ispropagation"];
                    nextItems = item["items"];
                    aid = "menu-uid-" + (++jazz.config.zindex);
                    item["auid"] = aid;

                    if (ispropagation === undefined) {
                        ispropagation = true;
                    }

                    menuHtml.push("<li class=\"jazz-menuitem\" >");
                    if (vtype) {
                        menuHtml.push("<a class=\"jazz-menuitem-content-component\" id='" + aid + "' value='" + value + "'>");
                        ...
                        if (nextItems && $.isArray(nextItems)) {
                            ...
                            menuHtml.push("<a class=\"jazz-menu-label\" id='" + aid + "' ispropagation='" + ispropagation + "' value='" + value + "'>");
                            ...
                            if (html) {
                                ...
                                menuHtml.push("<div id='" + htmlid + "'></div>");
                            } else {
                            ...
                            menuHtml.push("<a class=\"jazz-menuitem-content\" id='" + aid + "' ispropagation='" + ispropagation + "' value='" + value + "'>");
    ...
    $.widget("jazz.menu", $.jazz.basemenu, {
        options: {
            ...
            /**
             *@type boolean
             *@desc menu组件是否根据根据triggertype自动触发显示menu事件
             *@return false
             */
            autotrigger: false,
            /**
        ...
        _init: function() {
            ...
            this._bindTargetEvent();
            this._event("menuCreatedFinish");
        },

        /**
         * @desc 解除target默认触发绑定事件
         * @private
         */
        _cleanTargetEvent: function() {
            var triggertype = this.options.triggertype,
                target = this.options.target;

            //解除绑定事件
            target.off(triggertype + ".jazz-menu");
        },

        /**
         * @desc 绑定target默认触发绑定事件
         * @private
         */
        _bindTargetEvent: function() {
            if (!this.options.autotrigger || !this.options.target) {
                return;
            }
            var triggertype = this.options.triggertype,
                target = this.options.target,
            ...
            //绑定事件
            target.off(triggertype + ".jazz-menu").on(triggertype + ".jazz-menu", function() {
                that.element.show().position({
                    my: that.options.my,
                    at: that.options.at,
                    of: target
                });
        ...
        /**
         * @desc 直接展示menu菜单如果传入position信息则以position信息为准
         * @param {position}位置信息{at:"left top",my:"left bottom",of:$(xxx)}
         * @private
         */
        ...
        setTriggerType: function(triggertype) {
            this._cleanTargetEvent();
            this.options.triggertype = triggertype;
            this._bindTargetEvent();
        },
         ...
         * @private
         */
        setAutoTrigger: function(flag) {
            this.options.autotrigger = flag;
            if (flag) {
                //不用在解除绑定了，因为bind时候有解除操作
                this._bindTargetEvent();
            } else {
                this._cleanTargetEvent();
            }
