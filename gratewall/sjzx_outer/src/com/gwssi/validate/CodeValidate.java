...
public class CodeValidate {
	...
	@RequestMapping(value = "getCodeTwo")
	public void getCodeTwo(HttpServletRequest req, HttpServletResponse response)
			throws IOException {
		// 设置页面不缓存
		response.setHeader("Pragma", "No-cache");
		response.setHeader("Cache-Control", "no-cache");
		...
		// System.out.println(("生成验证码[{}] ,结果为:[{}]",calc,result);
		System.out.println("生成验证码[{"+calc+"}] ,结果为:[{"+ result+"}]");
		session.setAttribute("certCodeTwo", String.valueOf(result));
		// 图象生效
		g.dispose();
		// 输出图象到页面
		ImageIO.write(image, "JPEG", response.getOutputStream());
	 ...
	 * @throws OptimusException 
	 */
	@RequestMapping(value = "checkCodeTwo")
	@ResponseBody
	public String checkCodeTwo(OptimusRequest req, OptimusResponse resp) throws OptimusException {
		String code = req.getParameter("code");
		String sessionCode = (String) req.getHttpRequest().getSession().getAttribute("certCodeTwo");
		
		if(code!=null){
			if(code.equals(sessionCode)){
				//resp.addAttr("msg", "success");
				//resp.addResponseBody("1");
				return "jsonpCallback"+"({\"flag\":\"1\"})";//返回必须是json字符串
			}else{
				return "jsonpCallback"+"({\"flag\":\"0\"})";//返回必须是json字符串
				//resp.addAttr("msg", "fail");
			}
		}else{
			return "jsonpCallback"+"({\"flag\":\"0\"})";//返回必须是json字符串
