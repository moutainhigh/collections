function temp0(data) {
	...
	if ($.isEmptyObject(data)) {
		...
		if(opetype=="GS"){
			...
			_html += "<tr class='success'><td class='title'>认缴注册资本（万元）：</td><td class='titleDesc'>"+biZhong + " " + regcap + "  万元</td></tr>";
		}else if(opetype=="NZFR"){
			_html += "<tr class='success'><td class='title'>注册资金（万元）：</td><td class='titleDesc'>  " + regcap + "  万元</td></tr>";
		}else if(opetype=="HHQY"){
			_html += "<tr class='success'><td class='title'>认缴出资额（万元）：</td><td class='titleDesc'>  " + regcap + "  万元</td></tr>";
		}else if(opetype=="GRDZ"){
			_html += "<tr class='success'><td class='title'>出资额（万元） ：</td><td class='titleDesc'>  " + regcap + "  万元</td></tr>";
		}else if(opetype=="HZS"){
			_html += "<tr class='success'><td class='title'>成员出资总额 ：</td><td class='titleDesc'>  " + regcap + "  万元</td></tr>";
		}else if(opetype=="GT"){
			_html += "<tr class='success'><td class='title'>资金数额（万元）：</td><td class='titleDesc'>  " + regcap + "  万元</td></tr>";
		}else if(opetype=="SLYB"){
		...
		if(opetype=="SLYB"){
			...
			_html += "<tr class='success'><td class='title'>市场主体类型：</td><td class='titleDesc'>" + enttype + "</td></tr>";
		}
		if(opetype=="GT"){
...
function temp4(data, id){
	if ($.isEmptyObject(data)) {
		...
		for(var i=0;i<data.length;i++){
			...
			_html += '<tr class="active"><td class="alingCenter" style="width:20%">'+altdate+'</td><td class="alingLeft" style="width:60%">'+altitems+'</td><td class="alingLeft" style="width:100px"><a style="color:blue;text-decoration:underline" onclick="detail(\''+id+'\',\''+alttime+'\',\''+entName+'\',\''+altdate+'\')">'+'查看打印信息'+'</a></td></tr>';
		}
		$("#panel-5 table").empty().append(_html);
	}
}

function detail(id,alttime,entName,altdate){
	//window.open('biangeng.html?id='+id+"&alttime="+alttime+"&entName="+encode(entName)+"&altdate="+altdate);
	window.open('biangeng.html?id='+id+"&alttime="+alttime+"&entName="+encodeURIComponent(entName)+"&altdate="+altdate);
}
...
function temp7(data){
	...
	if ($.isEmptyObject(data)) {
		...
		for(var i=0;i<data.length;i++){
			...
			if (isNotEmpty(data[0].limittimeend)) {
				limittimeend = data[i].limittimeend.substring(0,10);
			}
			if (isNotEmpty(data[0].limitstatus)) {
				limitstatus = data[i].limitstatus;
			}
			if (isNotEmpty(data[0].Limitsm)) {
				Limitsm = data[i].Limitsm;
			}
			if (isNotEmpty(data[0].memo)) {
				memo = data[i].memo;
			}
			_html += "<tr class='active'><td class='title'>限制原因：</td><td class='titleDesc'>" + limityyvalue + "</td></tr>";
