...
//4.初始化（获取数据）	
	...
	if(nGroupId>0){
		...
		if(currGroup == null){
			throw new WCMException(ExceptionNumber.ERR_OBJ_NOTFOUND, LocaleServer.getString("wcm.auth.group.tree.errorinfo","指定的用户组[")+nGroupId+LocaleServer.getString("wcm.auth.group.tree.errorinfo1","]没有找到！"));
		}
	}
...
if(nOperType==<%=PageViewConstants.TREE_OPERATION_GRPUSER_SEL%>) {
	<%=CMyString.filterForJs(sObjTreeName)%>.addRootItem("<%=LocaleServer.getString("wcm.auth.group.tree.userlist","用户列表")%>" , "0", "<%=LocaleServer.getString("wcm.auth.group.tree.userlist","用户列表")%>", getRootHref(), false);
}
else {
	<%=CMyString.filterForJs(sObjTreeName)%>.addRootItem("<%=LocaleServer.getString("wcm.auth.group.tree.userorglist","用户组列表")%>", "0", "<%=LocaleServer.getString("wcm.auth.group.tree.userorglist","用户组列表")%>", getRootHref(), false);
}
...
<%
aTimer.stop();
if(IS_DEBUG)System.out.println(LocaleServer.getString("wcm.auth.group.tree.gouzaouser", "构造用户[")+loginUser.getName()+LocaleServer.getString("wcm.auth.group.tree.gouzaousertime", "]用户组树所用时间为[") +aTimer.getTime()+LocaleServer.getString("wcm.auth.group.tree.gouzaousermisecond","]毫秒！"));
%>
		...
		for (int i = 0; i < currGroups.size(); i++) {
			...
			if (_isShowAll || AuthServer.hasRight(_loginUser, currGroup)){
				sResult += _sObjTreeName + ".addItem(\"" + CMyString.filterForJs(currGroup.getName())
						+ "\", " + currGroup.getId() + ", \"" + LocaleServer.getString("wcm.auth.group.tree.userzu","用户组[")
