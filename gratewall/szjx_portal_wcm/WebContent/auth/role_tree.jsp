...
var <%=CMyString.filterForJs(sObjTreeName)%>	= new CTRSTree("<%=CMyString.filterForJs(currRequestHelper.getString("RoleIds"))%>",null,null,"<%=CMyString.filterForJs(strDisabledRoles)%>");
<%=CMyString.filterForJs(sObjTreeName)%>.nType	= nRoleTreeType;

<%=CMyString.filterForJs(sObjTreeName)%>.addRootItem("角色列表", "0", "角色列表", getRoleRootHref(), false);				
	<%=toTreeNavJavascript(loginUser, sObjTreeName, isShowAll)%>
...
<%
aTimer.stop();
if(IS_DEBUG)System.out.println("构造用户["+loginUser.getName()+"]角色树所用时间为["+aTimer.getTime()+"]毫秒！");
%>

<%!
	public String toTreeNavJavascript(User _currUser, String _sObjTreeName, boolean _isShowAll)throws WCMException {
        ...
        for (int i = 0; i < currRoles.size(); i++) {
			...
			if(_currUser.isAdministrator() || (aUserService.isManagerOfRole(_currUser) && sCrUserName.equalsIgnoreCase(_currUser.getName())) ){
				// add by hdg@2009-11-18 角色选择页面的各个角色的提示信息
				tip = "描述：" + PageViewUtil.toHtml(CMyString.filterForJs(currRole.getDesc()))
						+ " &#13类型：" + (currRole.getRoleType()<0?(currRole.getRoleType()==Role.TYPE_SYSTEM?"系统级角色":"站点级角色"):"普通角色");
				sResult += _sObjTreeName + ".addItem(\"" + CMyString.filterForJs(CMyString.transDisplay(currRole.getName()))
						+ "\", " + currRole.getId() + ", \""+tip+"\", getRoleNodeHref("
