...
<%
//4.初始化（获取数据）
	String strSourceName= currRequestHelper.getString("SourceName");
	
	WCMFilter filter = new WCMFilter("", "SRCNAME='"+CMyString.filterForSQL(strSourceName)+"'",
	"", "", 1);
	Sources currSources = Sources.openWCMObjs(loginUser, filter);
	Source currSource = null;
	if(!currSources.isEmpty()){
		currSource = (Source)currSources.getAt(0);
	}
	
	String sXML = "";
	if(currSource != null){
		ObjToXmlConverter aXMLConvert = new ObjToXmlConverter();
		sXML = aXMLConvert.toXMLString(currSource, null, "SOURCEID");
	}
//7.结束
	out.clear();
%>
