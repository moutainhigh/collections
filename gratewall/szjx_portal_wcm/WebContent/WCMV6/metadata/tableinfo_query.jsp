...
//2. 获取业务数据
	...
	MetaDBTables objects = (MetaDBTables)request.getAttribute(FrameworkConstants.ATTR_NAME_RESULT);
...
//5. 遍历生成表现
	for (int i = currPager.getFirstItemIndex(); i <= currPager.getLastItemIndex(); i++) {
		try{
			MetaDBTable object = (MetaDBTable)objects.getAt(i - 1);
			if (object == null)
				continue;
			String sObjectId = String.valueOf(object.getId());
			String sObjectName = CMyString.transDisplay(object.getName());
			String sObjectAnother = CMyString.transDisplay(object.getAnotherName());
                 ...
                 <tr>
                    ...
                    <td class="inner_tabular_tex" style="padding-top:3px;padding-left:1px;">
						<div class="ellipsisStyle">
							<span type="text" id="txt_dispname_<%=sObjectId%>" style="font-size:12px;" click_fun="clickDesc"><%=sObjectAnother%></span>
						<%if(rightValue.isHasRight(WCMRightTypes.SITE_EDIT)){%>
							<input click_fun="" id="txt_dispname_<%=sObjectId%>_" type="text" value="<%=sObjectAnother%>" style="display:none;" class="input_class" validation="type:'string',required:'',max_len:'60',desc:'元数据别名'" name="AnotherName">
						<%}%>
						</div>
                    </td>
                </tr>
...
<%!
    ...
    private RightValue getRightValue(MethodContext context, User loginUser, BaseObj _object) throws ConvertException {
		MetaDBTable object = (MetaDBTable) _object;
