...
/**
 * @class wcm.util.Observable Abstract base class that provides a common
 ...
 * 	});
 * }
 * Ext.extend(Employee, wcm.util.Observable);
 * </code></pre>
 */
Ext.ns("wcm.util");
...
wcm.util.Observable.prototype = {
	 ...
	 *            <ul>
	 *            <li>scope {Object} The scope in which to execute the handler
	 *            function. The handler function's "this" context.</li>
	 *            <li>delay {Number} The number of milliseconds to delay the
	 *            invocation of the handler after the event fires.</li>
	 *            <li>single {Boolean} True to add a handler to handle just the
	 *            next firing of the event, and then remove itself.</li>
	 *            <li>buffer {Number} Causes the handler to be scheduled to run
	 *            in an {@link wcm.util.DelayedTask} delayed by the specified
	 ...
	 * The method also allows for a single argument to be passed which is a
	 * config object containing properties which specify multiple handlers.
	 * <p>
	 * 
	 * <pre><code>
	 * 	foo.on({
