  ...
  */
   ...
   */
	setForm:function(formId, isUpload, secureUri)
	{
		...
		if(isUpload){
			this.createFrame(secureUri?secureUri:"about:blank");
   ...
   */
	uploadFile:function(id, callback, uri){
		...
		try{
			...
			(this.uploadSubmitError||Prototype.emptyFunction)(err);
			if(window.YUI_ThrowError)
				throw err;
			return;
		}
		var formNode = this._formNode;
...
//Extend
YUIConnect.uploadSubmitError = function(err){
	if(window.$alert&&window.$alert!=window.alert)
		$alert(err.message+'<br>原因：<b>文件路径不存在或者非本地文件路径。</b>');
	else{
		alert(err.message+'\n原因：文件路径不存在或者非本地文件路径。');
	}
