...
var FCKPanel = function( parentWindow ){
	...
	if ( _IE ){
		...
		oDocument.close() ;
		Event.observe(oIFrameWindow, 'focus',FCKPanel_Window_OnFocus.bindAsEventListener(null,this));
		Event.observe(oIFrameWindow, 'blur',FCKPanel_Window_OnBlur.bindAsEventListener(null,this));
	}

	oDocument.dir = 'ltr' ;
	
	oDocument.oncontextmenu = function(event){Event.stop(event);} ;
...
FCKPanel.prototype.AppendStyleSheet = function( cssFileUrlOrArray ){
	if ( typeof( cssFileUrlOrArray ) == 'string' )
		return FCKTools._AppendStyleSheet( this.Document, cssFileUrlOrArray ) ;
	else{
		for ( var i = 0 ; i < cssFileUrlOrArray.length ; i++ )
			FCKTools._AppendStyleSheet( this.Document, cssFileUrlOrArray[i] ) ;
	}
}
