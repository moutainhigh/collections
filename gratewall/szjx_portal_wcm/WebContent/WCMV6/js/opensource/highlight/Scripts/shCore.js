...
//				span.innerHTML = lines[i];
				sHtml += '<span class="'+css+'">'+lines[i]+'</span>';
//				this.div.appendChild(span);
				
				// don't add a <BR> for the last line
				if(i + 1 < lines.length){sHtml += '<br>';}
...
dp.sh.Highlighter.prototype.IsInside = function(match,_idx)
{
	if(match == null || match.length == 0)
		return false;
	return this.isInSideIndex[i];
...
dp.sh.Highlighter.prototype.SwitchToList = function(_sHtml)
{
	// thanks to Lachlan Donald from SitePoint.com for this <br/> tag fix.
	var html = _sHtml.replace(/<(br)\/?>/gi, '\n');
...
//		columns.appendChild(div);
		sHtml = '<div class=columns>'+sHtml+'</div>';
//		this.bar.appendChild(columns);
		this.bar.innerHTML = sHtml;
		sHtml = '';
	}
...
//		this.ol.appendChild(li);
		sHtml += '<li class='+((i % 2 == 0) ? 'alt' : '')+'><span>'+lines[i] + '&nbsp;</span>';
	}
	this.ol.innerHTML = sHtml;
//	this.div.innerHTML	= '';
}
...
dp.sh.Highlighter.prototype.Highlight = function(code)
{
	...
	{
		var sHtml = this.AddBit(this.code, null);
		this.SwitchToList();
	...
	for(var i = 0; i < this.matches.length; i++){
		var c = this.matches[i];
		if(c!=null){
//			this.isInSideIndex[i] = c.index>lastMax;
//			if((c.index+c.length)>lastMax){
//				lastMax = (c.index+c.length);
//			}
			if(c.index<lastMax){
				this.matches[i] = null;
			}
...
//	for(var i = 0; i < this.matches.length; i++)
//		if(this.IsInside(this.matches[i],i))
//			this.matches[i] = null;
	...
	for(var i = 0; i < this.matches.length; i++)
	{
		...
		sHtml += this.AddBit(Copy(this.code, pos, match.index), null);
		sHtml += this.AddBit(match.value, match.css);

		pos = match.index + match.length;
	}
	
	sHtml += this.AddBit(this.code.substr(pos), null);

	this.SwitchToList(sHtml);
//	this.div.innerHTML = sHtml;
