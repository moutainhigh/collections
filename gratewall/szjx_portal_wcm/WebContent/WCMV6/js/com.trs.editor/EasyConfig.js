...
$package("com.trs.editor.ImgUploader");
...
$package("com.trs.editor.ArticleSubmiter");
Object.extend(com.trs.editor.ArticleSubmiter,{
	...
	params:{
		article:'contentHtml',//需要与页面对应下面三个字段名
	...
	toolbar:function(){
		...
		for(var i in this.toolbarComponents)
		{
			if(first){
				...
				div.appendChild(sep.toElement());
			}
			var cmp=this.toolbarComponents[i];
			if(cmp&&cmp!='')
			{
				div.appendChild(this.components[cmp].toElement());
			}
		}
		return div;
	}
