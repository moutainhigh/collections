...
// $Id: CTRSCalendar_Obj.js,v 1.15 2007/07/31 11:23:36 huxiejin Exp $
...
Calendar.prototype.create = function (_par) {
	...
	table.width = 300;
	table.calendar = this;
	Calendar.addEvent(table, "mousedown", myActualTop.Calendar.tableMouseDown || Calendar.tableMouseDown);
	...
	if (this.showsTime) {
		...
		(function(){
			...
			function getNumberInput(){
				var event = myActualTop.event || window.event;
	...
	this.element.nextSibling.style.zIndex = 998;
	return this.element;
};
...
Calendar._blurEvent = function(ev){
	alert(Position.within(this,Event.pointerX(event),Event.pointerY(event)));
...
Calendar.prototype.show = function () {
	...
	if (this.isPopup) {
		...
		//Calendar.addEvent(myActualTop.document, "keydown", Calendar._keyEvent);
		//Calendar.addEvent(myActualTop.document, "keypress", Calendar._keyEvent);
		Calendar.addEvent(myActualTop.document, "mousedown", Calendar._checkCalendar);
		if(this.element.setCapture){
			//don't know the reason, if no setTimeout, the setCapture will no use.
			setTimeout(function(){
		...
		//Calendar.addEvent(this.element, "blur", Calendar._blurEvent.bind(this.element));
	}
	this.hideShowCovered();
};
...
Calendar.prototype.hide = function () {
	...
	if (this.isPopup) {
		//Calendar.removeEvent(myActualTop.document, "keydown", Calendar._keyEvent);
		//Calendar.removeEvent(myActualTop.document, "keypress", Calendar._keyEvent);
...
Calendar.prototype.showAtElement = function (el, opts) {
	//var position = Position.getAbsolutePositionInTop(el);alert(position);
	var position = Position.getPageInTop(el);
	var p = {x:position[0], y:position[1]};
	var displayType = Calendar.displayType || "rb";
	...
	var offsetParent = Position.offsetParent(divObj);
	if(p.y + h > offsetParent.style.top + offsetParent.offsetHeight){
		p.y -= h;//简单的定位
	}
	this.showAt(p.x, p.y);
};
