...
var ViewFieldInfoMgr = {
	serviceName : 'wcm6_MetaDataDef',
	deleteMethodName : 'deleteViewField',
	...
	mappingDataType : {
		...
		'relation' : {containerType : 'locateChannelContainer', dbType : 'char'},
	...
	belongToSelf : function(){
		...
		if(!mainWin || !mainWin.PageContext 
				|| mainWin.PageContext.ObjectType != "ViewFieldInfo"){
	...
	"delete" : function(_sObjectIds, _oPageParams){
		...
		if (confirm('确实要将这' + sHint + '个视图字段删除吗? ')){
	...
	changeViewFieldOrder : function(oPostData, fCallBack){
		this.getHelper().call(this.serviceName, 'changeViewFieldOrder', 
	...
	_doAddOrEdit : function(_nObjectId, _parameters){
		...
		FloatPanel.open('./metadata/viewfieldinfo_add_edit.jsp?' + urlParams, '新建/修改视图字段', 570, 380);
	}
};
