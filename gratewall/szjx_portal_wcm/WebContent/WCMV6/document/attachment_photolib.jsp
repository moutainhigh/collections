...
//				eDiv.style.backgroundImage = 'url('+getWebPicPath(sPhotoSrcs.split(',')[0])+')';
		...
		function SetPhotoSelected(_nPhotoId,sPhotoSrcs,_bChecked,_nPhotoDocId, _sPhotoDesc){
		...
		function Ok(){
			...
			if(true || ($('allReplace') && $('allReplace').checked)){
				var oWindow = window.dialogArguments.win;
...
//					oImage.src = sPhotoSrc;
					...
					tmpObj['src'] = arrPhotoSrcs[eTmpScale];
					tmpObj['url'] = getWebPicPath(arrPhotoSrcs[eTmpScale]);
					result.push(tmpObj);
				}
...
%>
				...
				<td width="130" valign="top" id="otherContainer">
					...
					<input name="button2" class="input_btn" style="margin-left:20px;width:80px" type="button" value="取消" onclick="window.close();" fcklang="DlgBtnCancel">
					...
					<div style="height:6px;overflow:hidden"></div>
					<span class="message">选中的图片:</span><br>
