...
//6.业务代码
	// 检查频道自定义编辑页面
	if(officeInfo==null && docChannel != null && !currRequestHelper.getBoolean("NoRedirect", false)){
...
<script>
	...
	if(!checkPage404('<%=CMyString.filterForJs(sTarget)%>')){
		(top.actualTop||top).location.href = '<%=CMyString.filterForJs(sTarget)%>';
	}
	else{
		(top.actualTop||top).location.href = 'document_addedit_404.html';
	}
	*/
	//去掉404的判断，提高性能，避免2次请求
	(top.actualTop||top).location.href = '<%=CMyString.filterForJs(sTarget)%>';
</script>
<%
			//currRequestHelper.sendRedirect(sContentAddEditPage);
			return;
		}
	}
	String sTitle = "";//文档标题
...
//		sChannelName = (docChannel!=null)?docChannel.getName():"";
		...
		sDocSourceName = CMyString.showNull(currDocument.getPropertyAsString("DocSourceName"));
		//获得当前文档的作者
		...
		//文档被引用到的频道
        DocumentMgr currDocumentMgr = (DocumentMgr) DreamFactory
                .createObjectById("DocumentMgr");
        quoteChannels = currDocumentMgr.getQutoedChannels(currDocument,ChnlDoc.MODAL_LINK, null);
...
<%!
	...
	private String getAllDocSourceJs(User _user,int _nSiteId) throws WCMException {
		...
		for(int i=0; i<nSize; i++){
			...
			sb.append(nSourceId);
			sb.append(",'label':\"");
			sb.append(CMyString.filterForJs(currSource.getDesc()));
			sb.append("\"}");
		}
		sb.append("]");
		currSources.clear();
	...
	private String showDocSource(User _user, int _nCurrSourceId,int _nSiteId) throws WCMException {
		String sSelectFields = "SOURCEID,SRCNAME,SRCDESC";
		WCMFilter filter = new WCMFilter("", "", "SourceOrder desc", sSelectFields);
		if(_nSiteId > 0){
			filter.setWhere("SiteId="+_nSiteId);
		}
		...
		Security currSecurity = null;
		StringBuffer sb = new StringBuffer();
		sb.append("<SELECT name=\"DocSource\" id=\"DocSource\" STYLE=\"WIDTH:150px\" pattern=\"string\" elname=\"来源\" ignore='1' onchange=\"DocSourceHelper.NotifyInput();\" onclick=\"DocSourceHelper.NotifySelect();\"" );
		sb.append(" value=\"");
		sb.append(_nCurrSourceId);
		sb.append("\">");
		sb.append("<option sourceid='0' value='' sourceDesc='--请选择--'>--请选择--</option>");
		for(int i=0; i<nSize; i++){
			currSource = (Source) currSources.getAt(i);
			int nSourceId = currSource.getId();
			if( currSource == null ){
			...
			currSecurity = currSource.getSecurity();
			sb.append("<option sourceid=\"");
			sb.append(nSourceId);
			sb.append("\" value=\"");
			sb.append(CMyString.filterForHTMLValue(currSource.getName()));
			sb.append("\" security=\"");
			sb.append(currSecurity == null?"":String.valueOf(currSecurity.getId()));
			sb.append("\"");
			if(nSourceId == _nCurrSourceId)	sb.append(" selected ");
			sb.append(" sourceDesc='").append(CMyString.filterForHTMLValue(currSource.getDesc())).append("'");
			sb.append(">");
	...
	private String getQuoteChannelIds(Channels quoteChannels){
		// wenyh@2008-05-09 10:05 修正获取Id序列的办法,原有逻辑中,tempQuteChannel可能为null.
		// why not use baseobjs.getIdsAs....?
		if(true) return getQuoteChannelIds0(quoteChannels);
	...
	private String getQuoteChannelIds0(Channels quoteChannels){		
		if(quoteChannels == null || quoteChannels.size() == 0) return "";
		StringBuffer sQuoteChannelIds = new StringBuffer(32);
		Channel tmpQuoteChannel = null;
		for(int i=0,size=quoteChannels.size();i<size;i++){			
			tmpQuoteChannel = (Channel)quoteChannels.getAt(i);
			if(tmpQuoteChannel != null){
				sQuoteChannelIds.append(',');
				sQuoteChannelIds.append(tmpQuoteChannel.getId());
			}
		}
		if(sQuoteChannelIds.length() > 1){
			return sQuoteChannelIds.substring(1);
		}
		return "";
	}
	...
	private String showQuoteChannels(Channels quoteChannels) throws Throwable{
		...
		if(quoteChannels != null){
			...
			for(int i=0; i<nQuoteChnlSize; i++){	
				...
				sRetVal += "</TD>";
				sRetVal += "<TD class='attr_quotechanel_column' title=\"" + getPath(quoteChannel)+ "\">";
	...
	private String getPath(Channel oChannel) throws Throwable{
		StringBuffer sbPath = new StringBuffer();
		Channel tempChnl = oChannel;
		while(tempChnl != null){
			sbPath.insert(0, "&gt;" + tempChnl.getDesc());
			tempChnl = tempChnl.getParent();
		}
		if(oChannel != null){
			sbPath.insert(0, oChannel.getSite().getDesc());
		}
