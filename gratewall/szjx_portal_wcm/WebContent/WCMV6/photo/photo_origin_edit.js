...
function reinitValid4PercentScale(width,height){
	...
	var temp = [minRatioW,maxRatioW,"desc:'宽度缩放比例'"];
	...
	var temp = [minRatioH,maxRatioH,"desc:'高度缩放比例'"];
...
function setOriginSize(){
	...
	var containerHeight = Element.getDimensions(document.body).height-100;
	var imgEl = $("imageElement");	
...
Object.extend(PageContext,{	
	...
	loadOriginImg : function(_photoId){
		BasicDataHelper.call("wcm6_photo","loadOriginImg",{PhotoId:_photoId || this.params.PhotoId},false,function(_transport,_json){			
			PageContext.loadWatermarks($v(_json,"libid")||0);	
		...
		BasicDataHelper.call("wcm6_watermark","query",{LibId:_libId},false,function(_transport,_json){
			...
			Event.observe($("selwatermark"),"change",function(){
				...
				if(op.value == -1){
					...
					Object.extend(PageContext.params,{WaterMarkFile:op._picfile});
				}
			});
		});
	},
...
function applyAction(){	
	...
	ProcessBar.init('正在处理，请稍候...');	
	ProcessBar.addState('正在处理，请稍候...');
	ProcessBar.start();	
...
function onOk(){
	...
	var postData = Object.extend(PageContext.params,{WaterMarkPos:wmpos.join(","),FileName:fn});
