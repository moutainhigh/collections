...
FCKCommentCommand.prototype.Execute = function(){
	switch(this.Name){
		case 'Comment':
			...
			var sValue = sDefault = '请在此输入批注',sBgColor = FCKCommentCommandBgColors[1];
			...
			var MSG_INCLUDE_SEL_TEXT = '是否在注释中包含指定文本？';
...
FCKComments.AddIntoDoc = function(addToDoc) {
	try {
		...
		var ask = addToDoc ? "是否将此批注加入到文档中？" : "删除批注";
		if (!confirm(ask + " " + "一旦点击\"确定\"就无法恢复")) {
			return;
		}
		var textToAdd = null;
		if (addToDoc) {
