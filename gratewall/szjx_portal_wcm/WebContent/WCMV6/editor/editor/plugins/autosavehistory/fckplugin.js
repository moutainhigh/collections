...
var sMyBasePath = myActualTop.BasePath;
if(sMyBasePath == null){
	sMyBasePath = "/wcm/WCMV6/document/";
}
FCKCommands.RegisterCommand( 'AutoSaveHistory', 
new FCKDialogCommand( 'AutoSaveHistory', FCKLang.AutoSaveHistoryDlgTitle,
sMyBasePath+'fck_autosave_history.html', 500, 320 ) ) ;
	...
	FCK.AttachSaveUserData = function(){
		...
		if(oCookies["LastFailure"]!=null&&oCookies["LastFailure"]!='false'){//保存失败
			try{
				...
				if(!(oData["Title"]==null&&oData["Content"]==null)&&confirm('您最近的一篇文章尚未发表成功，是否确认恢复？')){
		...
		if(parent.enableAutoSave || oTop.enableAutoSave){
...
//						}
						...
						if(bContentChanged){
							...
							try{
								...
								if(parent.showAutoSaveMessage){
									parent.showAutoSaveMessage();
								}else if(oTop.showAutoSaveMessage){
		...
		oTop.whenSaveFailure = function(){
			...
			if(!FCK.isBlankContent(oData["Content"])){
				...
				try{
					FCK.saveUserData(oEditFrame,oData,nCurrVersion);
					alert('文档提交失败，系统已自动备份当前版本，重新刷新页面可恢复该版本。');
				}catch(err){}
			}
		}
	}
