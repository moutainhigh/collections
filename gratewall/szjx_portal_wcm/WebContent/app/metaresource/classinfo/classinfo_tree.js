...
com.trs.tree.TreeNav.doActionOnClickA = function(_event, _elElementA){
	...
	refreshParent(_elElementA);
	return false;
}

function refreshParent(aElement){
	var divEl = aElement.parentNode;
	var nClassInfoId = divEl.getAttribute("objectId");
	if(divEl.getAttribute("isRoot")){
		nClassInfoId = "";
	}
	...
	Ext.apply(params, {CLASSINFOID : nClassInfoId});
	delete params[''];

	search = $toQueryStr(params);

	var sUrl = parent.location.href;
	var result = sUrl.match(/^([^\?]+)(\?|$)/);
	parent.location.href = result[1] + "?" + search;
}
...
Event.observe(window, 'load', function(){
	$("cls_0").fireEvent('onclick');
	
	/*var objType = getParameter('objType')||"cls";
	var objId = getParameter('objId')||5785;
	if(objType && objId){
	...
	//选中分类法节点，需要在click的回调里面做，TODO...
	setTimeout(function(){
		var nClassInfoId = getParameter('CLASSINFOID', parent.location.search);
		focusNode(nClassInfoId);
	},500);
...
com.trs.tree.TreeNav.makeGetChildrenHTMLAction = function(divElement){
	var urlPrefix = '/'+(location.pathname.split('/')[1]||'wcm')+'/center.do?serviceid=wcm61_classinfo&methodname=createClassInfoTreeHTML'; 
	var params = "&parentId=" + divElement.getAttribute("objectId");
	return urlPrefix + params;
}		
com.trs.tree.TreeNav.doActionOnClickA = function(event, aElement){
	var divEl = aElement.parentNode;
	var nClassInfoId = divEl.getAttribute("objectId");
	if(divEl.getAttribute("isRoot")){
		nClassInfoId = "";
...
function focusNode(nObjectId){
	//先找到该节点的root节点，对该节点的root节点进行更新
	//TODO...
	var root = $("cls_"+m_currRootId);
	var path = findPath(nObjectId);
	if(!path){
		com.trs.tree.TreeNav.focus($('cls_0'));
		return;
	}
	var oTopChildElement = com.trs.tree.TreeNav._getTopChildElement(root);
	if(oTopChildElement == null){
		com.trs.tree.TreeNav.focus($('cls_'+nObjectId));
		return false;
	}
	com.trs.tree.TreeNav.updateNodeChildrenHTML(
		root,
		'/wcm/center.do?serviceid=wcm61_classinfo&methodname=createClassInfoTreeHTMLOfIds&objectids='+path,
		function(){
			com.trs.tree.TreeNav.focus($('cls_'+nObjectId));
			try{
				//parent.document.getElementById('sepBar').click();
			}catch(error){
			}
		}
	);
...
function findPath(nObjectId){
	var sAction = '/wcm/center.do?serviceid=wcm61_classinfo&methodname=getpath&objectid='+nObjectId;
	var request = new Ajax.Request(
		sAction,
		{method: 'get', parameters: "", asynchronous: false}
	);
	if(request.responseIsFailure() || request.header('TRSException')){
		return null;
	}
