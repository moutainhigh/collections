...
if(nErrNo < 200000)if(nErrNo != 11111) sErrMsg = LocaleServer.getString("error.label.systemerror", "系统出现错误。");
...
sErrDetail = "true".equalsIgnoreCase(sShowDetailInfo) ? sErrDetail : LocaleServer.getString("error_for_dialog.jsp.syserror", "系统出现错误");
sErrMsg = "true".equalsIgnoreCase(sShowDetailInfo) ? sErrMsg : LocaleServer.getString("error_for_dialog.jsp.syserror", "系统出现错误");
out.clear();
%><%
...
<!--
	if(parent.parent.FloatPanel) Ext.Msg.warn('<%=CMyString.filterForJs(sErrMsg)%>');
	else if(parent.FloatPanel)Ext.Msg.warn('<%=CMyString.filterForJs(sErrMsg)%>');
//-->
</script>
<%
	}
