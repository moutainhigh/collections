...
<%@include file="../metaviewdata/element_processor.jsp"%>
<%
	Object result = request.getAttribute(FrameworkConstants.ATTR_NAME_RESULT);
	if(!(result instanceof MetaViewDatas)){
		throw new WCMException(CMyString.format(LocaleServer.getString("tmetarecdata_query.jsp.service", "服务(com.trs.components.metadata.service.MetaDataCenterServiceProvider.query)返回的对象集合类型不为MetaViewDatas，而为[{0}]，请确认。!"), new Object[]{result.getClass()}));
	}
...
<%
	...
	for(int i = 0;i < oMetaViewFields.size(); i++ ){
		...
		if(oMetaViewField.isSearchField()){
			...
			if(nDBType != 93 && nDBType != 2005 && nFieldType != 8 && nFieldType != 14){
				...
				String sSearchFieldName = oMetaViewField.getTableName()+"."+oMetaViewField.getDBName();
				sSearchFieldInfo.append("name:'").append(CMyString.filterForJs(sSearchFieldName)).append("',")
		...
		String sTitleFieldCls = oMetaViewField.isTitleField() ? "class='titleFieldHeader'" : "class='normalFieldHeader'";
	...
	%>
	<td <%=sTitleFieldCls%>><%=sAnotherName%></td>

	<%		}else{
	
	%>
		<td <%=sTitleFieldCls%> grid_sortby="<%=sTrueTableName%>.<%=sFieldName%>"><%=sAnotherName%><%=getOrderFlag(sTrueTableName + "." + sFieldName, sCurrOrderBy)%>
		</td>
...
%>
	<tr id="tr_<%=nChnlDocId%>" rowid="<%=nChnlDocId%>" recid="<%=nChnlDocId%>" class="grid_row  <%=sRowClassName%> <%=(bIsSelected)?" grid_row_active":""%>" isTopped="<%=bIsTop%>" right="<%=sRightValue%>" currchnlid="<%=nChnlID%>" docid="<%=nDocId%>">
...
<%
	...
	tx.printStackTrace();
	throw new WCMException(LocaleServer.getString("metarecdata_query.jsp.label.runtimeexception", "metarecdata_query.jsp运行期异常!"), tx);
}
%>
<%!
