...
String[] pStatSQL = new String[] { 
	"select COUNT(*) DataCount, AccountId"
	+ " from XWCMACCOUNTMICROCONTENT"
	+ " where (ISRETWEETED is null or ISRETWEETED = 0) and"
	+ " CrTime >=${StartTime}"
	+ " and CrTime <=${EndTime}"
	+ " group by ACCOUNTID" ,
	"select COUNT(*) AllCount, AccountId"
	+ " from XWCMACCOUNTMICROCONTENT"
	+ " where"
	+ " CrTime >=${StartTime}"
	+ " and CrTime <=${EndTime}"
...
for (int i = 0; i < nNum; i++) {
	...
	List<Float> oFloatList = new ArrayList<Float>();
	...
	for(int j=0;j<list.size();j++){
		...
		oFloatList.add(j,new Float(nOneDate));
	}
	String sAccountDesc = getAccountDesc(Integer.parseInt(sKey));
	String sTempData =  getChartData(sAccountDesc, oFloatList.toString());
	if(!CMyString.isEmpty(sAccountDesc)){
		oAccountList.add(sAccountDesc);
		oAccountDataMap.put(sAccountDesc, oFloatList);
	}
...
if(sPlatform.equals("all")){
	sTitle = nStatYear+ "年不同平台各帐号微博原创率";
}else{
	String sPlatChineseName = PlatformFactory.getPlatform(sPlatform).getChineseName();
	sTitle = nStatYear+ "年"+sPlatChineseName+"平台各帐号微博原创率";
}
%>