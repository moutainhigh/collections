...
<head>
	...
	<script>
		...
		function changeBackground(that){
			$(that).parent().css("background","#2291FF");
		}
		function clearBackground(id){
			$("#"+id).find("li").css("background","#fff");
		}
	</script>
</head>
...
<body>
		<div class="statSelectContainer marginLR18Px marginTop10Px">
			...
			<div class="first">
			<ul id="platSelect">
				<li <%if(sPlatform.equals("all")){%> class="selected"<%}%>><a href="<%=CMyString.filterForHTMLValue(sBaseUrl)%>/mc_count_stat.jsp?StatYear=<%=nStatYear%>&SCMGroupId=<%=nSCMGroupId%>&StatPlatform=all">全部</a></li>
				<%
					// 获取系统中的平台信息 TODO
				...
				%>
					<li <%if(sPlatform.equals(sName)){%> class="selected"<%}%>><a href="<%=CMyString.filterForHTMLValue(sBaseUrl)%>/mc_count_stat.jsp?StatYear=<%=nStatYear%>&SCMGroupId=<%=nSCMGroupId%>&StatPlatform= <%=CMyString.filterForHTMLValue(sName)%>"><%=CMyString.transDisplay(sChineseName)%></a></li>
				<%}%>
			</ul>
		...
		<div class="statSelectContainer marginLR18Px">
			...
			<div class="first">
			<ul id="groupSelect">
				<li <%if(nSCMGroupId == -1){%> class="selected"<%}%>><a href="<%=CMyString.filterForHTMLValue(sBaseUrl)%>/mc_count_stat.jsp?StatYear=<%=nStatYear%>&SCMGroupId=-1&StatPlatform=<%=CMyString.filterForHTMLValue(sPlatform)%>">全部</a></li>
				<%
				for(int i=0;i<oGroups.size();i++){
					SCMGroup oSCMGroup =(SCMGroup) oGroups.getAt(i);
					int nTempSCMGroupId = oSCMGroup.getId();
				%>
					<li <%if(oSCMGroup.getId() == nSCMGroupId){%> class="selected"<%}%>><a href="<%=CMyString.filterForHTMLValue(sBaseUrl)%>/mc_count_stat.jsp?StatYear=<%=nStatYear%>&SCMGroupId=<%=nTempSCMGroupId%>&StatPlatform=<%=CMyString.filterForHTMLValue(sPlatform)%>"><%=CMyString.transDisplay(oSCMGroup.getGroupName())%></a></li>
				<%
				}
				%>
			</ul>
		...
		<div class="statSelectContainer marginLR18Px">
			...
			<ul id="timeSelect">
				<%for(int i=nFirstYear;i<=now.getYear();i++){%>
					<li <%if(nStatYear == i){%> class="selected"<%}%>><a href="<%=CMyString.filterForHTMLValue(sBaseUrl)%>/mc_count_stat.jsp?StatYear=<%=i%>&SCMGroupId=<%=nSCMGroupId%>&StatPlatform=<%=CMyString.filterForHTMLValue(sPlatform)%>"><%=i%></a></li>
				<%}%>
			</ul>
			<div class="clearFloat"></div>
			</div>
		...
		<div class="statTableContainter">
			<div class="tableTitle">
				<div class="tableTitleFistDiv">
					...
					<div class="export_excel" onclick="exportExcel(<%=nStatYear%>,<%=nSCMGroupId%>,'<%=sPlatform%>','export_account_mc_count_stat.jsp')">Excel导出</div>
