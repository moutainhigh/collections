...
<%
	...
	if(!(result instanceof VideoDocs)){
		throw new WCMException( CMyString.format(
				LocaleServer.getString("video_thumb_query.jsp.confirm","服务(com.trs.ajaxservice.PhotoServiceProvider.query)返回的对象集合类型不为VideoDocs，而为[{0}]，请确认。"),new Object[] {result.getClass()}));
	}
	int nCurrDocId = 0;
...
//5. 遍历生成表现
	...
	for (int i = currPager.getFirstItemIndex(); i <= currPager.getLastItemIndex(); i++) {
		try{
			...
			//------------------------------------------------------
			boolean bCanDetail = hasRightToPlayVideo(loginUser, videoDoc, 34);
			...
			if(chnldoc == null){
				throw new WCMException(ExceptionNumber.ERR_PARAM_INVALID, CMyString.format(
						LocaleServer.getString("video_thumb_query.jsp.noID","没有找到ID为[{0}]的chnldoc"),new int[] {nRowId}));
			}
			int nModal = chnldoc.getPropertyAsInt("MODAL",0);;
			int nStatusId = videoDoc.getStatusId();
			String nStatusName = LocaleServer.getString("video_thumb_query.jsp","未知");
...
%>
    <div class="thumb_item<%=(bIsSelected)?" thumb_item_active":""%>" id="thumb_item_<%=nRowId%>" itemId="<%=nRowId%>" right="<%=sRightValue%>" docId="<%=nDocId%>" channelid="<%=nDocChannelId%>" currchnlid="<%=nChnlId%>" convertFlag= <%=convertFlag==-1 ? '0' : '1'%> converting= <%=convertFlag%> isTopped= <%=bTopped%>>
   		<div class="thumb" id="thumb_<%=nRowId%>">
			<img id="img_src<%=nRowId%>" style="cursor:pointer;position:absolute;" src="<%=adapatedImgSrc(convertFlag, thumbUrl)%>" title="视频<%=nDocId%>&#13;<%=nStatusName%>" WCMAnt:paramattr="title:video_thumb_query.jsp.video" onload="resizeIfNeed(this);" class="<%=(converting)?"imgConverting ":""%>wcm_pointer" rowid="<%=nRowId%>" docid="<%=nDocId%>" title="<%=sTitle%>" videoToken="<%=_srcFileName%>"  onclick="<%=getImgClickScript(bCanDetail, converting, convertFail, tokens.toString())%>" >		
		</div> 
        <div class="attrs" id="thumb_attrs_<%=nRowId%>">
            <input id="cbx_<%=nRowId%>" type="checkbox"<%=(bIsSelected)?" checked":""%> <%=(convertFlag==-1)?"disabled='true'":""%> />
			<%
				if(nModal == 2){
			%>
...
%>
<input id="thumbUrl" type="hidden" value="<%= thumbRoot%>"/>
<script>
	try{
		myThumbList.init({
...
<%
	...
	tx.printStackTrace();
	throw new WCMException(LocaleServer.getString("video_thumb_query.jsp.runError","watermark_query.jsp运行期异常!"), tx);
}
%>
...
<%!
	...
	private String playlink(String tokens) {
		return "javascript:newWindow('./player.jsp?v=" + tokens + "', '', 640, 460)";
	}
	...
	private String statusColor(String status, String title) {
		if (status == LocaleServer.getString("video_thumb_query.jsp.yifa","已发")) {
			return "<font color='green'>" + title + "</font>";
		} else {
			return title;
		}
	}
	private boolean hasRightToPlayVideo(User _currUser, CMSObj _objCurrent,int _nRightIndex) throws WCMException{
