...
<%
	...
	if (docId >= 0) {
		...
		if (doc != null) {
			videoStrings = doc.getAttributeValue("TOKEN");
		}
	} else {
...
<title WCMAnt:param="smallPlayer.jsp.playvideo">播放视频</title>
...
<div id="dvUpd" style="margin-top:5px;width:auto" align="center">
	...
	<div id="flashcontent" WCMAnt:param="smallPlayer.jsp.notDisplay">您的浏览器和Flash环境异常, 导致该内容无法显示!<a href="./flashversion.html"><font color="red">请尝试点击这里观看或报告环境信息</font></a></div>
	<noscript WCMAnt:param="smallPlayer.jsp.noScript">您使用的浏览器不支持或没有启用javascript, 请启用javascript后再访问!</noscript>
...
if (videos.length > 1) {
%>
	<input type="radio" name="playQuality" value="高清效果" WCMAnt:paramattr="value:smallplayer.jsp.hq" onclick="changePlayURL('<%=CMyString.filterForHTMLValue(videoHq) %>')" <%if(isHighQualityIp){%>checked<%}%>><span WCMAnt:param="smallplayer.jsp.hq" >高清效果</span>
	<input type="radio" name="playQuality" value="普通效果" WCMAnt:paramattr="value:smallplayer.jsp.lq" onclick="changePlayURL('<%=CMyString.filterForHTMLValue(videoPt) %>')" <%if(!isHighQualityIp){%>checked<%}%>><span WCMAnt:param="smallplayer.jsp.lq" >普通效果</span>
<%
}
%>
