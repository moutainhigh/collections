...
Ext.apply(PageContext, {
	...
	filterEnable : false,
	gridDraggable : false,
	serviceId : 'wcm61_channelsyn',
	methodName : 'jQuery',
	initParams : {
		"FieldsToHTML" : "",
		"SelectFields" : "",
		"CHANNELASTARGET" : false
	}
});
...
PageContext.setRelateType(
	!!PageContext.getParameter("ChannelId") ? "channelsynInChannel" :
		(!!PageContext.getParameter("SiteId") ? "channelsynInSite" : "channelsynInRoot")
);
...
PageContext.addGridFunctions({
	'edit' : function(event){//单击列表上修改图标时执行的操作
		var myMgr = wcm.domain.ChannelSynMgr;
		myMgr['edit'](event);
	},
...
/*-------------页面上下文环境信息--------------*/
Ext.apply(PageContext, {
	objectType : WCMConstants.OBJ_TYPE_CHANNELSYN
});
...
function convertStatus(){
	location.href = WCMConstants.WCM6_PATH + 'channelsyn/channelsyncol_classic_list.html' + location.search ;
	//alert(Ext.toSource(location.search));
	return;
}
ClassicList.cfg = {
	toolbar : [
		{
			...
			fn : function(event, elToolbar){
				wcm.domain.ChannelSynMgr.add(event);
			},
			name : wcm.LANG.CHANNELSYN_VALID_23 ||'新建',
			rightIndex : 13,
			isHost	: true
		}, {
			id : 'channelsyn_delete',
			fn : function(event, elToolbar){
				wcm.domain.ChannelSynMgr['delete'](event);
			},
