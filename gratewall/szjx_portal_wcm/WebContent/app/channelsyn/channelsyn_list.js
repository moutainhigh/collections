...
Event.observe(window, 'load', function(){
	wcm.ListQuery.register({
		...
		/*检索项的内容*/
		items : [
			{name : 'CHNLNAME', desc : wcm.LANG.CHANNELSYN_VALID_36 || '目标栏目', type : 'string'}
		],
		/*执行检索按钮时执行的回调函数*/
...
function convertStatus(){
	location.href = WCMConstants.WCM6_PATH + 'channelsyn/channelsyncol_list.html' + location.search ;
...
ValidationHelper.bindValidations([
	...
	Ext.applyIf({
		...
		methods : buildDateCompMethod('EDATE', function(myV, compV){
			var rst = myV <= compV;
			this.warning += rst ? "" : (wcm.LANG.CHANNELSYN_VALID_35 ||"分发开始时间大于分发结束时间");
			return rst;
		})
	}, m_oDateValidJson),
	Ext.applyIf({
