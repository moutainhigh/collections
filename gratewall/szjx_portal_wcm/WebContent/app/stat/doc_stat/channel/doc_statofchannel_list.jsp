...
<%
	...
	//获取栏目id
	int nChnlId = currRequestHelper.getInt("channelIds",0);
	Channel currChannel = Channel.findById(nChnlId);
	if(currChannel==null)throw new WCMException(ExceptionNumber.ERR_OBJ_NOTFOUND,CMyString.format(LocaleServer.getString("statofchannel_list.obj.not.found","没有找到栏目对象[Id={0}]"),new int[]{nChnlId}));
	String sChnlName = currChannel.getName();
	...
	//需要递归找子，即要指定参数ContainsChildren为true
	//获取文档集合
...
<html>
 <head>
  <title WCMAnt:param="statofchannel_list.title"> 栏目发稿量统计 </title>
...
<!--引入分页JS-->
 ...
 <body>
  <div class="container">
	...
	<div class="return_btn" id="return_btn" url="<%=sReturnUrl%>" style="cursor:pointer;" WCMAnt:param="statofchannel_list.back">返回</div>
	<div class="content">
		<div class="data-title">
			<div class="r">
				<div class="c" WCMAnt:param="statofchannel_list.box">
					栏目【<span class="detail-hostname"><%=sChnlName%></span>】的<span class="detail-doctype"><%=getDocTypeDesc(nType,nCase)%></span>稿件列表
				</div>
			</div>
		</div>
		<div class="data">
			<table cellspacing="0" cellpadding="0">
				<tbody>
					<tr class="table_title">
						<th class="first" width="40" WCMAnt:param="statofchannel_list.num">序号</th><th WCMAnt:param="statofchannel_list.name">稿件名称</th><th width="60" WCMAnt:param="statofchannel_list.user">发稿人</th><th width="60" WCMAnt:param="statofchannel_list.use">使用情况</th><th width="60" WCMAnt:param="statofchannel_list.doc">文档性质</th>
					</tr>
<%
...
<%!
	...
	private String getDocTypeDesc (int _nType, int _nCase){
		switch(_nType){
			...
			default:
				return LocaleServer.getString("statofchannel_list.unknow","未知");
		}
	}
	...
	private String getModalDesc(int _nModal){
		switch(_nModal){
			case 1:
				return LocaleServer.getString("statofchannel_list.src","原稿");
			case 2:
				return LocaleServer.getString("statofchannel_list.quote","引用");
			case 3:
				return LocaleServer.getString("statofchannel_list.quote","引用");
			case 4:
				return LocaleServer.getString("statofchannel_list.copy","复制");
			default:
				return LocaleServer.getString("statofchannel_list.src","原稿");
		}
	}
	...
	private String getDocFormDesc(int _nDocForm){
		switch(_nDocForm){
			case 1:
				return LocaleServer.getString("statofchannel_list.word","文字型");
			case 2:
				return LocaleServer.getString("statofchannel_list.pic","图片型");
			case 3:
				return LocaleServer.getString("statofchannel_list.audeo","音频型");
			case 4:
				return LocaleServer.getString("statofchannel_list.video","视频型");
			default:
				return LocaleServer.getString("statofchannel_list.word","文字型");
		}
	}
	...
	private String getDocStatusDesc(int _nStatus){
		switch(_nStatus){
			case 1:
				return  LocaleServer.getString("statofchannel_list.new","新稿");
			case 2:
				return  LocaleServer.getString("statofchannel_list.edited","已编");
			case 18:
				return  LocaleServer.getString("statofchannel_list.check","正审");
			case 16:
				return  LocaleServer.getString("statofchannel_list.signed","已签");
			case 10:
				return  LocaleServer.getString("statofchannel_list.sent","已发");
			case 15:
				return  LocaleServer.getString("statofchannel_list.back","返工");
			default:
				return  LocaleServer.getString("statofchannel_list.unknow","未知");
		}
	}
