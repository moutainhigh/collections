...
<%@ page import="com.trs.components.stat.DocStatHandler4UserSite" %>
...
<%
	...
	String[] pStatSQL = new String[] {
		// 总的发稿量
		"Select count(*) DataCount, CrUser, SiteId from WCMChnlDoc "
				// 统计指定时间段的数据
				...
				//排除被删除文档的引用
				+ " and Modal>0"
				+ " Group by CrUser, SiteId",

		// 按照状态统计
		"Select count(*) DataCount, CrUser, DocStatus, SiteId from WCMChnlDoc "
				// 统计指定时间段的数据
				+ " Where CrTime>=${StartTime}"
				+ " And CrTime<=${EndTime}"
				+ " and DocStatus>0 and ChnlId>0"
				...
				//排除被删除文档的引用
				+ " and Modal>0"
				+ " Group by CrUser, DocStatus, SiteId",

		// 按照创作方式(实体/引用)统计
		"Select count(*) DataCount, CrUser, abs(Modal), SiteId from WCMChnlDoc "
				// 统计指定时间段的数据
				+ " Where CrTime>=${StartTime}"
				+ " And CrTime<=${EndTime}"
				+ " and DocStatus>0 and ChnlId>0"
		...
		// 按照文档所属类型统计
		"Select count(*) DataCount, CrUser, DocForm, SiteId from WCMChnlDoc "
				// 统计指定时间段的数据
				+ " Where CrTime>=${StartTime}"
				+ " And CrTime<=${EndTime}"
				+ " and DocStatus>0 and ChnlId>0"
				...
				//排除被删除文档的引用
				+ " and Modal>0"
				+ " Group by CrUser, DocForm, SiteId"
		};

	//调用统计类进行统计
	StatAnalysisTool tool = new StatAnalysisTool(pStatSQL,new DocStatHandler4UserSite(nSiteId));
