...
<%@ page import="com.trs.components.stat.DocStatHandler4ChnlDeptChannel" %>
<%@ page import="com.trs.components.stat.DocStatHandler4UserDeptChannel" %>
...
<%
	...
	String[] pStatSQL = new String[] {
		// 总的发稿量
		"Select count(*) DataCount, " + sChnlDocStatScale +" , ChnlId from WCMChnlDoc "
				// 统计指定时间段的数据
				...
				//排除被删除文档的引用
				+ " and Modal>0"
				+ " Group by " + sChnlDocStatScale + " , ChnlId",
		// 按照状态统计
		"Select count(*) DataCount, " + sChnlDocStatScale + " , DocStatus , ChnlId from WCMChnlDoc "
				// 统计指定时间段的数据
				+ " Where CrTime>=${StartTime}"
				+ " And CrTime<=${EndTime}"
				+ " and DocStatus>0 and ChnlId>0"
				...
				//排除被删除文档的引用
				+ " and Modal>0"
				+ " Group by " + sChnlDocStatScale + " , DocStatus , ChnlId",

		// 按照创作方式(实体/引用)统计
		"Select count(*) DataCount, " + sChnlDocStatScale + " , abs(Modal) , ChnlId from WCMChnlDoc "
				// 统计指定时间段的数据
				+ " Where CrTime>=${StartTime}"
				+ " And CrTime<=${EndTime}"
				+ " and DocStatus>0 and ChnlId>0"
				...
				//排除被删除文档的引用
				+ " and Modal>0"
				+ " Group by " + sChnlDocStatScale + " , Modal , ChnlId",
		...
		// 按照文档所属类型统计
		"Select count(*) DataCount, " + sChnlDocStatScale + " , DocForm , ChnlId from WCMChnlDoc "
				// 统计指定时间段的数据
				+ " Where CrTime>=${StartTime}"
				+ " And CrTime<=${EndTime}"
				+ " and DocStatus>0 and ChnlId>0"
				...
				//排除被删除文档的引用
				+ " and Modal>0"
				+ " Group by " + sChnlDocStatScale + " , DocForm , ChnlId"
		};
	
	//调用统计类进行统计
	StatAnalysisTool tool = null;
	if(bStatByChannel){
		tool = new StatAnalysisTool(pStatSQL,new DocStatHandler4ChnlDeptChannel(nChannelId));
	}else{
		tool = new StatAnalysisTool(pStatSQL,new DocStatHandler4UserDeptChannel(nChannelId));
	}
