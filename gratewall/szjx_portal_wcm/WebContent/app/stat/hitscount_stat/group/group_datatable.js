...
var pageController = {
	...
	*/
	...
	Mouse_Click_Event : function(evt){
		...
		if(Element.hasClassName(element, "titletd")){
			location.href = "./user_datatable.jsp" + window.location.search + "&" + Object.toQueryString(params);
		}else{
	...
	makeBasicParam : function(element){
		...
		var paramValue = element.getAttribute("GroupId");
...
Ext.apply(SEARCH_CONFIG.GROUP_HITSCOUNT,{
	export_cmd:function(){
		var sRequestUrl = 'export_group_hitscount.jsp';
...
Event.observe(window,"load",function(){
	...
	new Stat.Tab().init(TABS_CONFIG.GROUP_HITSCOUNT);
	//初始化检索字段信息
	initSearchParam();
...
function hitsTimeOptionClick(nType){
	...
	location.href = 'group_datatable.jsp?' + paramStr;
}
function getUrlJson(){
	var json = {};
