...
function bindEvents(){
	...
	for (var i = 0; i < elements.length; i++){
		var eventElement = elements[i];//因为此处元素name和id都取的字段名，name = id
		var type = eventElement.getAttribute('_type');
		if(type='appendix'){
			var sElementName = eventElement.getAttribute("_name");
			Event.observe(sElementName, 'click', function(event){
				var _eventElement = Event.element(event || window.event);
				var sFileName = _eventElement.parentNode.getAttribute("value");
				if(sFileName != ""){
...
//从htmlElementParser.js中分离出来

function SetValueByEditor(_sElement, _sContent){
	var oElement = $(_sElement+"_frame");
