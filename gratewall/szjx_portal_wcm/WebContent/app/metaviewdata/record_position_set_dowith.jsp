...
//4.初始化(获取数据)
	...
	if(currDocument == null){
		throw new WCMException(ExceptionNumber.ERR_OBJ_NOTFOUND,CMyString.format(LocaleServer.getString("record_position_set_dowith.jsp.docId_not_found", "没有找到ID为[{0}]的文档！"), new int[]{nDocumentId}));
	}

	int nChannelId = currRequestHelper.getInt("ChannelId", 0);
	Channel currChannel = Channel.findById(nChannelId);
	if(currChannel == null){
		throw new WCMException(ExceptionNumber.ERR_OBJ_NOTFOUND,CMyString.format(LocaleServer.getString("record_position_set_dowith.jsp.channelId_not_found", "没有找到ID为[{0}]的栏目！"), new int[]{nChannelId}));
		//throw new WCMException(ExceptionNumber.ERR_OBJ_NOTFOUND, "没有找到ID为["+nChannelId+"]的栏目！");
	}
...
//6.业务代码
	...
	if(nTargetDocId != nDocumentId){
		//判断是否能够改变文档顺序
		if (!DocumentAuthServer.hasRight(loginUser, currChannel,currDocument, WCMRightTypes.DOC_EDIT)) {
			throw new WCMException(ExceptionNumber.ERR_OBJ_NOTFOUND,CMyString.format(LocaleServer.getString("record_position_set_dowith.jsp.noright_modify_currchannel", "没有权限在当前栏目[{0}][ID={1}]下改变记录顺序！"), new Object[]{currChannel.getName(),String.valueOf(currChannel.getId())}));
        }
