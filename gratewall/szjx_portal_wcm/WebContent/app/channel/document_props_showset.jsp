...
<%
	...
	Boolean bIsInheritFromWebSite = false;
	if(CMyString.isEmpty(sBaseProps)&&CMyString.isEmpty(sAdvanceProps)&&CMyString.isEmpty(sOtherProps)){
		...
		bIsInheritFromWebSite = true;
	}
	if(CMyString.isEmpty(sBaseProps)&&CMyString.isEmpty(sAdvanceProps)&&CMyString.isEmpty(sOtherProps)){
		//站点也空，系统默认的
		bIsInheritFromWebSite = false;
	}
...
window.m_fpCfg = {
	...
	function save(){
		...
		getHelper().JspRequest(WCMConstants.WCM6_PATH + "channel/document_props_showset_dowith.jsp", oPostData,  true, function(transport, json){
			notifyFPCallback();
			FloatPanel.close();
		});
		return false;
...
<body>
<div id="fromWebSiteDiv" style="color:red;display:none;font-size:12px;font-weight:600;margin-bottom:5px;">继承自站点!</div>
...
<%	
	if(bIsInheritFromWebSite) {
%>
	<script language="javascript">
		$("fromWebSiteDiv").style.display = "block";
	</script>
<%
	}
%>
