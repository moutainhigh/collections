	...
	Ext.apply(chnlThumbDragDrop, {
		...
		_moveAfter : function(currItem, prevItem){
			...
			if(info["chnlType"] == 2 || info["chnlType"] == 1) { //头条和图片总是位于前面，不需要移动位置
				Ext.Msg.$fail(wcm.LANG.CHANNEL_62||'不能改变头条、图片栏目的位置！');
				return false;
			}
			//get the after el
			...
			if(dom){
				...
				if(nChnlType == 2 || nChnlType == 1) { //头条和图片总是位于前面，其他栏目无需放到其前后
					Ext.Msg.$fail(wcm.LANG.CHANNEL_63||'不能将该栏目放在头条、图片栏目之前！');
					return false;
				}
			}
		...
		_moveTo : function(currItem, dstItem){
			...
			if(wcm.AuthServer.checkRight(srcInfo["right"], 12)//12：移动
				...
				var sMsg = String.format("确实要将栏目{0}移动为栏目{1}的子栏目?",srcChnlDesc,dstChnlDesc);
