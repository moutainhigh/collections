...
<%
	...
	//2013-03-05 注释by lky 原因由于母版保存缩略图报错
	...
	if (oFileHelper == null){
		sErrorMsg = LocaleServer.getString("file_upload_dowith.jsp.label.error2import_file", "传入的文件有误！");
	}else if (oFileHelper.getSize() <= 0){
		sErrorMsg = LocaleServer.getString("file_upload_dowith.jsp.label.import_files_content_is_null", "传入的文件没有内容！");
	}else if(isForbidFileExt(oFileHelper.getFileName())){
		sErrorMsg	= CMyString.format(LocaleServer.getString("file_upload_dowith.jsp.forbid_upload_file", "系统禁止上传[{0}]格式的文件!"), new String[]{CMyFile.extractFileExt(oFileHelper.getFileName()).toUpperCase()});
	}else{
...
<HEAD>
	...
	<TITLE WCMAnt:param="file_upload_dowith.jsp.title">缩略图处理页面</TITLE>
</HEAD>
<BODY>
<script>
	...
	<%
		...
		if (sErrorMsg == null){
			sErrorMsg = LocaleServer.getString("file_upload_dowith.jsp.label.runtimeexception", "传入的文件有误！");
		}
	%>
	alert("<%= sErrorMsg %>");
	<%
	...
	%>
	window.location.href="./file_upload.jsp";
</script>
</BODY>
</HTML>