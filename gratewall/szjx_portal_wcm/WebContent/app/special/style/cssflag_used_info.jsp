 ...
 *  Last EditTime	:none
 *  Update Logs:	none
 *  Parameters:		see cssflag_used_info.xml
 */
%>
...
<head>
	...
	<title WCMAnt:param="cssflag_used_info.jsp.title">当前标识已经被使用</title>
	<style type="text/css">
		body{
			background:#ffffff;
			font-size:14px;
...
<body>
	<div class="message">
		<span WCMAnt:param="cssflag_used_info.jsp.id_isUsed">标识[<span class="cssflag"><%=CMyString.transDisplay(sCssFlag)%></span>]已经被使用，详细信息如下:</span>
	</div>
	<div class="info_con">
		<%
			if(oResourceStyles.size()>0){
		%>
		<div class="style_type" WCMAnt:param="cssflag_used_info.jsp.widget_style">资源风格</div>
		<ul>
		<%
			}
			for(int i=0;i<oResourceStyles.size();i++){
				...
				if (nCurrPageStyleId > 0) {
					...
					sStyleInfo += LocaleServer.getString("cssflag_used_info.jsp.label.sys_level", "系统级别->");
				}
				sStyleInfo += oResourceStyle.getStyleName();
		%>
			<li><%=CMyString.transDisplay(sStyleInfo)%></li>
		<%
			}
			if(oResourceStyles.size()>0){
		%>
		...
		%>
		<div class="style_type" WCMAnt:param="cssflag_used_info.jsp.content_style">内容风格</div>
		<ul>
		<%
			}
