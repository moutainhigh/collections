...
<%
	...
	//权限的校验
	//获取参数
	int nSourceContentStyleId = currRequestHelper.getInt("SourceContentStyleId",0);
	int nPageStyleId = currRequestHelper.getInt("PageStyleId",0);
	
	// 获取资源分类
	JSPRequestProcessor processor = new JSPRequestProcessor(request, response);
	HashMap parameters = new HashMap();
	parameters.put("ObjectId", String.valueOf(nSourceContentStyleId));
	String sServiceId = "wcm61_contentstyle";
	String sMethodName = "findById";
	ContentStyle oContentStyle = ( ContentStyle ) processor.excute(sServiceId,sMethodName,parameters);
	if(oContentStyle==null){
		throw new WCMException(CMyString.format(LocaleServer.getString("copy_content_style_to.jsp.fail2get_style", "获取风格[Id={0}]失败！"), new int[]{nSourceContentStyleId}));
	}
	
	//获取对象属性
	String sPageStyleName = oContentStyle.getStyleName();//页面风格名称
	String sPageStyleDesc = oContentStyle.getStyleDesc();
...
<html>
<head>
	<title WCMAnt:param="copy_content_style_to.jsp.title">选择页面风格</title>
			...
			btns : [
				{
					...
					cmd : function(){
						...
						var copyInfo={
							SourceContentStyleId : <%=nSourceContentStyleId%>,
...
<div class="repeatnamearea">
	<table border="0" cellspacing="0" cellpadding="0" width="100%" height="50px" class="operations" style="margin:0px;">
	<tbody>
		<tr>
			<td class="td_bg" WCMAnt:param="copy_content_style_to.jsp.please_sel_rename">请选择风格重名后的处理方式：</td>
		</tr>
		<tr>
			<td height="30px">
				<span><input type="radio" name="StyleCopyMode" value="1"></span>
				<span WCMAnt:param="copy_content_style_to.jsp.auto_cover">自动覆盖</span>
				<span><input type="radio" name="StyleCopyMode" value="3"></span>
				<span WCMAnt:param="copy_content_style_to.jsp.auto_rename">自动更名</span>
				<span><input type="radio" name="StyleCopyMode" value="2" checked></span>
				<span WCMAnt:param="copy_content_style_to.jsp.skip">跳过</span>
			</td>
		</tr>
	</tbody>
	</table>
</div>
<span class="operations" style="margin:0px;padding-left:10px;" WCMAnt:param="copy_content_style_to.jsp.sel_target_style">请选择目标风格: </span>
<div class="selectarea"> 
	<table border="0" cellspacing="0" cellpadding="0" width="100%" style="height:25px;">
		<tr>
			<td width="40px" align="center" class="head_td_bg"><span onclick="selectall();return false;" style="cursor:pointer;" WCMAnt:param="copy_content_style_to.jsp.sel_all">全选</span></td>
			<td align="center" class="head_td_bg" WCMAnt:param="copy_content_style_to.jsp.zh_name">中文名</td>
			<td width="100px" align="center" class="head_td_bg" WCMAnt:param="copy_content_style_to.jsp.en_name">英文名</td>
		</tr>
	</table>
