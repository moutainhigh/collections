...
<table cellspacing=0 border="1" cellpadding=0 id="grid_table" class="grid_table" borderColor="gray">
	<tr id="grid_head" class="grid_head">
		<td width="40" WCMAnt:param="list.selectall" id="selAll" class="selAll" onclick="selectAll();">全选</td>
		...
		<td width="60" WCMAnt:param="template_query.temp.assign">模板分配</td>
	</tr>
...
//5. 遍历生成表现
	for (int i = currPager.getFirstItemIndex(); i <= currPager.getLastItemIndex(); i++) {
		try{
			...
			// 排除嵌套模板
			if(nTemplateType == PublishConstants.TEMPLATE_TYPE_NESTED){
				continue;
			}
...
%>
	<tr id="tr_<%=nRowId%>" rowid="<%=nRowId%>" itemId="<%=nRowId%>" class="grid_row" right="<%=sRightValue%>" rootId="<%=nRootId%>" folderId="<%=folderId%>" folderType="<%=folderType%>" tempType="<%=obj.getType()%>" VisualAble="<%=bIsVisual%>">
		...
		<td id="lastModifiedTime_<%=nRowId%>">
			<a href="#" onclick="selectChannel(<%=folderId%>,<%=folderType%>,<%=nRowId%>,<%=nTemplateType%>,'<%=sTempName%>')" WCMAnt:param="template_query.assign">分配给</a></td>
