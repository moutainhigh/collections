	...
	*/
	...
	var eventHandler = function(doms){	
		for(var i = 0; i < doms.length; i++){
			...
			data.descs = data.descs.split("$");			
			var scrollObj = com.trs.ScrollImage.get(data.container);
			if(scrollObj){
				scrollObj.destroy();
			}
	...
	for(var i = 0; i < eventTypes.length; i++){
		wcm.special.widget.InstanceMgr.addListener(eventTypes[i], function(dom){
			...
			if(!Element.hasClassName(dom, 'trs-scroll-image')){
				return;
			}
