...
function CTRSCalendar_openDialog(_id, _sInitDateString, _sDateFormat, _bShowTime){
	...
	if(!oEl){
		CTRSAction_alert(wcm.LANG.CTRSCalendar_101 || "没有找到存储日期值得元素，操作失败！");
		return;
	}
	var args = new Array();
	args[0] = _sDateFormat || "";
...
function checkDateString(bApplyEmpty){
	...
	if(regexp.test(sDateStr)){
		...
		if(nYear < 1900 || nYear > 2070){
			...
			CTRSAction_alert(wcm.LANG.CTRSCalendar_102 || "[年] 必须在 1900-2070 之间！");
		...
		if(nMonth < 1 || nMonth > 12){
			nMonth = 1;
			CTRSAction_alert(wcm.LANG.CTRSCalendar_103 || "[月] 必须在 1-12 之间！");
			oSrcEl.value = toNumberString(nYear) + "-" + toNumberString(nMonth) + "-" + toNumberString(nDay) + ((bHasTime)?(" " + toNumberString(dateArray[4]) + ":" + toNumberString(dateArray[5])):"");
			return;
		}
		if(nDay < 1 || nDay > 31){
			nDay = 1;
			CTRSAction_alert(wcm.LANG.CTRSCalendar_104 || "[日] 必须在 1-31 之间！");
			oSrcEl.value = toNumberString(nYear) + "-" + toNumberString(nMonth) + "-" + toNumberString(nDay) + ((bHasTime)?(" " + toNumberString(dateArray[4]) + ":" + toNumberString(dateArray[5])):"");
			return;
		}
		if(nMonth==2){ 
			if(isLeapYear(nYear)&&nDay>29){
				nDay = 29;
				CTRSAction_alert(wcm.LANG.CTRSCalendar_105 || "闰年的二月最多只有29天！");
				oSrcEl.value = toNumberString(nYear) + "-" + toNumberString(nMonth) + "-" + toNumberString(nDay) + ((bHasTime)?(" " + toNumberString(dateArray[4]) + ":" + toNumberString(dateArray[5])):"");
				return;
			}
			if(!isLeapYear(nYear)&&nDay>28){
				nDay = 28;
				CTRSAction_alert(wcm.LANG.CTRSCalendar_106 || "平年的二月有28天！");
				oSrcEl.value = toNumberString(nYear) + "-" + toNumberString(nMonth) + "-" + toNumberString(nDay) + ((bHasTime)?(" " + toNumberString(dateArray[4]) + ":" + toNumberString(dateArray[5])):"");
				return;
			}	
		}//end 2 month
		if((nMonth==4||nMonth==6||nMonth==9||nMonth==11)&&(nDay>30)){
			nDay = 30;
			CTRSAction_alert(nMonth+wcm.LANG.CTRSCalendar_107 || "月份最多只有30天！");
			oSrcEl.value = toNumberString(nYear) + "-" + toNumberString(nMonth) + "-" + toNumberString(nDay) + ((bHasTime)?(" " + toNumberString(dateArray[4]) + ":" + toNumberString(dateArray[5])):"");
			return;
		}//end 30days
		if(bHasTime){
			...
			if(nHour < 0 || nHour > 23){
				nHour = 0;
				CTRSAction_alert(wcm.LANG.CTRSCalendar_108 || "[时] 必须在 0-23 之间！");
			...
			if(nMinute < 0 || nMinute > 59){
				nMinute = 0;
				CTRSAction_alert(wcm.LANG.CTRSCalendar_109 || "[分] 必须在 0-59 之间！");
				oSrcEl.value = toNumberString(nYear) + "-" + toNumberString(nMonth) + "-" + toNumberString(nDay) + " " + toNumberString(nHour) + ":" + toNumberString(nMinute);
				return;
			}
		}
