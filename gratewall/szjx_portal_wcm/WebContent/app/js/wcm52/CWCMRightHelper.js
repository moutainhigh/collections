...
function getTypeHTML(_nType){
	switch(_nType){
		case TYPE_WCMOBJ_USER:
			return "<img src='../images/wcm52/icon_user.gif' title='" + (wcm.LANG.WCM52_USER || "用户") + "'>";
		case TYPE_WCMOBJ_GROUP:
			return "<img src='../images/wcm52/icon_group.gif' title='" + (wcm.LANG.WCM52_GROUP || "用户组") + "'>";
		case TYPE_WCMOBJ_ROLE:
			return "<img src='../images/wcm52/icon_role.gif' title='" + (wcm.LANG.WCM52_ROLE || "角色") + "'>";
		default:
			return (wcm.LANG.WCM52_UNKNOWN || "未知");
	}
}
...
function CWCMOperator_toHTML(){
	switch(this.nType){
		case TYPE_WCMOBJ_USER:
			return (wcm.LANG.WCM52_USER || "用户") + "[" +this.sName+"]";
		case TYPE_WCMOBJ_GROUP:
			return (wcm.LANG.WCM52_GROUP || "用户组") + "[" +this.sName+"]";
		case TYPE_WCMOBJ_ROLE:
			return (wcm.LANG.WCM52_ROLE || "角色") + "[" +this.sName+"]";
		default:
			return (wcm.LANG.WCM52_UNKNOWN || "未知") + "["+this.sName +"]";
	}
}
...
function CWCMRightHelper_draw(_nStartRightDefIndex, _nEndRightDefIndex, _elDiv,_sExtraIndexes,_bReadMode){
	var sHTML = ""
			...
			+((currMode == 'edit') ? ("<TD bgcolor=\"#BEE2FF\" onclick='selectAllCheck(this)' style='cursor:pointer;'>" + (wcm.LANG.WCM52_SELECTALL || "全选") + "</TD>") : ("<TD bgcolor=\"#BEE2FF\">" + (wcm.LANG.WCM52_NO || "序号") + "</TD>"))
			+"<TD bgcolor=\"#BEE2FF\">" + (wcm.LANG.WCM52_TYPE || "类型") + "</TD>"
			+"<TD bgcolor=\"#BEE2FF\"><A href=\"#\" onClick=\"WCMRightHelper.sort();init();return false;\">" + (wcm.LANG.WCM52_NAME || "名称");
	...
	for(var i=_nStartRightDefIndex; i<this.arRightDef.length && i<=_nEndRightDefIndex; i++){
		...
		if(currMode == 'edit'){
			sHTML += "<TD bgcolor=\"#BEE2FF\" title=\""+this.arRightDef[i]["Desc"]+"[" + (wcm.LANG.WCM52_ALERT_9 || "点击后可以选择一列") +"]\" "
	...
	for(var i=0; i < extraIndexes.length; i++){
		...
		if(currMode == 'edit'){
			sHTML += "<TD bgcolor=\"#BEE2FF\" title=\""+this.arRightDef[currIndex]["Desc"]+"[" + (wcm.LANG.WCM52_ALERT_9 || "点击后可以选择一列") +"]\" "+"onclick=\"WCMRightHelper.selectCol(this);\" style=\"cursor:hand\">"
	...
	if(currMode == 'edit')
		sHTML += "<TD bgcolor=\"#BEE2FF\">" + (wcm.LANG.WCM52_ALERT_10 || "删除") + "</TD>\n"
	...
	for(var i=0; i<this.arRight.length; i++){
		...
		if(currMode == 'edit'){
			sHTML += "<TD align=center><img alt='" + (wcm.LANG.WCM52_ALERT_11 || "删除当前设置信息") + "' src=\"../images/wcm52/button_delete_small.gif\" onclick=\"WCMRightHelper.removeAt("+i+");CWCMRightHelper_notifyRemove(this);\" style=\"cursor:hand;\"></TD>";
		}
		sHTML	+= "</TR>\n";		
	}
...
function CWCMRightHelper_removeAll(){
	if(this.arRight.length == 0)
		return;
	if($confirm(wcm.LANG.WCM52_ALERT_12 || "您确认删除全部的设置！", function(){
		$dialog().hide();
