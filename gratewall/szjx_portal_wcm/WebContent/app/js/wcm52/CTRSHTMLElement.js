...
function CTRSHTMLElement(){	
	...
	this.selectAllByName		= function(_sElementName, _bChecked, _oCheckValue){
		...
		//2.获取
		var arElements = document.getElementsByName(_sElementName); //document.all(_sElementName)
		if(arElements.length == 0)return;

		if(arElements.length <= 1){//单个元素
	...
	this.getElementValueByName	= function(_sElementName, _sDelim){
		...
		if(arElements.length<=1){
			if(arElements[0].checked)
				return arElements[0].value;
			return "";
		}
	...
	this.setElementCheckedByValue	= function(_sElementName, _sValue){
		...
		if(arElements.length <= 1){
			arElements[0].checked = (sValue.indexOf("," + arElements[0].value + ",")>=0);			
			return;
		}
