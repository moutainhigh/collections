Ext.ns('com.trs.ajaxframe', 'com.trs.web2frame');
/**
 * 置标解析的工具类
 */
...
var TempEvaler = com.trs.web2frame.TempEvaler = com.trs.ajaxframe.TempEvaler	= {
	...
	*/
	...
	value : function(_sExpression,_oJson){
		...
		if(oValue){
			if(Ext.isArray(oValue)){
				oValue	= oValue[0];
			}
			else if(Ext.type(oValue) == 'object'){
	...
	array : function(_sExpression,_oJson){
		...
		return ( !oArray || Ext.isArray(oArray) ) ? oArray : [oArray] ;
	},
	/**
	...
	valid : function(_sExpression,_oJson){
		...
		for(var index=0,tmpCmd='';index<cnt;index++){
			...
			if(tmpObject2 == null){
				if(tmpCmd=='0'&&!Ext.isArray(tmpObject)){//适应for置标中只有一个元素的情况
					//nothing
				}
				else if(Ext.isArray(tmpObject) && tmpObject.length>0){//适应数组中取首元素不带index的情况
...
TempEvaler.eval = TempEvaler.evaluateTemplater;