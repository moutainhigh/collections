...
Object.extend(com.trs.tree.TreeManager.prototype, {
	...
	*/
	...
	showOperator : function(element){
		...
		if(!this.params["operatorContainerId"]){
			...
			*/
			var sHTML = ['<div class="TreeViewManager">',
							'<div class="insertAsLastChild" type="insertAsLastChild" title="', wcm.LANG.TREE_ALERT_8 || '添加子节点', '"></div>',
							'<div class="delete" type="delete" title="', wcm.LANG.TREE_ALERT_9 || '删除', '"></div>',
						'</div>'].join("");
	...
	clickEvent : function(event){
		...
		if(this.lastClickElement == srcElement){
			this._currentTreeNode_ = srcElement;
			this.modify();
		}else{
			this.lastClickElement = srcElement;
		}
	},
	...
	mouseOutEvent : function(event){
		...
		var toEle = event.toElement || relatedTarget;
		if(this.params["_treeView_"].contains(toEle) 
				&& this.params["_treeView_"] != toEle){ 
			return;
		}
		var operatorContainer = $(this.params["operatorContainerId"]);
		if(operatorContainer && operatorContainer.contains(toEle)){
			return;
		}
		this.hideOperator();
	},
	cut : function(){
		if(this._cutTreeNode_){
			//对于剪切了一个，又再剪切一个的情况，需要去掉样式
			if(this._cutTreeNode_.parentNode&&Element.hasClassName(this._cutTreeNode_.parentNode,"iscut")){
				Element.removeClassName(this._cutTreeNode_.parentNode,"iscut");
			}
		}
		//剪切先不做删除，改变状态，并记录下来
		this._cutTreeNode_ = $TreeNav.oPreSrcElementA;
		//添加一个样式
		Element.addClassName(this._cutTreeNode_.parentNode,"iscut");
	},
	paste : function(){
		//发请求，只改变相互之间的关系
		//改变parentId，重新设置
		this._currentTreeNode_ = $TreeNav.oPreSrcElementA;
		if(this._cutTreeNode_==null)return;
		if(this._cutTreeNode_==this._currentTreeNode_){
			Ext.Msg.alert(wcm.LANG.Treemanager_3011 || "请选择节点。");
			return;
		}
		var oCurrDivNode = this._cutTreeNode_.parentNode;
		var detParentDivNode = this._currentTreeNode_.parentNode;
		this.changeRelation(oCurrDivNode,detParentDivNode);
		this._cutTreeNode_ = null;
	}, 
	...
	keyUpCtrl : function(event){
		...
		var currParentNode = Element.next(currDivNode);
		if(currParentNode && currParentNode.tagName != "UL"){
			currParentNode = null;
		}
		var previousParentNode = Element.previous(currDivNode);
		var previousDivNode = Element.previous(previousParentNode);
		...
		Element.first(currDivNode).focus();
	...
	keyDownCtrl : function(event){
		...
		var nextDivNode = Element.next(currParentNode || currDivNode);
		var nextParentNode = Element.next(nextDivNode);
	...
	getUpTarget : function(){
		if(!$TreeNav.oPreSrcElementA) return;
		var divNode = $TreeNav.oPreSrcElementA.parentNode;
		var tempNode = Element.previous(divNode);
		while(tempNode){
			if(tempNode.tagName.toUpperCase() == "UL"
					&& Element.visible(tempNode)){
				divNode = Element.last(tempNode);
				if(divNode){
					tempNode = divNode;
					continue;
				}
			...
			tempNode = Element.previous(tempNode);
		}
	...
	getDownTarget : function(){
		if(!$TreeNav.oPreSrcElementA) return;
		var divNode = $TreeNav.oPreSrcElementA.parentNode;
		var tempNode = Element.next(divNode);
		while(tempNode){
			if(tempNode.tagName.toUpperCase() == "UL"
					&& Element.visible(tempNode)){
				divNode = Element.first(tempNode);
				if(divNode){
					tempNode = divNode;
					continue;
				}
	...
	_validInput : function(oInputElement, sOldValue){
		if(oInputElement.value.trim() == ''){
			return wcm.LANG.TREE_ALERT_10 || "不能为空!";
		}
	},
	...
	*/
	...
	insertAsLastSibling : function(){
		...
		var parentElement = $TreeNav._getParentElement(this._currentTreeNode_.parentNode);
		var topChild = $TreeNav._getTopChildElement(parentElement);
		new Insertion.After(Element.last(topChild), this.getInsertContent());
		this.doAdd();
	},
	doAdd : function(oParams){
		...
		nodeInput.focus();
		nodeInput.onkeydown = this.inputKeyDownEvent.bind(this, nodeInput, "addURL", 'doAddAfter', wcm.LANG.TREE_ALERT_11  || "添加树节点失败!");
		nodeInput.onblur = this.inputBlurEvent.bind(this, nodeInput, "addURL", 'doAddAfter', wcm.LANG.TREE_ALERT_11 || "添加树节点失败!");
	...
	*/
	...
	'delete' : function(){
		...
		this.hideOperator();
		/*
		if(!confirm(String.format(wcm.LANG.TREE_ALERT_12 || "确认删除这个树节点[{0}]吗?",  this._currentTreeNode_.innerHTML.unescapeHTML()))){
			return;
		}
		*/
		...
		if(this.params["servicesName"]){
			...
			oHelper.Call(this.params["servicesName"], this.params["deleteURL"], 
					...
					this.failureCallBack.bind(this, wcm.LANG.TREE_ALERT_13 || "删除节点失败!"));
		}else{
			new Ajax.Request(this.params["deleteURL"], {
				...
				onFailure : this.failureCallBack.bind(this, wcm.LANG.TREE_ALERT_13 || "删除节点失败!")
			});		
		}
	},
	deleteAfter : function(response, json){
		...
		if(resultJson["ISSUCCESS"]){
			with($TreeNav){
				var divNode = this._currentTreeNode_.parentNode;
				var parentElement = _getParentElement(divNode);
				try{
					removeNode(divNode);
				}catch(error){
				}
				reloadNodeStyle(parentElement);
				this._deleteSuccessCallBack(divNode, response, json, resultJson);
			}
		}else{
	...
	modify : function(){
		...
		nodeInput.focus();
		nodeInput.select();
		nodeInput.onkeydown = this.inputKeyDownEvent.bind(this, nodeInput, "modifyURL", 'modifyAfter', wcm.LANG.TREE_ALERT_14 || "修改树节点失败!");
		nodeInput.onblur = this.inputBlurEvent.bind(this, nodeInput, "modifyURL", 'modifyAfter', wcm.LANG.TREE_ALERT_14 || "修改树节点失败!");
	},
	modifyAfter : function(response, json){
		var resultJson = this.getRequestResult(response, json);
		if(resultJson["ISSUCCESS"]){
			var divNode = this._currentTreeNode_.parentNode;
			this._modifySuccessCallBack(divNode, response, json, resultJson);
		}else{
...
$TreeNav.observe('onload', function(){
	...
	for (var i = 0, length = treeViews.length; i < length; i++){
		...
		var rootNode = Element.first(treeViews[i]);
		if(rootNode){
