...
function getParameter(_sName, _sQuery, encode){
	...
	var fn = Ext.isFunction(encode) ? encode : (encode === false ? Ext.emptyFn : decodeURIComponent);
...
Ext.Xml = com.trs.util.XML = {
	...
	createXMLDoc : function(_sRootName){
		var oXMLDoc = null;
		var sRootName = _sRootName || "root";
		//if(_IE) {
			oXMLDoc = new ActiveXObject("Microsoft.XMLDOM");
			oXMLDoc.async = false;
			oXMLDoc.documentElement = oXMLDoc.createElement(sRootName);
		/*
		}else{
			oXMLDoc = document.implementation.createDocument("", sRootName, null);
		}*/
		return oXMLDoc;
	},
	serializeToString : function(_oXmlDoc){
		var sXMLString = null;
		//if(_IE) {
			sXMLString = _oXmlDoc.xml;
		/*
		}else{
			var xmlSerializer = new XMLSerializer();
			sXMLString = xmlSerializer.serializeToString(_oXmlDoc)
		}*/
		return sXMLString;
	}
};
...
Object.deepClone = function(obj){
	...
	if(Ext.isArray(obj)){
		...
		for(var i=0,n=obj.length;i<n;i++){
			result.push(Object.deepClone(obj[i]));
		}
		return result;
	}
	...
	result = {};
	for(var name in obj){
		result[name] = Object.deepClone(obj[name]);
	}
	return result;
}
