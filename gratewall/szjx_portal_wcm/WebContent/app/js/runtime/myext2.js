...
var WCMConstants={DEBUG:true,WCM_APPNAME:'wcm',WCM6_PATH:'/wcm/app/',WCM_LOCAL_URL:'/wcm/app/localxml/',WCM_ROMOTE_URL:'/wcm/center.do',WCM_NOT_LOGIN_PAGE:'/wcm/console/include/not_login.htm'}

var MyExt=Ext=(function(){var ua=navigator.userAgent.toLowerCase();var isStrict=document.compatMode=="CSS1Compat",isOpera=ua.indexOf("opera")>-1,isSafari=(/webkit|khtml/).test(ua),isSafari3=isSafari&&ua.indexOf('webkit/5')!=-1,isIE=!isOpera&&ua.indexOf("msie")>-1,isIE7=!isOpera&&ua.indexOf("msie 7")>-1,isGecko=!isSafari&&ua.indexOf("gecko")>-1,isGecko3=!isSafari&&ua.indexOf("rv:1.9")>-1,isBorderBox=isIE&&!isStrict,isWindows=(ua.indexOf("windows")!=-1||ua.indexOf("win32")!=-1),isMac=(ua.indexOf("macintosh")!=-1||ua.indexOf("mac os x")!=-1),isAir=(ua.indexOf("adobeair")!=-1),isLinux=(ua.indexOf("linux")!=-1),isSecure=window.location.href.toLowerCase().indexOf("https")===0;if(isIE&&!isIE7){try{document.execCommand("BackgroundImageCache",false,true);}catch(e){}}
return{isOpera:isOpera,isSafari:isSafari,isSafari3:isSafari3,isSafari2:isSafari&&!isSafari3,isIE:isIE,isIE6:isIE&&!isIE7,isIE7:isIE7,isGecko:isGecko,isGecko2:isGecko&&!isGecko3,isGecko3:isGecko3,isBorderBox:isBorderBox,isLinux:isLinux,isWindows:isWindows,isMac:isMac,isAir:isAir,useShims:((isIE&&!isIE7)||(isMac&&isGecko&&!isGecko3)),isStrict:isStrict,isSecure:isSecure,emptyFn:function(){},ns:function(){var a=arguments,o=null,i,j,d,rt;for(i=0;i<a.length;++i){d=a[i].split(".");rt=d[0];eval('if (typeof '+rt+' == "undefined"){'+rt+' = {};} o = '+rt+';');for(j=1;j<d.length;++j){o[d[j]]=o[d[j]]||{};o=o[d[j]];}}},applyIf:function(o,c){if(o&&c){for(var p in c){if(typeof o[p]=="undefined"){o[p]=c[p];}}}
return o;},apply:function(o,c){if(o&&c){for(var p in c){o[p]=c[p];}}
return o;},$break:{},isDebug:function(){return(window.WCMConstants&&WCMConstants.DEBUG)||!!getParameter('isdebug',top.location.search);},errorMsg:function(_Error){if(Ext.isIE){var oCaller=arguments.caller;var tmpStack=[];while(oCaller&&oCaller.callee){tmpStack.push(oCaller.callee);oCaller=oCaller.caller;if(!oCaller||tmpStack.include(oCaller.callee)){break;}}
return _Error.message+'\n'+tmpStack.join('\n------------------------\n');}
else{return _Error.message+'\n'+_Error.stack;}},isArray:function(v){return v&&typeof v.length=='number'&&typeof v.splice=='function';},isFunction:function(_object){return typeof _object=="function";},isString:function(_object){return(typeof _object=="string")||((_object!=null)&&(_object.constructor!=null)&&(_object.constructor==String||_object.constructor.toString().trim().indexOf('function String()')==0));},isNumber:function(_object){return(typeof _object=="number")||((_object!=null)&&(_object.constructor!=null)&&(_object.constructor==Number||_object.constructor.toString().trim().indexOf('function Number()')==0));},isBoolean:function(_object){return(typeof _object=="boolean")||((_object!=null)&&(_object.constructor!=null)&&(_object.constructor==Boolean||_object.constructor.toString().trim().indexOf('function Boolean()')==0));},isSimpType:function(_object){return this.isString(_object)||this.isNumber(_object)||this.isBoolean(_object);}};})();

if(!window.$){function $(element){if(arguments.length>1){for(var i=0,elements=[],length=arguments.length;i<length;i++)
elements.push($(arguments[i]));return elements;}
if(element==null)return null;if(element.nodeName&&element.nodeType)
return element;return document.getElementById(element)||document.getElementsByName(element)[0]||element;}}
...
return returnValue;}}
Ext.applyIf(String.prototype,{lTrim:function(){return this.replace(/^\s*/,"");},rTrim:function(){return this.replace(/\s*$/,"");},trim:function(){return this.rTrim().lTrim();},endsWith:function(sEnd){return(this.substr(this.length-sEnd.length)==sEnd);},startsWith:function(sStart){return(this.substr(0,sStart.length)==sStart);},equals:function(_sc){return this==_sc;},equalsI:function(_sc){return this.toLowerCase()==_sc.toLowerCase();},byteLength:function(){var length=0;this.replace(/[^\x00-\xff]/g,function(){length++;});return this.length+length;},parseQuery:function(){var pairs=this.match(/^\??(.*)$/)[1].split('&');return pairs.inject({},function(params,pairString){var pair=pairString.split('=');params[pair[0]]=pair[1];return params;});}});Ext.applyIf(String,{escape:function(string){return string.replace(/('|\\)/g,"\\$1");},leftPad:function(val,size,ch){var result=new String(val);if(!ch){ch=" ";}
while(result.length<size){result=ch+result;}
return result.toString();},format:function(format){var args=Array.prototype.slice.call(arguments,1);return format.replace(/\{(\d+)\}/g,function(m,i){return args[i];});}});Ext.applyIf(Array.prototype,{each:function(iterator){try{for(var i=0;i<this.length;i++){iterator(this[i],i);}}catch(e){if(e!=Ext.$break)throw e;}},inject:function(memo,iterator){this.each(function(value,index){memo=iterator(memo,value,index);});return memo;},map:function(iterator){var results=[];this.each(function(value,index){results.push(iterator(value,index));});return results;},include:function(o){return this.indexOf(o)!=-1;},includeI:function(_object){if(Ext.isString(_object)){var matched=false;_object=_object.toLowerCase()
...
return unescape(document.cookie.substring(offset,endstr));};var Event={KEY_BACKSPACE:8,KEY_TAB:9,KEY_RETURN:13,KEY_ESC:27,KEY_LEFT:37,KEY_UP:38,KEY_RIGHT:39,KEY_DOWN:40,KEY_DELETE:46,element:function(event){return event.target||event.srcElement;},isLeftClick:function(event){return(((event.which)&&(event.which==1))||((event.button)&&(event.button==1)));},pointerX:function(event){return event.pageX||(event.clientX+
...
name='keydown';if(element.removeEventListener){element.removeEventListener(name,observer,useCapture);}else if(element.detachEvent){element.detachEvent('on'+name,observer);}}};Event.observe(window,'unload',Event.unloadCache,false);if(Ext.isIE){function fnCleanUp(){var p=Function.prototype;delete p.bind;window.detachEvent("onunload",fnCleanUp);}
window.attachEvent("onunload",fnCleanUp);}
Ext.ns('Element');Ext.applyIf(Element,{update:function(_el,html,loadScripts,callback){_el=$(_el);if(typeof html=="undefined"){html="";}
if(loadScripts!==true){_el.innerHTML=html;if(typeof callback=="function"){callback();}
return _el;}
var dom=_el;setTimeout(function(){var hd=document.getElementsByTagName("head")[0];var re=/(?:<script([^>]*)?>)((\n|\r|.)*?)(?:<\/script>)/ig;var srcRe=/\ssrc=([\'\"])(.*?)\1/i;var typeRe=/\stype=([\'\"])(.*?)\1/i;var match;while(match=re.exec(html)){var attrs=match[1];var srcMatch=attrs?attrs.match(srcRe):false;if(srcMatch&&srcMatch[2]){var s=document.createElement("script");s.src=srcMatch[2];var typeMatch=attrs.match(typeRe);if(typeMatch&&typeMatch[2]){s.type=typeMatch[2];}
...
if(typeof callback=="function"){callback();}},10);dom.innerHTML=html.replace(/(?:<script.*?>)((\n|\r|.)*?)(?:<\/script>)/ig,"");return dom;},visible:function(element){return $(element).style.display!='none';},toggle:function(element){element=$(element);return Element[Element.visible(element)?'hide':'show'](element);},hide:function(element){element=$(element);element.style.display='none';return element;},show:function(element){element=$(element);element.style.display='';return element;},_removeNode:Ext.isIE?function(){var d;return function(n){if(n&&n.tagName!='BODY'){d=d||document.createElement('div');d.appendChild(n);d.innerHTML='';}}}():function(n){if(n&&n.parentNode&&n.tagName!='BODY'){n.parentNode.removeChild(n);}},remove:function(_el){Element._removeNode($(_el));},addClassName:function(_el,className){_el=$(_el);if(Ext.isArray(className)){for(var i=0,len=className.length;i<len;i++){this.addClassName(className[i]);}}else{if(className&&!this.hasClassName(className)){_el.className=_el.className+" "+className;}}
return _el;},removeClassName:function(_el,_cls){_el=$(_el);if(!_cls||!_el.className){return _el;}
if(Ext.isArray(_cls)){for(var i=0,len=_cls.length;i<len;i++){this.removeClassName(_cls[i]);}}else{if(this.hasClassName(_cls)){var re=new RegExp('(?:^|\\s+)'+_cls+'(?:\\s+|$)',"g");_el.className=_el.className.replace(re," ");}}
return _el;},hasClassName:function(_el,_cls){_el=$(_el);return _cls&&(' '+_el.className+' ').indexOf(' '+_cls.trim()+' ')!=-1;},replaceClassName:function(_el,_clsOld,_clsNew){this.removeClassName(_clsOld);this.addClassName(_clsNew);}});Ext.ns('wcm.LANG');var WCMLANG=wcm.LANG={};window.onerror=function(message,url,line){if(Ext.isIE){s=message+"["+line+"@"+url+"]";var a=arguments.caller;var stack=[];stack.push(a.callee);while(true){a=a.caller;if(!a||stack.include(a.callee)){break;}
stack.push(a.callee);}
...
return false;}
Object.each=function(object,iterator){try{for(var sName in object){iterator(object[sName],sName);}}catch(e){if(e!=Ext.$break)throw e;}};var Form={serialize:function(form){var elements=Form.getElements($(form));var queryComponents=new Array();for(var i=0;i<elements.length;i++){var queryComponent=Form.Element.serialize(elements[i]);if(queryComponent)
queryComponents.push(queryComponent);}
return queryComponents.join('&');},getElements:function(form){form=$(form);var elements=new Array();for(tagName in Form.Element.Serializers){var tagElements=form.getElementsByTagName(tagName);for(var j=0;j<tagElements.length;j++){elements.push(tagElements[j]);}}
return elements;},getInputs:function(form,typeName,name){form=$(form);var inputs=form.getElementsByTagName('input');if(!typeName&&!name)
return inputs;var matchingInputs=new Array();for(var i=0;i<inputs.length;i++){var input=inputs[i];if((typeName&&input.type!=typeName)||(name&&input.name!=name))
continue;matchingInputs.push(input);}
return matchingInputs;},disable:function(form){var elements=Form.getElements(form);for(var i=0;i<elements.length;i++){var element=elements[i];element.blur();element.disabled='true';}},enable:function(form){var elements=Form.getElements(form);for(var i=0;i<elements.length;i++){var element=elements[i];element.disabled='';}},findFirstElement:function(form){return Form.getElements(form).find(function(element){return element.type!='hidden'&&!element.disabled&&['input','select','textarea'].includeI(element.tagName);});},reset:function(form){$(form).reset();}}
...
var $F=Form.Element.getValue;function $$F(_sName){var elements=document.getElementsByName(_sName);var values=[];for(var i=0;i<elements.length;i++){var element=elements[i];var method=element.tagName.toLowerCase();var parameter=Form.Element.Serializers[method](element);if(parameter)
...
return newElements;},loadXML:function(str){var xmlDoc=Try.these(function(){return new ActiveXObject('Microsoft.XMLDOM');},function(){return document.implementation.createDocument("","",null);})||false;xmlDoc.loadXML(str);return xmlDoc;},writeXML:function(_file,_sXML){if(Ext.isIE){var _lo=location.href.replace(/\/[^\/]*$/,'/');_file=_lo+_file;_file=_file.replace(/file:(\/)*/g,'').replace(/\//g,'\\');_file=unescape(_file);try{var fso=new ActiveXObject("Scripting.FileSystemObject");var oTextFile=fso.CreateTextFile(_file,true,true);oTextFile.Write(_sXML);oTextFile.Close();}catch(e){throw new Error("д�ļ�'"+file+"'ʧ��:"+e.message);}
return;}
_file=_file.replace(/file:(\/)*/g,'').replace(/\//g,'\\');_file=unescape(_file);try{netscape.security.PrivilegeManager.enablePrivilege("UniversalXPConnect");netscape.security.PrivilegeManager.enablePrivilege("UniversalBrowserWrite");var file=Components.classes["@mozilla.org/file/local;1"].createInstance(Components.interfaces.nsILocalFile);file.initWithPath(_file);if(!file.exists())
file.create(0,0664);var outputStream=Components.classes["@mozilla.org/network/file-output-stream;1"].createInstance(Components.interfaces.nsIFileOutputStream);outputStream.init(file,0x04|0x08|0x20,420,0);var sXML=this._convertFromUnicode("UTF8",_sXML);outputStream.write(sXML,sXML.length);outputStream.flush();outputStream.close();}catch(e){throw new Error("д�ļ�'"+_file+"'ʧ��:"+e.message);}},_convertFromUnicode:function(_sCharset,_sSrc){try{var unicodeConverter=Components.classes["@mozilla.org/intl/scriptableunicodeconverter"].createInstance(Components.interfaces.nsIScriptableUnicodeConverter);unicodeConverter.charset=_sCharset;return unicodeConverter.ConvertFromUnicode(_sSrc);}catch(e){}
return _sSrc;},toString:function(_oXmlDoc){return _oXmlDoc.xml;}};Ext.Json=com.trs.util.JSON={_json:function(_oJson,_sXPath,bCaseSensitive){var oRstJson=_oJson;if(_sXPath){try{var sXPath=_sXPath.trim();if(bCaseSensitive!=true){sXPath=sXPath.toUpperCase();}
...
if(Ext.isSimpType(oValue)){return _simpleJson;}
