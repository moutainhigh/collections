...
com.trs.tree.TreeNav.makeGetChildrenHTMLAction = function(_elElementLi){
	...
	return "tree_html_creator.jsp" + (location.search||'?1=1') + "&Type=0&FromSelect=1&ParentType=" + sParentType + "&ParentId=" + sParentId;
}
...
function getCheckValues4Disabled(){
	return TreeNav.getCheckValues('ChannelNav', false);
}
...
function expandTree(_nSiteId){
	...
	try{
		...
		if(sSelectedChannelIds != null && sSelectedChannelIds.length>0){
			...
			TreeNav.updateNodeChildrenHTML(sParentElementId, makeURLofGetHTMLContainsChannelIds(sSelectedChannelIds + ",<%=nCurrChannelId%>"), function(){
			
				if(nExpandNodeId>0){
					com.trs.tree.TreeNav.focus($(sExpandNodeType + "_"+ nExpandNodeId));
				}
			});
