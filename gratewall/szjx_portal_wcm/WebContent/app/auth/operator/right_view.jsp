<%@ page contentType="text/html;charset=UTF-8" pageEncoding="utf-8" errorPage="error.jsp" buffer="200kb"%>
...
<TITLE WCMAnt:param="right_view.jsp.title">权限查看</TITLE>
	...
	<table border=0 cellspacing=0 cellpadding=0 style="width:100%;height:100%;font-size:12px;padding:10px;">
		<tr>
			<td style="height:24px; padding:0px 20px;">
				...
				<div id="imgDescription" style="font-size:12px;clear:all;float:right;">
					<span WCMAnt:param="right_view.jsp.explain">(说明：</span>
					...
					<span style="padding-top:2px;font-size:12px;overflow:visible;" WCMAnt:param="right_view.jsp.description">表示权限来自用户，而非继承于用户组、角色或逻辑权限.</span>&nbsp;)
		...
		<tr>
			<td style="height:24px; padding:0px 20px;">
				<div>
					<span onclick="showType('site');" style="cursor:pointer;"><input type="radio" id="siteRadio" name="rightTypeTab" checked value=""><span WCMAnt:param="right_view.jsp.siteright">站点类操作权限</span></span>
					<span onclick="showType('channel');" style="cursor:pointer;"><input type="radio" id="channelRadio" name="rightTypeTab" value=""><span WCMAnt:param="right_view.jsp.channel">栏目类操作权限</span></span>
					<span onclick="showType('document');" style="cursor:pointer;"><input type="radio" id="documentRadio" name="rightTypeTab" value=""><span WCMAnt:param="right_view.jsp.doc">文档类操作权限</span></span>
					<span onclick="showType('template');" style="cursor:pointer;"><input type="radio" id="templateRadio" name="rightTypeTab" value=""><span WCMAnt:param="right_view.jsp.temp">模板类操作权限</span></span>
				<%--if(isAdmin(nOperType, nOperId)){--%>
					<span onclick="showType('flow');" style="cursor:pointer;"><input type="radio" id="flowRadio" name="rightTypeTab" value=""><span WCMAnt:param="right_view.jsp.workflow">工作流类操作权限</span></span>
				<%--}--%>
				</div>
			</td>
		</tr>
						...
						<!-- 左边的导航树//-->
							<table cellspacing=0 cellpadding=0 id="leftNavTree" class="TreeView" style="width:100%;border-left:1px soild gray;" >
								<tr class="fixTableHead grid_head" id="treeTitle">
									<td style="text-align:left;padding-left:10px;"><%=LocaleServer.getString("right_view.label.site", "站点:")%>
									<div id="r_-1" style="display:inline;">
									<select onchange="onChange(this);" id="siteSel" style="width:150px;">
										<%
											//复用treecreator中取站点的过程，
											//保持right_view.js(SitesLoadMgr.loadSites();)和right_view.jsp中所取站点逻辑的一致性
...
<%!
	...
	private String getOperDesc(int _nOperId,int _nOperType) throws WCMException{
		switch(_nOperType){
			...
			default : {
				throw new WCMException(LocaleServer.getString("right_view.jsp.label.invalidoperator","无效的操作者类型!"));
			}
		}
	}
