...
<TITLE WCMAnt:param="right_edit_whole.jsp.title">TRS WCM操作者视图权限设置</TITLE>
...
<script language="javascript" src="right_edit_whole.js" type="text/javascript"></script>
		...
		<tr>
			<td style="height:30px; padding-right:0px 20px;">
				<div style="float:left;">
					...
					<span style="background:url('../../images/auth/save_one.png') no-repeat; padding-left:16px;padding-top:1px;cursor:pointer;" id="saveCurrRowId" WCMAnt:param="right_edit_whole.jsp.save">保存选定行</span>
					(<span style="padding:0px 3px;background-color:#aabbcc;">&nbsp;</span><span WCMAnt:param="right_edit_whole.jsp.exp">标注</span>)&nbsp;&nbsp;
					<span style="background:url('../../images/auth/save_all.png') no-repeat; padding-left:16px;padding-top:1px;cursor:pointer;" id="saveAllRowsId" WCMAnt:param="right_edit_whole.jsp.saveall">保存全部</span>
					(<span style="padding:0px 3px;" class="updateRow">&nbsp;</span><span WCMAnt:param="right_edit_whole.jsp.exp">标注</span>)&nbsp;&nbsp;				
				</div>
				<div style="float:right;">
					<span onclick="showType('site');" style="cursor:pointer;"><input type="radio" id="siteRadio" name="rightTypeTab" checked value="site"><span WCMAnt:param="right_edit_whole.jsp.siteright">站点类操作权限</span></span>
					<span onclick="showType('channel');" style="cursor:pointer;"><input type="radio" id="channelRadio" name="rightTypeTab" value="channel"><span WCMAnt:param="right_edit_whole.jsp.channel">栏目类操作权限</span></span>
					<span onclick="showType('document');" style="cursor:pointer;"><input type="radio" id="documentRadio" name="rightTypeTab" value="document"><span WCMAnt:param="right_edit_whole.jsp.doc">文档类操作权限</span></span>
					<span onclick="showType('template');" style="cursor:pointer;"><input type="radio" id="templateRadio" name="rightTypeTab" value="template"><span WCMAnt:param="right_edit_whole.jsp.temp">模板类操作权限</span></span>
				<%--if(loginUser.isAdministrator()){--%>
					<span onclick="showType('flow');" style="cursor:pointer;"><input type="radio" id="flowRadio" name="rightTypeTab" value="flow"><span WCMAnt:param="right_edit_whole.jsp.workflow">工作流类操作权限</span></span>
				<%--}--%>
				</div>
				<div id="imgDescription" style="font-size:12px;clear:all;display:none;">
					(<span WCMAnt:param="right_edit_whole.jsp.explain">说明：</span>
					<span style="background:url('../../images/auth/enable.gif') no-repeat; padding-left:16px;padding-top:2px;font-size:12px;overflow:visible;" title="单击此类图标将删除本用户自身的权限&#13;此时用户权限将继承自用户组、角色或逻辑权限" WCMAnt:param="right_edit_whole.jsp.description">表示权限来自用户，而非继承于用户组、角色或逻辑权限</span>&nbsp;
					)
						...
						<!-- 左边的导航树//-->
							<table cellspacing=0 cellpadding=0 id="leftNavTree" class="TreeView" style="width:100%;border-left:1px soild gray;" >
								<tr class="fixTableHead grid_head" id="treeTitle">
									<td style="text-align:left;padding-left:10px;" WCMAnt:param="right_edit_whole.jsp.sourcename">资源名称</td>
								</tr>
								<tr>
									<td style="text-align:left;padding-left:10px;padding-right:5px;">
										<DIV class="SiteType0Root" id="r_0" title="文字库" WCMAnt:paramattr="title:right_edit_whole.jsp.wordsitetype" classPre="SiteType0">
											<A href="#" WCMAnt:param="right_edit_whole.jsp.wordsitetype">文字库</A>
										</DIV>
									</td>
								</tr>
								<tr>
									<td style="text-align:left;padding-left:10px;padding-right:5px">
										<DIV class="SiteType1Root plugins" id="r_1" title="图片库" WCMAnt:paramattr="title:right_edit_whole.jsp.imgsitetype" classPre="SiteType1">
											<A href="#" WCMAnt:param="right_edit_whole.jsp.imgsitetype">图片库</A>
										</DIV>
									</td>
								</tr>
								<tr>
									<td style="text-align:left;padding-left:10px;padding-right:5px">
										<DIV class="SiteType2Root plugins" id="r_2" title="视频库" WCMAnt:paramattr="title:right_edit_whole.jsp.videositetype" classPre="SiteType2">
											<A href="#" WCMAnt:param="right_edit_whole.jsp.videositetype">视频库</A> 
										</DIV>
									</td>
								</tr>
								<tr>
									<td style="text-align:left;padding-left:10px;padding-right:5px">
										<DIV class="SiteType4Root plugins" id="r_4" title="资源库" WCMAnt:paramattr="title:right_edit_whole.jsp.source" classPre="SiteType4">
											<A href="#" WCMAnt:param="right_edit_whole.jsp.source">资源库</A> 
										</DIV>
									</td>
								</tr>
							</table>
...
private String getOperDesc(int _nOperId, int _nOperType)
			throws WCMException {
		switch (_nOperType) {
		case User.OBJ_TYPE: {
			...
			if (currUser == null)
				throw new WCMException(CMyString.format(LocaleServer.getString(
						"right_edit_whole.jsp.usernotfoundexception",
						"没有找到[id={0}]的用户!"), new int[] { _nOperId }));
			return "<span style='background:url(\"../../images/auth/icon_user.gif\") no-repeat;padding-left:16px;'>"
					+ currUser + "</span>";
		}
		case Group.OBJ_TYPE: {
			...
			if (currGroup == null)
				throw new WCMException(CMyString.format(LocaleServer.getString(
						"right_edit_whole.jsp.orgnotfoundexception",
						"没有找到[id={0}]的组织!"), new int[] { _nOperId }));
			return "<span style='background:url(\"../../images/auth/icon_group.gif\") no-repeat;padding-left:16px;'>"
					+ currGroup + "</span>";
		}
		case Role.OBJ_TYPE: {
			...
			if (currRole == null)
				throw new WCMException(CMyString.format(LocaleServer.getString(
						"right_edit_whole.jsp.rolenotfoundexception",
						"没有找到[id={0}]的角色!"), new int[] { _nOperId }));
			return "<span style='background:url(\"../../images/auth/icon_role.gif\") no-repeat;padding-left:16px;'>"
					+ currRole + "</span>";
		}
		default: {
			throw new WCMException(LocaleServer.getString(
					"right_edit_whole.jsp.label.invalidoperator", "无效的操作者类型!"));
		}
		}
	}
