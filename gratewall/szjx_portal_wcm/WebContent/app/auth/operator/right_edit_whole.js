...
function setTreeNodeHTML(newRowObj, newRowInfoObj, parentRowInfoObj){
	...
	var sHtml = [
		...
		'>',
			'<a href="#">',
				'<input name="c', newRowInfoObj.id, '" type="checkbox" boxType="tree"/>',
					newRowInfoObj.desc,
			'</a>',
		'</div>'
	].join("");
...
function saveRights(params){
	BasicDataHelper.call('wcm6_auth', 'saveOperatorRights', params.parseQuery(), true, saveSuccessCallBack, failure);
}
