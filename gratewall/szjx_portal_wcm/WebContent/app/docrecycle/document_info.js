...
function getOperationDesc(_sType){
	if(window.m_arrOptDescs == null) {
		window.m_arrOptDescs = {
			_copy : wcm.LANG.DOCRECYLE_DESC_12 || '您正准备复制如下文档：',
			_move : wcm.LANG.DOCRECYLE_DESC_13 || '您正准备移动如下文档：',
			_quote : wcm.LANG.DOCRECYLE_DESC_14 || '您正准备引用如下文档：',
			_forcedelete : wcm.LANG.DOCRECYLE_DESC_1 || '您正准备强制删除如下文档：',
			_delete : wcm.LANG.DOCRECYLE_DESC_2 || '您正准备删除如下文档：',
			_restore : wcm.LANG.DOCRECYLE_DESC_3 || '您正准备还原如下文档：',
			_cannot_restore_all : wcm.LANG.DOCRECYLE_DESC_4 || '您正准备还原如下文档（含有不能还原的文档）：',
			_trash : wcm.LANG.DOCRECYLE_DESC_5 || '您正准备将如下文档放入废稿箱：',
			_cannot_restore : wcm.LANG.DOCRECYLE_DESC_6 || '由于以下原因，您无法进行文档还原操作！',
			_channel_deleted : wcm.LANG.DOCRECYLE_DESC_7 || '无法还原：文档所在栏目尚在栏目回收站内',
			_docchnl_deleted : wcm.LANG.DOCRECYLE_DESC_8 || '无法还原：文档所属栏目尚在栏目回收站内',
			_entity_deleted : wcm.LANG.DOCRECYLE_DESC_9 || '无法还原：引用文档的实体尚在废稿箱内'
		}
	}
	var result = window.m_arrOptDescs[_sType] ||  (wcm.LANG.DOCRECYLE_DESC_11 || '未知操作');
//*/
	return result;
}
...
function updateRefChannels(index, channels){
	if(channels.length == 1){
		...
		var sHtml = "[" + channels.slice(0, channels.length - 1).join("], [") + (wcm.LANG.DOCRECYLE_DESC_29||"] 和 [") + channels[channels.length-1] + "]";
		Element.update('refChannels_'+ index, sHtml);
	}else{
		var sHtml = "[" + channels[0] + (wcm.LANG.DOCRECYLE_DESC_29||"] 和 [") + channels[channels.length-1] + "]";
		Element.update('refChannels_'+ index, sHtml);
	}
}