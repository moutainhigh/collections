...
//2. 获取业务数据
	...
	String selectType = CMyString.showEmpty(oMethodContext.getValue("selectType"), "checkbox");
...
<table cellspacing=0 border="1" cellpadding=0 id="grid_table" class="grid_table" borderColor="gray">
	<tr id="grid_head" class="grid_head">
		<td <%=selectType.equals("radio") ? "" : "onclick=wcm.Grid.selectAll('RecId');"%> width="50" WCMAnt:param="document_select_query.jsp.selectorselectall"><%=selectType.equals("radio") ? "选择" : "全选"%></td>
...
%>
	<tr id="tr_<%=nRecId%>" rowid="<%=nRecId%>" class="<%=(bCanDetail)?"grid_row":""%> <%=sRowClassName%><%=(bIsSelected)?" grid_row_active":""%> <%=(bCanDetail)?"grid_selectable_row":"grid_selectdisable_row"%>" right="<%=sRightValue%>" doctype="<%=nDocType%>" isTopped="<%=bTopped%>" channelid="<%=nDocChannelId%>" currchnlid="<%=nChnlId%>" docid="<%=nDocId%>" doctitle="<%=sTitleHtmlValue%>">
		<td><input type="<%=CMyString.filterForHTMLValue(selectType)%>" id="cb_<%=nRecId%>" class="grid_checkbox" name="RowId" value="<%=nRecId%>" <%=(bCanDetail)?"":"disabled"%> <%=(bIsSelected)?"checked":""%>/><span class="grid_index" id="grid_index_<%=nRecId%>"><%=i%></span></td>
