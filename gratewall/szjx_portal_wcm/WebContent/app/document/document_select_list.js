...
Ext.apply(PageContext, {
	...
	pageFilters : (function(){
		...
		var bOnlyPublished = PageContext.getParameter("OnlyPublished")
		if(!bOnlyPublished || bOnlyPublished == 0){
...
$MsgCenter.on({
	...
	afterunselect : function(event){
		if(event.from()==wcm.getMyFlag())return;
		var nRecId = objs.getAt(i).getId();
		var row = wcm.Grid.getRow(nRecId);
		if(!row)return;
		wcm.Grid.unselectRow(row, true);
	},
	afterselect : function(event){
		if(event.from()==wcm.getMyFlag())return;
