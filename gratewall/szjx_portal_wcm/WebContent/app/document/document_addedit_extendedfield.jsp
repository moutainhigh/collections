...
<%!
		private String showExtendFields(ContentExtFields currExtendedFields,Document currDocument,String strDBName,boolean bReadOnly,Channel oChannel,boolean isInheritFormWebSite) throws Throwable{
		String sRetVal = "";
		if(currExtendedFields != null && !currExtendedFields.isEmpty()){
			...
			for(int i=0; i<currExtendedFields.size(); i++){
				try{
					...
					//另外，当栏目继承自站点的扩展字段（站点的扩展字段同步到栏目），需要在站点上做判断 by@sj 2013-1-11
					if(isInheritFormWebSite) {
						if(isHasShowField(oChannel.getSite(),sExtFieldName))continue;
					}else {
	...
	private boolean isHasShowField(WebSite currWebSite,String sExtFieldName){
