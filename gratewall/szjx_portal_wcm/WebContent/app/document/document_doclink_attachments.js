...
Event.observe(window, 'load', function(){
	Ext.get('AppLink').on('click', function(event, target){
		if(target.getAttribute('grid_function', 2)){
			return doGridFunction(event, target);
		}
	});

	Ext.get('AppLink').on('keydown', function(event, target){
	...
	if(currDocumentId > 0){
		initLinksAppendixInfo(currDocumentId);
	}else{
		showLinksGrid();
	}
...
function initLinksAppendixInfo(currDocumentId){
	/**
	...
	BasicDataHelper.call('wcm61_viewdocument','queryAppendixes',postData,true,function(_transport,_json){
		var result = {
			Type_40 : Ext.Json.parseXml(_transport.responseXML)
		};
		m_Appendixes = result;
		showLinksGrid();
	});
