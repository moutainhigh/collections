...
//decodeURIComponent//encodeURIComponent//unescape//escape
...
window.onload = function()
{
	...
	if(FCKConfig.LinkSimple){
		Element.hide('spLinkType');
		SetLinkType( 'url' ) ;
	}
	else{
	...
	SetDefaultTarget() ;
	//设置选中的文本值
	var  text =FCK.EditorDocument.selection.createRange().text;
	if(text && text.length>0){
		GetE('LinkTextInput').value=text;
	}
...
function LoadSelection()
{
	...
	if ( sProtocol )
	{
		...
		if (false && sProtocol == 'mailto:' )	// It is an e-mail link.
		{
			sType = 'email' ;
...
//		GetE('cmbAnchorName').value = GetE('cmbAnchorId').value = sHRef.substr(1) ;
	...
	{
		sUploadFileName = oLink.getAttribute( 'uploadPic' , 2 );
		if(sUploadFileName!=null){
			sType = 'file' ;
			GetE('spnCurrFile').style.display = '';
			GetE('CurrFile').innerHTML = GetE('CurrFile').title = sHRef;
		}
		else{
			sType = 'url' ;
...
function SetLinkType( linkType )
{
	...
	ShowE('divLinkTypeFile'		, (linkType == 'file') ) ;

	ShowE('divLinkTarget'			, (linkType != 'anchor') ) ;
	ShowE('divLinkText'				, (linkType != 'anchor'))
	if ( !FCKConfig.LinkDlgHideTarget )
...
{
	if(!_bUploaded&&GetE('cmbLinkType').value=='file'){
	...
	switch ( GetE('cmbLinkType').value )
	{
		...
		case 'file' :
			var sFileUrl = GetE('FileUpload').value ;
			//TODO
			sUri = sFileUrl ;
			break;
	}
...
//					sInnerHtml = asLinkPath[1];  // use matched path
	...
	for ( var i = 0 ; i < aLinks.length ; i++ )
	{
		...
		if(!(oOrig&&GetE('cmbLinkType').value=='file'&&sUri.length==0)){
			oLink.href = sUri ;
			SetAttribute( oLink, '_fcksavedurl', sUri ) ;
			if(GetE('cmbLinkType').value=='file'){
		...
		// 不管有选中还是没有选中，都使用输入的值
		var value=GetE('LinkTextInput').value.trim();
		//对于多个链接，链接描述文字不能简单的定义为一个
		if(value.length>0 && aLinks.length == 1)
			oLink.innerHTML = value;
		else
...
function UploadFile(_fCallBack){
	...
	try{
		FileUploadHelper.validFileExt(sFileName, ".*");
	}catch(err){
	...
	try{
		YUIConnect.asyncRequest('POST','../../../system/file_upload_dowith.jsp?ResponseType=2&FileParamName=FileUpload&Type=LOCAL_FILE_LIMIT',callBack);
		clearInterval(lTimeout);
		Element.hide('upload_message');
	}catch(err){
	}
...
function setDefaultShowText(_filePath){
	if(FCK.EditorDocument.selection.createRange().text.length>0 &&GetE('LinkTextInput').value.length!=0 )
		return;
	var nIndex=_filePath.lastIndexOf("\\");
	var fileName = _filePath.substring(nIndex+1);
	GetE('LinkTextInput').value=fileName;
