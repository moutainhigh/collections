...
function init(sAttributes){
	...
	*/
	//初始化状态按钮
	if(oAttributes['isForWord']=="1") {
		$("defautlFormat_forword_checked").checked = true;
	}else {
		$("defautlFormat_forword_checked").checked = false;
	}
	if(oAttributes['isStatus']=="1") {
		$("defautlFormat_status_checked").checked = true;
	}else if(oAttributes['isInherit']=="1") {
		$("defautlFormat_status_inherit").checked = true;
	}else if (oAttributes['isStatus']=="0") {
	...
	*对选中项产生css预览的操作用监听处理
	*/
	var element_mt = $("d_inpmargintop");
	var element_mtv = $("v_inpmargintop");
	var element_mb = $("d_inpmarginbottom");
	var element_mbv = $("v_inpmarginbottom");
	var element_lh = $("d_inplineheight");
	var element_lhv = $("v_inplineheight");
	var element_fontSize = $("d_selfontsize");
	var element_fontName = $("d_selfontname");
	var handler = function(){
		var oCurrStyle = getCssStyle(EventUtil.getAttributes());
		var strCurrStyle = transObj2Str(oCurrStyle);
		var text = getStyle(oCurrStyle,strCurrStyle);
		$("preview_box").innerHTML = text;
	}
	...
	var displayForChecked = function(){
		if($("defautlFormat_status_inherit").checked || $("defautlFormat_status_nochecked").checked) {
	...
	handler();aboutAllDelCheck();aboutAllKeepCheck();
	displayForChecked();
...
function myDo(){
	...
	var oPostData = {
		ObjType		:	getParameter('ObjType'),
		ObjId		:	getParameter('ObjId'),
		Status		:	bStatus,//$("defautlFormat_status_checked").checked,
		Inherit		:	$("defautlFormat_status_inherit").checked ? 1 : 0,//1:使用继承设置;0:使用自身设置
...
function getCssStyle(_attribute){
	...
	oCurrStyle[".TRS_Editor P"] = {
		"font-family" : oStyle["d_selfontname"] ? oStyle["d_selfontname"] : "",
		"font-size" : oStyle["d_selfontsize"] ? oStyle["d_selfontsize"] : "",
		"margin-top" : (oStyle["d_inpmargintop"]=="1") ? oStyle["v_inpmargintop"] : 0,
		"margin-bottom" : (oStyle["d_inpmarginbottom"]=="1") ? oStyle["v_inpmarginbottom"] : 0
	}
	//两端对齐
	if(oStyle["d_addjustify"] == "1") {
		oCurrStyle[".TRS_Editor P"]["align"] = "justify";
	}
	oCurrStyle[".TRS_Editor P"]["line-height"] = (oStyle["d_inplineheight"]=="1") ? oStyle["v_inplineheight"] : 1;
	...
	oCurrStyle[sPreCssText]["line-height"] = (oStyle["d_inplineheight"]=="1") ? oStyle["v_inplineheight"] : 1;
