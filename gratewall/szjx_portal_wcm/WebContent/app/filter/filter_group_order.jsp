...
<%
	...
	if(currFilter == null){
		throw new WCMException(ExceptionNumber.ERR_OBJ_NOTFOUND,CMyString.format(LocaleServer.getString("filter_group_order.jsp.notfindfilterid", "没有找到ID为{0}的筛选器！"), new int[]{nFilterId}));
	}
	HashMap parameters = new HashMap();
	parameters.put("ObjectId", sFilterId);
	FilterOptionGroups oFilterOptionGroups = (FilterOptionGroups)processor.excute("wcm61_filtercenter", "queryFilterOptionGroupsByFilter", parameters);
	out.clear();
%>
<HTML>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title WCMAnt:param="filter_group_order.jsp.trswcmfiltergrouporder">TRS WCM ::::::过滤选项组排序</title>
			...
			<TR>
				...
				<TABLE width="100%" border="0" cellpadding="0" cellspacing="1"
					...
					<TR bgcolor="#BEE2FF" class="list_th">
						<TD bgcolor="#BEE2FF" WCMAnt:param="filter_group_order.jsp.optiongroupname">选项组名称</TD>
						<TD bgcolor="#BEE2FF" WCMAnt:param="filter_group_order.jsp.order">排序</TD>
					</TR>
					<tbody id="table_tbody">
		<%
			 for(int i=0;i<oFilterOptionGroups.size();i++){
				 FilterOptionGroup filterOptionGroup = (FilterOptionGroup)oFilterOptionGroups.getAt(i);
				 int nOptionGroupId = filterOptionGroup.getId();
				 int nOrder = filterOptionGroup.getOrder();
		%>
					<TR class="list_tr" id="tr_<%=nOptionGroupId%>">
						<TD align="center"><%=CMyString.transDisplay(filterOptionGroup.getOptionGroupName())%></TD>
						<TD align="center"><input type="text" name="orderText" id="orderText_<%=nOptionGroupId%>" value="<%=nOrder%>" class="orderText" _id="<%=nOptionGroupId%>" rowNum ="<%=nOrder%>" _value="<%=nOrder%>"/>
						</TD>
					</TR>
		<%
			}
...
var OrderHandler = {
	...
	changeValue : function(event, dom){
		...
		BasicDataHelper.call("wcm61_filtercenter", 'saveFilterOptionGroup', oPostData, true, function(transport, json){
