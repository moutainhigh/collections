...
var AbstractCustomTabDealer = {
    ...
    */
	newOrUpdate : function(newUpdateFlag){
		...
		var sUrl = WCMConstants.WCM6_PATH + 'individuation/custom_channel.html';
...
//定制的栏目
var CustomChannelTabDealer = {};
Object.extend(CustomChannelTabDealer, AbstractCustomTabDealer);
Object.extend(CustomChannelTabDealer, {
	initCustomChannelTabEvent : function(){
		this.initAbstractCustomTabEvent();
	},

	initCustomChannelTabValue : function(){
		//this.refresh();
		this.initAbstractCustomTabValue();
	},

	initCustomChannelTab : function(){//初始化自定义栏目页
		this.customContainer = $('customChannelContainer');
		this.customType = 'Channel';
		this.customContainer = $('customChannelContainer');
		this.newBtn = $('customChannelNew');
		this.updateBtn = $('customChannelUpdate');
		this.removeBtn = $('customChannelRemove');
		this.selectAllBtn = $('customChannelSelectAll');
		this.deselectAllBtn = $('customChannelDeselectAll');

		this.initCustomChannelTabEvent();
		this.initCustomChannelTabValue();
	}
});

//点击个性化设置页面的"自定义栏目"节点时，触发的动作
function customChannelTabLoad(){
	if(!CustomChannelTabDealer.isLoaded){
		CustomChannelTabDealer.initCustomChannelTab();
		CustomChannelTabDealer.isLoaded = true;
	}
}

//Event.observe(window, 'load', CustomChannelTabDealer.initCustomChannelTab.bind(CustomChannelTabDealer));