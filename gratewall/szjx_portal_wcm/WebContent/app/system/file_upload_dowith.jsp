...
try{
	...
	HttpServletRequest newRequest = oUploadHelper;

	String sFileParamName = newRequest.getParameter("FileParamName");
	String sResponseType = newRequest.getParameter("ResponseType");
	String sType = newRequest.getParameter("Type");
	int nChannelId = 0;
	String sysConfigImageSize = null;
	int nSysConfigImageSize = 0;
	boolean bWebSetConfig = true;
	if(!CMyString.isEmpty(sType)){
		...
		if(!CMyString.isEmpty(sWebSiteAttName)){
			String sChannelId = newRequest.getParameter("channelId");
		...
		if (oFileHelper != null && oFileHelper.getSize() > 0){
			try{
				...
				if(!CMyString.isEmpty(sysConfigImageSize)
					...
					// 如果是取的站点上设置的配置值
					if(bWebSetConfig){
						sErrorMsg =CMyString.format(LocaleServer.getString("file_upload_dowith.filesize.toolarge1","上传文件大小超过当前站点设置的最大值 {0}K，请重新选择文件！"),
			...
			String sWmpic = newRequest.getParameter("wmpic");
			if(sWmpic != null && sWmpic.trim().length() > 0){//加水印
				String sPos = newRequest.getParameter("wmpos");
				int[] arPos = {3};
				if(sPos != null){
...
//7.结束
	...
	out.flush();
	if(true) return;//avoid tailed LR/LF.
}catch(Throwable ex){
	int errorCode = 0;
