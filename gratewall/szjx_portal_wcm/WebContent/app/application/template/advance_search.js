...
Object.extend(ViewDataEditor, {
	bindEvents : function(){
		...
		Event.observe('copyToClipBoard', 'click', function(){
			try{
				var mainWin = (top.actualTop||top).document.getElementById("main").contentWindow;
				...
				for (var sName in params){
					if(params[sName] === '' || params[sName] == null) continue;
					sParams += "&" + sName + "=" + params[sName];
				}
				if(sParams.charAt(0) == '&') sParams = "?" + sParams.substring(1);
				Ext.copy(sParams);
				Ext.Msg.alert(wcm.LANG.METAVIEWDATA_88 || "已经复制到剪切板中！");
			}catch(ex){
				Ext.Msg.alert(wcm.LANG.METAVIEWDATA_89 || '您的浏览器不支持自动复制操作');
			}
		});
...
/*-----------------------------------------command button end---------------------------------*/
function search(){
	//这里利用return 为了使当有弹出框时，弹出框关闭时return false可以阻断事件传播，使页面不关闭。
	checkSQL(null, function(){
		return ViewDataEditor.search();
	}, function(sHtml){
		Ext.Msg.warn(sHtml);
	});
	return false;
...
function checkSQL(_oPostData, _fValid, _fInValid){
	if($('_sqlWhere_').value.trim() == ''){
		(_fValid || Ext.emptyFn)();
		return;
	}
	var oHelper = new com.trs.web2frame.BasicDataHelper();
	var params = formatParamsForMode(getParams(false));
	Ext.apply(params, {ChannelId : getParameter('ChannelId') || 0});
	oHelper.JspRequest('metaviewdata_query.jsp', params, true, function(oTrans, oJson, ajaxObj){
		var isError = ajaxObj.header('TRSException');
		if(isError == null || isError=="") {
			(_fValid || Ext.emptyFn)();
			return;
		}
		var sDetail = oJson.FAULT.DETAIL;
		(_fInValid || Ext.emptyFn)(sDetail);
	}, function(oTrans, oJson, ajaxObj){
		var isError = ajaxObj.header('TRSException');
		if(isError == null) {
			(_fValid || Ext.emptyFn)();
			return;
