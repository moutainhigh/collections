...
<title WCMAnt:param="metaviewdata_addedit.jsp.title">新建/修改[下级公开信息]数据</title>
		...
		jQuery.noConflict()(function() {
			jQuery("#tabs").tabs({
				show: function(event, ui) {
					/*
					 *注释以下，这样会在ie10中导致页面有时加载不出来及按钮初始化问题
					 *被迫暂时牺牲性能
					 */
					//已经加载，则不再加载
...
<!--
	new com.trs.ui.XText({
		...
		validation:"type:'string',required:'0',max_len:'200',desc:'发布机构'"
	}).render();
//-->
</script>
								</div>
...
<!--
	new com.trs.ui.XEditor({
		...
		validation:"type:'string',required:'0',desc:'检索的内容'",
		value : '<%=CMyString.filterForJs(makeHtmlCon(getValue(obj, "SearchContent", "")))%>'
	}).render();
//-->
</script>
								</div>
...
<!--
	new com.trs.ui.XText({
		disabled : 0,
		name : 'svobjcat',
		value : '<%=CMyString.filterForJs(getValue(obj, "svobjcat", ""))%>',
		validation:"type:'string',required:'0',max_len:'200',desc:'服务对象'"
	}).render();
//-->
</script>
								</div>
...
//-->
		...
		<div id="tabs-others">
				...
				%>
					<div class="row" id="selectChannelDiv">
