...
(function(){
	...
	var imgUploadingTip = '-uploading-tip';
	var template = [
		'<div class="XAppendix">',
			...
			'<form name="{0}-frm" id="{0}-frm" method="post" enctype="multipart/form-data">',
				...
				'<span id="{0}', imgUploadingTip, '" style="display:none;" class="appendix-uploading-tip">uploading...</span>',
			'</form>',
		'</div>'
	].join("");
	...
	com.trs.ui.XAppendix = Ext.extend(com.trs.ui.BaseComponent, {
		...
		initActions : function(){
			...
			Event.observe(sBrowserBtn, 'change', function(){
				...
				//上传附件
				Element.show(sName + imgUploadingTip);
				caller.upload(function(){
					Element.hide(sName + imgUploadingTip);
				});
			});
		...
		*/
		...
		setValue : function(info){
			if(typeof info == 'string'){
				var sName = this.initConfig['name'];
				$(sName).value = info;
				Element.update(this.initConfig['name'] + appendixText, info);
				Element.show(sName + appendixDeleteBtn);
			}
		},
		upload : function(fSuccess, fFailure){
			...
			sParams+= "&Type="+Type;
			// 可以设置上传文件的路径
			if(this.initConfig.pathFlag)
				sParams+="&pathFlag="+this.initConfig.pathFlag;
			...
			YUIConnect.asyncRequest('POST',
				this.uploadUrl + '?'+sParams, {
					"upload" : function(_transport){
						...
						caller.setValue(result["Message"]);
						caller.fireEvent('upload', result);
						$(sName + appendixForm).reset();//清空form，防止上传出错时，下次再次上传
						if(fSuccess) fSuccess(result["Message"],result["Path"]);
					}
				}
			);
		}
