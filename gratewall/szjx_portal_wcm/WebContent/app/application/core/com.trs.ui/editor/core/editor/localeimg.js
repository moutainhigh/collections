...
var FileUploadHelper = {
	validFileExt :function(_strPath,_sAllowExt,_fAlert){
		...
		if(_strPath.length<=0){
			(_fAlert)(wcm.LANG.localeimg_1000 || "没有选择文件");
			return false;
		}
			...
			if(texts[0]==0){
				...
				if(window.FaultDialog){
					FaultDialog.show({
						...
						detail		: texts[2],
						suggestion  : ''
					}, wcm.LANG.localeimg_1003 || '与服务器交互时出错啦');
				}
			}
			if(_fErr)(_fErr)();
...
function toggleOptions(){
	...
	if (icon && link) {
		...
		if (!showOptions) {
			...
			icon.alt = link.innerHTML = wcm.LANG.localeimg_1004 || '隐藏图片选项';
			...
			icon.alt = link.innerHTML = wcm.LANG.localeimg_1005 || '显示图片选项';
