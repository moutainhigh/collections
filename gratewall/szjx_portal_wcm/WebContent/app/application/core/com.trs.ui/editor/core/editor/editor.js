...
/// Word////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var editorUrl = window.location.protocol + "//" + window.location.host + "/wcm/app/neweditor"
EDiaryEditor = {
	...
	initView: function (sParentID) {
		...
		var _toolbarStr = "\
			<div id='EDiaryEditor'style='width: 495px; padding-left:2px;'>\
				...
				<div style='height: 250px; width: 582px!important ; width: 495px;' id='EDiaryEditorContent'>\
					<iframe id='editor_body_area' name='editor_body_area' style='width: 493px!important ; width: 493px; height: 250px; border:1px solid #abc0db; overflow: atuo; display: nones;' frameborder='0'></iframe>\
					<textarea id='editor_body_textarea' name='editor_body' style='width: 493px; height: 252px; border-width: 0px; padding-top:8px; display: none;'></textarea>\
		...
		for(var key in _toolBarInfo) {
			_toolbarStr2 += "<img unselectable='on' src='images/editor/blank.gif' title='" + _toolBarInfo[key]["a"] + "' alt='" + _toolBarInfo[key]["a"] + "' id='EDiaryEditor" + _toolBarInfo[key]["n"] + "' style='-moz-user-select:none;position: absolute; left: " + (_toolBarInfo[key]["l"]-72) + "px; top: " + _toolBarInfo[key]["t"] + "px; width: " + _toolBarInfo[key]["w"] + "px; height: " + _toolBarInfo[key]["h"] + "px;' func='" + _toolBarInfo[key]["n"] + "' action='" + _toolBarInfo[key]["c"] + "'>";
		}
			...
			for(var key2 in _toolMenuInfo[key]) {
					...
					if(key2 == 0) {// head
						_toolbarStr3 += "\
							...
							<div style='cursor: default; position: absolute; width: 163px; top: 32px; left: 6px; display: none;' id='EDiaryEditorFontFamilyItem'>\
								<div style=' height: 210px!important ;height: 160px; background-image: url(images/editor/menu_bg.gif);' id='EDiaryEditorFontFamilyBox'>\
						";
					}
					...
					if(key2 == 0) {// head
						_toolbarStr3 += "\
							...
							<div style='cursor: default; position: absolute; width: 163px; top: 32px; left: 103px; display: none;' id='EDiaryEditorFontSizeItem'>\
								<div style='height: 270px!important ;height: 160px; background-image: url(images/editor/menu_bg.gif);' id='EDiaryEditorFontSizeBox'>\
						";
					}
					if(key2 < _toolMenuInfo[key].length) {// body
				...
				if(key == "Style") {
					...
					if(key2 == 0) {// head
						_toolbarStr3 += "\
							<!-- Font Style Menu -->\
							<div style='cursor: default; position: absolute; width: 163px; top: 32px; left: 171px; display: none;' id='EDiaryEditorFontStyleItem'>\
								<div style='text-align: center; height: 20px; background-image: url(images/editor/menu_title.gif); padding-top: 6px; padding-left: 6px;'><b WCMAnt:param='editor.html.duanLuo'>段落</b></div>\
									<div style='height: 180px!important ;height: 187px; background-image: url(images/editor/menu_bg.gif);' id='EDiaryEditorFontStyleBox'>\
						";
					}
					if(key2 < _toolMenuInfo[key].length) {// body
	...
	initControl: function (bWordMode, sContent) {
		...
		function viewDialog(el, Act) {
			...
			if(window._isIE) {
				...
				if(Act == "img") {
					window.showModalDialog("editor/localeimg.htm", window, "dialogWidth: 500px; dialogHeight: 200px; help: no; scroll: no; status: no");
				}
			}
			else {
				if(Act == "img") {
	...
	runCMD: function (CMD, sValue) {
		...
		if(CMD == "" || CMD == null)return;
			...
			*/
			...
			//update for complex editor.
			//var feature = "fullscreen =yes, resizable=no, menubar=no, toolbar =no, scrollbars=yes, titlebar=no";

			var url = '/wcm/app/editor/editor.jsp?ChannelId='
	...
	addHTML: function (oParentNode, sHTML) {
		...
		if(window.addEventListener && !(ua.indexOf("msie 9") > -1)) {// for MOZ
