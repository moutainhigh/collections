...
<%
	String sStepId = processor.getParam("StepId");
	int nStepId = Integer.parseInt(sStepId);
	Step currStep = Step.findById(nStepId);
	if(currStep == null){
		throw new WCMException(ExceptionNumber.ERR_OBJ_NOTFOUND, CMyString.format(LocaleServer.getString("advisor_optiongroup_order.jsp.notfindadvisoridstep", "没有找到ID为{0}的顾问步骤！"), new int[]{nStepId}));
	}
...
<title WCMAnt:param="advisor_optiongroup_order.jsp.trswcmoptiongrouporder">TRS WCM ::::::选项组排序</title>
			...
			<TR>
				...
				<TABLE width="100%" border="0" cellpadding="0" cellspacing="1"
					...
					<TR bgcolor="#BEE2FF" class="list_th">
						<TD bgcolor="#BEE2FF" WCMAnt:param="advisor_optiongroup_order.jsp.optiongroupname">选项组名称</TD>
						<TD bgcolor="#BEE2FF" WCMAnt:param="advisor_optiongroup_order.jsp.order">排序</TD>
					</TR>
					<tbody id="table_tbody">
		<%
			 for(int i=0;i<oOptionGroups.size();i++){
				 OptionGroup optionGroup = (OptionGroup)oOptionGroups.getAt(i);
				 int nOptionGroupId = optionGroup.getId();
				 int nOrder = optionGroup.getOrder();
		%>
					<TR class="list_tr" id="tr_<%=nOptionGroupId%>">
						<TD align="center"><%=CMyString.transDisplay(optionGroup.getOptionGroupName())%></TD>
...
var OrderHandler = {
	...
	changeValue : function(event, dom){
		...
		BasicDataHelper.call("wcm61_advisorcenter", 'saveOptionGroup', oPostData, true, function(transport, json){
