...
<%
	...
	if(currAdvisor == null){
		throw new WCMException(ExceptionNumber.ERR_OBJ_NOTFOUND, CMyString.format(LocaleServer.getString("advisor_step_order.jsp.notfindadvisorid", "没有找到ID为{0}的顾问！"), new int[]{nAdvisorId}));
	}
...
<title WCMAnt:param="advisor_step_order.jsp.trswcmadvisorsteporderpage">TRS WCM ::::::顾问步骤排序页面</title>
			...
			<TR>
				...
				<TABLE width="100%" border="0" cellpadding="0" cellspacing="1"
					...
					<TR bgcolor="#BEE2FF" class="list_th">
						<TD bgcolor="#BEE2FF" WCMAnt:param="advisor_step_order.jsp.stepname">步骤名称</TD>
						<TD bgcolor="#BEE2FF" WCMAnt:param="advisor_step_order.jsp.order">排序</TD>
					</TR>
					<tbody id="table_tbody">
		<%
			 for(int i=0;i<oSteps.size();i++){
				 Step step = (Step)oSteps.getAt(i);
				 int nStepId = step.getId();
				 int nOrder = step.getOrder();
		%>
					<TR class="list_tr" id="tr_<%=nStepId%>">
						<TD align="center"><%=CMyString.transDisplay(step.getStepName())%></TD>
						<TD align="center"><input type="text" name="orderText" id="orderText_<%=nStepId%>" value="<%=nOrder%>" class="orderText" _id="<%=nStepId%>" rowNum ="<%=nOrder%>" _value="<%=nOrder%>"/>
						</TD>
					</TR>
		<%
			}
...
var OrderHandler = {
	...
	changeValue : function(event, dom){
		...
		BasicDataHelper.call("wcm61_advisorcenter", 'saveStep', oPostData, true, function(transport, json){
