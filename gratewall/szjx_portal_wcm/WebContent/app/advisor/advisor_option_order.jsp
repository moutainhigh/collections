...
<%
	String sOptionGroupId = processor.getParam("OptionGroupId");
	int nOptionGroupId = Integer.parseInt(sOptionGroupId);
	OptionGroup currOptionGroup = OptionGroup.findById(nOptionGroupId);
	if(currOptionGroup == null){
		throw new WCMException(ExceptionNumber.ERR_OBJ_NOTFOUND, CMyString.format(LocaleServer.getString("advisor_option_order.jsp.notfindadvisorgroupid", "没有找到ID为{0}的顾问选项组！"), new int[]{nOptionGroupId}));
	}
...
<title WCMAnt:param="advisor_option_order.jsp.trswcmoptionorder">TRS WCM ::::::选项排序</title>
			...
			<TR>
				...
				<TABLE width="100%" border="0" cellpadding="0" cellspacing="1"
					...
					<TR bgcolor="#BEE2FF" class="list_th">
						<TD bgcolor="#BEE2FF" WCMAnt:param="advisor_option_order.jsp.optionname">选项名称</TD>
						<TD bgcolor="#BEE2FF"  WCMAnt:param="advisor_option_order.jsp.order">排序</TD>
					</TR>
					<tbody id="table_tbody">
		<%
			 for(int i=0;i<oOptions.size();i++){
				 Option option = (Option)oOptions.getAt(i);
				 int nOptionId = option.getId();
				 int nOrder = option.getOrder();
		%>
					<TR class="list_tr" id="tr_<%=nOptionId%>">
						<TD align="center"><%=CMyString.transDisplay(option.getOptionName())%></TD>
...
var OrderHandler = {
	...
	changeValue : function(event, dom){
		...
		BasicDataHelper.call("wcm61_advisorcenter", 'saveOption', oPostData, true, function(transport, json){
