...
function disableCommand(){
	FloatPanel.disableCommand("onPrevious",(m_nCurrIndex==0));
	FloatPanel.disableCommand("onNext",(m_nCurrIndex==m_nAll-1));
}
...
function onNext(){
	FloatPanel.disableCommand("onNext",true);
	setTimeout(function(){
		FloatPanel.disableCommand("onNext",(m_nCurrIndex==m_nAll-1));
		}, 1000);
...
Event.observe(window,'load',function(){
	...
	FloatPanel.disableCommand("onPrevious",true);
	if(m_nAll < 2){		
		FloatPanel.disableCommand("onNext",true);
	}
...
var imgInfoOfCrop;
var bound;
function crop(nSiteId,sWatermarkFile,sWatermarkPos){
	...
	var parameters = "photo="+encodeURIComponent(oImg.src)+"&SiteId="+nSiteId;
	//var sUrl = WCMConstants.WCM6_PATH + "photo/photo_crop.html?"+parameters;
	var sUrl = WCMConstants.WCM6_PATH + "photo/photo_compress.jsp?"+parameters+"&WaterMarkFile="+sWatermarkFile+"&WatermarkPos="+sWatermarkPos;
