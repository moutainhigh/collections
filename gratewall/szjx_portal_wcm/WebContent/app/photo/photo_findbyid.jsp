...
<%
	...
	if(!(result instanceof ViewDocument)){
		throw new WCMException(CMyString.format(LocaleServer.getString("photo_findbyid.jsp.objtype_isnot_ViewDocument","服务(com.trs.ajaxservice.PhotoServiceProvider.findbyid)返回的对象类型不为ViewDocument，而为{0}，请确认。"),new Object[]{result.getClass()}));
	}
...
<%
	...
	params.put("ChannelId", obj.getChannelId() +"");
	params.put("ChnlDocIds", obj.getChnlDocProperty("RECID", 0) +"");
...
<div id="template_photo" select="ViewDocument">	
	...
	<div class="attribute_row <%=sEditable%>">
		<span class="wcm_attr_name" style="width:40px" WCMAnt:param="photo_findbyid.jsp.time">时间：</span>
		<span class="wcm_attr_value" id="DOCRELTIME_SPAN" name="DOCRELTIME_SPAN" _fieldName="DOCRELTIME" _fieldValue="<%=trimNull(currDocument.getReleaseTime().toString("yyyy-MM-dd HH:mm:ss"))%>" style="color:#09549F" validation="type:date,date_format:yyyy-mm-dd hh:MM:ss" _serviceId="wcm6_viewdocument"><%=trimNull(currDocument.getReleaseTime().toString("yyyy-MM-dd HH:mm:ss"))%></span><button type="button" id="embed" class="calendarShow"><IMG alt="" src="../images/icon/TRSCalendar.gif" border=0 class="img"></button>
	</div>
	<div class="attribute_row <%=sEditable%>">
...
<%
	...
	if(!bContainStatus){
%>
				<option value="<%=nStatusId%>" title="<%=CMyString.filterForHTMLValue(nStatusName)%>">
					<%=CMyString.filterForHTMLValue(nStatusName)%>
				</option>
<%}%>
			</select>
	...
	<%
		if(obj.getChnlDocProperty("DOCSTATUS",0) == 10){
	%>
		<div class="attribute_row readonly">
...
<%
	...
	tx.printStackTrace();
	throw new WCMException(LocaleServer.getString("photo_findbyid.jsp.label.runtimeexception", "photo_findbyid.jsp运行期异常!"), tx);
}
%>