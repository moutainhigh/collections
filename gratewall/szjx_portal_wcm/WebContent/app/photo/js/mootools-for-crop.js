...
}return new Elements(Array.filter(this,(typeof A=="string")?function(C){return C.match(A);}:A,B));}});Document.implement({newElement:function(A,B){var L;try{var x=document.createElement("<input name='leafgray'>");L=(x.name=='leafgray')}catch(e){}if(Browser.Engine.trident&&B&&L){["name","type","checked"].each(function(C){if(!B[C]){return ;
