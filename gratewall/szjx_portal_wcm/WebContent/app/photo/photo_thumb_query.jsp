...
<%
	...
	if(!(result instanceof ViewDocuments)){
		throw new WCMException(CMyString.format(LocaleServer.getString("photo_thumb_query.jsp.objtype_isnot_ViewDocument","服务(com.trs.ajaxservice.PhotoServiceProvider.query)返回的对象类型不为ViewDocument，而为{0}，请确认。"),new Object[]{result.getClass()}));
	}
	int nCurrDocId = 0;
	...
	for (int i = currPager.getFirstItemIndex(); i <= currPager.getLastItemIndex(); i++) {
		try{
			...
			if(currDocument == null){
				throw new WCMException(ExceptionNumber.ERR_PARAM_INVALID,CMyString.format(LocaleServer.getString("photo_thumb_query.jsp.doc_notfound", "没有找到ID为[{0}]的文档!"), new int[]{ obj.getDocId()}));
			}
			...
			if(chnldoc == null){
				throw new WCMException(ExceptionNumber.ERR_PARAM_INVALID, CMyString.format(LocaleServer.getString("photo_thumb_query.jsp.chnldoc_notfound", "没有找到ID为[{0}]的chnldoc!"), new int[]{ nRowId}));
			}
...
%>
    <div class="thumb_item<%=(bIsSelected)?" thumb_item_active":""%>" id="thumb_item_<%=nRowId%>" itemId="<%=nRowId%>" right="<%=sRightValue%>" docId="<%=nDocId%>">
        <div contextmenu="1" class="thumb" id="thumb_<%=nRowId%>" onclick='$openMaxWin("/wcm/app/photo/photo_show.jsp?DocumentId=<%=nDocId%>&RecId=<%=nRowId%>&ChannelId=<%=nChannelId%>","","yes")'>
        ...
        <div class="attrs" id="thumb_attrs_<%=nRowId%>">
			...
			%>
            <span id="desc_<%=nRowId%>" title="<%=sTitle%>" style="color:<%=chnldoc.getStatusId() == 10 ? "green" :""%>;text-align:left;"><%=filter(sTitle)%></span>
        </div>
    </div>
...
<%
	...
	tx.printStackTrace();
	throw new WCMException(LocaleServer.getString("photo_thumb_query.jsp.label.runtimeexception", "watermark_query.jsp运行期异常!"), tx);
}
%>
<%!
