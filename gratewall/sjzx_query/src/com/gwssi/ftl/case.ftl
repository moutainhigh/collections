		...
		<#if cou%2==0>
			...
			<#elseif key=="案发时间" || key=="立案日期" || key=="执行日期" || key== "结案日期" || key== "执照有效期限" || key== "注销日期"  || key== "吊销日期" || key== "处罚决定书签发日期"  || key== "登记时间" || key== "案源终止时间" || key== "移送日期" >
		  	...
		  	<#elseif key=="案值" || key=="罚款金额" || key=="没收金额" || key=="变价金额">
		  		...
		  		<#assign cou=cou+1 /> 
		  	<#elseif key=="处罚决定书文号">
		  		<tr><td style="text-align: right;padding-right: 1px;">${key}:</td><td style="padding-left: 5px;padding-bottom: 8px;padding-top: 8px;"><a href="javascript:void(0);" onclick="viewDataSource();">${weaponMap[key]?default("")}</a></td>
		  		<#assign cou=cou+1 />  
		 	<#else>
		 		<tr><td style="text-align: right;padding-right: 1px;">${key}:</td><td style="padding-left: 5px;padding-bottom: 8px;padding-top: 8px;">${weaponMap[key]?default("")}</td>
		 		<#assign cou=cou+1 />
		 ...
		 <#else>
		 	...
		 	<#elseif key=="案发时间" || key=="立案日期" || key=="执行日期" || key== "结案日期" || key== "执照有效期限" || key== "注销日期"  || key== "吊销日期" || key== "处罚决定书签发日期"  || key== "登记时间" || key== "案源终止时间" || key== "移送日期" >
		  	...
		  	<#elseif key=="案值" || key=="罚款金额" || key=="没收金额" || key=="变价金额">
		  		...
		  		<#assign cou=cou+1 />
		  	<#elseif key=="处罚决定书文号">
		  		<td style="text-align: right;padding-right: 1px;">${key}:</td><td style="padding-left: 5px;padding-bottom: 8px;padding-top: 8px;"><a href="javascript:void(0);" onclick="viewDataSource();">${weaponMap[key]?default("")}</a></td></tr>
		  		<#assign cou=cou+1 />
		 	<#else>
		  		<td style="text-align: right;padding-right: 1px;">${key}:</td><td style="padding-left: 5px;padding-bottom: 8px;padding-top: 8px;">${weaponMap[key]?default("")}</td></tr>
		  		<#assign cou=cou+1 />
