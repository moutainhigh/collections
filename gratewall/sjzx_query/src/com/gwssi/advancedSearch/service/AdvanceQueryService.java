...
public class AdvanceQueryService extends BaseService{
	...
	public List<Map> getTableByTopic(String topicId) throws OptimusException{
		...
		sql.append("select d.table_name_cn as title, d.table_name_en as code,d.pk_dc_table as key from dc_table d ");
		sql.append(" where d.dc_topic = ? and d.effective_marker = 'Y'  and d.table_name_cn is not null ");
	 ...
	 * @throws OptimusException
	 */
	public Map getAdvQueryList(String sql,int currentPage,int pageSize) throws OptimusException
	{
...
//		}
		String defauls = "500";
		try {
		    int b = Integer.valueOf(pageSize).intValue();
		    if(b>500){
		    	defauls = String.valueOf(b);
		    }
		} catch (NumberFormatException e) {
		    e.printStackTrace();
		}
		
		//String testsql =patternReg(sql, defauls);
		//String testsql =  "select * from ("+sql+")  e where rownum <= 500";
		
		System.out.println("AdvanceQueryService.getAdvQueryList(String, int, int),高级查询列表结果查询的语句===>"  + sql);
		
		int total = jdbcTemplate.queryForInt("select count(1) from (" +sql+ ")");
		System.out.println(total);
		
		/*int pageNo = Integer.valueOf(pageIndex);
		int pageEndNo = total % Integer.valueOf(pageSize);*/
		int pageNo = (currentPage-1) * pageSize+1;
		int pageEndNo =(currentPage-1) * pageSize +pageSize;
		
		String tempSql ="SELECT * FROM (SELECT A.*, ROWNUM RN FROM ("+sql+") A  WHERE ROWNUM <= "+pageEndNo+" ) WHERE RN >= "+pageNo+" ";
		...
		for (int i = 0; i < list.size(); i++) {
			Map<String, Object> map = list.get(i);
			  for (Entry<String, Object> entry : map.entrySet()) {
				  //System.out.println("key= " + entry.getKey() + " and value= " + entry.getValue().getClass());
		...
		maps.put("rows", list);     //每页显示的记录数
		maps.put("total", total);   //存放总记录数 ，必须的
	...
	public boolean check(String sql){
		  ...
		  boolean flag = span.matches(pattern);
...
//		if(matcher.find()){}
	...
	public String saveAdvQueryInfo(String params) throws OptimusException {
	...
	/*	Map<String,Object> conditionMap = new HashMap<String,Object>();
		...
		String uid = insertAdvQueryBaseInfo(baseInfoMap);
	...
	private String insertAdvQueryBaseInfo(Map<String, Object> map) throws OptimusException {
		IPersistenceDAO dao = getPersistenceDAO(DATASOURS_DC_QUERY);
...
//		baseInfoList.add((String) map.get((String) map.get("queryNameTransfer")));
	...
	public static void main(String[] args) {
		...
		AdvanceQueryService s = new AdvanceQueryService();
		
		s.check("SELECT DC_RA_MER_BASE.CURRENCY ,DC_RA_MER_BASE.DOM ,DC_RA_MER_BASE.DOMCODE ,DC_RA_MER_BASE.DOMDISTRICT ,DC_RA_MER_BASE_UPDATE.REGORG ,DC_RA_MER_BASE_UPDATE.ID ,DC_RA_MER_BASE_UPDATE.APPRDATE ,DC_RA_MER_BASE_EXT.REGCAPUSD ,DC_RA_MER_BASE_EXT.RESIBUILENCO  FROM DC_RA_MER_BASE,DC_RA_MER_BASE_EXT,DC_RA_MER_BASE_UPDATE");
