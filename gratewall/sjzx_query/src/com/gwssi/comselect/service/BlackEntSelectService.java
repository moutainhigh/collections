...
@Service(value = "blackEntSelectService")
public class BlackEntSelectService extends BaseService{
	
	public List<Map> getList(Map<String,String> map) throws OptimusException{
		IPersistenceDAO dao = getPersistenceDAO("dc_dc");
		StringBuffer sql = new StringBuffer();
		sql.append("select * from V_NS_ENTERPRISE_BLACK_LIST t where 1=1");
	...
	public Map getListDetail(String id) throws OptimusException{
		IPersistenceDAO dao = getPersistenceDAO("dc_dc");
		StringBuffer sql = new StringBuffer();
		sql.append("select * from V_NS_ENTERPRISE_BLACK_LIST t where id = ? order by LAST_UPDATE desc");
		List<String> params = new ArrayList<String>();
		params.add(id);
		List<Map> res = dao.pageQueryForList(sql.toString(), params);
	...
	public void updateListDetail(Map<String, String> form, String id) throws OptimusException{
		...
		IPersistenceDAO dao = getPersistenceDAO("dc_dc");
		StringBuffer sql = new StringBuffer();
		sql.append("update DC_NS_ENTERPRISE_BLACK_LIST t set ADDRESS = ?,HOUSE_CODE = ?,REMARK = ?,LAST_UPDATE = ?,UPDATE_USER=?,UPDATE_NAME=? where id = ?");
	...
	public void updateListInsert(Map<String, String> form) throws OptimusException{
		...
		IPersistenceDAO dao = getPersistenceDAO("dc_dc");
		StringBuffer sql = new StringBuffer();
		sql.append("insert into DC_NS_ENTERPRISE_BLACK_LIST(ID, ADDRESS, HOUSE_CODE, REMARK, CREATE_DATE, LAST_UPDATE, CREATE_USER, CREATE_NAME, UPDATE_USER, UPDATE_NAME) VALUES(dc_ns_enterprise_black_list_s.nextval,?,?,?,?,?,?,?,?,?)");
		List params = new ArrayList<String>();
		params.add(form.get("address"));
		params.add(form.get("houseCode"));
		params.add(form.get("remark"));
	...
	public void updateListDelete(String id) throws OptimusException{
		IPersistenceDAO dao = getPersistenceDAO("dc_dc");
		StringBuffer sql = new StringBuffer();
		sql.append("delete from DC_NS_ENTERPRISE_BLACK_LIST t where t.id = ?");
		List params = new ArrayList<String>();
		params.add(id);
		dao.execute(sql.toString(), params);		
	}
	
	public List<Map> exportExcel(String address,String houseCode) throws OptimusException{
		IPersistenceDAO dao = getPersistenceDAO("dc_dc");
		StringBuffer sql = new StringBuffer();
		sql.append("select *  from V_NS_ENTERPRISE_BLACK_LIST t where 1=1 ");
		List<String> params = new ArrayList<String>();
