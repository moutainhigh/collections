...
function uploadSuccess(file, serverData) {
	try {
		...
		stid=serverData;
		//展示比对结果列表数据
		$("#gridpanel").gridpanel("option", "dataurl","../../datas/getList.do?tid=" + stid +"&select=0");
		$("#gridpanel").gridpanel("reload");
		
		//获取比对结果汇总
		$.ajax({
			url : "../../datas/getResults.do",
			type : "get",
			dataType : "json",
			data : {
				tid : stid
			...
			success : function(data) {
				tid= stid;
				var countM = data.data[0].data;
				var total = countM.countSum;
				var match = countM.countMatch;
				var noMatch = total - match;
				$(".results").css("display","block");
				$("#countSum").html(total);
				$("#countMatch").html(match);
				$("#countNotMatch").html(noMatch);
