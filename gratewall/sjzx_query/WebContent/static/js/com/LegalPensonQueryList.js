		 ...
		 */
			_init: function(){
				...
				require(['domReady'], function (domReady) {
					    ...
					    /*	var gridUrl="../../LegalPensonselect/LegalPensonQueryList.do";
				    		$("#LegalPensonQueryListGrid").gridpanel("option",'datarender',torch.backFunction);
				    		$("#LegalPensonQueryListGrid").gridpanel("option",'dataurl',gridUrl);
				    		$("#LegalPensonQueryListGrid").gridpanel('query', ['LegalPensonQueryListPanel'],null);*/
				    		
				    		$("#save_button").off('click').on('click',$_this.queryLegalPensonList);
						    $("#reset_button").off('click').on('click',torch.resetEditCondition);
						    $("#exportFile").off('click').on('click',torch.exportFile);
						    $("#openFile").off('click').on('click',torch.openFile);
	    	...
	    	getCount:function(){
	    		var params = {
	    				url : '../../LegalPensonselect/LegalPensonQueryList.do?flag=1',
	    				components: ['LegalPensonQueryListPanel'],
	    	...
	    	queryLegalPensonList : function(){
	    		...
	    		clearTimeout(timer);  
	    			timer = setTimeout(function() { 
	    	    		...
	    	    		$("#save_button").on('click',$_this.queryLegalPensonList);
	    		}, 1800);
...
//					//torch.getCount();
	    	...
	    	regDetail:function(id, enttype, opetype,entname,regno, entstatus, entid){
	    		...
	    		var url = $_this.getContextPath() + "/" + urlright + "?flag=" + encode("0")
	    				...
	    				+ encode(regno) + "&entid=" + encode(entid);
	    		window.open(url);
...
//	    		alert(pathName);
	    	...
	    	openFile : function(){
	    		//parent.location.href = "www.baidu.com";
	    		
	    		},
	    	edit_fromNames : [ 'LegalPensonQueryListPanel']
		};
