		 ...
		 */
			_init: function(){
				...
				require(['domReady'], function (domReady) {
					  ...
					  /*  	var gridUrl="../../CPRShow/CPRQueryList.do";
				    		$("#CPRQueryListGrid").gridpanel("option",'datarender',torch.backFunction);
				    		$("#CPRQueryListGrid").gridpanel("option",'dataurl',gridUrl);
				    		$("#CPRQueryListGrid").gridpanel('query', ['CPRQueryListPanel'],null);*/
				    		
				    		$("#save_button").off('click').on('click',$_this.queryCPRList);
						    $("#reset_button").off('click').on('click',torch.resetEditCondition);
						    util.exports('CPRDetail',$_this.CPRDetail);
	    	...
	    	getCount:function(){
	    		var params = {
	    				url : '../../CPRShow/CPRQueryList.do?flag=1',
	    				components: ['CPRQueryListPanel'],
	    	...
	    	queryCPRList : function(){
	    		...
	    		clearTimeout(timer);  
	    			timer = setTimeout(function() { 
	    	    		...
	    	    		$("#save_button").on('click',$_this.queryCPRList);
	    		}, 1800);
...
//				$("#tips").css("display","block");
//				$("#contactsTips").css("display","block");
	    		var data = obj.data;
	    		if (data.length == 0) {
	    		...
	    		for (var i = 0; i < data.length; i++) {
	    			data[i]["regino"] = '<a href="javascript:void(0);" title="'+data[i]["regino"] +'" onclick="ebaic.CPRDetail(\''
	    			    +data[i]["invmaiid"]//涉及主体 
	    				+ '\',\''
						+ data[i]["invobjid"]//涉及客体
	    				+ '\',\''
	    				+ data[i]["infproid"]//信息提供方信息
	    				+ '\',\''
	    				+ data[i]["infowareid"]//分流信息
	    				+ '\',\''
	    				+ data[i]["feedbackid"]//反馈信息;加入反馈信息存在，绕后根据mediationid在DC_CPR_MEDIATION表查询调解信息信息
	    			    +'\')">' + data[i]["regino"] + '</a>';
	    		}	
	    		return data;
	    	},
	    	 ...
	    	 */
	    	CPRDetail:function(invmaiid, invobjid ,infproid,infowareid,feedbackid){
	    		var url = $_this.getContextPath()+"/page/comselect/CPREntDetail.jsp?invmaiid="+invmaiid+
	    		"&invobjid="+invobjid+
	    		"&infproid="+infproid+
...
//	    		alert(pathName);
	    	...
	    	edit_fromNames : [ 'CPRQueryListPanel']
