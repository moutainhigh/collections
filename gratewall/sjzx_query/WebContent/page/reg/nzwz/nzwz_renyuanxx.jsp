	...
	function fixColumn(event, obj) {// 维护按钮
		...
		for(var i=0;i<data.length;i++){
			...
			if(isNotEmpty(data[i]["cerno"])){
				cerNo = data[i]["cerno"];
				data[i]["cerno"] = '<div class="cerno"><a href="javascript:void(0);" data-cerno="'+cerNo+'" onclick="viewCerno(this'+','+ "'"+ data[i]["name"]+"'"+');">' + "查看(该操作会记录日志)" + '</a></div>';
			}else{
