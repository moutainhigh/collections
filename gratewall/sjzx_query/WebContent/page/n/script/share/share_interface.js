...
/*function getTopic(key, container) {
	...
	$.ajax({
				type : "get",
				url : "/txn401007.ajax?attribute-node:record_page-row=10000&key="
						+ key,
				async : false,
				success : function(xmlResults) {
					if (xmlResults.selectSingleNode("//context/error-code").text != "000000") {
						alert(xmlResults
								.selectSingleNode("//context/error-desc").text);
						return false;
					} else {
						var datas = _getXmlNodeValues(xmlResults,
								"/context/dataString");
...
function getTable(key, container) {
	...
	$.ajax({
				type : "get",
				url : "/txn401008.ajax?attribute-node:record_page-row=10000&key="
						+ key,
				async : false,
				success : function(xmlResults) {
					if (xmlResults.selectSingleNode("//context/error-code").text != "000000") {
...
function getTableSelectItemByKey(key,container,tableId,parentTableKey){
	if (typeof (currentTopic) == "undefined") {
		currentTopic = key;
	}
	
	colTotable(key,tableId,parentTableKey);
	
}


//将用户点击的列添加到结果集
function colTotable(key,tableId,parentTableKey) {
//	alert(key);
  ...
  var tab1_col_all = [];
	  ...
	  $.ajax({
		  type: "post",
		  url: "../../advQuery/getColumn.do?tableId=" + parentTableKey,
		  async: false,
		  dataType:"json",
		  success: function(data){
			  tab1_col_all = data.data[0].data;
		  }
    ...
    if (tab1_col_all[ii].key == key) {
     ...
     // console.log(selected_col);
      rowData = [{
        //        "sort":sort,
        "table": {
          //"id": selected_col.parentTableKey,
          "id": parentTableKey,
...
function createSql(step) {
	...
	if (step == '1') {
		...
		var dataStr =  $("#tab1_col_selected_div").tablet("getAllData"); //得到选择的列
		
		var jsonObj =  eval('(' + dataStr + ')');
		var _html = "";
		for(var i=0;i<jsonObj.data.length;i++){
			//_html += jsonObj.data[i].table+jsonObj.data[i].column.code;
			//_html += jsonObj.data[i].column.dcTableNameEn +" "+ jsonObj.data[i].column.code;
			//console.log(jsonObj.data[i]);
			//console.log(jsonObj.data[i].column.name_en);
			//console.log(jsonObj.data[i].column.name_cn);
			//console.log(jsonObj.data[i].table.name_cn);
			//console.log(jsonObj.data[i].table.name_en);
			
			_html += jsonObj.data[i].table.name_en+"."+jsonObj.data[i].column.name_en +" ,";
		}
		
		$.cookie("tableInfo",dataStr)
		_html = _html.substring(0,_html.length-1)
		//console.log(_html);
		
		tableCode  = $.cookie("record:table_code");
		//sql = "SELECT * FROM " + tableCode;
		sql = "SELECT "+_html+" FROM " + tableCode;
	} else if (step == '2') {
		...
		_html = _html.substring(0,_html.length-1);  //截取最后一个 , 
		//console.log(_html);
		$.cookie("tableInfo",dataStr);
		//sql = "SELECT * FROM " + tableCode + ' WHERE ' + selectStr + ' ';
		sql = "SELECT " + _html + " FROM " + tableCode + ' WHERE ' + selectStr + ' ';
		// alert(sql);
		//setFormFieldValue("record:sql", sql);
		$.cookie("record:sql",sql);
	} else {
		sql = $.cookie("record:sql");
		//sql = getFormFieldValue("record:sql");
...
function preview() {
	...
	$.cookie("record:table_id",tableIds);
	$.cookie("record:table_name_cn",tableNames);
	$.cookie("record:table_code",tableCodes);
	$.cookie("record:sql",createSql('1'));
...
//	setFormFieldValue("record:table_id", tableIds);
	...
	if (relation != '{data:[]}') {
		...
		$.cookie("record:condition",relation);
		//setFormFieldValue("record:sql", createSql('2'));
		$.cookie("record:sql",createSql('2'));
	}
	// 获取查询条件值
	var param = $("#table_param").tablet("getAllData");
	if (param != '{data:[]}') {
		$.cookie("record:param",param);
		$.cookie("record:sql",createSql('3'));
	...
	if (checkResult == false) {
		...
		var tableSelects = $("#cond_checkTable").html();
		
		$.cookie("skyTable",tableSelects);
		
		$.cookie("skySql",sql);
		$("#preview_data").attr("src","list.jsp");
		$("#preview_data").css("height","550px");
	}
}
