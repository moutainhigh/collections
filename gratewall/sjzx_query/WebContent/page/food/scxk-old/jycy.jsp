...
function getUrlParam(name) {
	...
	function fixColumn(event, obj) {// 维护按钮
		...
		for(var i=0;i<data.length;i++){
			data[i]["opt"] ='<a href="javascript:void(0);" title="查看" onclick="viewDataSource(\''
						+ data[i]["id"]
						+'\');">查看</a>';
		}
//		if(data.length==0){
...
//	}
	function viewDataSource(id){
		post(rootPath + "/food/sample.do", {"id" :id});
	}	
...
<body>
	<div vtype="gridpanel" name="jbxxGrid" height="536" width="100%"
		...
		title="检验抽样详情信息" defaultview="table" showborder="false"
		isshowselecthelper="false" selecttype="2">
		<!-- 表头 -->
		<div vtype="gridcolumn" name="grid_column" width="100%">
			<div>
				<div name='foodCategory' text="食品品种名称" textalign="center" width="30%"></div>
				<div name='sampleDate' datatype="date" dataformat="YYYY-MM-DD" text="抽样日期" textalign="center" width="20%"></div>
				<div name='sampleJyjgResult' text="检验结果" textalign="center" width="30%"></div>
				<div name='opt' text="抽检报告" textalign="center" width="20%"></div>
			</div>
		</div>
		<!-- 表格 -->
