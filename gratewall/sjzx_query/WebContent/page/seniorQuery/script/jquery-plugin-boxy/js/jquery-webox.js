...
$.extend({
	webox:function(option){
		...
		$(document).ready(function(e){
			...
			var scroll=option['scroll']?option['scroll']:"no";
			$('body').append('<div class="background" style="background-color:black;display:none;">'+
			'</div><div class="webox" style="width:'+width+'px;height:'+height+'px;display:none;">'+
			'<div id="inside" style="height:'+height+'px;"><h1 id="locked" onselectstart="return false;">'+
			(option['title']?option['title']:'webox')+'<a class="span" href="javascript:void(0);"></a></h1>'+
			(option['iframe']?'<iframe class="w_iframe" src="'+option['iframe']+
			'" frameborder="0" width="100%" scrolling="yes" style="border:none;overflow:hidden;height:450px;">'+
			'</iframe>':option['html']?option['html']:'')+'</div></div>');
			//修改 .background 的background-color ，可以变换背景颜色
			...
			if(option['bgvisibel']){
				$('.background').fadeTo('slow',0.5);	//设置动画变化速度和背景透明度
			...
			$('#'+option['locked']+' .span').click(function(){
				...
				document.parentWindow.top.swit.switchSysBar();
			});
