...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		var url = "";
		if(parent.type=='case'){
			$("#enttypeName").text("案件信息");
			url = "left-caseInfo.json";
		}else if(parent.type == "CPR"){
			$("#enttypeName").text("12315信息");
			url = "left-CPRInfo.json";
		}else if(parent.type == "gt"){
			$("#enttypeName").text("个体年报基本信息");
			url = "left-gtInfo.json";
		}else if(parent.type == "qy"){
			$("#enttypeName").text("企业年报基本信息");
		...
		$.getJSON(url,function(data){
			for(var i=0 ;i<data.length;i=i+2){
				...
				if((i+1)<data.length){
					...
					$('<tr height="30px"><th width="50%"><a  href="#" onclick=rightPageSet("'+url+parent.infowareid+'&headTitle='+encode(data[i].content)+'&toBackUrl='+data[i].toBackUrl+'")>'+data[i].content+'</a></th><th>|</th><th width="50%"><a  href="#" onclick=rightPageSet("'+data[i+1].url+parent.infowareid+'&headTitle='+encode(data[i+1].content)+'&toBackUrl='+data[i+1].toBackUrl+'")>'+data[i+1].content+'</a></th></tr>').appendTo($('#container'));

				}else{
						$('<tr height="30px"><th width="50%"><a  href="#"  onclick=rightPageSet("'+url+parent.infowareid+'&headTitle='+encode(data[i].content)+'&toBackUrl='+data[i].toBackUrl+'")>'+data[i].content+'</a></th></tr>').appendTo($('#container'));
		     	}
			}
		});
	});
