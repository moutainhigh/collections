...
<script type="text/javascript">
	...
	$(function(){
		priPid=getUrlParam("priPid");//主键
  		...
  		caseType=getUrlParam("caseType"); 
  		if(caseType=="0"){//一般案件
  			$("#main").panel("option","frameurl","<%=request.getContextPath()%>/page/caseType/query-panel-right-commonCase.jsp");
  			<%-- main.location.href="<%=request.getContextPath()%>/page/reg/query-panel-right.jsp"; --%>
  			
  		}else if(caseType=="1"){//简易案件
  	

  			$("#main").panel("option","frameurl","<%=request.getContextPath()%>/page/caseType/query-panel-right-simpleCase.jsp");
  		}
  		$("#left").panel("option","frameurl","<%=request.getContextPath()%>/page/caseType/query-tree-left-commonCase.jsp");
 	...
 	function rightPageSet(url){
         var url2="";
         url2="<%=request.getContextPath()%>/page/caseType/"+url;
