...
*/		for(var groupCode in treeBoxHelpers){
	...
	report._getInitHtml = function(name){
		var html =  "<div name=\""+name+"Report\" class=\"report\"> "+
					...
					"	<div class=\"report-query\"><div class='report-query-banner'><img align='absmiddle' style='display:none' src='../../gwaic/images/icon/application_view_list.png' /><span class='report-title'></span></div></div> "+
					"	<div class=\"report-content\"> "+
					" 		<div class='report-btn-banner'>"+
					//"			<a href='javascript:void(0)'  id ='doExcelDownload1' ><span style='display:inline-block;line-height:23px;' class='defaultButton button-excel' >导出大类</span></a>" +
					"			<div style='text-align:left'><font size='5' color='red'>以下信息仅供参考，不得对外提供</font></div>" +
					"			<a href='javascript:void(0)'  id ='doExcelDownload' ><span style='display:inline-block;line-height:23px;padding:0 4px;' class='defaultButton button-excel'>导出详细菜单</span></a>" +
					"			<button name='"+name+"_query_back' onclick='report.goBack(\""+name+"\");' class='defaultButton button-back' >返回</button>" +	
					"		</div>"+
	...
	report._getParamsFormHtml = function(config,treeBoxes,treeBoxHelpers){
		...
		var hasStatType = false;
		var fieldIdx ; // 表单域个数，用于计算何时换行
		for(var i = 0 ; i<config.params.length ; ++i){
			paramConfig = config.params[i];
			if(!paramConfig.label || !paramConfig.name || !paramConfig.type){
				alert('查询表单域必须设置 label 、  name 和  type 属性。');
				return '';
			}
			
			if(paramConfig.name=='stat_type'){
				hasStatType = true;
			}
			
			paramId = config.name+'_'+paramConfig.name+"_param";
			
			var label = paramConfig.label || '';
			if(paramConfig.type!='hidden'){
				retHtml += "<td width='20%' align='right' nowrap='nowrap' class='report-query-li-field'>";
				retHtml += "<label for='"+paramId+"' class='report-query-label'>"+(paramConfig.label||'')+"：</label>";
				retHtml += "</td>";
				retHtml += "<td width='30%' class='report-query-li-input'>";  // 参数 td 开始
			}
			
			if(paramConfig.type=='text'){
				retHtml += "<input name='"+paramConfig.name+"' id='"+paramId
							  +"' type='text' class='report-query-field report-query-text' ";
				retHtml += " label='"+label+"' ";
				if(paramConfig.defaultvalue!=''){
					retHtml += " value='"+(paramConfig.defaultvalue||'')+"' ";
				}
				if(paramConfig.rule){
					retHtml += " rule='"+(paramConfig.rule||'')+"' ";
				}
				retHtml += " />";
			}
			if(paramConfig.type=='combox'){
				retHtml += "<select name='"+paramConfig.name+"' id='"+paramId
				  +"' class='report-query-field report-query-combox' >";
				retHtml += " label='"+label+"' ";
				if(paramConfig.rule!='must'){
					retHtml += "<option value=''>全部</option>";
				}
				
				if(Object.prototype.toString.apply(paramConfig.options) != '[object Array]'){
					paramConfig.options = report.parseJson(paramConfig.options);
				}
					
				if(paramConfig.options && paramConfig.options.length){
					var option ;
					for(var o = 0 ; o < paramConfig.options.length; ++o){
						option = paramConfig.options[o];
						retHtml += "<option value='"+option.value+"' ";
						if(paramConfig.defaultvalue == option.value){
							retHtml += " selected='true' ";
						}
						retHtml += ">"+option.text+"</option>";
					}
				}
				retHtml += "</select>";
			}
			if(paramConfig.type=='treebox'){
				retHtml += "<input name='"+paramConfig.name+"_text' id='"+paramId
				  +"Text' type='text' class='report-query-text report-treebox-text' readonly='true' ";
				retHtml += " />";
				
				retHtml += "<input name='"+paramConfig.name+"' id='"+paramId
				  +"' type='hidden' class='report-query-field report-query-treebox' ";
				retHtml += " label='"+label+"' ";
				if(paramConfig.defaultvalue!=''){
					retHtml += " value='"+(paramConfig.defaultvalue||'')+"' ";
				}
				...
				treeBoxes[paramId] = paramConfig.options ;
				
			}
			if(paramConfig.type=='date'){
				retHtml += "<input name='"+paramConfig.name+"' id='"+paramId
				  +"' type='text' type='text' class='report-query-field report-query-date' "
				  + "onclick=\"new WdatePicker(this)\" ";
				retHtml += " label='"+label+"' ";
				if(paramConfig.defaultvalue!=''){
					retHtml += " value='"+(paramConfig.defaultvalue||'')+"' ";
				}
			...
			if(paramConfig.type=='hidden'){
				hiddenFieldHtml += "<input name='"+paramConfig.name+"' id='"+paramId
				  +"' type='hidden' value='"+(paramConfig.defaultvalue||'')+"' label='"+label+"' class='report-query-field report-query-hidden' />";
			}
			
			// helper
			if(paramConfig.helper){
				// helperConfig 是数组格式，第一个元素是helper的名字，第二个元素是helper的类型，从第三个元素开始是helper的初始化参数
				var helperConfig = paramConfig.helper;
				if(helperConfig.length && helperConfig.length>0){
					var helperName = helperConfig[0] ;
					var helperType = helperConfig[1] ;
					if(helperType=='date'){
						var helperHtml = "<div name='"+helperName+"' style='report-query-field report-query-helper'>";
						// 最多支持 5 个初始化参数
						helperHtml += report.initHelper(helperConfig[1]||'',helperConfig[2]||'',
								helperConfig[3]||'',helperConfig[4]||'',helperConfig[5]||'',
								helperConfig[6]||'');
						helperHtml += "</div>";
						
						retHtml += helperHtml;
					}
					if(helperType=='treebox'){
						var helperGroupCode = helperConfig[2]||'';
						treeBoxHelpers[helperGroupCode] = [];
						
						retHtml += "<input id='"+paramId+"Helper' type='hidden' ";
						retHtml += " value='" + helperGroupCode + "' />";
					}
			...
			if((fieldIdx+1)%2==0 && (fieldIdx+1)<config.params.length){
				retHtml += "</tr><tr>";
			}
		...
		var padTdCnt ;
		if(hasStatType){
			padTdCnt = 3 - (config.params.length-1)%3;
		}else{
			padTdCnt = 3 - config.params.length%3;
		}
		for(var i = 0; i < padTdCnt ; ++i){
			retHtml += "<td width='10%' align='right' nowrap='nowrap'>&nbsp;<td>";
			retHtml += "<td width='23%'>&nbsp;<td>";
		}

		retHtml += "<tr>";
		retHtml += "</table>" ;
		
		retHtml += hiddenFieldHtml;
	...
	report.goBack = function(name){
		var oReport = $("div[name='"+name+"Report']");
		
		oReport.find('.report-query').show();
		oReport.find('.report-content').hide();
		oReport.find('.report-remark').hide();
		/*
	...
	report.refresh = function(name){
		...
		oReport.find('.report-query').hide();
		oReport.find('.report-content').show();
		oReport.find('.report-remark').show();
		var config = report.config(name);
		// 0、显示为正在加载数据
...
//		}
		...
		$(".report-data-tr:even").css({"background":"#FFFFFF"});
		$(".report-data-tr:odd").css({"background":"#EFF6FA"});
		...
		if(reportData && reportData.paging){ 
			var pagingHtml = "<tr class='report-paging-tr'><td colspan=\""+(config.headerDataConfig.length+1)+"\" style='border-right-width: 0px;'><div style='float:right'>";
			...
			pagingHtml += "</div></td></tr>";
			var oReport = $("div[name='"+name+"Report']");
			oReport.find('.report-content-table').append(pagingHtml);
			
			
			var add = reportData.paging.pageNo;
			var pageTotal = reportData.paging.pageCnt;
			if(add==1){
				$(".report-paging-nav-first").css({"background":"#F4F4F4","padding":"5px","border":"1px solid #C0D3DD"});
				$(".report-paging-nav-pre").css({"background":"#F4F4F4","padding":"5px","border":"1px solid #C0D3DD"});
				$(".report-paging-nav-next").css({"background":"#EBF5FA","padding":"5px","border":"1px solid #C0D3DD"});
				$(".report-paging-nav-last").css({"background":"#EBF5FA","padding":"5px","border":"1px solid #C0D3DD"});
			}else if(add == pageTotal){
				$(".report-paging-nav-first").css({"background":"#EBF5FA","padding":"5px","border":"1px solid #C0D3DD"});
				$(".report-paging-nav-pre").css({"background":"#EBF5FA","padding":"5px","border":"1px solid #C0D3DD"});
				$(".report-paging-nav-next").css({"background":"#F4F4F4","padding":"5px","border":"1px solid #C0D3DD"});
				$(".report-paging-nav-last").css({"background":"#F4F4F4","padding":"5px","border":"1px solid #C0D3DD"});
			}else{
				$(".report-paging-nav-first").css({"background":"#EBF5FA","padding":"5px","border":"1px solid #C0D3DD"});
				$(".report-paging-nav-pre").css({"background":"#EBF5FA","padding":"5px","border":"1px solid #C0D3DD"});
				$(".report-paging-nav-next").css({"background":"#F4F4F4","padding":"5px","border":"1px solid #C0D3DD"});
