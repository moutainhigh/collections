			...
			$.ajax({
				...
				success:function(data){
					...
					for (var i=0;i<jsonStr.length;i++) {//<td style='width:117px'>"+_html+"</td>
						$("#columnList").append("<option data-keys="+jsonStr[i].key+" data-name_en="+jsonStr[i].dcTableNameEn+" data-name_cn="+jsonStr[i].dcTableNameCn+" data-collength="+jsonStr[i].colLenght+" data-column_cn ="+jsonStr[i].title+" data-column_en ="+jsonStr[i].code +" data-columntype = "+jsonStr[i].colType+">"+jsonStr[i].title+"</option>");
					...
					$("#columnList option").off("click").on("click",function(){
						var _this = $(this);
						...
						columnDetail.each(function(){
							var _this = $(this);
							var cn = _this.data("column_cn"); //得到当前表的列中已经加入到当前的列表之中
							
						});
						
						if(!isExist){
							_this.remove();
							$("#selectColumnList").append("<option data-keys="+key+" data-name_en="+name_en+" data-name_cn="+name_cn +" data-column_en ="+column_en +" data-column_cn="+column_cn+" data-collength="+collength+" data-columntype = "+columntype+" >"+column_cn+"</option>");
						}
						...
						$("#selectColumnList option").off("click").on("click",function(){
								...
								var collength =_this.data("collength");
								_this.remove();
								$("#getColumDetail").append("<tr data-keys="+key+" data-name_en="+name_en+" data-name_cn="+name_cn +" data-column_en ="+column_en +" data-column_cn="+column_cn+" data-collength="+collength+" data-columntype = "+columntype+"><td>"+name_cn+"</td><td>"+column_cn+"</td><td>"+columntype+"</td><td>"+collength+"</td><td><a href='javascript:;' onclick='clearSelect(this)'>删除</td></tr>");
							});
					});
		...
		<tr><td colspan="2"><a href="javascript:preview();">点击查看预览</a></td></tr>
		 ...
		 <td colspan="2">
			 <div class="dataListTable">
			 	<iframe id="listResult"  width="100%"></iframe>
			 </div>
		 </td></tr> 
	</table><br /></div>
  </dd><!-- 第3个标签页结束 -->
