...
<script type=text/javascript>
		...
		function allaction(){
			...
			if($.trim(pn)!="" && '请输入内容'!=$.trim(pn)){
				 location.href = "<%=request.getContextPath()%>/page/trs/trscase.jsp?search="+encode(pn);//location.href实现客户端页面的跳转  
			}else{
		...
		function searchCount(queryKeyWord){
			$.ajax({
				url : rootPath+'/trsSearchaj/countDualcase.do',
				async: false,
				data : {
		...
		function returnOrgNo(orgno,queryKeyWord,labelFlag){
			...
			$('.results').empty();
			$.ajax({
				url : rootPath+'/datatrscase/querydatacase.do',
				async: false,
				data : {
					pages : orgno,
					queryKeyWord : queryKeyWord,
