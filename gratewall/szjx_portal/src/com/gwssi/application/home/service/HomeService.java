...
public class HomeService extends BaseService {
	 ...
	 */
	public List<Map> getApp() throws OptimusException {

		IPersistenceDAO dao = getPersistenceDAO();

		// 编写查询sql
		StringBuffer sql = new StringBuffer();
		sql.append("select sm.pk_sys_integration,sm.system_name,sm.system_code,sm.system_code as id,sm.integrated_url,sm.system_img_url,sm.system_img_url_code from SM_SYS_INTEGRATION sm where sm.effective_marker = ? order by sm.order_no");
	...
	public List<Map> getFunc(String appCode) throws OptimusException {
		...
		sql.append(" and fun.pk_sys_integration = (select s.pk_sys_integration from SM_SYS_INTEGRATION s where s.system_code = ?)");
	...
	public List<Map> getMessageList(List postList, String type)
		...
		if (postList.size() > 0) {
			...
			for (int i = 0; i < postcode.length; i++) {
				...
				condtion.append(" instr(a.SEND_TO," + postcode[i] + ") > 0 "
						+ operate);
			}
		}
		condtion.append(" ) ");
	 ...
	 * @throws OptimusException
	 */
	public List<Map> getTipSchedule(String userid, String state)
			throws OptimusException {
		IPersistenceDAO dao = getPersistenceDAO();
		List paramList = new ArrayList();
		paramList.add(userid);
	 ...
	 * @throws OptimusException
	 */
	public void updateTipSchedule(String userid, String state)
			throws OptimusException {
		IPersistenceDAO dao = getPersistenceDAO();
		List paramList = new ArrayList();
		paramList.add(state);
