var comparePlugin = function(config){
	if (!config.id){
	...
	this.tableConnectTableHtml =   '  <table width="95%" border="0" align="center" id="' + this.id + 'Container" style="display:none" class="frame-body" cellpadding="0" cellspacing="0">' + 
									'     <tr class="title-row">' + 
									'       <td colspan="9"><table cellspacing="0" cellpadding="0" width="100%"><tr><td class="leftTitle"></td><td class="secTitle">比较条件</td><td class="rightTitle"></td></tr></table></td>' + 
									'     </tr>' + 
									'     <tr><td width="100%"><table border="0" cellspacing="0" style="border:2px solid #006699;" id="' + this.id + '_topTopTable" cellpadding="0" width="100%">' +
									...
									'       <td style="border-right:1px solid #CCCCCC;">比较关系</td>' + 
									'       <td width="14%" style="border-right:1px solid #CCCCCC;">比对表</td>' + 
									'       <td width="18%" style="border-right:1px solid #CCCCCC;">表字段</td>' + 
									'       <td width="6%" style="border-right:1px solid #CCCCCC;">括弧</td>' + 
									'       <td width="7%">操作</td>' + 
									'     </tr>' + 
									...
									'       <td><select name="' + this.id + 'Relation" id="' + this.id + 'Relation" class="connectConditionSelect"><option value="=" selected="selected">等于</option><option value="<>">不等于</option></select></td>' + 
...
// 添加事件
comparePlugin.prototype.addEvent = function(){
	var p = this;
	// 选择表1时
 ...
 */
//comparePlugin.prototype.removeOptionFromSelect = function(selectId, optionValue){
...
// 默认选中columnTwo中的选项
comparePlugin.prototype.setDefaultColumn = function(selectId, optionText){	
	var p = this;
...
// 设置选择数据表
comparePlugin.prototype.setDataFromSelect = function(selectId){
//	var tableGhost = document.getElementById(this.id + "TableGhost");
...
// 设置TableTwo的内容
comparePlugin.prototype.setTableTwoData = function(optionName, optionValue, tblName){
	var tableTwo = document.getElementById(this.id + "TableTwo");
	tableTwo.innerHTML = "";
	var tempTableOption = document.createElement("OPTION");
	tempTableOption.appendChild(document.createTextNode( optionName ));
	tempTableOption.value = optionValue;
	tempTableOption.tblName = tblName;
	tableTwo.options.appendChild(tempTableOption);
}

// 获取表下的字段列表
comparePlugin.prototype.getColumns = function(tableNo, columnSelectId){
	var p = this;
	...
	$.get(p.src + tableNo, function(xml){
		fillXmlToSelect(xml, "record", columnSelectId, "column_no", "column_name_cn", {type: "edit_type", codeValue: "demo", colName:"column_name", colByName:"column_byname"});
 ...
 * 添加一条表连接条件
 */
comparePlugin.prototype.addConnCondition = function(){
	var p 	     = this;
	...
	var connJSONObj = {condition:con1.options[con1.selectedIndex].value, 
					   ...
					   columnOneByName:leftCol.options[leftCol.selectedIndex].colByName,
					   ...
					   columnTwoByName:rightCol.options[rightCol.selectedIndex].colByName,
					   columnTwoNameCn:rightCol.options[rightCol.selectedIndex].text,
					   postParen:rightKH.options[rightKH.selectedIndex].value,
					   paramText:content};
//	alert("添加开始:");
//	alert(connJSONObj);
//	for (var tt in connJSONObj){
//		alert(tt + ":" + connJSONObj[tt]);
//	}				
//	alert("添加完毕!");
...
comparePlugin.prototype.deleteCondition = function(idx){
	var p = this;
	deleteOneRow("conditionTbl_" + p.id, idx);
	var objTable = document.getElementById("conditionTbl_" + p.id);
	if(objTable){
		if(objTable.rows.length == 0){//如果显示条件的表格没有行时，则进行隐藏
			document.getElementById("conn_topTopTable").style.borderBottomWidth = "2px";
 ...
 */
comparePlugin.prototype.validateConnCondition = function(selectedTblId){
	var p = this;
	//获得“已选数据表”对象
 ...
 */
comparePlugin.prototype.getTableId = function(){
	var p = this;
 ...
 * @return String
 */
comparePlugin.prototype.getconnConditionStr = function(){
	var p = this;
	var paramStr = "";
 ...
 * @param 用来比较的JSON对象
 */
comparePlugin.prototype.exist = function(obj){
	if(!obj){
		return false;
	}
	var p = this;
 ...
 * 清除所有连接条件
 */
comparePlugin.prototype.Clear = function(){
	var p = this;
