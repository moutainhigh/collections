var connectCondition = function(config){
	...
	this.tableConnectTableHtml =   '  <table width="95%" border="0" align="center" id="' + this.id + 'Container" style="display:none" class="frame-body" cellpadding="0" cellspacing="0">' + 
									...
									'       <td class="odd2"><select name="' + this.id + 'Condition" id="' + this.id + 'Condition" class="connectConditionSelect" disabled="true">' + 
									'         <option selected="selected"></option>' + 
									'       </select></td>' + 
									'       <td class="odd2"><select name="' + this.id + 'PreParen" id="' + this.id + 'PreParen" class="connectConditionSelect">' + 
									'         <option selected="selected"></option>' + 
									'         <option value="(">(</option>' + 
									'       </select></td>' + 
									'       <td class="odd2"><select name="' + this.id + 'TableGhost" id="' + this.id + 'TableGhost" class="connectConditionSelect" style="display:none"></select>' + 
									'          <select name="' + this.id + 'TableOne" id="' + this.id + 'TableOne" class="connectConditionSelect"></select></td>' + 
									'       <td class="odd2"><select name="' + this.id + 'ColumnOne" id="' + this.id + 'ColumnOne" class="connectConditionSelect"></select></td>' + 
									'       <td class="odd2"><select name="' + this.id + 'Relation" id="' + this.id + 'Relation" class="connectConditionSelect"><option value="=" selected="selected">等于</option><option value="=%2B">左连接</option></select></td>' + 
									'       <td class="odd2"><select name="' + this.id + 'TableTwo" id="' + this.id + 'TableTwo" class="connectConditionSelect"></select></td>' + 
									'       <td class="odd2"><select name="' + this.id + 'ColumnTwo" id="' + this.id + 'ColumnTwo" class="connectConditionSelect"></select></td>' + 
									'       <td class="odd2"><select name="' + this.id + 'PostParen" id="' + this.id + 'PostParen" class="connectConditionSelect">' + 
									'          <option selected="selected"></option>' + 
									'          <option value=")">)</option>' + 
									'       </select></td>' + 
									'       <td class="odd2"><table cellspacing="0" cellpadding="0" class="button_table"><tr><td class="btn_left"></td><td><input type="button" id="'+ this.id + 'AddButton" name="'+ this.id + 'AddButton" value="添加" class="menu" /></td><td class="btn_right"></td></tr></table></td>' + 
									'     </tr></table></td></tr>' + 
									'     <tr id="'+ this.id +'hideRow" style="display:none" class="framerow">'+
                                    '       <td colspan="9" style="padding:0px;">'+
                                    '           <table id="conditionTbl_'+ this.id +'" cellpadding="0" cellspacing="0" width="100%" style="border:2px solid #006699;border-top:none;"></table>'+
                                    '       </td>'+
                                    '     </tr>'+
									'   </table>';
...
connectCondition.prototype.addConnCondition = function(){
	document.getElementById("conn_topTopTable").style.borderBottomWidth = "0px";
	var p 	     = this;
		...
		for(var i = 0; i < connArray.length; i++){
			...
			paramStr += jsonObj.condition +" "
					 ...
					 if(jsonObj.relation != '='){
					 	...
					 	paramStr +=  jsonObj.relation;
					 }
			         paramStr += jsonObj.tableTwoName +"."
					 +  jsonObj.columnTwoName;
					 if(jsonObj.relation == '=%2B'){
					 	paramStr += '(+)';
					 }
			paramStr +=  jsonObj.postParen +" ";
		}
	}
	return paramStr;
}
