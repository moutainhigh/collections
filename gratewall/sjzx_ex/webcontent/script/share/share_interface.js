...
function getDataItemByKey(key, div_container) {
	...
	$
			.ajax({
				type : "post",
				url : "/txn401009.ajax?attribute-node:record_page-row=10000&key="
						+ key,
				async : false,
				success : function(xmlResults) {
					if (xmlResults.selectSingleNode("//context/error-code").text != "000000") {
						...
						if (div_container == 'condition_item_div') {
							opt.onChange = function() {
								...
								if (code_table && code_table != '') {
									...
									$.ajax({
												type : "post",
												url : "/txn20301026.ajax?attribute-node:record_page-row=10000&select-key:code_table="
														+ code_table,
												cache : true,
												async : false,
												dataType : 'xml',
												success : function(xmlResults) {
													if (xmlResults
															.selectSingleNode("//context/error-code").text != "000000") {
														alert(xmlResults
																.selectSingleNode("//context/error-desc").text);
														return false;
													} else {
																		...
																		function() {
																			$(
																					...
																					.append(
																							'<option title="'
																									+ $(this)[0]
																											.selectSingleNode("jcsjfx_dm").text
																									+ '---'
																									+ $(this)[0]
																											.selectSingleNode("jcsjfx_mc").text
																									+ '" value="'
																									+ $(this)[0]
																											.selectSingleNode("jcsjfx_dm").text
																									+ '">'
							...
							for ( var ii = 0; ii < right_options.length; ii++) {
								for ( var jj = 0; jj < datas.length; jj++) {
									if (datas[jj].key == right_options[ii].value) {
										$(right_options[ii]).data("code_table",
												datas[jj].code_table);
										$(right_options[ii]).data("col_type",
												datas[jj].type);
										break;
									}
								}
