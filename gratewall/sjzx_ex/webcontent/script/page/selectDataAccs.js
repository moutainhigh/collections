...
var pageSelectDataAccs = new Object();

pageSelectDataAccs.create = function(contextPath,dataaccgrpidArray,dataaccgrpid,zdyDataaccrule,rolefunids){
	this.contextPath = contextPath;
	this.dataaccgrpidArray = dataaccgrpidArray;
	this.dataaccgrpid = dataaccgrpid;//自定义项id
	this.zdyDataaccrule = zdyDataaccrule;
	this.rolefunids = rolefunids;
	this.init();
}

pageSelectDataAccs.create.prototype = {
	init:function(){
		...
		if(zdyDataaccrule){
			var dataaccruleSelect = $(document.getElementById("record:dataaccrule"));
			$("option[@value="+zdyDataaccrule+"]").attr("selected","selected");
		}
	
		document.getElementById("record:selectGroups").value = this.dataaccgrpidArray.join();
	...
	initDataObjectTree:function(){
		...
		var nodePageUrl = this.contextPath+"/txn103017.ajax?select-key:rolefunids="+this.rolefunids+"&select-key:dataaccgrpid="+this.dataaccgrpid;//not use
	...
	makeSaveXml:function(){
		...
		$("input.jTree_checkbox[@type=checkbox]",this.dataObjectTree).each(function(){
			...
			if(pid!="item_0"){
				...
				if((oChecked=="true"&&checked=="false")||(checked=="true")){
