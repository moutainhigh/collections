...
//						sql += " AND (" + isNoNeedValue ? qcObj.getQueryConditionStrWithoutValue() : qcObj.getQueryConditionStr() + ")";
			...
			if (funcType && funcType=="download" && reg_org == ""){
				...
				if (choosedRegIndBas && !choosedRegBusEnt){
					sql += sqlConn + " reg_indiv_base.reg_org = '" + reg_org + "' ";
	  	...
	  	document.getElementById("preViewButton").onclick = function(){
			...
			document.getElementById("preViewButton").disabled = true;
	  		$.post(rootPath+"/testsql",{testsql:encodeURI(sql)}, function(xml){
				var result = xml.selectSingleNode("/results/sql").text;
				if(result=="false"){
					_hideProcessHintWindow();
					alert("SQL校验失败！请检查后重试");
	    ...
	    function createXMLHttpRequest() {
	  	...
	  	function toAdd(paramArray){
			...
			var post = "record:name=" + paramArray.cxmc
			         ...
			         + "&record:display_columns_en_array="+document.getElementById("record:columnsEnArray").value;
			...
			if (xmlObj.readyState == 4) { // 判断对象状态
				...
				if (xmlObj.status == 200) { // 信息已经成功返回，开始处理信息
					...
					if(errCode != "000000"){
				    	...
				    	alert("保存成功！");
				    	window.location.href = rootPath+"/txn60210001.do?<%=request.getQueryString()%>";
				} else { //页面不正常
 ...
 * 下载回调
 */
function downloadHandler(totalRecord){
	...
	var sUrl = rootPath + "/dw/aic/download/query/sendApply.jsp?max_result="+ max_result +"&totalRecords="+totalRecord;
	showModalDialog(sUrl, self, "dialogWidth:600px;dialogHeight:400px;status:no;");
}