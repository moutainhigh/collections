...
//						sql += " AND (" + isNoNeedValue ? qcObj.getQueryConditionStrWithoutValue() : qcObj.getQueryConditionStr() + ")";
		...
		function handleRespose(){
			...
			if (xmlObj.readyState == 4) { // 判断对象状态
				...
				if (xmlObj.status == 200) { // 信息已经成功返回，开始处理信息
					...
					var url=rootPath+"/txn6025002.do?primary-key:sys_advanced_query_id="+query_id+"&select-key:queryType=1";
					document.getElementById('squeryid').value=query_id;
					//新增高级数据
					toAddQuery(url);
				} else { //页面不正常
	    ...
	    function toAddQuery(url){
		
			xmlObj = createXMLHttpRequest();
			xmlObj.open ('post',url,false);
			xmlObj.onreadystatechange = addQueryRespose;
			xmlObj.setrequestheader("cache-control","no-cache"); 
			xmlObj.setrequestheader("Content-Type","application/x-www-form-urlencoded");
			//xmlObj.setrequestheader('charset','GBK'); 
			xmlObj.send(null);
		...
		function addQueryRespose(){
			var xmlResults = xmlObj.responseXML;
			if (xmlObj.readyState == 4) { // 判断对象状态
				_hideProcessHintWindow();
				if (xmlObj.status == 200) { // 信息已经成功返回，开始处理信息
					alert("保存成功！");	
					//window.location.href = "/txn6025001.do";
	  	...
	  	var paramStr = "?record:downloadflag=1&record:sys_id=";
	  	/*if ( funcType && funcType == "download" ){
...
function downloadHandler(totalRecord, isDownload){
	...
	var regid = document.getElementById('squeryid').value;
