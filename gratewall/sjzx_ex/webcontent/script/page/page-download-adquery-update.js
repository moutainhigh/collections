...
//						sql += " AND (" + isNoNeedValue ? qcObj.getQueryConditionStrWithoutValue() : qcObj.getQueryConditionStr() + ")";
			...
			// alert(sql);
	  		$.post(rootPath+"/testsql",{testsql:sql}, function(xml){
				...
				if(result=="false"){
					...
					//alert(document.getElementById('totalTableDiv').innerHTML);
					document.getElementById('sqlCondition').value=document.getElementById('totalTableDiv').outerHTML;
		...
		if (document.getElementById("saveUpdateButton")){
			document.getElementById("saveUpdateButton").onclick = function(){
				 ...
				 var id = ajaxXml.selectSingleNode("//record/sys_advanced_query_id").text;
				 paramArray = {cxmc:name,cxid:id};
				 xnCode = "/txn60210002.ajax";
				 toUpdate(paramArray);
			}
		}
		if (document.getElementById("downloadButton")) {
			document.getElementById("downloadButton").onclick = downloadHandler;
		}
		...
		function handleRespose(){
			...
			if (xmlObj.readyState == 4) { // 判断对象状态
				...
				if (xmlObj.status == 200) { // 信息已经成功返回，开始处理信息
					...
					alert("修改成功！");
					window.location.href = rootPath+"/txn6025001.do?primary-key:sys_advanced_query_id="+query_id;
				} else { //页面不正常
		...
		function toUpdate(paramArray){
			...
			post=encodeURI(post); 
			xmlObj = createXMLHttpRequest();
			var URL = "/txn60210002.ajax";
			xmlObj.open ('post',URL,false);
			xmlObj.onreadystatechange = handleUpdate;
			xmlObj.setrequestheader("cache-control","no-cache"); 
			xmlObj.setrequestheader("Content-Type","application/x-www-form-urlencoded");
			//xmlObj.setrequestheader('charset','GBK'); 
			xmlObj.send(post);
		...
		function handleUpdate(){
	  	...
	  	var paramStr = "";
	  	if(document.getElementById('queryType').value=='0' ){
	  		paramStr = "?record:queryflag=1&record:sys_id=";
	  	}
	  	if(document.getElementById('queryType').value=='1' ){
	  		paramStr = "?record:downloadflag=1&record:sys_id=";
	  	}
 ...
 * 下载回调
 */
function downloadHandler(totalRecord, isDownload){
	var has_purv = document.getElementById("has_purv").value ;
	if(has_purv != "1"){
		alert("您没有下载的权限！");
		return;
	}
	...
	var regid = ajaxXml.selectSingleNode("//record/sys_advanced_query_id").text;
	var max_result = document.getElementById("max_result").value ;
	var sUrl = rootPath + "/dw/aic/download/query/sendApply.jsp?max_result="+ max_result +"&totalRecords="+totalRecord+"&adquery_id="+regid;
	if (isDownload){// 如果是数据下载
		sUrl += "&download=1";
	}else{// 否则是高级查询
		sUrl += "&download=0";
	}
	var result=showModalDialog(sUrl, self, "dialogWidth:600px;dialogHeight:400px;status:no;");
	if(result=='1'){
	   var url="/txn6025010.ajax?select-key:sys_advanced_query_id="+regid;
	   var page1 = new pageDefine( url, "修改下载时间" );
	   page1.callAjaxService('UpdateTimeBack');
	}
	document.getElementById('goBack1').onclick();
}

	function UpdateTimeBack(errCode, errDesc, xmlResults){
	  if(errCode == '000000'){
			window.location='/txn6025001.do';
	  }
	}