...
function checkEnName(enName, msg, rowIndex) {
	if (typeof(enName) == 'undefined' || enName == '') {
		alert('第' + rowIndex + '行[' + msg + ']不能为空!');
	} else if (/^\d+$/.test(enName) || /['"<>]/.test(enName)) {
		alert('第' + rowIndex + '行[' + msg + ']不能全部是数字!');
		return false;
	} else if (/.*[\u4e00-\u9fa5]+.*$/.test(enName)) {
		alert('第' + rowIndex + '行[' + msg + ']不能含有中文!');
		return false;
	} else if (!/[a-zA-Z]+/.test(enName)) {
		alert('第' + rowIndex + '行[' + msg + ']至少含有一个字母!');
		return false;
	} else if (!/^[a-zA-Z0-9_]{1,100}$/.test(enName)) {
		alert('第' + rowIndex + '行[' + msg + ']不能含有特殊字符!');
		return false;
	} else {
		...
		if (chEnd == "_") {
			alert('第' + rowIndex + '行[' + msg + ']不能以_结尾!')
			return false;
		} else if (!((chBegin >= "a" && chBegin <= "z") || (chBegin >= "A" && chBegin <= "Z"))) {
			alert('第' + rowIndex + '行[' + msg + ']必须以字母开头!')
			return false;
		} else {
			return true;
		}
...
function checkItemLength(item, len, msg, rowIndex) {
	...
	if (item == null || item == "") {
		...
		alert('第' + rowIndex + '行[' + msg + ']长度不能超过'+len);
		return false;
	}else{
		return true;
	}
...
function check_int(id, rowIndex) {
	...
	if (id == null || id == "") {
		...
		alert('第' + rowIndex + '行数据项长度不能超过4位数!');
		return false;
	} else {
		return true;
	}
