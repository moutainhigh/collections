...
%>
		...
		**/
		...
		function search(collId,rows,page,query){
			...
			$.get(url,{"collId":collId,"query":query,"rows":rows,"page":page },
			 function(xml){
			 	$.get("<%=request.getContextPath()%>/search-pages/searchResult2.jsp",function(xslt){
	...
	<form style="margin:0;" action="<%=request.getContextPath()%>/search-pages/search2.jsp" onsubmit="return checkInput()" method="post">
		<div style="margin:10 0 0 0;padding:0 20 0 20">
			<input type="text" name="query" id="query" style="width:300px;" value="<%=query%>"/>
			<input type="submit" value="  搜 索" style="background:url('../../images/search/searchButton.jpg') #EBEDDF no-repeat"/>
		</div>
	</form>
	<div id="result"></div>
</freeze:html>
