...
<%
	...
	String execute_type = "";
	if(db != null){
		state = db.getRecord("select-key").getValue("state");
...
/*	
  var page = new pageDefine( "/txn50207002.do", "数据交换统计");
...
*/  
  ...
  txnCode = '/txn50207002.do';	//_querySubmit();
  _formSubmit(null, "正在查询，请稍候.... ", txnCode);
}
...
function reset(){
	...
	setFormFieldValue("select-key:state", 0, " ");
...
<freeze:form action="/txn50207002">
  <freeze:block property="select-key" caption="查询条件" width="95%">
      ...
      <freeze:select property="state" caption="查询状态" style="width:90%" />
      <freeze:select property="execute_type" caption="交换类型" style="width:36.5%" colspan="2" />
  </freeze:block>
  <br>
  <freeze:grid property="record" caption="数据交换统计列表" width="95%" keylist="sys_svr_user_name" rowselect="false" checkbox="false" navbar="bottom" fixrow="false">
    ...
    <freeze:cell property="sys_svr_user_name" caption="用户名称" width="15%" />
    <freeze:cell property="sys_svr_service_name" caption="服务名称" width="15%" />
    <freeze:cell property="execute_start_time" caption="执行开始时间" width="10%" />
    <freeze:cell property="execute_end_time" caption="执行结束时间" width="10%" />
    <freeze:cell property="execute_type" caption="交换类型" width="5%" />
    <freeze:cell property="state" caption="执行结果" width="5%" />
    <freeze:cell property="records_mount" caption="返回记录数"  align="middle" width="6%"/>
    <freeze:cell property="error_msg" caption="错误描述" width="16%"/>
    <freeze:cell property="client_ip" caption="客户端IP" />
  </freeze:grid>
