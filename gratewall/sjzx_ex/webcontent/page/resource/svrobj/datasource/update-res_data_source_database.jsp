...
function func_testConn()
{
	...
	if(db_instance==null || db_instance==""){
		alert("【数据库实例】不能为空!");
		return;
	}
...
<freeze:form action="/txn20102013">
  ...
  <freeze:block property="record" caption="修改数据源信息" width="95%">
      ...
      <freeze:select property="data_source_type" caption="数据源类型" show="name" valueset="资源管理_数据源类型" notnull="true" readonly="true"  style="width:95%"/>
      <freeze:text property="data_source_name" caption="数据源名称" datatype="string" maxlength="100" notnull="true" style="width:95%"/>
      <freeze:select property="db_type" caption="数据库类型" valueset="资源管理_数据库类型" notnull="true" style="width:95%"/>
      <freeze:text property="db_username" caption="数据源用户名" datatype="string" notnull="true" maxlength="50" style="width:95%"/>
      <freeze:text property="db_password" caption="数据源密码" maxlength="50" notnull="true" style="width:95%"/>
      <freeze:text property="data_source_ip" caption="数据源IP" datatype="string" notnull="true" maxlength="50" style="width:95%"/>
      <freeze:text property="access_port" caption="访问端口" datatype="string" notnull="true" maxlength="8" style="width:95%"/>
      <freeze:textarea property="db_instance" caption="数据源实例" colspan="2" rows="4" maxlength="2000" style="width:98%"/>
      ...
      <freeze:hidden property="access_url" caption="访问URL" datatype="string" maxlength="255" colspan="2" style="width:98%"/>
