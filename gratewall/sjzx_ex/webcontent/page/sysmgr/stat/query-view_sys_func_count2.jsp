...
<freeze:form action="/txn60900001">
  <freeze:block property="select-key" caption="按功能统计" width="100%">
      ...
      <freeze:hidden property="func_index" />
  ...
  <br>
  <%if(context != null){
  	Map map = ContextToMap.parse2Map(context);
  %>
  <div align="center">
	  <div style="border-right:1px solid #2975AF;border-left:1px solid #2975AF;padding-bottom:18px;width:100%;overflow-x:auto;overflow-y:visible;">
		  <TABLE id="frame_record" border="1" cellSpacing="0" cellPadding="0" width="100%" height="100%" align="center" bordercolor="#2975AF">
		  	<tr class="title-row">
		  		<td colspan="<%=3+jgmcArray.length%>"> 按功能统计使用情况列表</td>
		  	</tr>
		  	<tr class="grid-headrow">
		  		<td nowrap="true" class="veticalScrollLockTd">序号</td>
		  		<td nowrap="true" class="veticalScrollLockTd" align="center">日期</td>
		  	...
		  	for(Iterator iter = map.keySet().iterator(); iter.hasNext();){
		  		String queryDate = (String)iter.next();
		  		Map data = (Map)map.get(queryDate);
		  		
		  		rowid ++;
		  	%>
		  		<tr class="<%=rowid%2==0?"oddrow":"evenrow"%>">
		  			<td nowrap="true" class="veticalScrollLockTd" align="center"><%=rowid%></td>
		  			<td nowrap="true" class="veticalScrollLockTd" align="center"><%=queryDate%></td>
		  			<td nowrap="true" class="veticalScrollLockTd" align="center"><%=data.get("total")%></td>
		  			<%for(int i = 0; i < jgidArray.length; i++){
		  				...
		  				%>
		  				<td nowrap="true" align="center"><%=o%></td>
		  			<%}%>
		  		</tr>
		  	<% }%>
		  </table>
