(function() {
	...
	//u为中间格子的宽度
	//r 为圆形半径
	//var a = 650, c = objG.pageHeight, h = c, U = 220, K = objG.gridHeight, S = 20, s = 8, R = 15, J = 35, o = 15, t = 15, w = 1000, F = "elastic", N = "#B79300";
	var a = 850, c = 500, h = c, U = 200, K = 25, S = 20, s = 8, R = 15, J = 35, o = 15, t = 15, w = 1000, F = "elastic", N = "#B79300";
	var T, q, x, j, H, A, P;
	var L = {}, k = {};
	var i, y;
	...
	var d = d3.select("#graph").append("svg").attr("width", a)
			.attr("height", c).append("g").attr("transform",
					"translate(" + a / 2 + "," + c / 2 + ")");
	...
	var B = d.append("g").attr("class", "links"), f = d.append("g").attr(
			"class", "episodes"), E = d.append("g").attr("class", "nodes");
	...
	function O() {
		...
		var ad = 180 + J, Z = 360 - J, ac = (Z - ad)
				/ (T.get("themes").length - 1);
	...
	function G(Y, X) {
		//取消网图钻取功能   add by lxb 2014-2-17
		return ;
		
		/*if(X){
		  if(!isNaN(X)){
			  document.getElementById('turnDetail').href="/page/zwt/zwtDetail.jsp#"+Y.name;
			  document.getElementById('turnDetail').click();
			  return;
		  }	
		}*/
		if (L.node === Y && X !== true) {
			if (Y.type === "episode") {
			    //2013-11-19 begin 当 Y.slug 为 undefined 直接返回
				if(Y.slug==undefined){
					return false;
				}
				//2013-11-19 end			
	...
	function b(X) {
		...
		Y.append("text").attr("class", "label-stroke");
	...
	function C(Z) {
		...
		if (ab && ab.type === "episode") {
			...
			aa.append("text").attr("fill", N).attr("text-anchor", "middle")
					.attr("y", (o + t) * -1).text(function(ae) {
						return "";
					})
		} else {
			if (ab && ab.type === "theme") {
				Y.append("text").attr("fill", "#aaa").attr("text-anchor",
				...
				if (ab && ab.type === "perspective") {
					...
					ad.enter().append("text").attr("fill", "#aaa").attr(
		...
		X.enter().append("text").attr("text-anchor", "start").attr("x",
				a / -2 + t).attr("y", c / 2 - t).text("返回主页面").attr(
				"class", "all-episodes").on("click", O);
		X.exit().remove()
	}
	function D(Y) {
		var Y = f.selectAll(".episode").data(Y, u);
		var X = Y.enter().append("g").attr("class", "episode").on("mouseover",
				g).on("mouseout", n).on("click", G);
	...
	function z() {
		f.selectAll("rect").attr("fill", function(X) {
			return l(X, "#3182BD", N, "#3182BD")
		});
		B
				.selectAll("path")
				.attr("stroke", function(X) {
					return l(X, "#53BEE0", N, "#aaa")
				})
		...
		E.selectAll("text.label").attr(
				"fill",
				function(X) {  //默认字体颜色
					return (X === L.node || X.isGroup) ? "#fff" : l(X, "#3182BD",
							N, "#999")
				})
	}
	function p(X) {
