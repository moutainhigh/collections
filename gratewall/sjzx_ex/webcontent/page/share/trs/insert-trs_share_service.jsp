...
function toEditContent(dbName, dbNamecn){
	...
	var columnArr = columnMap[dbName+'_key'].split(',');
	var columnArrVal = columnMap[dbName+'_value'].split(',');
	var htm = '<tr style=" background: #f2f2f2;"><td colspan="2" style="border: 1px solid #ddd;height:30px;text-align:center;"><span> '
		+dbNamecn+'</span></td></tr>';
	htm += '';
	var searchFlag = true;
	...
	if($('.first a.nSelected').length>1){
		...
		for(var i=0; i<columnArr.length;i++){
			...
			if($('#div'+dbName+' td:eq(1) div:eq('+i+') span.'+dbName+"_query").text().indexOf('检索') > -1){
	   		...
	   		+'<label style="line-height: 1.2;white-space: nowrap; margin:0 5px;"><input value="0" type="checkbox" onclick="display_select_all();" name="'
...
<freeze:body>
<freeze:title caption="新增trs接口"/>
<freeze:errors/>

<freeze:form action="/txn40300003">
  <freeze:block property="record" caption="修改trs接口信息" width="95%">
      ...
      <freeze:textarea property="service_description" caption="服务说明" colspan="1" rows="1" maxlength="2000" style="width:98%"/>
