...
function func_record_testRecord_socket(){
	...
	var id = getFormFieldValues("record:service_id");
	var old_service_no = getFormFieldValues("record:old_service_no");
	var page = new pageDefine( "test-user_svr_socket.jsp","服务参数测试","modal", document.body.clientWidth, document.body.clientHeight);
	page.addValue( id, "svrId" );
	page.addValue( old_service_no, "old_service_no" );
	//page.addValue( "lztest1", "svrId");
	page.updateRecord( );
...
function func_record_testRecord_jms(){
	...
	var id = getFormFieldValues("record:service_id");
	var old_service_no = getFormFieldValues("record:old_service_no");
	var page = new pageDefine( "test-user_svr_jms.jsp","服务参数测试","modal", document.body.clientWidth, document.body.clientHeight);
	page.addValue( id, "svrId" );
	page.addValue( old_service_no, "old_service_no" );
	//page.addValue( "lztest1", "svrId");
	page.updateRecord( );
...
function func_record_addRecord()
{
	...
	var page = new pageDefine( "insert-share_service.jsp", "增加服务表","modal", document.body.clientWidth, document.body.clientHeight);
	page.addRecord();
}
...
function func_record_updateRecord(idx)
{
	...
	var page = new pageDefine( "/txn40200004.do", "修改服务表", "modal", document.clientWidth, document.clientHeight);
...
{
	var svrId = getFormFieldValue("record:service_id", idx);
	var page = new pageDefine( "/txn40200014.do", "查看服务表", "modal", document.clientWidth, document.clientHeight);
	page.addValue( svrId, "primary-key:service_id" );
...
<freeze:form action="/txn40209002">
  <freeze:grid property="record" checkbox="false" multiselect="false" caption="查询服务表列表" keylist="service_id" width="100%" navbar="bottom" fixrow="false">
      ...
      <freeze:cell property="service_targets_id" caption="服务对象" align="center" valueset="资源管理_共享服务对象名称" style="width:15%" />
       <freeze:hidden property="service_targets_name" caption="服务对象" align="center" style="width:15%" />
      <freeze:cell property="service_name" caption="服务名称"  style="" />
      <freeze:hidden property="service_no" caption="服务编号" style="width:10%" />
      <freeze:cell property="old_service_no" caption="服务编号" align="left" style="width:10%" />
      <freeze:cell property="interface_id" caption="接口名称" valueset="共享服务_接口名称" style="width:15%" />
      <freeze:cell property="service_type" caption="服务类型" align="center" valueset="资源管理_数据源类型" style="width:10%" />
      <freeze:hidden property="service_state" caption="服务状态" align="center" valueset="资源管理_归档服务状态" style="width:60px;" />
      <freeze:cell property="created_time" nowrap="true" caption="创建日期" align="center" style="width:75px;" /> 
      <freeze:cell property="oper" nowrap="true" caption="操作" align="center" style="width:145px;" />
