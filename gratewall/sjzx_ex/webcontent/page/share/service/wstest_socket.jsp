...
function collectResCallBack(errorCode, errDesc, xmlResults){
	...
	<%
		...
		fileParam.append("#~#"+CollectConstants.QNAME+"="+CollectConstants.QNAME_NEW);
		fileParam.append("#~#SERVICE_ID="+service_id);
		...
		HashMap fileMap = (HashMap)map.clone();
		...
		String dom = XmlToMapUtil.map2Dom(map);
		dom=dom.replace("\n", "");
		String ip=InetAddress.getLocalHost().getHostAddress();
		System.out.println("ip========"+ip);
		//String socketPort=java.util.ResourceBundle.getBundle("app").getString("socketport");
		String socketPort = config.getServletContext().getInitParameter("socketPort");
		
		
		System.out.println("socket端口========"+socketPort);
		Socket socket = new Socket(ip, Integer.parseInt(socketPort));
		PrintWriter pw = null;
