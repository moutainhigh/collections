...
<freeze:title caption="增加接口信息" />
    ...
    <div id="step1DIV">
			<table width="95%" border="0" align="center" class="frame-body" cellpadding="0" cellspacing="0"> 
			    <tr class="title-row">
			      ...
			      <table width="100%" border="0" cellSpacing="0" cellPadding="0">
					<tr><td class="leftTitle"></td><td class="secTitle">共享字段选择</td>
			    ...
			    <tr class="framerow">
		          <td style="padding:0px;">
		          	<table id="colTbl" width='100%' style="border:2px solid #006699;" cellpadding='0' cellspacing='0' border="0">
						<tr class="grid-headrow">
						    <td width="1%" align="center" nowrap="nowrap">
						    <label style="padding-left:22px;" class="checkboxNew" id="cAll">全选</label></td>
			...
			<table cellspacing="0" cellpadding="0" class="button_table">
				<tr>
					<td class="btn_left"></td>
					<td>
						<input type='button' id='goBack0' class="menu" value=" 关 闭 "
							onclick="closeOpen();" />
					</td>
					<td class="btn_right"></td>
				</tr>
	...
	<tr>
		<td class="btn_left"></td>
		<td>
     	<input type="button" id="goBack1" class="menu" value=" 关 闭 " onclick="closeOpen();"/>
     	</td>
		<td class="btn_right"></td>
	</tr>
</table>
	...
	<tr>
		<td class="btn_left"></td>
		<td>
     	<input type='button' id='goBack2' class="menu" value=" 关 闭 " onclick="closeOpen();"/>
     	</td>
		<td class="btn_right"></td>
	</tr>
</table>
...
queryCondition2addEvent = function(){
    ...
    if (ColumnOne){
        ColumnOne.onchange = function(){
            //alert(this.options[this.selectedIndex].type);
            var columnType=1;
...
var  actiont =  "<%= request.getParameter("actiont")%>";
...
function doInitQuery(){
	...
	$.get("<%=request.getContextPath()%>/txn50203004.ajax?select-key:sys_svr_user_id=" + userId + "&select-key:sys_svr_service_id=" + svrId, function(xml){
		...
		for(var i = 0; i < cols.length; i++){
			...
			//add by dwn20121105 td 样式
			var styleIndex= 2-i%2;
			newTd0.className="even"+styleIndex;
			newTd1.className="odd"+styleIndex;
			newTd2.className="even"+styleIndex;
			newTd3.className="odd"+styleIndex;
			
			if(permitCols){
				for(var j = 0; j < permitCols.length; j++){
					if(colNo == permitCols[j]){
						newTd0.innerHTML = "<input style='margin-left:-1000px;' id='check_"+colNo+"' type='checkBox' name='permitColumn' colName='"+colName+"' colNameCn='"+colNameCn+"' tblName='"+tblName+"' tblNameCn='"+tblNameCn+"' value='"+colNo+"' checked='checked'/>"+
						"<label style='' onclick='changeCheck(this)' name='label_permitColumn' class='checkboxNew' for='check_"+colNo+"' style='background-position-y:top;'></label>";
						break;
					}
					else{
						newTd0.innerHTML = "<input id='check_"+colNo+"' style='margin-left:-1000px;' type='checkBox' name='permitColumn' colName='"+colName+"' colNameCn='"+colNameCn+"' tblName='"+tblName+"' tblNameCn='"+tblNameCn+"' value='"+colNo+"' />"+
						"<label style='' onclick='changeCheck(this)' name='label_permitColumn' class='checkboxNew' for='check_"+colNo+"'></label>";
					}
				}
				//添加别名
				if(aliasJSON){
				...
				newTd0.innerHTML = "<input id='check_"+colNo+"' style='margin-left:-1000px;' type='checkBox' name='permitColumn' colName='"+colName+"' colNameCn='"+colNameCn+"' tblName='"+tblName+"' tblNameCn='"+tblNameCn+"' value='"+colNo+"' />"
				+"<label style='' onclick='changeCheck(this)' name='label_permitColumn' class='checkboxNew' for='check_"+colNo+"' id='label_'"+colNo+"'></label>";
				newTd3.innerHTML = "<input type='text' name='colAlias' />";
			}
			newTd1.innerHTML = colName;
			newTd2.innerHTML = tblNameCn+"："+colNameCn;
	    }
	    forQCondition = null;
    	...
    	if(jump){
    		if(actiont){
    			if(actiont=='new'){
					...
					if(a){
						  a[0].addNode(svrName,svrId,userId);
					}    	
    			}else if(actiont=='edit'){
    				var a  =  window.dialogArguments
					if(a){
						  a[0].resetParentTree(svrName,svrId,userId);
					}  
    			}
    		}
...
function changeCheck(obj){
	var checkId = obj.getAttributeNode("for").value;
	var check = document.getElementById(checkId);
	if(check.checked==true){
		obj.style.backgroundPositionY='bottom';
	}else{
		obj.style.backgroundPositionY='top';
	}
...
function __userInitPage(){
	...
	var saveButtons = document.getElementsByName("toSaveButton");
 	...
 	document.getElementById("cAll").onclick = function(){ 
	    ...
	    var m = document.getElementsByName("permitColumn");
	    var ss = document.getElementsByTagName("label");
	    var l = m.length;
	    if(f.style.backgroundPositionY=='bottom'){
		    for ( var i=0; i< l; i++)
		    {
		        m[i].checked = true;
		        ss[i+1].style.backgroundPositionY='top';
		    }
		    f.style.backgroundPositionY='top';
	    }else{
	    	for ( var i=0; i< l; i++)
		    {
		        m[i].checked = false;
		        ss[i+1].style.backgroundPositionY='bottom';
		    }
