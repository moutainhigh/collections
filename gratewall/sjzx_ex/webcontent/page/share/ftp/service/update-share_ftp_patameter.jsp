...
var service_id = getFormFieldValue('record1:service_id');
...
function func_record_addRecord(){
	...
	$('#add_table .btn_save').unbind("click");
	$('#add_table .btn_save').bind("click", function(){addParamValue(1)});
...
function addParamValue(type){
	...
	var srv_param_id = $('input[name="srv_param_id"]').val();
	var ftp_service_id=getFormFieldValue('record1:ftp_service_id');
	//var ws_id = document.getElementById('primary-key:ftp_service_id').value;
	...
	var page = new pageDefine("/txn40402003.ajax", "添加参数值");
	if(type==0){
		page = new pageDefine("/txn40402002.ajax", "修改参数值");	
		page.addValue(srv_param_id, "record:srv_param_id");
	}
	
	page.addValue(ftp_service_id, "record:ftp_service_id");
	page.addValue(service_id, "record:service_id");
	page.addValue(service_targets_id, "record:service_targets_id");
	...
	function addParamValueCallBack(errCode, errDesc, xmlResults) {
		...
		//var href='http://localhost:7001/txn40401004.do?primary-key:service_id=' + getFormFieldValue("record1:service_id")
		//+'&primary-key:service_targets_id='+getFormFieldValue("record1:service_targets_id"); 
		//alert(href);
		//$('#reload').attr("href", href); 
		$('#reload')[0].click();
		
		//$('#reload').attr("href", '/txn40401004.do?primary-key:service_id=' + getFormFieldValue("record1:service_id")
		//+'&primary-key:service_targets_id='+getFormFieldValue("record1:service_targets_id")); 
		//$('#reload')[0].click();
	}
	...
	if(confirm("是否删除选中的记录")){
		if(typeof(id) != 'undefined'){
			var page = new pageDefine("/txn40402005.ajax", "删除参数值");	
			page.addValue(id, "record:srv_param_id");
			page.addValue(service_id, "record:service_id");
			page.addValue(service_targets_id,"record:service_targets_id");
			//page.addParameter("record-key:ftp_service_id", "primary-key:ftp_service_id");
	...
	function func_record_updateRecord(id){
		...
		if(typeof(id) != 'undefined' && lineNo > -1){
			...
			var srv_param_id = getFormFieldValue("record:srv_param_id", lineNo);
			...
			setParamType();
			//$('#add_table #param_type option[text="'+param_value_type+'"]').attr("selected", true);
			...
			$('#add_table input[name="srv_param_id"]').val(srv_param_id);
			
			//addParamValue(0); //调用修改方法
			$('#add_table .btn_save').unbind("click");
			$('#add_table .btn_save').bind("click", function(){addParamValue(0)});
	...
	function __userInitPage() {
		...
		var ids = getFormAllFieldValues("record:srv_param_id");
...
<freeze:form action="/txn40402002"> 
  <freeze:frame property="primary-key" width="95%">
      <freeze:hidden property="srv_param_id" caption="参数ID" style="width:95%"/>
  </freeze:frame>
  <freeze:frame property="param" width="95%">
   ...
   <freeze:grid property="record" rowselect="true" checkbox="false" caption="维护参数信息" width="95%">
      ...
      <freeze:cell property="param_value_type" valueset="采集任务_参数类型" caption="参数类型" datatype="string"  style="width:20%"/>
      <freeze:cell property="patameter_name" caption="参数名" datatype="string"  style="width:15%"/>
      <freeze:cell property="patameter_value" caption="参数值" datatype="string"  style="width:20%"/>
      <freeze:cell property="style" valueset="采集任务_日期格式" caption="参数格式" datatype="string"  style="width:15%"/>
      <freeze:cell property="showorder" caption="参数顺序" style="width:10%"/>
      <freeze:hidden property="srv_param_id" caption="参数值id" datatype="string"  style=""/>
      <freeze:cell property="oper" caption="操作" align="center" style="width:15%" />
  </freeze:grid>
  <div id='add_table' style='display:none;' ><br />
		<table class='frame-body' align='center' cellpadding='0' cellspacing='0' style='width: 95%; border: 1px solid #ccc;'>
			<tr height='30' class='grid-headrow'>
				...
				<td id='date_format_title' style='display:none;'>日期格式</td>
			...
			<tr height='30' class='oddrow'>
				...
				<input type='hidden' name='srv_param_id' />
				</td>
			</tr>
			<tr height='30' class='oddrow'>
