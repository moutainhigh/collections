...
function func_record_saveRecord(){
	...
	document.forms["CollectTaskUpdateForm"].setAttribute("action", "/txn30101063.do");
	saveRecord( '', '保存采集任务信息' );
	/**
...
function getParameter(){
   ...
   var collectType = "<%=ExConstant.TYPE_SJYLX_JMS%>";
...
function func_record_addRecord()
{
    ...
    if(collect_task_id==null||collect_task_id==""){
	 ...
	 var page = new pageDefine( "/txn30102017.do", "增加方法");
	 page.addValue(collect_task_id,"primary-key:collect_task_id");
	 page.addValue("","primary-key:webservice_task_id");
...
{
	var page = new pageDefine( "/txn30102018.do", "修改方法信息");
...
if(confirm("是否删除选中的记录")){
	var page = new pageDefine( "/txn30102021.do", "删除方法信息" );
	page.addValue(idx,"primary-key:webservice_task_id");
...
{
	var page = new pageDefine( "/txn30102020.do", "查看方法信息", "modal" );
	page.addValue(idx,"primary-key:webservice_task_id");
	page.addParameter("select-key:service_targets_id","select-key:service_targets_id");
	page.addParameter("select-key:collect_type","select-key:collect_type");
	page.addParameter("select-key:task_status","select-key:task_status");
...
function changeTarget()
{
	if (getFormFieldValue("record:service_targets_id") != null && getFormFieldValue("record:service_targets_id") != "")
	{
		...
		setFormFieldValue("record:task_name",name+"_JMS");
...
<freeze:form action="/txn30101062" enctype="multipart/form-data">
...
<br>
   <freeze:grid property="dataItem" caption="方法列表" keylist="webservice_task_id" multiselect="false" checkbox="false" width="95%" navbar="bottom" fixrow="false" >
     <freeze:button name="record_addRecord" caption="添加方法" txncode="20202003" enablerule="0" hotkey="ADD" align="right" onclick="func_record_addRecord();"/>
