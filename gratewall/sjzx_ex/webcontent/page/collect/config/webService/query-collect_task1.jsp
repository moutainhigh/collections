...
{
	if(type=="WebService"||type=="FTP"||type=="SOCKET"){
...
{
	if(type=="WebService"||type=="SOCKET"){
...
{
	if(type=="WebService"||type=="SOCKET"||type=="FTP"||type=="文件上传"||type=="数据库"){
...
function __userInitPage()
{
	...
	for(var i=0; i<ids.length; i++){
	   ...
	   if(task_status[i]=="Y"){
	   htm+='<a href="#" title="停用" onclick="func_record_changeOneStatus(\''+ids[i]+'\',\''+task_status[i]+'\',\''+type[i]+'\');"><div class="stop"></div></a>&nbsp;';
	   }else
	   {
	   htm+='<a href="#" title="启用" onclick="func_record_changeOneStatus(\''+ids[i]+'\',\''+task_status[i]+'\',\''+type[i]+'\');"><div class="run"></div></a>&nbsp;';
	   }
...
<freeze:form action="/txn30101001">
  <freeze:grid property="record" caption="查询采集任务列表" keylist="collect_task_id" width="100%"  multiselect="false" checkbox="false" navbar="bottom" fixrow="false">
      ...
      <freeze:cell property="service_targets_id" caption="所属服务对象" show="name" valueset="资源管理_服务对象所有名称" style="width:13%" />
      <freeze:cell property="data_source_id" caption="数据源"  show="name"  valueset="资源管理_数据源"  style="width:18%"/>
      ...
      <freeze:cell property="collect_status" caption="采集状态"  style="width:13%" show="name" valueset="采集任务_采集状态" />
      <freeze:cell property="created_time" caption="创建时间" style="width:12%" />
      <freeze:cell property="oper" caption="操作" style="width:12%" />
