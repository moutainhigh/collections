...
{
	var type=document.getElementById("record:scheduling_type").value;
	if(!type){
		if(confirm("没有选择计划任务类型，采集周期设置不会生效，确定要这样做吗？")){
	...
	var page = new pageDefine( "/txn30801003.ajax", "保存任务调度信息");
	page.addParameter("record:collect_task_id","record:collect_task_id");
	page.addParameter("record:task_scheduling_id","record:task_scheduling_id");
...
function insertTaskSchedule(errCode,errDesc,xmlResults){
		...
		if(errCode != '000000'){
		    ...
		    var task_scheduling_id=_getXmlNodeValues(xmlResults,'record:task_scheduling_id');
		    ...
		    window.dialogArguments.document.getElementById("record:task_scheduling_id").value = task_scheduling_id; 
		    window.dialogArguments.document.getElementById("record:scheduling_type").value = scheduling_type; 
		    window.dialogArguments.document.getElementById("record:start_time").value = start_time; 
		    window.dialogArguments.document.getElementById("record:end_time").value = end_time; 
		    window.dialogArguments.document.getElementById("record:scheduling_week").value = scheduling_week; 
		    window.dialogArguments.document.getElementById("record:scheduling_day").value = scheduling_day; 
		    window.dialogArguments.document.getElementById("record:scheduling_count").value = scheduling_count; 
		    window.dialogArguments.document.getElementById("record:interval_time").value = interval_time; 
		    window.dialogArguments.document.getElementById("record:scheduling_day1").value = cjzq; 
		    window.close();
		}
}
...
document.getElementById("record:collect_task_id").value=window.dialogArguments.document.getElementById("record:collect_task_id").value;
document.getElementById("record:task_scheduling_id").value=window.dialogArguments.document.getElementById("record:task_scheduling_id").value;
