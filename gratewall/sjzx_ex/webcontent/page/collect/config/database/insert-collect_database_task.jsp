...
function checkTaskBack(errorCode, errDesc, xmlResults){
	if(errorCode!="000000"){
		...
		if(task_num>0){
			...
			collect_task_id = getFormFieldValue("record:collect_task_id");
			saveAndExit( '', '保存采集数据库表','/txn301010311.do?record:collect_task_id='+collect_task_id);	// /txn30501001.do
		}
...
<freeze:body>
<freeze:title caption="增加采集数据库信息"/>
<freeze:errors/>

<freeze:form action="/txn30501003">
  <freeze:block property="record" caption="增加采集数据库信息" width="95%">
