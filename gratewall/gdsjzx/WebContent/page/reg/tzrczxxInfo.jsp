		...
		$('.jazz-grid-row').click(function () {
			...
			if($(tds[3]).html()=="法人"){
				for(var i=1;i<tds.length;i++){
					$('#row'+i).text("");
					if($(tds[i]).html()=="&nbsp;"){
						$('#formpanel1 #row'+i).text("");
					}else{				
						$('#formpanel1 #row'+i).text($(tds[i]).html());
					}
				}
				$("#formpanel1").show(); 
				$("#formpanel2").hide(); 
				...
				for(var i=1;i<tds.length;i++){	
					$('#row'+i).text("");
					if($(tds[i]).html()=="&nbsp;"){
						$('#formpanel2  #row'+i).text("");
					}else{		
						$('#formpanel2  #row'+i).text($(tds[i]).html());
					}
				}
				$("#formpanel2").show(); 
				$("#formpanel1").hide(); 
...
function renderdata(event, obj){
	...
	//	console.info($('#hiddenSum',window.parent.document));
		...
		if(regtype=='gt'){
			if(conam!=undefined){
				data[i]["conam"] = conam+'元';
			}
			if(subconam!=undefined){
				data[i]["subconam"] = subconam+'元';
			}
			if(acconam!=undefined){
				data[i]["acconam"] = acconam+'元';
			}
			if(acconam==null){
				data[i]["conprop"] = '0%';
			}else{
...
<div vtype="gridpanel" name="jbxxGrid" height="236" width="100%"  id='jbxxGrid' dataloadcomplete=""  datarender="renderdata()"
	...
	<div vtype="gridcolumn" name="grid_column" width="100%">
		...
		<div name='subconam' text="认缴出资额" textalign="center"  width="10%"></div>
 ...
 -->	</div>
	...
	<div vtype="gridtable" name="grid_table"></div>
  	...
  	<div id="formpanel1" class="formpanel_table">
	...
	<div id="formpanel2" class="formpanel_table">
		<div class="font_title">详细信息</div>
