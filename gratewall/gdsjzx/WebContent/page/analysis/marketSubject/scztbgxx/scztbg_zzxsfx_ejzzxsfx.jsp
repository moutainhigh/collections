...
$(function () {
	...
	$("#organizationScope").hide();
	$("#organizationScopeLabe").hide();
	$("#selData_2").hide();
});
...
function insertNew(){
	y1.push(0);y2.push(0);y3.push(0);y4.push(0);y5.push(0);y6.push(0);y7.push(0);y8.push(0);y9.push(0);y10.push(0);
}
//根据code判断产业类型
function type(code2){
	...
	if(code2 == "公司")
		...
		return y2;
	 else if(code2 == "内资非公司企业法人")
		return y3;
	else if(code2 == "合伙企业")
		return y4;
	 else if(code2 == "集团")
		return y5;
	else if(code2 == "内资非法人企业")
		return y6;
	 else if(code2 == "农民专业合作经济组织")
		return y7;
	else if(code2 == "个人独资企业")
		return y8;
	else if(code2 == "外资非公司企业")
		return y9;
	else
		return y10;
}
...
function getData(exe){
	...
	$.ajax({
		url: '<%=request.getContextPath()%>/analysis/enterpriseMarketSubjectInfo.do',
	    //	async : false,
		dataType : 'json',
		type : "POST", //请求方式
		data:data,
...
function drawHighCharts(xs,y1,y2,y3,y4,y5,y6,y7,y8,y9,y10,title, code,j){
	$('#container').highcharts({
        ...
        series: [{ m_t: 0, name: '公司',data: y1,}, 
                 { m_t: 1, name: '外国企业',data: y2,}, 
                 { m_t: 2, name: '内资非公司企业法人',data: y3, },
                 { m_t: 3, name: '合伙企业',data: y4, },
                 { m_t: 4, name: '集团',data: y5,}, 
                 { m_t: 5, name: '内资非法人企业',data: y6, },
                 { m_t: 6, name: '农民专业合作经济组织',data: y7, },
                 { m_t: 7, name: '个人独资企业',data: y8, },
                 { m_t: 8, name: '外资非公司企业',data: y9, },
                 { m_t: 9, name: '个体工商户',data: y10, }]
    });
}
...
function initIndusCodes(m_t,seriesName){
	//根据区域加载市级列表
	$.ajax({
		url: '<%=request.getContextPath()%>/analysis/sencondEnterpriseMarketSubjectInfo.do',
		dataType : 'json',
		type : "POST", //请求方式
		data:{type:m_t},
		success : function(data) {
			$("#organizationScope").html('');
			for(var i = 0; i < data.length; i ++){
				if(seriesName==data[i].v1){
				$("#organizationScope").append("<option selected value='" + data[i].c1 + "'>" + data[i].v1 + "</option>");
				}else{
				$("#organizationScope").append("<option value='" + data[i].c1 + "'>" + data[i].v1 + "</option>");
				}
			}
...
function initSecondAna(startTime,endTime,dateType, code, m_t, title1, title2, organizationScope){
	//加载charts图表
	$.ajax({
		url: '<%=request.getContextPath()%>/analysis/secondByEnterpriseByMarketSubject.do',
	    //	async : false,
		dataType : 'json',
		type : "POST", //请求方式
		data:{startTime:startTime, endTime:endTime, measureCode:code, type:m_t,dateType:dateType,organizationScope:organizationScope},
...
<body>
<div id="Container">
    <div class="headertop" id="Header" style="background:url(<%=request.getContextPath()%>/static/images/system/index/B-05.png); height:78px; width:98.5%;">
       	<input class="btu" type="radio" name="gender" id="x" value="0" checked="checked"><label for="x">按日</label>
        <input class="btu" type="radio" name="gender" id="y" value="1"><label for="y">按月</label><br/><br/>
		...
		<!-- <input class="btu" class="btu" id="showtime" type="text" readonly="readonly" class="Wdate"
			value='2016-07-01' onclick="createWdatePickerEnd();"/> -->
		度量维度:<select class="btu" id="busType" name="dept" style="width: 150px;"></select>
		<label id="organizationScopeLabe" for="区域范围">组织形式：</label><select class="btu" id="organizationScope" name="dept_1" style="width: 150px;"></select>
