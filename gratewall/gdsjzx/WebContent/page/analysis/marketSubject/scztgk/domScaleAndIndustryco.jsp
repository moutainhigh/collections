...
function initSecCharts(){
    ...
    initSecondAna(null,null,null, $('#Header input[name="gender"]:checked ').val(), 
    		     //$("#induType").val(), $("#induType option:selected").text(), 
    		     tempParentCode, null, 
    		     $("#busType option:selected").text(),getIndusCode() ,getSelectedIndusCode());
}
//第一层
function initCharts(){
	//设置加载gif @TODO
				    ...
				    //    groupPadding : 0.5, 
						events: {
						    click: function(e) {
							   ...
							   tempParentCode = t;
							   initIndusCodes(t);
							   initSecondAna($("#showstarttime").val(), $('#Header input[name="gender"]:checked ').val(),
...
function getData(exe){
	...
	$.ajax({
		url: '<%=request.getContextPath()%>/analysis/domScaleAndIndustycoPageInfo.do',
	    //	async : false,
		dataType : 'json',
		type : "POST", //请求方式
		data:data,
...
<div id="Container">
    <div class="headertop" id="Header">
       	时间维度: <input style="display:none" class="btu" type="radio" name="gender" id="x" value="0" checked="checked"><label style="display:none" for="x">按日</label>
        <input style="display:none" class="btu" type="radio" name="gender" id="y" value="1"><label style="display:none" for="y">按月</label>
		<label id="showstart" for="从">从:</label><input class="btu" id="showstarttime" type="text" readonly="readonly" class="Wdate"
			value='2016-05-01' onclick="createWdatePickerStart();" /> 
		<label style="display:none" id="showend" for="至">至:</label><input style="display:none" class="btu" id="showendtime" type="text" readonly="readonly" class="Wdate"
		...
		<!-- <input class="btu" class="btu" id="showtime" type="text" readonly="readonly" class="Wdate"
			value='2016-07-01' onclick="createWdatePickerEnd();"/> -->
		分析指标:<select class="btu_select" id="busType" name="dept" style="width: 150px;"></select>
		<label id="indusTypeLabe" for="产业类型">产业类型:</label><select class="btu_select" id="indusType" name="dept_1" style="width: 150px;"></select>
		<label id="enterpriseTypeLabe" for="注册资金规模">注册资金规模:</label><select class="btu_select" id="enterpriseType" name="dept_1" style="width: 150px;"></select>
