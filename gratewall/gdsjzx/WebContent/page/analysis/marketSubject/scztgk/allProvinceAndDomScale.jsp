...
function getData(exe){
   ...
   /*  var economicCode = $("#indusType option:selected").val(); */
	...
	$.ajax({
		url: '<%=request.getContextPath()%>/analysis/allProvinceAndDomScalePageInfo.do',
	    //	async : false,
		dataType : 'json',
		type : "POST", //请求方式
		data:data,
...
function initSecondAna(startTime,endTime,dateType, code, m_t, title1, title2, indusCode){
	...
	$.ajax({
		...
		dataType : 'json',
		type : "POST", //请求方式
		data:{startTime:startTime, endTime:endTime, measureCode:code, type:m_t,dateType:dateType,indusCode:indusCode},
...
<div id="Container">
    <div class="headertop" id="Header">
       	日期格式: <input style="display: none" class="btu" type="radio" name="gender" id="x" value="0" checked="checked"><label style="display:none" for="x">按日</label>
        <input style="display: none" class="btu" type="radio" name="gender" id="y" value="1"><label  style="display:none" for="y">按月</label>
		<label id="showstart" style="display:none" for="从">从:</label><input class="btu" id="showstarttime" type="text" readonly="readonly" class="Wdate"
		...
		<label id="enterpriseTypeLabe" for="全省各市">全省各市:</label><select class="btu_select" id="enterpriseType" name="dept_1" style="width: 150px;"></select>
		<label id="indusTypeLabe" for="注册资金规模">注册资金规模:</label><select class="btu_select" id="indusType" name="dept_1" style="width: 150px;"></select>
