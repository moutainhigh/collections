...
/* if("1" == t){//为市场主体概况，否则为设立登记
  ...
  function drawHighCharts(rederTo,chartType,title,xName,yName,series){
   draw = {
		...
		$(function () {
			var organizationmode = getUrlParam("organizationmode");
			...
			//	$("#businessCount").append("<option value='002005'>本期登记注册资本(金)(万元)</option>");
			    ...
			    var data = {organizationmode:organizationmode,startdt:startdt};
		    	if(_data_type_table)
		    		data.isBG = 1;
				$.ajax({
  			 url: '<%=request.getContextPath()%>/analysis/iniZzxsAnalystsArea.do',
					async : false,
					dataType : 'json',
					data:data,
					type : "POST", //请求方式
				...
				var data={selType:setType,startdt:startdt,enddt:enddt,businessCount:measure,organizationmode:organizationmode};
	...
	function selData(){
		//	var selType=$("selType").val();
			...
			var data={organizationmode:getSelected(),selType:selType,startdt:showstarttime,enddt:showendtime,businessCount:businessCount};
			loadChars(title,rederTo,chartType,yName,data);
	}
...
function loadChars(title,rederTo,chartType,yName,data){
	...
	function  groupSeries(data,name,xName){
		var transdt=[];
	//	var selType=$("#selType").val();gender
		var selType = $('input[name="gender"]:checked ').val();
		var showstarttime=$("#showstarttime").val();
		var showendtime=$("#showendtime").val();
		for(var k=0;k<xName.length;k++){
			...
			if(name==data[i].value && data[i].transdt==xName[k]){
					var  seriesData; 
					var date=data[i].measurevalue;
					seriesData={name:data[i].organizationmode,/*dataLabels: {enabled: true,align:'left'},*/y:date};
