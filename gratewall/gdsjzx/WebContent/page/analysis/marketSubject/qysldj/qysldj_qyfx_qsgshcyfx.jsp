...
function getData(exe){
	...
	if(_data_type_table)
		data.isBG = 1;
	$.ajax({
		url: '<%=request.getContextPath()%>/analysis/industryAndProvinceInfo.do',
	    //	async : false,
		dataType : 'json',
		type : "POST", //请求方式
		data:data,
...
function initSecondAna(startTime,endTime,dateType, code, m_t, title1, title2, indusCode){
	var data = {startTime:startTime,  measureCode:code, type:m_t,indusCode:indusCode, dateType:dateType};
	if(_data_type_table)
		data.isBG = 1;
	//加载charts图表
	$.ajax({
		url: '<%=request.getContextPath()%>/analysis/valueByIndustry.do',
	    //	async : false,
		dataType : 'json',
		type : "POST", //请求方式
		data:data,
		success : function(data_all) {
			showSec();
			//{"t":"广东省阳江市工商行政管理局","s":323350.000000,"c":"水果种植"}
			if(data_all != null){
				var x = new Array();
				...
				if(data_all.length > 0) {
					...
					for(i = 0 ; i < data_all.length; i ++){
						(series[getIndex(names, data_all[i].c)]).data.push({x:getIndex(x,data_all[i].t),y:data_all[i].s});
					}
...
<div id="Container">
    <div class="headertop" id="Header">
		...
		<label style="display:none"  id="showstart" for="开始">从:</label><input class="btu" id="showstarttime" type="text" readonly="readonly" class="Wdate"
			value='2016-05-01' onclick="createWdatePickerStart();" /> 
		<label style="display:none"  id="showend" for="结束">至:</label><input style="display:none"  class="btu" id="showendtime" type="text" readonly="readonly" class="Wdate"
