...
function loadChars(title,rederTo,chartType,yName,data){
	if(_data_type_table)
		data.isBG = 1;
	$.ajax({
  			 url: '<%=request.getContextPath()%>/analysis/zzxsAnalystsSum.do',
				//	async : false,
					dataType : 'json',
					type : "POST", //请求方式
					data:data,
				...
				//	alert(xName.length);
					...
					for(var k=0;k<data.length;k++){
						var parentValue=data[k].parentValue;
						if(regorgrStr.indexOf(parentValue)==-1){
						regorgrStr=regorgrStr+"/"+parentValue;
						}
					}
					...
					// 按区域分析 ---- 标题
					drawHighCharts(rederTo,chartType,title,xName,yName,series,xName.length-1);
						} 
					}
				});
	}
	...
	//		var selType=$("#selType").val();
		...
		for(var k=0;k<xName.length;k++){
			...
			var url="<%=request.getContextPath()%>/page/analysis/marketSubject/qysldj/qysldj_zzxsfx_drillejzzxsfx.jsp";
			if(name==data[i].parentValue && data[i].transdt==xName[k]){
					var  seriesData; 
					var date=data[i].measurevalue;
					seriesData={name:data[i].organizationmode,/*dataLabels: {enabled: true,align:'left'},*/y:date,url:url+"?organizationmode="+data[i].organizationmode};
...
<div id="Container">
    <div class="headertop" id="Header">
		...
		<label id="showstart" for="开始">开始:</label><input class="btu_select" id="showstarttime" type="text" readonly="readonly" class="Wdate"
			value='2016-05-01' onclick="createWdatePickerStart();" /> 
		<label id="showend" for="结束">结束:</label><input class="btu_select" id="showendtime" type="text" readonly="readonly" class="Wdate"
