...
changeTitle(t);
  ...
  function drawHighCharts(rederTo,chartType,title,xName,yName,series,j){
   draw = {
                ...
                cursor: 'pointer',
                 point: {
                    events: {
                        click: function (e) {
                        if(_data_type_table){
                          location.href = this.options.url+"&measure="+$('#businessCount').val()+"&startdt="+$('#showstarttime').val()+"&enddt="+$('#showendtime').val()+"&setType="+	$('input[name="gender"]:checked').val()+"&bt=1"+"&at="+t;
                        }else{
                          location.href = this.options.url+"&measure="+$('#businessCount').val()+"&startdt="+$('#showstarttime').val()+"&enddt="+$('#showendtime').val()+"&setType="+	$('input[name="gender"]:checked').val()+"&at="+t;
                        }
                        }
                    }
                }
		...
		//
		var title="<h3>企业设立登记全省各市和组织形式分析</h3>";
...
$(function () {
			...
			//	$("#businessCount").append("<option value='002005'>本期登记注册资本(金)(万元)</option>");
				...
				var data={selType:"0",level:"1",startdt:strDate,enddt:entDate,businessCount:$("#businessCount").val()};
...
function loadChars(title,rederTo,chartType,yName,data){
	if(_data_type_table)
		data.isBG = 1;
	$.ajax({
  			 url: '<%=request.getContextPath()%>/analysis/businessCityAnalystsType.do',
				//	async : false,
					dataType : 'json',
					type : "POST", //请求方式
					data:data,
				...
				//	alert(xName.length);
					...
					// 按区域分析 ---- 标题
					drawHighCharts(rederTo,chartType,title,xName,yName,series,xName.length);
						} 
					}
				});
	}
	...
	function  groupSeries(data,name,xName){
		...
		for(var k=0;k<xName.length;k++){
			...
			var url="<%=request.getContextPath()%>/page/analysis/marketSubject/qysldj/qysldj_zzxsfx_drillqsgsfx.jsp";
			if(name==data[i].c && data[i].t==xName[k]){
					var  seriesData; 
					var date=data[i].s;
					seriesData={/*dataLabels: {enabled: true,align:'left'},*/y:date,url:url+"?drillType="+data[i].d+"&showstarttime="+showstarttime+"&showendtime="+showendtime + (t=="1"?"&at=1":"")};
					...
					var date=0;	
					var  seriesData; 
					seriesData={/*dataLabels: {enabled: true,align:'left'},*/y:date,url:url+"?organizationmode="+data[i].organizationmode+"&showstarttime="+showstarttime+"&showendtime="+showendtime+ (t=="1"?"&at=1":"")};
					transdt.push(seriesData);
						}
					}
				}
...
function initMeasureData(){
	$.ajax({
		...
		success : function(data) {
			...
			if(data != null){
			    if(data.length > 0)
			    	$("#businessCount").append("<option selected value='" + data[0].mc + "' mk='"+(data[0].mk==null ? 0 : data[0].mk)+"'>" + data[0].mn + "</option>");
				for(var i = 1; i < data.length; i ++) {
					$("#businessCount").append("<option value='" + data[i].mc + "' mk='"+(data[i].mk==null ? 0 : data[i].mk)+"'>" + data[i].mn + "</option>");
				}
			}
 		}
		//
...
<body>
    ...
    <div class="headertop" id="Header">
		...
		<label id="businessCountLabel" for="分析指标"><label id='busTypeLabel'>分析指标:</label></label><select class="btu_select" id="businessCount" name="dept" style="width: 150px;"></select>
		<label style="display:none" id="organizationmodeLabel" for="组织形式">组织形式:</label><select class="btu_select" id="organizationmode" name="dept_1" style="width: 150px;display:none"></select>
