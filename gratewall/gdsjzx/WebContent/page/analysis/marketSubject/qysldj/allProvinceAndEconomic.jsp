...
function getData(exe){
   ...
   /*  var economicCode = $("#indusType option:selected").val(); */
	...
	var data={startTime:showstarttime,endTime:showendtime,measureCode:measureCode,enterpriseCode:enterpriseCode};
	if(_data_type_table)
		data.isBG = 1;
	$.ajax({
		url: '<%=request.getContextPath()%>/analysis/allProvinceAndEconomicPageInfo.do',
	    //	async : false,
		dataType : 'json',
		type : "POST", //请求方式
		data:data,
...
<div id="Container">
    <div class="headertop" id="Header">
       	...
       	<input class="btu" type="radio" name="gender" id="x" value="0" checked="checked">
       	<label for="x">按日</label>
        <input class="btu" type="radio" name="gender" id="y" value="1">
        <label for="y">按月</label>
        </span>
		<label style="display:none" id="showstart" for="从">从:</label><input class="btu" id="showstarttime" type="text" readonly="readonly" 
			value='2016-05-01' onclick="createWdatePickerStart();" /> 
		<label style="display:none" id="showend" for="至">至:</label><input style="display:none" class="btu" id="showendtime" type="text" readonly="readonly" 
