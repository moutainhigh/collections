			...
			drillType = getUrlParam("drillType");
  			 ...
  			 url: '<%=request.getContextPath()%>/analysis/iniBusinessAnalystsBusiness.do',
				//	async : false,
					...
					var html="<option value="+data[3][k].code+">"+data[3][k].value+"</option>"
				...
				$("#regType").val("area");
				$("#businessCount").val("1");
				var data={selType:"1",businessCount:"1",level:"2"};
...
/* 	function loadMultiSelectValue(name){ 
	...
	function selData(){
			...
			var data={selArea:getSelected(regType),regType:regType,startdt:showstarttime,accepttype:accepttype,businessCount:businessCount,level:"2"};
			loadChars(title,rederTo,chartType,yName,data);
	}
...
function loadChars(title,rederTo,chartType,yName,data){
	...
	function  groupSeries(data,name,xName){
		...
		for(var k=0;k<xName.length;k++){
			...
			if(name==data[i].qyvalue && data[i].ywvalue==xName[k]){
					var  seriesData; 
...
<div id="Container">
    <div class="headertop" id="Header" >
		...
		<label style="display:none" id="showstart" for="开始">开始</label><input class="btu Wdate" style="width:110px" id="showstarttime" type="text" readonly="readonly" 
    	...
    	<label >业务类型:</label><select id="accepttype" name="dept" class="btu_select" style="width: 110px;">
    		</select>
		<label >区域维度:</label>
    		<select id="regType"  class="btu_select"  onchange="changeArea(this)" name="dept" style="width: 120px;">
