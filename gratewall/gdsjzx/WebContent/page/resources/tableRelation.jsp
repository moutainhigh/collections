...
function renderdata(event, obj){
	...
	for(var i=0;i<data.length;i++){
		...
		data[i]["custom"] = '<div class="jazz-grid-cell-inner"><a href="javascript:void(0);" onclick="detail(\''+pripid+'\')">维护表关系</a></div>';
	}
	return data;
}
function detail(dmbId){
	winEdit = jazz.widget({
  	     vtype: 'window',
	   	     frameurl: './tableRelationEdit.jsp?priPid='+dmbId+'&update=true',
	  			name: 'win',
	  	    	title: '维护表关系',
	  	    	titlealign: 'left',
	  	    	titledisplay: true,
...
<div>位置：资源管理>数据资源管理>表关系管理</div>
	<div vtype="formpanel" id="formpanel" displayrows="1" name="formpanel" titledisplay="true" width="100%" layout="table" 
		...
		<div name='industryphy' vtype="comboxfield" label="业务系统" dataurl="[{checked: true,value: '1',text: '数据中心'},{value: '2',text: '执法办案'}]" labelAlign="right"  labelwidth='100px' width="410"></div>
		<div name='entname' vtype="textfield" label="物理表" labelAlign="right"  labelwidth='100px' width="410"></div>		
		<div name='enttype' vtype="textfield" label="物理表中文名" labelAlign="right"  labelwidth='100px' width="410"></div>
		<div name='industryco' vtype="comboxfield" label="表类型" dataurl="[{checked: true,value: '1',text: '业务表'},{value: '2',text: '系统表'}]" labelAlign="right"  labelwidth='100px' width="410"></div>				
	...
	<div vtype="gridpanel" name="zzjgGrid" height="400" width="100%"  id='zzjgGrid' datarender="renderdata()"
		titledisplay="true" title="表关系信息"  dataurl="" layout="fit" showborder="false" isshowselecthelper="false" selecttype="2">
		<!-- 表头 -->
