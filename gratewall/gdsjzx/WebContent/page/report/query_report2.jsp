...
<body>
	...
	<!-- 	<div name='reportname' vtype="comboxfield" label="报表名称"
			...
			dataurl='[{"text":"内资222表", "value":"内资1表" },
...
<div name='reportname' vtype="comboxfield" label="报表名称" labelAlign="right" labelwidth='100px' width="410" multiple="false" 
			dataurl=''></div>
...
$(function(){
	...
	//根据URL 上面的参数得到对应的内外资等明细信息
	var dataStr1 = "[{text:'外资111表',value:'外资1表 '}]";
	var dataStr2 = "[{text:'内资1表',value:'内资1表 '},{text:'内资22表',value:'内资22表 '}]";
	var dataStr3 = "[{text:'个体1表',value:'个体1表 '}]";
	var dataStr4 = "[{text:'综合1表',value:'综合1表 '}]";
	var dataStr5 = "[{text:'农资1表',value:'农资1表 '}]";
	//string转json对象
	dataStr1 =  eval('(' + dataStr1 + ')');
	dataStr2 =  eval('(' + dataStr2 + ')');
	dataStr3 =  eval('(' + dataStr3 + ')');
	dataStr4 =  eval('(' + dataStr4 + ')');
	dataStr5 =  eval('(' + dataStr5 + ')');
	
	if(rid=="nz"){
		$('div[name="reportname"]').comboxfield("option","dataurl", dataStr2);
		$('div[name="reportname"]').comboxfield("reload");
	}
	
	if(rid=="wz"){
		$('div[name="reportname"]').comboxfield('addOption', '111', '2222');
		//$('div[name="reportname"]').comboxfield("option","dataurl", dataStr1);
		//$('div[name="reportname"]').comboxfield("reload");
	}
 
	if(rid=="gt"){
		
		$('div[name="reportname"]').comboxfield("option","dataurl", dataStr3);
		$('div[name="reportname"]').comboxfield("reload");
	}
	
	if(rid=="zh"){
		$('div[name="reportname"]').comboxfield("option","dataurl", dataStr4);
		$('div[name="reportname"]').comboxfield("reload");
	}

	
	if(rid=="nh"){
		$('div[name="reportname"]').comboxfield("option","dataurl", dataStr5);
		$('div[name="reportname"]').comboxfield("reload");
	}
