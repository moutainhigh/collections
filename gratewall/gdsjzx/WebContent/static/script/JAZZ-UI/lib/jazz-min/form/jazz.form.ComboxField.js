...
//            }
        ...
        _isshowblankitem: function(){
        	...
        	if(this.options.isshowblankitem && !this.options.multiple){
        		this.blankitem = [{text: blanktext, value: ""}];
...
//	        		}else{
//	        			this._bindEvent(this.options.vtype, this.iData.length);
//	        		}
//	        		break;	 
//        	}
...
//        },
        ...
        addOption: function(text, value) {
            ...
            this.dataset.push({'text': text, 'value': value});
            this.datakeyvalue[value] = text;
        },
        ...
        getValue: function() {
        	...
        	if(this.options.editable == true){
        		...
        		if(text != this.options.valuetip && text != this.options.blanktext){
	                for(var i = 0; i < this.dataset.length; i++) {
	                    var option = this.dataset[i], label = option['text'];
	                    text = $.trim(text);
	                    if(label===text){
	                    	text = option['value'];
	                    	break;
