    ...
    (function($, undefined) {
        ...
        $.widget("jazz.gridpanel", $.jazz.panel, {
            ...
            _init: function() {
                ...
                // 此处类似于在panel的_init()中items处理(即widget形式创建子组件) this._createItems(this.content);  
    ...
    (function($) {
        ...
        $.widget("jazz.gridcolumn", $.jazz.boxComponent, {
            ...
            _create: function() {
                ...
                var el = this.element;
                el.attr("id", this.compId + '_gridcolumn').addClass("jazz-gridcolumn").css({
                    'width': '100%',
                    'overflow': 'hidden'
                });
                ...
                for (var i = 0; i < cellColumnIndexArray.length; i++) {
                    ...
                    for (var j = 0, len = cellColumnIndexArray[i].length; j < len; j++) {
                        ...
                        if (celltype) { //表格头特殊列；序号列、checkbox列或者radio列
                            if (celltype == 'lineno') {
                                header2 += '<td index="' + colindex + '" class="jazz-grid-headerCell" rowspan="' + headrows + '">' + '<div class="jazz-grid-headerCell-inner" style="text-align:center;">序号</div></td>';
                            ...
                            if (celltype == 'select') {
                                if (selecttype == 2 || selecttype == '2') {
                                    header2 += '<td index="' + colindex + '" class="jazz-grid-headerCell" rowspan="' + headrows + '">' + '<div class="jazz-grid-headerCell-box">' + '<input type="checkbox" name="' + this.compId + '_box" id="' + this.compId + '_box" />' + '</div>' + '</td>';
                                } else {
                            ...
                            header2 += '<td index="' + colindex + '" name="' + _name + '" class="jazz-grid-headerCell' + _bottomclass + '" ' + _rowspan + _colspan + '>' + '<div class="jazz-grid-headerCell-outer">'
            ...
            calculateGridcolumnWidth: function() {
                ...
                if (grid) {
                    var gridtable = $(grid)['gridpanel']('getGridTable');
                    if (gridtable) {
