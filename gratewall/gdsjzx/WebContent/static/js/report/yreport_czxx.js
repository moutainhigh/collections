	...
	$(function(){
 	...
 	function queryUrl() {
		...
		$('#jbxxGrid1').gridpanel('option', 'dataurl',rootPath+'/datacompre/queryZzjgDm.do');
 	...
 	function queryHistory(entityNo,priPid,sourceflag,year){
		...
		$('#jbxxGrid1').gridpanel('option', 'dataurl',rootPath+
				'/report/detail.do?flag='+(parseInt(entityNo)+1)+'&priPid='+priPid+'&sourceflag='+sourceflag+'&year='+year);
		$('#jbxxGrid1').gridpanel('query', [ 'formpanel']);
	}

	function reset() {
		$("#jbxxGrid").formpanel('reset');
		$("#jbxxGrid1").formpanel('reset');
	}
