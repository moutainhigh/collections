  	 $(function(){
  		...
  		if(entityNo!=null){
 			...
 			queryHistory(entityNo,priPid,sourceflag,type,economicproperty,regno);
  		}
	...
	function queryUrl() {
		...
		$('#jbxxGrid').gridpanel('option', 'dataurl',rootPath+'/datacompre/queryZzjgDm.do');
		$('#jbxxGrid').gridpanel('query', [ 'formpanel']);
	}
	
	function queryHistory(entityNo,priPid,sourceflag,type,economicproperty,regno){
		$('#jbxxGrid').gridpanel('option', 'dataurl',rootPath+'/reg/detail.do?flag='+entityNo+'&priPid='+priPid+'&sourceflag='+sourceflag+'&regno='+regno+'&economicproperty='+economicproperty+'&type='+type);
		$('#jbxxGrid').gridpanel('query', [ 'formpanel']);
	}
    ...
    */
	...
	function handler(){
		$('.jazz-grid-row').live("click",function () {
			var tds=$(this).children("td");
			for(var i=1;i<tds.length;i++){	
				$('#row'+i).text("");
				if($(tds[i]).html()=="&nbsp;"){
					$('#formpanel  #row'+i).text("");
				}else{		
					$('#formpanel  #row'+i).text($(tds[i]).html());
				}
			}
			$("#formpanel").show(); 
	 	});
