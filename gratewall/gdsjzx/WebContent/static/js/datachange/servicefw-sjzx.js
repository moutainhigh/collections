var setting = {
	...
	$(function(){
		...
		if(update=="true"){
			...
			$('div[name="createtype"]').hiddenfield('setValue', "sjzx");
		 	$('div[name="defaulttime"]').textfield('setValue', "30000");
		}

		$('div[name="serviceconentid"]').comboxfield('option','dataurl',rootPath+'/dataservice/serviceContentSelect.do?serviceid='+serviceid+'&update='+update);
		$('div[name="serviceconentid"]').comboxfield('reload');
		$('div[name="serviceconentid"]').comboxfield("option", "change", function(event, ui){  
			$('div[name="serviceconentshow"]').hiddenfield('setValue', ui.newText);
		});
