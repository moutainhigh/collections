$(function() {
	...
	var mainparams = {
		url: rootPath+"/home/loadAppByBusionDomain.do?random="+Math.random(),
		callback : function(data,obj,res) {
			//console.log("==qq=data="+jazz.jsonToString(data));
			if(data==null){
				...
				if(data.data.length > 0){
					...
					if(len <= 8){
						...
						$(".right-footer-right").addClass("right-footer-right2");
					}
	...
	$("#contentpanel").parent().css("overflow","hidden");
...
/*			if(!!url && url.indexOf(":") > 0){
			...
			if(data.systemCode=="BACK"){
				url = rootPath+data.systemImgUrl;
			}else{
				url = rootPath+"/page/common/file_read.jsp?fileName="+data.systemImgUrl;
			}	
		...
		click : function(e, ui) {
			if(ui.systemType=="NOSYS"){
				var url = rootPath+"/home/loadAppForIndex.do?random="+Math.random();
				var pkBusiDomain = ui.pkSysIntegration;
				$('#gnlb').icon("option", "dataurlparams", {"pkBusiDomain":pkBusiDomain});
				$('#gnlb').icon("reload", url, function(options){
					if(options.iconpages < 2){ //总页数小于2
						$(".right-footer").hide();
					}else{
						$(".right-footer").show();
						if(options.activeindex == 0){ //首页时
							$(".right-footer-left").removeClass("right-footer-left2");
							$(".right-footer-right").addClass("right-footer-right2");
						}else if(options.activeindex+1 == options.icontotal){ //未页时
							$(".right-footer-left").addClass("right-footer-left2");
							$(".right-footer-right").removeClass("right-footer-right2");							
						}else{ //非首页 非未页
							$(".right-footer-left").addClass("right-footer-left2");
							$(".right-footer-right").addClass("right-footer-right2");							
						}
				...
				if(ui.systemCode == "BACK"){
					var url = rootPath+"/home/loadAppByBusionDomain.do?random="+Math.random();
					$('#gnlb').icon("loadData",url);
				}else{
...
function initIconButton(num){
	//总页数
	var pageNum = $("#gnlb").icon("getPages");
	
	$(".right-footer-left").click(function(){
		var flag = $("#gnlb").icon("leftside");
		var activeIndex = $("#gnlb").icon("option", "activeindex");
		if(flag){
			$(this).addClass("right-footer-left2");
		}else{
			$(this).removeClass("right-footer-left2");
		}
		if(activeIndex == 0){
			$(this).removeClass("right-footer-left2");
			$(".right-footer-right").addClass("right-footer-right2");
	...
	$(".right-footer-right").click(function(){
		...
		var activeIndex = $("#gnlb").icon("option", "activeindex");
		if(flag){
			$(this).addClass("right-footer-right2");
		}else{
			$(this).removeClass("right-footer-right2");
		}
		if(pageNum == (activeIndex+1)){
			$(this).removeClass("right-footer-right2");
			$(".right-footer-left").addClass("right-footer-left2");
		}else{
			$(".right-footer-left").addClass("right-footer-left2");
