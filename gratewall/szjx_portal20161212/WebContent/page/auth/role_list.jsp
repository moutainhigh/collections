...
var pkRole =null;
	function initData(res){
		systemCode=res.getAttr("systemCode");
	...
	function treeOnClick(event, treeId, treeNode){
		...
		pkRole =RoleId;
		type="update";
  		$("#maincontent").panel("option", "frameurl",rootPath+'/auth/getRole_crudHtml.do?systemCode='+systemCode+'&type='+type+'&pkRole='+pkRole);
	}
	function refreshright(sys,id){
		...
		type="update";
		$("#maincontent").panel("option", "frameurl",
  				rootPath+'/auth/getRole_crudHtml.do?pkSysIntegration='+sys+'&type='+type+'&pkRole='+id);
	}
	var tree;
	$(document).ready(function(){
		...
		refreshFuncTree();
		var type="onlyShow";
  		//$("#maincontent").panel("option", "frameurl",rootPath+'/auth/getRole_crudHtml.do?pkSysIntegration='+pkSysIntegration+'&type='+type+'&pkRole='+pkRole); 
	});
	...
	function refreshFuncTree(){
		...
		var params = {
			url : rootPath+'/auth/queryRoleTree.do?systemCode='+systemCode+"&pkSysIntegration="+pkSysIntegration,
			callback : function(data, r, res) { 
				var data = data["data"];
				var flag =data[0]["flag"];
				//0查询为空
				if(flag==0){
					var str = "<div style='color:red;'>当前角色为空</div>";
					$("#funcTree").html(str);
					return false;
				}
	...
	function addRole(){
		type="add";
  		$("#maincontent").panel("option", "frameurl",
  				rootPath+'/auth/getRole_crudHtml.do?systemCode='+systemCode+'&type='+type+'&pkRole='+pkRole);  	
	}
	function deleteFunc(){
		//alert(rID);
		//roleId='', roleName='',upRoleId='',upRoleName='';
...
<div id="rolelist" vtype="panel" width="100%" height="100%" layout="fit" titledisplay="true" title="角色维护" bgcolor="white" showborder="true">
	<div id="table" height="100%" vtype="panel" layout="column" showborder="false" 
        ...
        <div id="demo" >
        	...
        	<div id="toolbar" name="toolbar" vtype="toolbar" align="left" style="display:none" >
