...
public class LdapbyUser {
	DirContext dc = null;
	String root = "dc=msatest,dc=com"; // LDAP的根节点的DC
...
//		close();
	...
	public void init() {
		...
		String LDAP_URL = "ldap://10.3.70.1:389"; // LDAP访问地址
		String adminName = "testadmin@msatest.com"; // 注意用户名的写法：domain\User或
		String adminPassword = "1234@abcd"; // 密码
	...
	public void add(String newUserName) {
		try {
			...
			dc.createSubcontext("ou=" + newUserName + "," + "ou=用户（测试）" + root,
					attrs);
		} catch (Exception e) {
			e.printStackTrace();
