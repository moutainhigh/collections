...
@Service
public class SMSRateEmailServiceImpl implements SMSRateStockEmailService {
	private static Logger logger = LogManager.getLogger(EmailServiceImpl.class);
	@Autowired
	private JavaMailSender javaSMSRateMailSender;

	@Autowired
	private SimpleMailMessage simpleSMSRateMailMessage;
	
	@Value("${smsjinzhenTitle}")
	private String emailTitle;

	public void senderShortEmailByRate(String toEmail, List<SMSStock> stocks) {
		SMSRateMailModel mail = new SMSRateMailModel();
		// 主题
		...
		builder.append("<tr><td colspan='7' style='text-align:center;font-size:14px;font-weight:bold;padding:5px 10px;border:1px solid #C1D9F3;background:#C1D9F3'>监控宝</td></tr>");
		builder.append("<tr>");
		...
		for (int i = 0; i < stocks.size(); i++) {
			if (i % 2 == 0) {
				...
				builder.append("<td style='font-size:14px;text-align:center;background:#B2D7EA;border:1px solid #C1D9F3'> </td>");
				builder.append("</tr>");
			}
		}
	...
	public void sendEmail(SMSRateMailModel mail) {
		// 建立邮件消息
		MimeMessage message = javaSMSRateMailSender.createMimeMessage();
		...
		try {
			...
			if (mail.getEmailFrom() != null) {
				...
				messageHelper.setFrom(simpleSMSRateMailMessage.getFrom());
			}
			...
			if (mail.getToEmails() != null) {
				...
				messageHelper.setTo(simpleSMSRateMailMessage.getTo());
			...
			if (mail.getSubject() != null) {
				...
				messageHelper.setSubject(simpleSMSRateMailMessage.getSubject());
			}
			...
			// 发送邮件
			javaSMSRateMailSender.send(message);
			logger.info("------------发送邮件完成----------");
