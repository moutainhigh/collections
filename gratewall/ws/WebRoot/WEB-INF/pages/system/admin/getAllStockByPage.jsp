...
<script>
		...
		$('#list').bootstrapTable(
				{
					...
					columns : [
							...
							},
							{
								...
								formatter : function(value, row, index) {
									if (row.currentPrice > row.prevClose) {
										return "<span class='red dp' onclick='changeStockInfos(this)' data-codes='"+row.stockCode+"' data-type='price'>" + value + "</span>";
									} else if (row.currentPrice == row.prevClose) {
										return "<span class='#ccc dp' onclick='changeStockInfos(this)' data-codes='"+row.stockCode+"' data-type='price'>" + value + "</span>";
									} else {
										return "<span class='green dp' onclick='changeStockInfos(this)' data-codes='"+row.stockCode+"' data-type='price'>" + value + "</span>";
									}
								}
							},
							{
								...
								formatter : function(value, row, index) {
									if (row.currentPrice > row.prevClose) {
										return "<span class='red dp' onclick='changeStockInfos(this)' data-codes='"+row.stockCode+"' data-type='rate'>" + value + "</span>";
									} else if (row.currentPrice == row.prevClose) {
										return "<span class='#ccc dp' onclick='changeStockInfos(this)' data-codes='"+row.stockCode+"' data-type='rate'>" + value + "</span>";
									} else {
										return "<span class='green dp' onclick='changeStockInfos(this)' data-codes='"+row.stockCode+"' data-type='rate'>" + value + "</span>";
									}
								}
							}],
					data : list
	...
	function changeStockInfos(obj){
		var _this = $(obj);
		var stockCode = _this.data("codes");
		var type = $(obj).data("type");
		var flag  = _this.parent().find(".editsInfos");
		if(flag.html()==undefined){
			_this.after("<p class='editsInfos'><input type='text'  autofocus='autofocus' /><a href='javascript:;' onclick='saveEditInfo(this)' data-stockcode='"+stockCode+"' data-type='"+type+"'>保存</a><a href='javascript:;' onclick='cancel(this)'>取消</a></p>")
		}
	}
	
	function saveEditInfo(obj){
		var vals = $(obj).prev().val();
		var stockCode = $(obj).data("stockcode");
		var type = $(obj).data("type");
		$.ajax({
			url:"stock/updateInfoByWeb",
			data:{
				stockCode:stockCode,
				stockPrice:vals,
				type:type,
			...
			success:function(data){
				$(obj).parent().siblings().html(vals);
				$(obj).parents(".editsInfos").remove();
