...
function renderdata(event, obj){
	...
	for(var i=0;i<data.length;i++){
		...
		data[i]["custom"] += '<a href="javascript:void(0);" title="管理" onclick="editorTable(\''+data[i]["pkDcTable"]+'\',\''+data[i]["pkDcDataSource"]+'\');"><img src="'+rootPath+'/static/images/other/forma-1.png" width="11px" height="11px" border="0"/></a>&nbsp;&nbsp;'
							...
							+ '<a href="javascript:void(0);" title="查询" onclick="updateQueryTable(\''+data[i]["pkDcTable"]+'\',\''+data[i]["isQuery"]+'\');"><img src="'+rootPath+'/static/images/other/fengbei.png" width="11px" height="12px" border="0"/></a>'
		data[i]["custom"] += '</div>';
...
function updateQueryTable(pkDcTable,isQuery){
	if(isQuery=="Y"||isQuery=="是"){
		var params = {
				url:rootPath+"/tableManager/deleteQueryTable.do", 
				params: {"pkDcTable":pkDcTable},
				callback : function(data,obj,res) {
	   				if (res.getAttr("back") == 'success') {  	
	   			        $('div[name="gridpanel"]').gridpanel('reload'); 
	...
	var params = {
			url:rootPath+"/tableManager/updateQueryTable.do", 
			params: {"pkDcTable":pkDcTable},
			callback : function(data,obj,res) {
   				if (res.getAttr("back") == 'success') {  	
   			        $('div[name="gridpanel"]').gridpanel('reload'); 
