...
<script>
	...
	$(function(){
		...
		query();
		$('div[name="pkDcBusiObject"]').comboxfield('option','dataurl',rootPath+'/functionList/queryBusiObjectName.do');
		$('div[name="pkDcBusiObject"]').comboxfield('reload');
	});
	
	
	function query(){
		$('div[name="gridpanel"]').gridpanel('option','dataurl',rootPath+'/functionList/query.do');
	...
	function renderdata(event, obj){
		...
		for(var i=0;i<data.length;i++){
			data[i]["custom"] = '<div class="jazz-grid-cell-inner">'
								+'<a href="javascript:void(0);" title="管理" onclick="updateData(\''+data[i]["pkDcTrigger"]+'\');"><img src="'+rootPath+'/static/images/other/forma-1.png" width="11px" height="11px" border="0"/></a>&nbsp;&nbsp;&nbsp;'
								+'</div>';
			data[i]["triggerName"] = '<a href="javascript:void(0)" title="查看" onclick="checkData(\''+data[i]["pkDcTrigger"]+'\')">'+data[i]["triggerName"]+' </a>';
		}
		return data;
	}
	...
	function updateData(pkid){
		var title = "修改触发器信息";
		var type = "update";
		var frameurl= ""+'/functionList/updateFunction.do?pkDcTrigger='+pkid+'&type='+type;
		createNewWindow(title,frameurl,390);
	...
	function checkData(pkid){
		var title = "查看触发器信息";
		var type = "check";
		var frameurl= ""+'/functionList/checkFunction.do?pkDcTrigger='+pkid+'&type='+type;
 ...
 <div class="title_nav">当前位置：资源管理> 数据资源管理> <span>触发器管理</span></div>
	...
	<div vtype="gridpanel" name="gridpanel" width="100%" showborder="false" isshowselecthelper="true" 
		...
		<div vtype="gridcolumn" name="grid_column" width="100%">
			<div>
				<!-- 单行表头 -->
				<div name='pkDcTrigger' key="true" visible="false" ></div>
				...
				<div name='triggerName' text="触发器名称" textalign="center" sort="true" width='20%'></div>
				<div name='triggerUseDesc' text="触发器用途" textalign="center" sort="true" width='55%'></div> 
