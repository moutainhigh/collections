...
<script>
	 ...
	 function getUrlParam(name) {
	   ...
	   function renderdata(event,obj){
			...
			for(var i=0;i<data.length;i++){
				var htm = '<a href="javascript:void(0);" onclick="reportDis(\''+data[i]["id"]+'\')">'
					+ "报表分发" + '</a>';
				data[i]["edit"] = htm; 
	...
	function reportDis(id){
		//window.open (rootPath+"/page/report/report_distribute.jsp?id="+id, '_blank' ) ;
		winEdit = jazz.widget({
	 	     vtype: 'window',
	   	     frameurl: 'report_distribute.jsp?id='+id,
	  			name: 'win',
	  	    	title: '报表分发',
	  	    	titlealign: 'left',
	  	    	titledisplay: true,
