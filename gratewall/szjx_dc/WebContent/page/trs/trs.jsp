...
<script type=text/javascript>
		...
		function searchCount(queryKeyWord){
			$.ajax({
				url : rootPath+'/trsSearch/countDual.do',
				async: false,
				data : {
...
<%-- 			for(var i=0;i<pagedata.length;i++){
			...
			for (var i = listbegin; i <= listend; i++) {
	        	...
	        	if (i != pages) {
	          		...
	          		$("<span><a onclick='lazyload("+ i +")'>" + i + "</a></span>").appendTo($('.page'));
	            } else {
			...
			h3.each(function(){
				var _this = $(this);
				var beginIndex = _this.html().lastIndexOf("(");
				var endIndex = _this.html().lastIndexOf(")");
				//console.log(beginIndex+" ==== " +endIndex)
				//var temp = _this.html().replace(beginIndex,endIndex+1);
				var temp = _this.html().replace(beginIndex,"<font color='red'>11</font>");
				console.log(temp);
			});
		}
...
<body>
	<div id="allcontent" vtype="panel" width="100%" height="100%" showborder="false" 
		...
		<div class="yincangdiv" region="north" height="65">
			<jsp:include page="../../page/home/header.jsp"/>
			<jsp:include page="trs_header.jsp"/>
		</div>

	<div region="center" style="background: #f3f6f6;">
	
		<div class="float_left imagemenu">
			<div id ="search_1" style="z-index:1;/* margin-top:2%; */text-align: center;width: 920px;background: #fff;margin: 0 auto;padding-top: 20px">
				<div class="sjzx_content" style="width:820px;margin:0 auto;">
				<div class="serach" style="z-index:1;margin-top:2%;text-align: center;width:820px;margin:0 auto;">
			...
			<div class="lists" style=" background:#fff;width: 920px;margin: 0 auto;">
			<!--查询耗时  <div  id="tishi">热词搜索<span id="countnum"></span><span id="timeuseing"></span></div>-->
			<div id="result" class="results" style="/* margin-top:3% */;text-align: center;"></div>
			</div>
			
			<div class="pageRegion" style="background: #fff;width: 920px;margin: 0 auto;">
				<div class="p" style="float: right"></div> 
