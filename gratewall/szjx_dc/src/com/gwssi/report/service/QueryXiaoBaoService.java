...
public class QueryXiaoBaoService  extends BaseService implements ReportSource{
	...
	public List<Map> zhiJieJieDa(String beginTime, String endTime, int i,
		...
		String sql=
				" select  REGEXP_REPLACE ( t.infotype,'[0-9]+','') infotype  ,count(t.infotype)count from (\n" +
						"select (case i.inftype when '2' then '1举报'\n" + 
						"                       when '3' then '2咨询'\n" + 
						"                       when '4' then '3建议'\n" + 
						"                       when '6' then '4消委会投诉'\n" + 
						"                       else '5其他' end\n" + 
						...
						"  on  i.proinsid=o.proinsid  and o.action='直接回复'\n" + 
						"where i.regtime>=to_date(?,'yyyy-mm-dd hh24:mi:ss')\n" + 
						"and i.regtime<=to_date(?,'yyyy-mm-dd hh24:mi:ss')\n" + 
						"\n" + 
						")t\n" + 
	...
	public List<Map> ziXunJianYiYeWuFanWei(String beginTime, String endTime,
		...
		list.add(endTime+" 23:59:59");
		String sql=
				"select  distinct nvl(tt.name,(case when substr(t.businessrange,7,2)<>'00' then  '　　　'\n" +
						"                                   when substr(t.businessrange,5,4)<>'0000' then  '　　'\n" + 
						"                                   when substr(t.businessrange,3,6)<>'000000' then  '　'\n" + 
						"end)||t.businessrange) name,t.businessrange,t.sums from (\n" + 
						"with x as (\n" + 
						"select o.businessrange from dc_dc.dc_cpr_infoware i left join dc_dc.dc_CPR_INQUIRY o on\n" + 
						"i.inquiryid=o.inquiryid and I.REGTIME>=to_date(?,'yyyy-mm-dd hh24:mi:ss')\n" + 
						"and I.REGTIME<=to_date(?,'yyyy-mm-dd hh24:mi:ss'))\n" + 
						...
						"(select c1.code,(case when c1.parentcode='0' then c1.name\n" + 
						" when c1.parentcode<>'0' and  substr(c1.code,7,2)<>'00' then  '　　　'||c1.name\n" + 
						" when c1.parentcode<>'0' and  substr(c1.code,5,4)<>'0000' then  '　　'||c1.name\n" + 
						" when c1.parentcode<>'0' and  substr(c1.code,3,6)<>'000000' then  '　'||c1.name\n" + 
						"   end)as  name from dc_code.dc_12315_codedata c1 where   c1.codetable='ZH01'\n" + 
						"  union all\n" + 
						"  select '99999999' as code,'未指定' as name from dual\n" + 
						"  union all\n" + 
						"  select '999999999'as code, '全部' as name from dual\n" + 
						"  union all\n" + 
						"  select '50000000' as code,'其他' as name from dual\n" + 
						"   ) c order by c.code)tt\n" + 
						"on t.businessrange=tt.code\n" + 
	...
	public List<List<Map>> shenSuJuBaoSheJiKeTi(String beginTime, String endTime,
		...
		if (infoType != null && "1".equals(infoType)) {
			...
			infotype += "and i.inftype='3' \n";
		} else {
			infotype += "and i.inftype in ('3','2','1') \n";
		}
	...
	public List<Map> fuWuZhanXinXiJian(Map map, int i, HttpServletRequest req) {
		...
		List<Map> list1=new ArrayList<Map>();
		sb.append(
				"select p.persname ,m.invname ,i.content,to_char(t.transtime,'yyyy-mm-dd') transtime,\n" +
