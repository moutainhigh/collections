...
function fixColumn(event, obj) {// 维护按钮
	...
	for(var i=0;i<data.length;i++){
		var htm = '<div class="jazz-grid-cell-inner">'
				+'<a href="javascript:void(0);" title="维护" onclick="fixSys(\''+data[i]["pkSysIntegration"]+'\');"><img src="'+rootPath+'/static/images/other/forma-1.png" width="11px" height="11px" border="0"/></a>&nbsp;&nbsp;&nbsp;'
				+'</div>';
		data[i]["fix"] = htm; 
	}
	return data;
...
var fixwin;
function fixSys(pkSysIntegration) {
	$("iframe", parent.document).attr("src", rootPath+'/auth/getMenu_fixHtml.do?pkSysIntegration='+pkSysIntegration);
	//$("#column_id").panel("option", "frameurl",rootPath+'/auth/getMenu_fixHtml.do?pkSysIntegration='+pkSysIntegration);
}
...
<body>
	<div class="title_nav">当前位置：权限管理 > <span>菜单管理</span></div>
	   	<div name="formpanel" id="formpanel" vtype="formpanel" titledisplay="false" width="100%" layout="table" showborder="false" layoutconfig="{cols:2, columnwidth: ['50%','*']}" >
			<div name='systemName' id='systemName' vtype="textfield" label="子系统名称" labelalign="right" labelwidth="200" width="80%" editable="true"></div>
			<div name='systemCode' id='systemCode' vtype="textfield" label="子系统编码" labelalign="right" labelwidth="200" width="80%" editable="true"></div>
	   	...
	   	<div vtype="gridpanel" name="gridpanel" id="gridpanel" width="100%" layout="fit" showborder="false" datarender="fixColumn()" 
			...
			<div vtype="gridcolumn" name="grid_column" id="grid_column">
				<div> <!-- 单行表头 -->
					...
					<div name="fix" text="操作" textalign="center" width="9%" ></div>
				</div>
			</div>
			<!-- 表格 -->
