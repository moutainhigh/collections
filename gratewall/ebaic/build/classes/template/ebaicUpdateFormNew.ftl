...
<#if config?exists && ((config.primaryKey)!)!="">
	<div name="${(config.primaryKey)!}" vtype="hiddenfield" label="主键" labelwidth="142" height="46" labelalign="right" width="700"></div>
</#if>	
...
<#list config.columns as column>
	...
	<div name="${(column.columnName)!}" vtype="textareafield" label="${(column.columnTitle)!}" labelwidth="142" height="46" labelalign="right" width="${(column.columnWidth)!"700"}" tooltip="${(column.columnTips)!}" rule="${(column.columnRule)!}" height="110" readonly="<#if ((column.columnEditable)!)=="0">true</#if>"></div>
		</#if>
