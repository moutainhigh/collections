...
var torch = {
	...
	<#if ((column.columnFieldType)!)=="edit">
		...
		for(var i=0;i<data.length;i++){
			var htm = '<a href="javascript:void(0);" onclick="">编辑</a><a href="javascript:void(0);" onclick="">删除</a>';
	...
	${(fbo.functionName)!}Reset : function(){
	 ...
	 require(['domReady'], function (domReady) {
		...
		<#list queryList as query>
		<#if ((query.queryType)!)=='Grid'>
			$("div[name='${(config.configName)!}_Form']").gridpanel("option","datarender",torch.${(query.configName)!}_datarender);
			$("div[name='${(config.configName)!}_Form']").gridpanel("reload");
		</#if>
		</#list>
		</#if>
