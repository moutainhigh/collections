define(['jquery', 'jazz','util' ], function($, jazz,util){
	...
	torch = {
    	...
    	_init: function(){
			...
			require(['domReady'], function (domReady) {
					    domReady(function () {
				    		...
				    		if(configId){
				    			$_this.reloadEditConfig(configId);
				    		}
				    		...
				    		$("#save_button").off('click').on('click',$_this.addEditConfig);
				    		$("#back_button").off('click').on('click',torch.goBack);
				    		$("#reset_button").off('click').on('click',torch.resetEditConfig);
				    		$("#add_edit_column_detail_button").off('click').on('click',torch.addEditColumnDetail);
    	...
    	addEditConfig:function (){
    		var model = $("#model").comboxfield('getValue');
    		var params = {
    			url : '../../../../torch/config/torchConfig.do',
    			components : [ 'sysEditConfigPanel' ],
    			params:{
    				model:model
    			},
    	...
    	reloadEditConfig:function (configId){
    		var params = {
    			configId:configId
    		};
    		var formUrl='../../../../torch/config/queryEditConfig.do';
    		$("#sysEditConfigPanel").formpanel("option","dataurlparams",params);
    		$("#sysEditConfigPanel").formpanel("option","dataurl",formUrl);
    		$("#sysEditConfigPanel").formpanel("reload");
    	...
    	queryEditColumnDetail:function (columnId){
    		util.openWindow("queryEditColumnDetail","编辑列组件信息","editColumnDetail.html?columnId="+columnId,1050,450);
    		/*var url = $_this.getContextPath()+"/mgr/torch/sysconfig/editColumnDetail.html?columnId="+columnId;
    		window.location.href=url;*/
    	},
    	addEditColumnDetail:function (){
    		var configId = jazz.util.getParameter("configId")||"";
    		util.openWindow("addEditColumnDetail","新增编辑列信息","editColumnDetail.html?configId="+configId,1050,450);
    		/*var url = $_this.getContextPath()+"/mgr/torch/sysconfig/editColumnDetail.html?configId="+configId;
    		window.location.href=url;*/
    	},
    	/**
    	 * 重置方法
    	...
    	edit_fromNames : [ 'sysEditConfigPanel'],
