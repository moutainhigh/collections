...
<!-- 数据展现 -->
<div vtype="formpanel" wid="${(config.configId)!}" name="${(config.configName)!}_Form" dataview="false"
	titleDisplay="true" title="${(config.configTitle)!}" width="100%" showborder="true" 
	dataurl="" layout="table" layoutconfig="{cols:1, columnwidth: ['100%']}" labelstyleclass="labelstyle">
<#if config?exists && ((config.primaryKey)!)!="">
	<div name="${(config.primaryKey)!}" vtype="hiddenfield" label="主键" labelwidth="200" labelalign="right" width="500"></div>
</#if>	
...
<#list config.columns as column>
	...
	<div name="${(column.columnName)!}" vtype="textareafield" label="${(column.columnTitle)!}" labelwidth="200" labelalign="right" width="${(column.columnWidth)!"500"}" tooltip="${(column.columnTips)!}" rule="${(column.columnRule)!}" height="110" readonly="<#if ((column.columnEditable)!)=="0">true</#if>"></div>
		</#if>
