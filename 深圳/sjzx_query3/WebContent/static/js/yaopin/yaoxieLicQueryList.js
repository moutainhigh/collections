	...
	torch = {
		 ...
		 */
	    	...
	    	//	var isGtOrQe = jazz.util.getParameter('isGtOrQe')||'';
	    		...
	    		var gridUrl="../../yaoxie/YXQyQueryDetail.do?entid="+entid;
	    		/*if(isGtOrQe==0){
	    		...
	    		$("#SPLicQueryListGrid").gridpanel("option",'datarender',torch.backFunction);
	    		$("#SPLicQueryListGrid").gridpanel("option","dataurlparams",params);
	    		$("#SPLicQueryListGrid").gridpanel("option",'dataurl',gridUrl);
	    		$("#SPLicQueryListGrid").gridpanel("reload");
	    	...
	    	regFood:function(entid, pripid,regno,entname,typeName,id,licno){
	    		var url = "/query/page/yaopin/yjm.jsp";
	    		var param ="?entid="+entid+"&pripid="+pripid+"&regno="+regno+"&entname="+encodeURIComponent(entname)+"&certificateType="+encodeURIComponent(typeName)+"&certno="+encodeURIComponent(licno)+"&id="+id;
	    		url = url +param;
	    		window.open(url);
