$(function() {
	...
	if (entityNo != null) {
		...
		queryHistory(entityNo, priPid, economicproperty,alttime,sign);
	}
});
function queryUrl() {
...
function queryHistory(entityNo, priPid, sourceflag,alttime,sign) {
	//alert(entityNo);
	$('#jbxxGrid').gridpanel(
			'option',
			'dataurl',
			rootPath + '/reg/detail.do?flag=' + entityNo + '&priPid=' + priPid
					+ '&sourceflag=' + sourceflag+'&alttime='+alttime+'&sign='+sign);
