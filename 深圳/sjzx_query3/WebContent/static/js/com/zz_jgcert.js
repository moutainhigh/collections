...
//							$_this.initManagerData();
			...
			saveData:function(){		
				...
				if(type == "add"){
					...
					var addrFlag = $('div[name="addrFlag"]').radiofield('getValue');
					if(address == '' || addrFlag == ''){
						jazz.info("企业地址未填写或片区未选择，保存失败");
					}
					else{
						var params = {
								url:'../../WhiteEntSelect/WhiteEntInsert.do', 
								components: ['whiteEntEditOrAddPannel'],
				...
				if(type == "edit"){
					...
					if(address == '' || addrFlag == ''){
						...
						var params = {
								url:'../../WhiteEntSelect/WhiteEntUpdate.do', 
								components: ['whiteEntEditOrAddPannel'],
								params:{
									"id":id
								},
