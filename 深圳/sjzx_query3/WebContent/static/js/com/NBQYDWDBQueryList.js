	...
	torch = {
		 ...
		 */
	    	...
	    	queryNBDetail : function(){
	    		...
	    		var id = decode(jazz.util.getParameter('id'))||'';
	    		...
	    		$("#NBGTQYQueryDetailGrid").gridpanel("option",'datarender',torch.backFunction);
	    		$("#NBGTQYQueryDetailGrid").gridpanel("option","dataurlparams",params);
	    		$("#NBGTQYQueryDetailGrid").gridpanel("option",'dataurl',gridUrl);
	    		$("#NBGTQYQueryDetailGrid").gridpanel("reload");
			...
			backFunction:function (event, obj) {
	    		...
	    		for (var i = 0; i < data.length; i++) {
	    			...
	    			data[i]["opt"] = '<a href="javascript:void(0);" onclick="ebaic.NBDetailList(\''+data[i].ancheid+'\')">' + "详情" + '</a>'
	    	...
	    	NBDetailList:function(ancheid){
	    		util.openWindow("NBQYCDWDBQueryDetail","保证担保信息详情",$_this.getContextPath()+"/page/comselect/NBQYCDWDBQueryDetail.html?ancheid="+ancheid,680,392);
	    	},
