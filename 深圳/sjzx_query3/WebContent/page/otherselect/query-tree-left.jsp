...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		var url = "";
		if(parent.type=='blackent'){
			$("#enttypeName").text("黑名单信息");
			url = "left-blackent.json";
		}else if(parent.type == "YR"){
			$("#enttypeName").text("一人公司信息");
			url = "left-yr.json";
		}
		else{
   ...
   function getUrlParam(name) {
	   ...
	   var locations = window.location.toString();
		if(locations.indexOf("script")=="-1"||locations.indexOf("%3E")==-1||locations.indexOf("%3C")==-1||locations.indexOf("alert")==-1||locations.indexOf(")")==-1){
			 var reg = new RegExp("(^|&)" + name + "=([^&]*)(&|$)"); //构造一个含有目标参数的正则表达式对象
	         var r = window.location.search.substr(1).match(reg);  //匹配目标参数
	         if (r != null) return unescape(r[2]); return null; //返回参数值
		}else{
			jazz.error("传入的是xss非法参数，系统自动拦截");
		}
     }
		    		//选择年份
   function showSel(){
