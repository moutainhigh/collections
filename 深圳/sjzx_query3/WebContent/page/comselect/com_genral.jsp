<%@ page import="com.gwssi.application.common.query.CommonQueryUtil,com.alibaba.fastjson.JSONArray,com.alibaba.fastjson.JSONObject,java.util.Map,com.gwssi.application.common.JSonUtil"%>
<%@ page contentType="text/html; charset=utf-8"%>
<%
Map<String,Object> c = CommonQueryUtil.getConditionConfig("food");
%>
<!DOCTYPE html>
<html>
<head>
...
<body>
<div class="title_nav">当前位置：通用查询> 通用查询> <span><%=c.get("name") %></span></div>
<form method="post" target="_blank"  id="formID" action="">
					...
					<tr>
					<% 
					JSONArray conditions = (JSONArray)c.get("condition");
					for(int i=0;i<conditions.size();++i){
						JSONObject condition = conditions.getJSONObject(i);
						
						String label = condition.getString("label");
					...
					%>
						...
						<th class="class_td4"><%=label %>：</th>
						<th class="class_td5 thleft" id="row5" >
							<%
								if("text".equals(type)){
							%>
									<input type="text" style="height:25px; width:40%" name="<%=name %>" value="" />
							<%
								}
								if("combox".equals(type)){
							%>
									<select name="regcapcur" id="regcapcur" name="<%=name %>" style="height:25px; width:80%">
					...
					<tr>
						...
						<th class="class_td5 thleft" id="row5" >
							...
							<div  id ="div1" style="width:17px;height:1px;margin:5px auto;padding:0px;background-color:black;overflow:hidden;"></div>至&nbsp;&nbsp;&nbsp;
