...
//				if(confirm("确认要删除所有数据吗?")){
	...
	function _insertRowParam(data){
		...
		//tmp += "<td name='param_value_type'></td>";	//2参数类型
		...
		tmp += "<td name='patameter_value'></td>";	//4 参数值
		//tmp += "<td name='style'></td>";	//5参数格式
		tmp += "<td class='oper'><select name='param_value_type' >";
		...
		tmp += "<td name='showorder'></td>";//6参数顺序
		tmp += "<td class='oper'>"+
	...
	function _initData(data){
		...
		tmp += "<td name='patameter_name'>"+data.patameter_name+"</td>";	//3 参数名
		tmp += "<td name='patameter_value'>"+data.patameter_value+"</td>";	//4 参数值
		//tmp += "<td name='style'></td>";	//5参数格式
		tmp += "<td class='oper'><select name='param_value_type' >";
		for(var ii=0; ii<settings.style.length; ii++){
			var rowTmp = settings.style[ii];
		...
		tmp += "<td name='showorder'>"+data.showorder+"</td>";//6参数顺序
		tmp += "<td class='oper'>"+
			"<a href='javascript:;' title='删除' onclick='deleteDataItem(\""+rowindex+"\");' ><div class='delete'></div></a>"+
			"</td>";	//7操作
	...
	function _insertRow(data){
		var rowindex = $this.find("tr").length;
		var tmp = "<tr><td class='oper'>"+rowindex+"</td>";	//1 序号
		tmp += "<td name='dataitem_name_en'></td>"; //2 数据项名称
		tmp += "<td name='dataitem_name_cn'></td>";	//3 中文名
		...
		tmp += "</select></td>";	//4 数据项类型
		tmp += "<td name='dataitem_long'></td>";	//5 数据项长度
		tmp += "<td class='oper'><select name='is_key'><option value='0' selected>否</option><option value='1'>是</option></select></td>"	;	//6 是否主键
		...
		/*"<option selected value='00'>无</option>"+
			"<option value='01'>企业类型</option><option value='02'>行业大类</option><option value='03'>国别</option>"+
		*/
		tmp += "</select></td>";	//7 对应代码表
		tmp += "<td class='oper'><a href='javascript:;' title='删除' onclick='deleteDataItem();' >"+
	...
	function _initRow(data){
		...
		for(var ii=0; ii<data.length; ii++){
			...
			for(var jj=0; jj<settings.dataitem_type.length; jj++){
				...
				+"</select></td>";	//4 数据项类型
			tmp += "<td name='dataitem_long'>"+row.dataitem_long+"</td>";	//5 数据项长度
			tmp += "<td class='oper'><select name='is_key'><option " +
				(row.is_key=="0" ? "selected" : "") +" value='0' selected>否</option><option " +
				(row.is_key=="1" ? "selected" : "") +" value='1'>是</option></select></td>"	;	//6 是否主键
			tmp += "<td class='oper'><select id='code_table' >";
			...
			/*"<option " +
				(row.code_table=="00" ? "selected" : "") +" value='00'>无</option>"+
				"<option " + (row.code_table=="01" ? "selected" : "") +" value='01'>企业类型</option><option " +
				(row.code_table=="02" ? "selected" : "") +" value='02'>行业大类</option><option " + 
				(row.code_table=="03" ? "selected" : "") +" value='03'>国别</option>"+*/
			tmp += "</select></td>";	//7 对应代码表
			tmp += "<td class='oper'><a href='javascript:;' title='删除' onclick='deleteDataItem(\""+(ii+1)+"\");' >"+
