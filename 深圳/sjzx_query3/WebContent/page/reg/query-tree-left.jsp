	...
	$(document).ready(function() {
				...
				if((i+1)<data.length){
					...
					//_ $('<tr height="30px"><th width="50%"><a  href="#" onclick=rightPageSet("'+url+encode(parent.entid)+'&economicproperty='+parent.economicproperty+'")>'+data[i].content+'</a></th><th>|</th><th width="50%"><a  href="#" onclick=rightPageSet("'+data[i+1].url+encode(parent.entid)+'&economicproperty='+parent.economicproperty+'")>'+data[i+1].content+'</a></th></tr>').appendTo($('#container2'));
					 ...
					 if((i+1)<5){
						...
						$('<tr height="30px"><th width="50%"><a  href="#" onclick=rightPageSet("'+url+encode(parent.entid)+'&economicproperty='+parent.economicproperty+'")>'+data[i].content+'</a></th><th>|</th><th width="50%"><a  href="https://app03.szmqs.gov.cn/xyjggs.webui/xyjggs/NBInfo.aspx?Entid='+parent.entid+'&NBYear='+year+'" target="_blank" >'+data[i+1].content+'</a></th></tr>').appendTo($('#container2'));
					} 
					
				}else{
					 if(data[i].content=="即时公示信息"){
						$('<tr height="30px"><th width="50%"><a  href="https://app03.szmqs.gov.cn/xyjggs.webui/xyjggs/NBInfo.aspx?Entid='+parent.entid+'&NBYear='+year+'" target="_blank" >'+data[i].content+'</a></th></tr>').appendTo($('#container2'));
					}else{
						$('<tr height="30px"><th width="50%"><a  href="#"  onclick=rightPageSet("'+url+encode(parent.entid)+'&economicproperty='+parent.economicproperty+'")>'+data[i].content+'www</a></th></tr>').appendTo($('#container2'));
					} 
				}
			}
		});
		...
		$.getJSON(caseurl,function(data){
			for(var i=0 ;i<data.length;i=i+2){
				...
				if((i+1)<data.length){
					//	$('<tr height="30px"><th width="50%"><a target="main" href="'+url+parent.priPid+'&sourceflag='+parent.sourceflag+'">'+data[i].content+'</a></th><th>|</th><th width="50%"><a target="main" href="'+data[i+1].url+parent.priPid+'&sourceflag='+parent.sourceflag+'">'+data[i+1].content+'</a></th></tr>').appendTo($('#container'));
					$('<tr height="30px"><th width="50%"><a  href="#" onclick=rightPageSet("'+url+encode(parent.entid)+'&entname='+encode(parent.entname)+'&regno='+encode(parent.regno)+'&bregno='+encode(parent.bregno)+'")>'+data[i].content+'</a></th><th>|</th><th width="50%"><a  href="#" onclick=rightPageSet("'+data[i+1].url+encode(parent.entid)+'&entname='+encode(parent.entname)+'&regno='+encode(parent.regno)+'&bregno='+encode(parent.bregno)+'")>'+data[i+1].content+'</a></th></tr>').appendTo($('#container3'));
				}else{
					$('<tr height="30px"><th width="50%"><a  href="#" onclick=rightPageSet("'+url+encode(parent.entid)+'&entname='+encode(parent.entname)+'&regno='+encode(parent.regno)+'&bregno='+encode(parent.bregno)+'")>'+data[i].content+'</a></th></tr>').appendTo($('#container3'));
		     	}
			}
		});
		...
		var yaoXieUrl = ""; //食品信息
		$("#yaoXieName").text("药品信息");
		yaoXieUrl = "yaoXie.json";
		$.getJSON(yaoXieUrl,function(data){
			for(var i=0 ;i<data.length;i=i+2){
				var url=data[i].url;
				if((i+1)<data.length){
					//	$('<tr height="30px"><th width="50%"><a target="main" href="'+url+parent.priPid+'&sourceflag='+parent.sourceflag+'">'+data[i].content+'</a></th><th>|</th><th width="50%"><a target="main" href="'+data[i+1].url+parent.priPid+'&sourceflag='+parent.sourceflag+'">'+data[i+1].content+'</a></th></tr>').appendTo($('#container'));
					$('<tr height="30px"><th width="12%"><a  href="#" onclick=rightPageSet("'+url+encode(parent.entid)+'&entname='+encode(parent.entname)+'&regno='+encode(parent.regno)+'")>'+data[i].content+'</a></th><th>|</th><th width="50%"><a  href="#" onclick=rightPageSet("'+data[i+1].url+encode(parent.entid)+'&entname='+encode(parent.entname)+'&regno='+encode(parent.regno)+'")>'+data[i+1].content+'</a></th></tr>').appendTo($('#container5'));
				}else{
					$('<tr height="30px"><th width="12%"><a  href="#" onclick=rightPageSet("'+url+encode(parent.entid)+'&entname='+encode(parent.entname)+'&regno='+encode(parent.regno)+'")>'+data[i].content+'</a></th></tr>').appendTo($('#container5'));
		     	}
			}
		});
...
<body style="background-color: #f2f9f9;"><!-- #F8F9FB; -->
	...
	<div id="div5" height="100%">
		<div class="divboder">
			<div id="yaoXieName" class="title" style="text-align: center;"></div>
			<table class="container" style="margin:0px;" id="container5"></table>
			<table style="display:none" class="container" style="margin:0px;border-collapse: collapse;"
				id="ndreport2"></table>
		</div>
	</div>
