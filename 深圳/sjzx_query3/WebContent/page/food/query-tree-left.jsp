	...
	$(document).ready(function() {
		...
		$.getJSON(url,function(data){
			for(var i=0 ;i<data.length;i=i+2){
				var url=data[i].url;
				if((i+1)<data.length){
					$('<tr height="30px"><th width="50%"><a  href="#" onclick=rightPageSet("'+url+parent.id+'&lictype='+parent.lictype+'")>'+data[i].content+'</a></th><th>|</th><th width="50%"><a  href="#" onclick=rightPageSet("'+data[i+1].url+parent.id+'&lictype='+parent.lictype+'")>'+data[i+1].content+'</a></th></tr>').appendTo($('#container'));
				}else{
					$('<tr height="30px"><th width="50%"><a  href="#"  onclick=rightPageSet("'+url+parent.id+'&lictype='+parent.lictype+'")>'+data[i].content+'</a></th></tr>').appendTo($('#container'));
		     	}
			}
		});
