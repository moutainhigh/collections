			...
			$.ajax({
				...
				success:function(data){
					...
					for (var i=0;i<jsonStr.length;i++) {//<td style='width:117px'>"+_html+"</td>
						$("#columnList").append("<option data-name_en="+jsonStr[i].dcTableNameEn+" data-name_cn="+jsonStr[i].dcTableNameCn+" data-collength="+jsonStr[i].colLenght+" data-column_cn ="+jsonStr[i].title+" data-column_en ="+jsonStr[i].code +" data-columntype = "+jsonStr[i].colType+">"+jsonStr[i].title+"</option>");
					...
					$("#columnList option").on("click",function(){
						var _this = $(this);
						_this.remove();
						...
						var isHas = $("#selectColumnList option").data("column_en");
						$("#selectColumnList").append("<option data-name_en="+name_en+" data-name_cn="+name_cn +" data-column_en ="+column_en +" data-collength="+colLength+" data-columntype = "+columntype+" >"+column_cn+"</option>");
					
						
						$("#selectColumnList option").on("click",function(){
							var $this = $(this);
							var name_en = _this.data("name_en");
							var name_cn = _this.data("name_cn");
							var column_cn = _this.data("column_cn");
							...
							var collength =_this.data("collength");
							$this.remove();
							$("#getColumDetail").append("<tr><td>"+name_cn+"</td><td>"+column_cn+"</td><td>"+columntype+"</td><td>"+collength+"</td><td><a>删除</a></td></tr>");
						});
...
<select id="pid" onchange="gradeChange()" style="width: 400px;margin:10px auto">
    <option  value="0">请选择主题</a>
</select>
			...
			<tr>
				...
				<td align="center" valign="middle" width="5%" valign="top">
	   			...
	   			<td valign="top">
	   				<style>
	   					.fl{float: left;}
	   					.showSelectColumTips{position: absolute;width:70px;top: 100px;}
	   				</style>
	   				<div class="list">
	   					<div class="fl">
	   					...
	   					<div class="fl" style="position: relative;">
	   						<span class="showSelectColumTips">选择查询的列名：</span>
	   						<select style="position: absolute;left: 58px;" id="selectColumnList" multiple="multiple" size="10" class="cssSelect"></select>
	   					</div>
	   					
	   					<div class="fl" style="position: relative;">
	   						<div class="" style="height: 200px;overflow-y: hidden">
	   						<table  style="position: absolute; left:220px;width: 580px">
	   							<thead>
			   					<tr>
		   							<th width='100'>表名</th>
		   							<th width='150'>数据项名</th>
		   							<th width='80'>类型</th>
	   								<th width='80'>长度</th>
	   								<th width='80'>操作</th>
	   							</tr>
	   								...
	   								<style>
	   									#getColumDetail td{border-bottom: 1px solid #f6f6f6;text-align: center;}
	   								</style>
