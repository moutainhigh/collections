...
var tableCode = '';
function createSql(step,prama) {

	if(step=="1"){
		var aliaName = getAliaName().tableEn;
		var cloum =  aliaName+"."+getAliaName().columnEn;
		
		
		sql = "SELECT "+ cloum + " FROM " + prama;
		sql = replacePos(sql, sql.indexOf(", FROM")+1, "");
		tableCode = prama;
	}
	...
	if (step == '2') {
		...
		sql = "SELECT "+cloum+" FROM " + tableCode + ' WHERE ' + selectStr + ' ';
		sql = replacePos(sql, sql.indexOf(", FROM")+1, "");
		//alert(sql);
	} 
...
function preview() {
	...
	var aliaName =  getAliaName();
	
	console.log("列名和别名:" +aliaName.keyEn + aliaName.keyCn + aliaName.keyAlia + aliaName.tableCn  + aliaName.tableEn);
	var queryTableJson = {baseInfo:{}};
	
	queryTableJson.baseInfo.queryName="";
	
	queryTableJson.baseInfo.tableNames=[];
	queryTableJson.baseInfo.tableNames.push({
		name_en:tableCodesList,/*中文名*/
		name_cn:tableNamesList,/*英文名*/
		column_en:aliaName.keyEn,
		column_cn:aliaName.keyCn,
		column_ais :aliaName.keyAlia
	});
	
	
	//console.log(queryTableJson);
	...
	if (checkResult == false) {
		...
		$.ajax({
			...
			type:"post",
			data:{sql:sql,queryParams:relation},
...
function getAliaName(){
	...
	$(".alias").each(function(){
		var _this = $(this);
		if(_this.html()!=""){
			//columnEn += _this.data("column_en")+"  as  "+_this.html()+" , ";
			columnAlia = columnEn;
		}else{
			columnEn += _this.data("column_en") + " ,";
		}
	});
	
	 $(".alias").each(function(){
			var _this = $(this);
			columnCn +=  _this.data("column_cn")+" ,"
		});
	 $(".alias").each(function(){
			var _this = $(this);
			tableCn +=  _this.data("name_cn")+" ,"
		});
	 
	 $(".alias").each(function(){
			var _this = $(this);
			tableEn +=  _this.data("name_en") +"."+ _this.data("column_en")+" ,"
		});
		return {
			columnEn:columnEn,
			columnCn:columnCn,
			columnAlia:columnAlia,
			tableCn:tableCn,
			tableEn:tableEn
		}
}
...
function  getJsonObj(){
	...
	$(".alias").each(function(){
		...
		var _this = $(this);
		 ...
		 if(_this.html()!=""){
			 column_ais = _this.html();
		 }
		 obj.name_en = name_en;
		 obj.name_cn = name_cn;
		 obj.column_en = column_en;
		 obj.column_cn = column_cn;
		 obj.column_ais = column_ais;
		 tableNames.push(obj);
	});
	
	
	for ( var i in tableNames) {
		console.log(i + "  "  + tableNames[i].name_en);
	}
