...
function preview() {
	...
	alert("share_interface " + checkResult);
...
function getSearchSelectColumn(){
	...
	if(columns==""||columns=="undefined"||columns.length==0){
		...
		columns.each(function(){
			 var _this = $(this);
			 name_en = decode(_this.data("name_en"));
			 name_cn =  decode(_this.data("name_cn"));
			 column_cn =  decode(_this.data("column_cn"));
			 column_en =  decode(_this.data("column_en"));
...
function  getJsonObj(sql){
	...
	$("#getColumDetail tr").each(function(){
		...
		// var obj = {};
		 name_en += decode(_this.data("name_en"))+",";
		 name_cn += decode(_this.data("name_cn"))+",";
		 column_en += decode(_this.data("name_en"))+"."+decode(_this.data("column_en"))+",";
		 column_cn +=  decode(_this.data("name_cn"))+"."+decode(_this.data("column_cn"))+",";
...
function saveTableInfoTable(jsonStr){
	...
	$.ajax({
		...
		data:{datas:jsonStr},
		success:function(data){
			var uid = data.data[0].data;
			var dataStr = data.data[1].data;
			if(dataStr=="1"){
				$("#listResult").attr("src","queryForPage.jsp?uid=1&queryId="+uid);
				//$("#listResult").attr("src","queryForSql.jsp");
