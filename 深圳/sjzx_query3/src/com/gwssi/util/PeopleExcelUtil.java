...
public class PeopleExcelUtil {
	...
	public static void PeopleExcelOut(List<Map> list,String excelTitle,
		...
		//设置表格数据的样式
		CellStyle ztStyle1 =  xs.createCellStyle();  
		ztStyle1 = getValueStyle1(xs);
		...
		if(code2StrKeys!=null && code2StrKeys.size()!=0 && codes2String!=null && codes2String.size()!=0){
			...
			for(int i = 0;i<list.size();i++){   //遍历数据集合
				...
				for(int y = 0;y<rowDataKeys.size();y++){  //遍历Key集合
		            ...
		            ztCell.setCellStyle(ztStyle1);               // 样式应用到该单元格上  
				}
			}
		}
	...
	private static XSSFCellStyle getValueStyle1(XSSFWorkbook xs) {
	...
	private static XSSFCellStyle getValueStyle2(XSSFWorkbook xs, int y) {
		XSSFCellStyle ztStyle = (XSSFCellStyle) xs.createCellStyle();  
		//rowdata.createCell((short)y).setCellValue(map.get(rowDatakey)==null?"":map.get(rowDatakey).toString());
		 // 创建字体对象  
		Font ztFont = xs.createFont();  
