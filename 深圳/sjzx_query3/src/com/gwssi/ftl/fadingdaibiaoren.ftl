		...
		<#if cou%2==0>
			...
			<#elseif key=="成立日期" || key=="核准日期" || key=="经营期限自" || key== "经营期限至" || key== "执照有效期限" || key== "注销日期"  || key== "吊销日期" || key== "出生日期" || key== "生产经营期限止" || key== "生产经营期限起" || key == "营业期限自" || key == "营业期限至" ||key =="经营(驻在)期限自" ||key=="经营(驻在)期限至" || key=="合伙期限自" || key=="合伙期限至">
		  	...
		  	<#elseif key=="姓名">
		  		<#if weaponMap[key]??>
		  			<#if weaponMap["证照号码"]??>
				  		<tr><td style="text-align: right;padding-right: 1px;">${key}:</td><td style="padding-left: 5px;padding-bottom: 8px;padding-top: 8px;"><a id="persname" href="javascript:void(0);" onclick="viewDataSource('${weaponMap["证照号码"]}');">${weaponMap[key]}</a></td>
				  		<#assign cou=cou+1 />
				  	<#else>
		  				<tr><td style="text-align: right;padding-right: 1px;">${key}:</td><td style="padding-left: 5px;padding-bottom: 8px;padding-top: 8px;">${weaponMap[key]?default("无")}</td>
				  		<#assign cou=cou+1 />
