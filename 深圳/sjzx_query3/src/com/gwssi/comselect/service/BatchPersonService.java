		...
		if (map!=null) {
			if(map.get("select")!=null){
				...
				if("0".equals(select)){//历史记录
					sql.append("select  func_getcode('C00017',a.regorg) as regorg,a.dom, b.type_flag,a.id as pripid,a.pripid as entid,a.opetype,a.regno,a.entstatus as status,a.enttype as type,a.entname,a.unifsocicrediden,d.old_code_name as enttype,a.regcap,to_char(a.estdate,'yyyy-MM-dd') as estdate,(case when a.entstatus = '2' then to_char(t2.REVODATE, 'yyyy-MM-dd') when a.entstatus = '4' then to_char(t2.CANCDATE, 'yyyy-MM-dd')  end) as apprdate,e.old_code_name as entstatus,b.persname,b.cerno,b.perflag,(case when b.perflag='股东' then '出资额'||c.old_code_name|| TO_CHAR(b.SUBCONAM)||'万元，出资比例'||TO_CHAR(b.CONPROP)||'%' when b.perflag='高管人员' then b.post end) as memo ");
					sql.append("from dc_ra_mer_base a, dc_ra_mer_base_ext t2,(select * from v_persons_query_old where 1=1  ");
				}else{// 当前记录
					sql.append("select  func_getcode('C00017',a.regorg) as regorg,a.dom, a.id as pripid,a.pripid as entid,a.opetype,a.regno,a.entstatus as status,a.enttype as type,a.entname,a.unifsocicrediden,d.old_code_name as enttype,a.regcap,to_char(a.estdate,'yyyy-MM-dd') as estdate,(case when a.entstatus = '2' then to_char(t2.REVODATE, 'yyyy-MM-dd') when a.entstatus = '4' then to_char(t2.CANCDATE, 'yyyy-MM-dd')  end) as apprdate,e.old_code_name as entstatus,b.persname,b.cerno,b.perflag,(case when b.perflag='股东' then '出资额'||c.old_code_name|| TO_CHAR(b.SUBCONAM)||'万元，出资比例'||TO_CHAR(b.CONPROP)||'%' when b.perflag='高管人员' then b.post end) as memo ");
					sql.append("from dc_ra_mer_base a, dc_ra_mer_base_ext t2,(select * from v_persons_query where 1=1  ");
				}
			}
			...
			sql.append(" ) b,(select * from codedata where code_index='CA04') c,(select * from codedata where code_index='CA16') d,(select * from codedata where code_index='CA19') e where a.id=b.main_tb_id  and a.id = t2.main_tb_id(+) and a.currency=c.old_code_value(+) and a.enttype=d.old_code_value(+) and a.entstatus=e.old_code_value(+)  order by a.entname ");
