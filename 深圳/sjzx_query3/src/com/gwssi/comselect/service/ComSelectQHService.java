...
@Service(value = "comSelectServiceQH")
public class ComSelectQHService extends BaseService{
	...
	public Map<String,Object> queryEnt(EntSelectQueryBo bo,String querytype, JSONObject res, String id,HttpServletRequest httpServletRequest) throws OptimusException {
		...
		if(querytype.equals("excel")){
			...
			if(bo.getEnttype() != null){
				sql.append(" from V_QIANHAI_RA_MER_BASE_QUERY t, com_enttype_new a where 1=1 ");
			}else{
				sql.append(" from V_QIANHAI_RA_MER_BASE_QUERY t where 1=1 " );
			}
			
		}else if(bo.getEnttype() != null){
			sql.append("select pripid,regno,entname,enttype,type,estdate,REGCAPCUR,RECCAP,REGSTATE,entstatus,opetype,estdate,apprdate,lerep,opfyears,entid from V_QIANHAI_RA_MER_BASE_QUERY t, com_enttype_new a where 1=1  ");
		}else{
			sql.append("select pripid,regno,entname,enttype,type,estdate,REGCAPCUR,RECCAP,REGSTATE,entstatus,opetype,estdate,apprdate,lerep,opfyears,entid from V_QIANHAI_RA_MER_BASE_QUERY t where 1=1  ");
		}
		...
		if(StringUtils.isNotBlank(bo.getAddrflag())){
			
			sql.append( " and t.addr_flag=" ).append("?").append("   ");
			params.add(bo.getAddrflag());
		}
...
//			}
		...
		if("1".equals(bo.getYichangminglu())){ //包含异常名录
			sql.append("and t.entid in (select entid from dc_ms_abnormal_diretory where type in(?,?) and entid in(select pripid from dc_ns_enterprise_list))");
			params.add("1");
			params.add("2");
		}else if("0".equals(bo.getYichangminglu())){ //不包含异常名录
			sql.append("and t.entid not in (select entid from dc_ms_abnormal_diretory where type in(?,?) and entid in(select pripid from dc_ns_enterprise_list))");
			params.add("1");
			params.add("2");
		}
		
		if("1".equals(bo.getGangzi())){ //包含港资企业
			sql.append("and t.pripid in (select main_tb_id from dc_ra_mer_invest where countrycode= ? union all select main_tb_id from dc_ra_mer_base_ext where countrycode= ?) and enttype in(select code from COM_ENTTYPE_NEW t where pid like 'C%')");
			params.add("344");
			params.add("344");
		}else if("0".equals(bo.getGangzi())){ //不包含港资企业
			sql.append("and (not exists (select main_tb_id from dc_ra_mer_invest i where t.PRIPID = i.main_tb_id and countrycode= ?  union all select main_tb_id from dc_ra_mer_base_ext e where t.PRIPID = e.main_tb_id and countrycode= ?) or enttype not in(select code from COM_ENTTYPE_NEW t where pid like 'C%'))");
			params.add("344");
			params.add("344");
		}
	...
	public List<Map<String, Object>> queryEntTypeTree(String radiovalue) throws OptimusException {
		...
		sql.append("select code as ID, type_name AS NAME , nvl(pid,'all') as pid, is_stand AS STANDE from com_enttype_new  ");
