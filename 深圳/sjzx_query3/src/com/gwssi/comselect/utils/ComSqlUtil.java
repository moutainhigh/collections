...
public  class ComSqlUtil {
	 ...
	 * @return
	 */
	public static Map<String,Object> getORSql(String[] term,String[] value,String filename,String logic){
					...
					if("or".equals(logic)){
						if(StringUtils.isNotBlank(entname_term[i])&&StringUtils.isNotBlank(entname[i])){
							if("include".equals(entname_term[i])){
								sql2.append(" or  ").append(filename).append(" like ?");
								//list2.add(entname[i]+"%");
								list2.add("%"+entname[i]+"%");
							}else if("notinclude".equals(entname_term[i])){
								sql2.append(" or  ").append(filename).append(" not like ?");
								//list2.add(entname[i]+"%");
								list2.add("%"+entname[i]+"%");
							}else{
								sql2.append(" or  ").append(filename).append(" = ?");
								list2.add(entname[i]);
							}
						}
					}else {
