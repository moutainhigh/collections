	function initData() {
			...
			var code = $('#index_code').val();  //获取输入的验证码
			var flag = checkCode();		//获取验证码
							...
							success : function(data) {
								...
								if (data.data[0].data == "0") {
									...
									if(data.data[0].data=="您查询次数过多，请稍后再查！"){
										...
										var entTotalNums = data.data[0].data.length ;
									    ...
									    if(entTotalNums==1){ 
										 ...
										 getMutiRowsByUnifCode(entTotalNums);
									 }
									}else{
										var entname = $('#entNameInput').val();
...
//根据不同条件掉用不同的方法
function getQueryStr(unifsocicrediden,entname,isFull){
	if(isFull=="1"){
		//exactQuery(unifsocicrediden,entname,1)
		exactQuery(unifsocicrediden,entname,1);
	}else{
		//uncertainQuery(unifsocicrediden,entname,1,isFull)
		uncertainQuery(unifsocicrediden,entname,1);
	}
}
...
//模糊查询
function uncertainQuery(unifsocicrediden,entname,flag){
	$.ajax({
		url : '../entEnt/entList.do',
		type : 'POST',
		dataType : 'json',
...
//通过统一信用代码或者注册号查询到多条的
function getMutiRowsByUnifCode(entTotalNums){
	var unifsocicrediden = $('#unifsocicredidenInput').val();
	$.ajax({
		url : '../entEnt/entList.do',
		type : 'POST',
		...
		success:function(data){
			...
			$("#listTips p").eq(1).html("您通过【<strong class='keyWordsDesc'> "+unifsocicrediden+"</strong>】查询到的企业数量共 【 <strong class='keyWordsDesc'> " +entTotalNums+ "</strong>】 家(注：对应的商事主体数量过大 ，系统只显示最多20家)。");
		},error:function(data){
			$("#submit2").text("查询");
			alert(data.responseText);
		}
