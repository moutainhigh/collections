...
public class ReceiveHrAppImpl implements IReceiveHrApp {
	...
	public User getUserInfo(String userId) {
		...
		try {
			...
			user.setOrgCode("4530");
			user.setUpOrgCode(postService.getUpOrgCode(user.getOrgCode()));
...
/*				if (isSuperAdmin){
	...
	public User getUserInfoByUserRoles(String userId) {
		...
		String istest = optimus.getProperty("org.test");
		...
		try {
			...
			if("true".equalsIgnoreCase(istest)){
				String orgno = optimus.getProperty("org.no");
				user.setOrgCode(orgno);
			}else{
				user.setOrgCode(postService.getOrgCode(userId));
			}
