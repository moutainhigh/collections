...
public class FoodstuffUtils {
	...
	public String queryShiPinXuKe(List<Map> num) {
		...
		for (int i = 0,ii=num.size(); i < ii; i++) {
			...
			if(!StringUtils.isEmpty(num.get(i).get("辖区"))) {
				tm  = (String) num.get(i).get("辖区");
			}else {
				tm = "合计";
			}
	...
	public String downExcelShiPinZhunRu(Map map,String beginTime) {
		StringBuffer sb=new StringBuffer();
		
		
		sb.append("<table  border='1' cellspacing='0' cellpadding='0' width='1024px'>");
		sb.append("<tr><td colspan='5' style='text-align:center;    font-size: 25px;'>食药准入业务工作完成情况报表（食品）</td></tr>");
		sb.append("<tr><td colspan='5' style='text-align:center;    font-size: 18px;'>食品生产许可</td></tr>");
		sb.append("<tr><td style='text-align:center'>辖区</td>");
		sb.append("<td style='text-align:center'>单位</td>");
		sb.append("<td></td>");
		sb.append("<td >受理数(件)</td>");
		sb.append("<td >办理数(件)</td>");
		sb.append("</tr>");
				
		
		
		
		List<Map> shengChangList = (List<Map>) map.get("shengChangList");
		List<Map> jingYingList = (List<Map>) map.get("jingYingList");
		
		
		List<Map> num = shengChangList;
		/*for (int i = 0; i < num.size(); i++) {
			if(num.get(i).get("nyFlag").equals("当月数")){
				sb.append("<tr height=19 style='height:14.25pt'>" );
				sb.append("  <td rowspan=2 height=38 class=xl6630409 width=101 style='height:28.5pt;\n" );
				sb.append("  border-top:none;width:76pt'>"+num.get(i).get("单位")+"</td>\n"  );
				sb.append("  <td class=xl6430409 width=100 style='border-top:none;border-left:none;\n" );
				sb.append("  width:75pt'>当月数</td>\n" );
			    sb.append("  <td class=xl7330409 width=100 style='border-top:none;border-left:none;\n" );
			    sb.append("  width:75pt'>"+num.get(i).get("食品生产受理")+"</td>" );
			    sb.append(" <td class=xl7330409 width=100 style='border-top:none;border-left:none;\n " );
			    sb.append("   width:75pt'>"+num.get(i).get("食品生产办理")+"</td>\n"  );
	 		    sb.append(" </tr>");
			}else{
				sb.append("<tr height=19 style='height:14.25pt'>");
				sb.append("<td height=19 class=xl6430409 width=100 style='height:14.25pt;border-top: ");
				sb.append(" none;border-left:none;width:75pt'>本年度累计</td>");
				sb.append("<td class=xl7330409 width=100 style='border-top:none;border-left:none;"  );
				sb.append("width:75pt'>"+num.get(i).get("食品生产受理")+"</td>");
		        sb.append(" <td class=xl7330409 width=100 style='border-top:none;border-left:none;");
				sb.append(" width:75pt'>"+num.get(i).get("食品生产办理")+"</td>");
		...
		for (int i = 0; i < num.size(); i++) {
			if(num.get(i).get("nyFlag").equals("当月数")){
				sb.append("<tr> ");
				sb.append("<td style='text-align:center' >"+num.get(i).get("辖区")+"</td>" );
				sb.append("<td>"+num.get(i).get("单位")+"</td>" );
				sb.append("  <td>当月数</td>"); 
				sb.append("  <td >"+num.get(i).get("食品生产受理")+"</td>");
				sb.append("  <td>"+num.get(i).get("食品生产办理")+"</td>" );
				sb.append( " </tr>");
			}else{
				sb.append(	"<tr> ");
				sb.append(	"<td  style='text-align:center' >"+num.get(i).get("辖区")+"</td>" );
				sb.append(	"<td>"+num.get(i).get("单位")+"</td>" ); 
				sb.append("  <td>本年度累计</td>"); 
				sb.append("  <td >"+num.get(i).get("食品生产受理")+"</td>" );
				sb.append("  <td>"+num.get(i).get("食品生产办理")+"</td>" );
				sb.append(" </tr>");
			} 
		...
		sb.append("<tr><td colspan='5' style='height:30px;'>&nbsp;</td></tr>\n" );
		sb.append( "<tr><td colspan='5' style='height:30px;'>&nbsp;&nbsp;</td></tr>\n" );
		sb.append( "<tr><td colspan='5' style='height:30px;'>&nbsp;&nbsp;&nbsp;&nbsp;</td></tr>\n" );
		sb.append("<tr><td colspan='5' style='text-align:center ;   font-size: 18px;'>食品经营许可（不含原餐饮许可数据）</td></tr>");
		sb.append("<tr><td style='text-align:center;'>辖区</td>");
		sb.append("<td style='text-align:center'>单位</td>");
		sb.append("<td></td>");
		sb.append("<td >受理数(件)</td>");
		...
		/*for (int i = 0; i < num.size(); i++) {
			if(num.get(i).get("nyFlag").equals("当月数")){
				...
				sb.append("  width:75pt'>当月数</td>\n" );
	 			sb.append("  <td class=xl7330409 width=100 style='border-top:none;border-left:none;\n" );
	 			sb.append(" 	  width:75pt'>"+num.get(i).get("食品经营受理")+"</td>\n"  ); 
	 			sb.append("  <td class=xl7330409 width=100 style='border-top:none;border-left:none;\n " );
	 		    sb.append("   width:75pt'>"+num.get(i).get("食品经营办理")+"</td>\n"); 
	 		    sb.append(" </tr>");
			}else{
				sb.append("<tr height=19 style='height:14.25pt'>");
				sb.append("<td height=19 class=xl6430409 width=100 style='height:14.25pt;border-top: ");
		...
		for (int i = 0; i < num.size(); i++) {
			if(num.get(i).get("nyFlag").equals("当月数")){
				...
				sb.append("<td>"+num.get(i).get("单位")+"</td>" );
				sb.append("  <td>当月数</td>"); 
				sb.append("  <td >"+num.get(i).get("食品经营受理")+"</td>");
				sb.append("  <td>"+num.get(i).get("食品经营办理")+"</td>" );
				sb.append( " </tr>");
			}else{
				sb.append(	"<tr> ");
				sb.append(	"<td  style='text-align:center' >"+num.get(i).get("辖区")+"</td>" );
