...
function renderdata(event, obj){
	...
	for(var i=0;i<data.length;i++){
		...
		if(data[i]["issuper"]=="1"){
			data[i]["custom"] += '<a href="javascript:void(0);" title="编辑" onclick="editorSystem(\''+data[i]["pkSysIntegration"]+'\');"><img src="'+rootPath+'/static/images/other/forma-1.png" width="11px" height="11px" border="0"/></a>&nbsp;&nbsp;&nbsp;'
							  +'<a href="javascript:void(0);" title="维护" onclick="fixMenu(\''+data[i]["pkSysIntegration"]+'\');"><img src="'+rootPath+'/static/images/other/fengbei.png" width="11px" height="11px" border="0"/></a>&nbsp;&nbsp;&nbsp;'
							  +'<a href="javascript:void(0);" title="查看" onclick="viewSystem(\''+data[i]["pkSysIntegration"]+'\');"><img src="'+rootPath+'/static/images/other/mag.png" width="11px" height="12px" border="0"/></a>&nbsp;&nbsp;&nbsp;'
							  +'<a href="javascript:void(0);" title="删除" onclick="deleteSystem(\''+data[i]["pkSysIntegration"]+'\');"><img src="'+rootPath+'/static/images/other/close.png" width="13px" height="13px" border="0"/></a>'
		}else{
...
function editorSystem(pkid){
	...
	newwindow.window('option','frameurl',rootPath+"/page/integeration/xt_add.jsp?type=1&pkId="+pkid);
	newwindow.window('open');
}

function viewSystem(pkid){
	if (newwindow.length == 0) {
		createNewWindow();
	}
	newwindow.window('option','frameurl',rootPath+"/page/integeration/xt_add.jsp?type=2&pkId="+pkid);
	newwindow.window('open');
}

function fixMenu(pkSysIntegration) {//维护菜单
