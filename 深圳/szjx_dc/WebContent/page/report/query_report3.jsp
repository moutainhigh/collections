			...
			if ("${sessionScope.USER_DOOR_ROLE_CODE}"==2) {
				for(var i=0;i<data.length;i++){
					...
					if(isValid == '0'){ //报表是已启用的
						data[i]["isvalid"] = '<a>'+'<font color="green">已启用</font></a>';
					}else{//停用中
						data[i]["isvalid"] = '<a>'+'<font color="red">已停用</font></a>';
					}
				}
			}else{
				for(var i=0;i<data.length;i++){
					...
					data[i]["regcode"] = getDistrict(data[i]["regcode"]);
					if(isValid == '0'){ //报表是已启用的
						data[i]["isvalid"] = '<a href="javascript:void(0);" onclick="changeValid(\''+data[i]["id"]+'\','+isValid+')"><font color="green">已启用</font></a>';
					}else{//停用中
						data[i]["isvalid"] = '<a href="javascript:void(0);" onclick="changeValid(\''+data[i]["id"]+'\','+isValid+')"><font color="red">已停用</font></a>';
						}
					}
				}
				return data;
