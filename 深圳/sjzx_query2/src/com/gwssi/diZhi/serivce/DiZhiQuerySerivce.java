...
public class DiZhiQuerySerivce  extends BaseService{
	...
	private void commonMethod(Map map, StringBuffer sql, List list) {
		if (map!=null) {
			...
			if (map.get("yiChangContains")!=null) {
				Object object2 = map.get("yiChangContains");
				if (object2.toString().trim().length()>0) {
					String yiChangContains = object2.toString();
					
					if("1".equals(yiChangContains)) {
						if("全部".equals(SpQymark)){
							if (map.get("yiChangType")!=null) {
								Object object3 = map.get("yiChangType");
								if (object3.toString().trim().length()>0) {
									sql.append("  and ( t.ABNORMALTYPE  in ( ");
									String temp = object3.toString().trim();
									if(temp.contains(",")) {
									...
									sql.append(" ) ");
									sql.append( " OR t.ABNORMALTYPE IS NULL )");
								}
							}
						}else {
							if (map.get("yiChangType")!=null) {
						...
						if("全部".equals(SpQymark)){
							if (map.get("yiChangType")!=null) {
								Object object3 = map.get("yiChangType");
								if (object2.toString().trim().length()>0) {
									sql.append(" and ( t.ABNORMALTYPE  not in ( ");
									String temp = object3.toString().trim();
									if(temp.contains(",")) {
										String [] arr = temp.split(",");  
										for (int i = 0; i < arr.length; i++) {
	...
	public List<Map> exportExcel(Map map, HttpServletRequest httpServletRequest) throws OptimusException{
		...
		String testsql =  "select * from ("+sql+")  e "; //修改为8万条数据
