...
public class ComSelectService extends BaseService{
	...
	public List<Map> queryExcelEntQuery(EntSelectQueryBo bo, JSONObject res, String id,HttpServletRequest request) throws OptimusException{
		...
		/*
		 * 该处为新增“所属监管所”或“工作网格”选项
		 * 如果有“所属监管所”或“工作网格”选项，则对其进行转码处理，
		 * 没有选择“所属监管所”和“工作网络”选项时，则不进行转码处理，直接返回原来的结果集，
		 * 对list.get(0).containsKey("adminbrancode")条件的处理，只有在list不为null并且有值时，才会执行该条件。
		*/
		if (list != null && list.size() > 0 && (list.get(0).containsKey("adminbrancode") || list.get(0).containsKey("gongzuowangge") )) {
			List<Map> transCodingList = transCoding(list);
			return transCodingList;
		}
	...
	public Map<String,Object> queryEnt(EntSelectQueryBo bo,String querytype, JSONObject res, String id,HttpServletRequest httpServletRequest) throws OptimusException {
		...
		if(querytype.equals("excel")){
			...
			}if(!"".equals(res.get("gongzuowangge"))) {
				sql.append(" ,gongzuowangge");
			}
			if(!"".equals(res.get("enttype"))){
				...
				sql.append(" ,opfyears");
			}if(!"".equals(res.get("fddbrMobtel"))){
				sql.append(" ,fddbr_mobtel");
			}if(!"".equals(res.get("gsllyMobtel"))){
				sql.append(" ,gslly_mobtel");
			}if(!"".equals(res.get("gsllyName"))){
				sql.append(" ,gslly_name");
			}
		...
		if(StringUtils.isNotBlank(bo.getYichangminglu())){
			if(!(bo.getYichangminglu().equals("2"))) {
				sql.append( " and t.ms_type= " ).append("?").append("   ");
				params.add(bo.getYichangminglu());
			}
			
		}
		
		
		/*if(StringUtils.isNotBlank(bo.getYear())){
			StringBuffer sbTemp = new StringBuffer();
			sbTemp.append(" AND EXISTS (SELECT 1 FROM (SELECT T3.PRIPID FROM DC_NB_QY_JBXX T3 WHERE T3.IFPUB IN ('2','3') AND T3.ANCHEYEAR IN (?)  ");
			sbTemp.append("     UNION ALL ");
			sbTemp.append(" SELECT T4.PRIPID FROM DC_NB_GT_JBXX T4 WHERE T4.IFPUB IN ('2','3','5') AND T4.ANCHEYEAR IN (?) ) N WHERE N.PRIPID = T.PRIPID ) ");
				
			sql.append(sbTemp.toString());
			params.add(bo.getYear());
			params.add(bo.getYear());
			
		}*/
		
		if(StringUtils.isNotEmpty(bo.getNbflag())) {
			String nbFlag = bo.getNbflag();
			if(nbFlag.equals("全部")) {
				/*StringBuffer sbTemp = new StringBuffer();
				sbTemp.append(" and 1 = 1  ");
					
				sql.append(sbTemp.toString());*/
			}else if(nbFlag.equals("0")) {
				if(StringUtils.isNotBlank(bo.getYear())){
					StringBuffer sbTemp = new StringBuffer();
					
					if(!bo.getYear().contains(",")) {
						sbTemp.append(" AND not EXISTS (SELECT 1 FROM (SELECT T3.PRIPID FROM DC_NB_QY_JBXX T3 WHERE  T3.ANCHEYEAR IN (?)  ");
						sbTemp.append("     UNION ALL ");
						sbTemp.append(" SELECT T4.PRIPID FROM DC_NB_GT_JBXX T4 WHERE  T4.ANCHEYEAR IN (?) ) N WHERE N.PRIPID = T.Entid ) ");
							
						sql.append(sbTemp.toString());
						params.add(bo.getYear());
						params.add(bo.getYear());
					}else {
						String [] arr = bo.getYear().split(",");
						
						/*StringBuffer  sqlWrap =  new StringBuffer();
						...
						StringBuffer sqlWrap = new StringBuffer();
						for (int i = 0; i < arr.length; i++) {
							sqlWrap.append("'"+arr[i]+"',");
							
						}
						
						String sqlWrapStr = sqlWrap.toString();
						String tempSQLWrap = sqlWrapStr.substring(0,sqlWrapStr.length()-1);
						
						sbTemp.append(" AND not EXISTS (SELECT 1 FROM (SELECT T3.PRIPID FROM DC_NB_QY_JBXX T3 WHERE  T3.ANCHEYEAR IN ("+tempSQLWrap+")  ");
						sbTemp.append("     UNION ALL ");
						sbTemp.append(" SELECT T4.PRIPID FROM DC_NB_GT_JBXX T4 WHERE  T4.ANCHEYEAR IN ("+tempSQLWrap+") ) N WHERE N.PRIPID = T.Entid ) ");
				...
				if(StringUtils.isNotBlank(bo.getYear())){
					...
					if(!bo.getYear().contains(",")) {
						sbTemp.append(" AND  EXISTS (SELECT 1 FROM (SELECT T3.PRIPID FROM DC_NB_QY_JBXX T3 WHERE T3.IFPUB IN ('2','3') AND T3.ANCHEYEAR IN (?)  ");
						sbTemp.append("     UNION ALL ");
						sbTemp.append(" SELECT T4.PRIPID FROM DC_NB_GT_JBXX T4 WHERE T4.IFPUB IN ('2','3','5') AND T4.ANCHEYEAR IN (?) ) N WHERE N.PRIPID = T.Entid ) ");
						...
						sbTemp.append(" AND  EXISTS (SELECT 1 FROM (SELECT T3.PRIPID FROM DC_NB_QY_JBXX T3 WHERE T3.IFPUB IN ('2','3') AND T3.ANCHEYEAR IN ("+tempSQLWrap+")  ");
						sbTemp.append("     UNION ALL ");
						sbTemp.append(" SELECT T4.PRIPID FROM DC_NB_GT_JBXX T4 WHERE T4.IFPUB IN ('2','3','5') AND T4.ANCHEYEAR IN ("+tempSQLWrap+") ) N WHERE N.PRIPID = T.Entid ) ");
		...
		Map map2= ComSqlUtil.getORSql(bo.getEntname_term(),bo.getEntname(),"t.entname", bo.getEntname_logic());
		if(map2!=null){
		...
		Map map1= ComSqlUtil.getORSql(bo.getOpscope_term(),bo.getOpscope(),"t.opscope", bo.getOpscope_logic());
		if(map1!=null){
		...
		Map map3= ComSqlUtil.getORSql(bo.getDom_term(),bo.getDom(),"t.dom", bo.getDom_logic());
		if(map3!=null){
...
//			}
			if(bo.getEnttype()!=null&&bo.getEnttype().length>0){
				sql.append(" and t.newenttype=a.code and t.newenttype in (");
		...
		if("page".equals(querytype) && httpServletRequest == null){
			String querySql = sql.toString().replace("pripid,regno,entname,enttype,type,estdate,REGCAPCUR,RECCAP,REGSTATE,entstatus,opetype,estdate,apprdate,lerep,opfyears,entid", "/*+parallel(16)*/ count(1) as cnt ");
	...
	public List<Map<String, Object>> queryCode_value(String string) throws OptimusException {
		...
		if("regcapcur".equals(string)){//币种
			 //sql.append("select * from (select t.codeindex_code as value, t.codeindex_value as text from DC_STANDARD_CODEDATA t where t.standard_codeindex = 'C00013' order by t.illustrate) where rownum<=18 ");
			 sql.append("select * from v_dc_money_type ");
		...
		else if ("adminbrancodeEXL".equals(string)) {//excel输出，"所属监管所"转码查询处理
			sql.append("select distinct gssdm as value,gssmc as text from v_jg_ent order by gssdm ");
			list=dao.queryForList(sql.toString(),null);	
		}
		else if ("gongzuowanggeEXL".equals(string)) {//excel输出，"工作网络"转码查询处理
			sql.append("select distinct gzwgdm as value,gzwgmc as text from v_jg_ent order by gzwgdm ");
			list=dao.queryForList(sql.toString(),null);	
		}
	...
	/**
	 * 导出excel数据时，将所选的“所属监管所”和“工作网格”进行转码处理
	 * @param list
	 * @return List<Map>
	 * @throws OptimusException 
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public List<Map> transCoding(List<Map> list) throws OptimusException {
		//定义返回结果集
		List<Map> resultList = new ArrayList<Map>();
		//查询“所属监管所”的代码值
		List<Map<String, Object>> admCodeList = queryCode_value("adminbrancodeEXL");
		//查询“工作网络”的代码值
		List<Map<String, Object>> gzwgList = queryCode_value("gongzuowanggeEXL");
		
		Map map = null;
		for (int i=0; i<list.size(); i++) {
			map = list.get(i);
			//获取所属监管所转码后中文名字，例如：code=5000 转码后为 text=华强北所，最后将转码的text值写入到原来的集合中
			if (map.containsKey("adminbrancode") && map.get("adminbrancode") != null) {
				//所属监管所代码值 code
				String admcode = (String) map.get("adminbrancode");
				//进行转码处理，转码成功，就直接break该for循环，
				for (int n=0; n<admCodeList.size(); n++) {
					String value = (String) admCodeList.get(n).get("value");
					//if (value.equals(admcode)) {
					if (admcode.equals(value)) { ///防止为监管所为空的导出时报异常
						String adminbrancode = (String) admCodeList.get(n).get("text");
			...
			//获取工作网格转码后的中文名字，例如：code=5000 转码后为 text=华强北所，最后将转码的text值写入到原来的集合中
			if (map.containsKey("gongzuowangge") && map.get("gongzuowangge") != null) {
				//工作网格代码值 code
				String gzwg  = (String) map.get("gongzuowangge");
				for (int n=0; n<gzwgList.size();n++) {
					String value = (String) gzwgList.get(n).get("value");
					if (gzwg.equals(value)) {///防止为工作网格为空的导出时报异常
						String  gongzuowangge = (String) gzwgList.get(n).get("text");
						map.put("gongzuowangge", gongzuowangge);
						break;
					}
				}
			}
