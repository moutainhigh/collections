...
public class FoodLicController{
	...
	public void FoodLicQueryList(OptimusRequest req, OptimusResponse resp) throws OptimusException {
		...
		if(flag !=null){
			String count= foodLicSelectService.getSpQyListCount(form);//总条数
	...
	public void exportExcel(OptimusRequest req, OptimusResponse resp) throws OptimusException, IOException {
		...
		String ytsl =  req.getParameter("ytsl");
		String fztime_begin =  req.getParameter("fztime_begin");
		String fztime_end =  req.getParameter("fztime_end");
		
		List<Map> list = foodLicSelectService.exportExcel(entname,licno,supunitbysup,supdepbysup,newflag,lictype,street,ztyt,jyxm,ytsl,fztime_begin,fztime_end,req.getHttpRequest());
		...
		sortedMap.put("street", "街道名称");
		sortedMap.put("businessArea", "经营面积");
		...
		Workbook xs=new SXSSFWorkbook(1000);
		   ...
		   row.createCell((int)3).setCellValue("发证日期");
		   row.createCell((int)4).setCellValue("有效期限");
		   row.createCell((int)5).setCellValue("许可证类型");
		   row.createCell((int)6).setCellValue("管辖区域");
		   row.createCell((int)7).setCellValue("所属工商所");
		   row.createCell((int)8).setCellValue("主体业态");
		   row.createCell((int)9).setCellValue("经营项目");
		   row.createCell((int)10).setCellValue("地址");
		   row.createCell((int)11).setCellValue("街道名称");
		   row.createCell((int)12).setCellValue("经营面积【平方】");
