...
public class OtherSelectService extends BaseService{
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public List<Map> getSXList(Map<String, String> str, HttpServletRequest httpServletRequest,int pageIndex,int pageSize) throws OptimusException {
		
		IPersistenceDAO dao = getPersistenceDAO(DATASOURS_DC_DC);
		StringBuffer sql=new StringBuffer();
		
		
		//String cols = "  t.ID, t.INAME, t.SEX_NAME,t.AGE, t.CARDNUM, t.COURT_NAME, to_char(t.REG_DATE,'yyyy-MM-dd HH:mm:ss') as REG_DATE,t.S_EXT_SEQUENCE   ";
		String cols = "  t.id, t.iname, t.SEX_NAME as sexName,t.age, t.cardnum, t.COURT_NAME as courtName ,t.reg_date as regDate,t.s_ext_sequence  as sExtSequence  ";
		List<String> params = new ArrayList<String>();
		
		
		
		StringBuffer sbSql = new StringBuffer();
		sbSql.append(" select top "+pageSize + cols );
		sbSql.append(" from (select row_number() ");
		sbSql.append(" over(order by id asc) as rownumber,* ");
		
		
		
		if(StringUtils.isNotEmpty(str.get("iname"))){
			sql.append(" and t.iname like ?");
			params.add("%" + str.get("iname").trim() + "%");
			//params.add("%" + str.get("iname").trim() + "%");
		}
		
		if(StringUtils.isNotEmpty(str.get("cardnum"))){
			sql.append(" and t.cardnum = ?");
			params.add(str.get("cardnum").trim());			
		}
		
		
		sql.append(" and S_EXT_VALIDFLAG = '1' ");
		
		sbSql.append(" from LAOLAI   t where 1 = 1"+ sql.toString()  +") t ");
		sbSql.append(" where rownumber>(("+pageIndex+"-1)*"+pageSize+")");
		
		Object [] info = (Object[]) params.toArray();
		List list = jdbcTemplate2.queryForList(sbSql.toString(), info);
		LogUtil.insertLog("失信被执行人查询", sbSql.toString(), list.toString(), httpServletRequest, dao);
		return list;
	}
	...
	*/
	...
	public Map getSXQueryById(String sExtSequence) throws OptimusException {
		IPersistenceDAO dao = getPersistenceDAO(DATASOURS_DC_DC);
		String sql=" select t.iname, t.cardnum, t.sex_name as sexName, t.age, case t.party_type_name when '0' then '自然人' when '1' then '企业' else  '其他'end as partyTypeName, t.performance, t.disreput_type_name as disreputTypeName,t.CASE_CODE as caseCode, t.COURT_NAME as courtName, t.GIST_CID as gistCid, t.GIST_UNIT as gistUnit, t.buesinessentity, t.AREA_NAME as areaName,t.PERFORMED_PART as performedPart, t.UNPERFORM_PART as unperformPart, t.PUBLISH_DATE as publishDate, t.REG_DATE as regDate"
					+" from LAOLAI t where t.S_EXT_SEQUENCE = ?";
		
		
		
		
		List<String> params = new ArrayList<String>();
		params.add(sExtSequence);
		Object [] info = (Object[]) params.toArray();
	...
	public List<Map> queryHPList(Map<String, String> str, HttpServletRequest httpServletRequest,int pageIndex,int pageSize) throws OptimusException {
		IPersistenceDAO dao = getPersistenceDAO("dc_dc");
		List<String> params = new ArrayList<String>();
		StringBuffer sql= new StringBuffer();
		String cols = " pripid,\r\n" + 
		...
		sbSql.append("select top "+pageSize + cols );
		sbSql.append(" from (select row_number() ");
		sbSql.append("over(order by pripid asc) as rownumber,* ");
		sbSql.append("from ENTER_BLACK) t ");
		sbSql.append("where rownumber>(("+pageIndex+"-1)*"+pageSize+")");
		
		sql.append(sbSql.toString() + " and  t.S_EXT_VALIDFLAG = '1' ");
		if(StringUtils.isNotEmpty(str.get("entname"))){
			sql.append(" and t.entname like ?");
			params.add("%" + str.get("entname").trim() + "%");
		}
		if(StringUtils.isNotEmpty(str.get("regno"))){
			sql.append(" and t.regno = ?");
			params.add(str.get("regno").trim());			
		}
		if(StringUtils.isNotEmpty(str.get("enttype"))){
			sql.append(" and t.enttype = ?");
			params.add(str.get("enttype"));			
		}
		if(StringUtils.isNotEmpty(str.get("name"))){
			sql.append(" and t.name = ?");
			params.add(str.get("name").trim());			
		}
		if(StringUtils.isNotEmpty(str.get("certype"))){
			sql.append(" and t.certype = ?");
			params.add(str.get("certype"));			
		}
		if(StringUtils.isNotEmpty(str.get("cerno"))){
			sql.append(" and t.cerno = ?");
			params.add(str.get("cerno").trim());			
		}
		/*if(StringUtils.isNotEmpty(str.get("revdate_start"))){
			sql.append(" and t.revdate >= to_date(?,'yyyy-mm-dd')");
			params.add(str.get("revdate_start").trim());		
		}
		if(StringUtils.isNotEmpty(str.get("revdate_end"))){
			sql.append(" and t.revdate <= to_date(?,'yyyy-mm-dd')");
			params.add(str.get("revdate_end").trim());			
		}*/
		
		if(StringUtils.isNotEmpty(str.get("revdate_start"))){
			sql.append(" and t.revdate >= cast(? as datetime)");
			params.add(str.get("revdate_start").trim());		
		}
		if(StringUtils.isNotEmpty(str.get("revdate_end"))){
			sql.append(" and t.revdate <= cast(? as datetime)");
	...
	//	List<Map> pageQueryForList = dao.pageQueryForList(testsql.toString(), params);
		LogUtil.insertLog("黑牌企业查询", sql.toString(), params.toString(), httpServletRequest, dao);
	...
	public String queryHPListCount(Map<String, String> str) throws OptimusException {
		...
		List<String> params = new ArrayList<String>();
		StringBuffer sql= new StringBuffer();
		sql.append("select count(1) as count from ENTER_BLACK t where S_EXT_VALIDFLAG = '1' ");
		if(StringUtils.isNotEmpty(str.get("entname"))){
			sql.append(" and t.entname like ?");
			params.add("%" + str.get("entname").trim() + "%");
		}
	...
	/*	if(!"".equals(str.get("certype"))){
		...
		Object [] info = (Object[]) params.toArray();
		Integer counts  = jdbcTemplate2.queryForObject(sql.toString(), info,Integer.class);
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public List<Map> getYRList(Map map, HttpServletRequest httpServletRequest,int pageIndex,int pageSize) throws OptimusException{
		IPersistenceDAO dao = getPersistenceDAO(DATASOURS_DC_DC);
		StringBuffer sql=new StringBuffer();
		String cols = " pripid, entname , regno, name , cerno, inv cernoInv  ";
		StringBuffer sbSql = new StringBuffer();
		
		sql.append(" where  t.S_EXT_VALIDFLAG = '1' ");
		//sql.append("select top 100 "+ cols +" from ENTER_ONEPERSON t where t.s_ext_validflag = '1' ");
		List list=new ArrayList();
		if (map!=null) {
			Object object = map.get("entname");
			if (object!=null) {
		...
		sbSql.append(" from (select row_number() ");
		sbSql.append("over(order by pripid asc) as rownumber,* ");
		sbSql.append("from ENTER_ONEPERSON  t "+ sql.toString()  +") t ");
		sbSql.append("where rownumber>(("+pageIndex+"-1)*"+pageSize+")");
		
		Object [] info = (Object[]) list.toArray();
		List lists = jdbcTemplate2.queryForList(sbSql.toString(), info);
		LogUtil.insertLog("一人责任有限公司查询", sql.toString(), list.toString(), httpServletRequest, dao);
		return lists;
	}
	...
	public String getYRListCount(Map map) throws OptimusException{
		...
		StringBuffer sql=new StringBuffer();
		//sql.append("select * from DC_BL_ENTER_ONEPERSON t where t.s_ext_validflag = '1' ");
		sql.append("select count(1) as count from ENTER_ONEPERSON t where S_EXT_VALIDFLAG = '1' ");
		List list=new ArrayList();
		if (map!=null) {
			Object object = map.get("entname");
			if (object!=null) {
	...
	public String getSXListCount(Map<String, String> str) throws OptimusException {
		IPersistenceDAO dao = getPersistenceDAO("dc_dc");
		List<String> params = new ArrayList<String>();
		StringBuffer sql= new StringBuffer();
		sql.append("select count(1) as count from LAOLAI t where 1 = 1   ");
