...
@Service
public class YaoXieZZService extends BaseService{
	private static  Logger log=Logger.getLogger(YaoXieZZService.class);
	...
	//获取证照类型
	public List<Map<String, Object>> queryZhentZhaoType() throws OptimusException {
		IPersistenceDAO dao_dc=getPersistenceDAO("dc_dc");
		StringBuffer sql = new StringBuffer();
		
		sql.append(" select t.TYPE_NAME as text, t.TYPE_NAME as value from V_DC_YAOXIE_CODE t where 1=1 ");
		List list = null;
	...
	public List<Map> getYaoXieList(Map<String, String> map, HttpServletRequest httpRequest) throws OptimusException {
		...
		LogUtil.insertLog("药械证照查询", testsql, list.toString(), httpRequest, dao);
	...
	public List<Map>  getSpQyDetail(String pripid) throws OptimusException {
		...
		StringBuffer sql=new StringBuffer();
		List list=new ArrayList();
		sql.append("select * from V_DC_YAOXIE_ZZ where pripid=? ");
		list.add(pripid);
	...
	private void commonMethod(Map map, StringBuffer sql, List list) {
		if (map!=null) {
			...
			if ("0".equals(SpQymark)) {//全部
				sql.append(" select * from V_DC_YAOXIE_ZZ t where 1=1 ");
			}else if("1".equals(SpQymark)){//1是有效
				sql.append(" select * from V_DC_YAOXIE_ZZ t where t.flag = '0' ");
			}else if("2".equals(SpQymark)){//2是无效
				sql.append(" select * from V_DC_YAOXIE_ZZ t where t.flag <> '0' ");
			}
			...
			if (map.get("certno")!=null) {
				Object object2 = map.get("certno");
				if (object2.toString().trim().length()>0) {
					sql.append(" and t.certno = ?");
					list.add(object2.toString().trim());
				}
			}
			...
			if (map.get("zhenZhaoType")!=null) {
				Object object2 = map.get("zhenZhaoType");
				if (object2.toString().trim().length()>0) {
					sql.append(" and t.typename = ?");
					list.add(object2.toString().trim());
				}
			}
