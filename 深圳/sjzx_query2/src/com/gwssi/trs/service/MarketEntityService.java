...
/*			if(StringUtils.isNotBlank(sourceflag)){
		...
		/*	
			String regnio = (String) map.get("regnio");
			if(StringUtils.isNotBlank(pripid)){
				sql.append(" and regnio = ? ");
				list.add(regnio);
			}*/
...
/*			String sourceflag=(String)map.get("sourceflag");
	...
	public List<?> queryNZWZBianGenXx(Map<String, String> params) throws OptimusException {
		...
		if(params!=null&&params.size()>0){
			...
			String regino = (String)params.get("regnio");
			sql.append(" where  1=1" );
			if(StringUtils.isNotBlank(pripid)){
				sql.append(" and  priPid = ? ");
				list.add(pripid);
			...
			if(StringUtils.isNotBlank(regino)){
	...
	public List<?> nzwzBianGengDetailTitile(String priPid) throws OptimusException {
		IPersistenceDAO dao= this.getPersistenceDAO(getDetail_datasourcekey());
		String sql = "select to_char(t.altdate,'yyyy/MM/dd') as altdate,t.ALTTIME, wm_concat(t.ALTITEM) ALTITEMS,t.entname,t.REGINO from v_qy_nzwz_biangeng_xx t "
				+ "where t.PRIPID = ? group by t.ALTTIME,t.altdate,t.entname ,t.REGINO order by  t.altdate desc, t.ALTTIME desc";
		List<String> list = new ArrayList<String>();
		list.add(priPid);
	...
	public List<?> nzwzBianGengPersons(String priPid, String sign) throws OptimusException {
		IPersistenceDAO dao= this.getPersistenceDAO(getDetail_datasourcekey());
		StringBuffer sql = new StringBuffer();
		sql.append("select to_char(t.altdate,'yyyy/MM/dd') as altdate,t.ALTTIME, wm_concat(t.ALTITEM) ALTITEMS,t.entname,t.REGINO from v_qy_nzwz_biangeng_xx t where t.PRIPID = ?");
		List<String> list = new ArrayList<String>();
		list.add(priPid);
		if("1".equals(sign)){
		...
		sql.append("group by t.ALTTIME,t.altdate,t.entname ,t.REGINO order by t.ALTTIME desc");
	 ...
	 * @throws OptimusException 
	 */
	public void queryPhone(String flag, String SqlParams, OptimusRequest req,String userid) throws OptimusException{
		IPersistenceDAO dao= this.getPersistenceDAO("dc_dc");
	 ...
	 * @throws OptimusException 
	 */
	public String queryCerno(String flag, String SqlParams, OptimusRequest req,String userid) throws OptimusException{
		IPersistenceDAO dao= this.getPersistenceDAO("dc_dc");
		String msg = "";
		
		String querySql = "SELECT cx_cnt,xz_cnt FROM ZHCX_SFZ t where t.user_id = ?  ";
		String runSQL ="";
		List queryParams = new ArrayList();
		queryParams.add(userid);
		List<Map> ret = dao.queryForList(querySql, queryParams);
		List params = new ArrayList();
		if(ret!=null&&ret.size()==0) {
			params.clear();
			 runSQL = "INSERT into ZHCX_SFZ(user_id, cx_cnt, upatetime) VALUES (?, 1, SYSDATE)";
			 params.add(userid);
			int i = dao.execute(runSQL, params);
		}else {
			Map map = ret.get(0);
			params.clear();
			String xzCnt = (String) map.get("xzCnt").toString();
			String cxCnt = (String) map.get("cxCnt").toString();
			int limtTimes = Integer.valueOf(xzCnt);
			int queryTimes = Integer.valueOf(cxCnt);
			if(queryTimes<limtTimes) {
				params.add(queryTimes+1);
				params.add(userid);
				runSQL = "UPDATE ZHCX_SFZ SET CX_CNT = ?, UPATETIME = SYSDATE WHERE user_id =? ";
				int i = dao.execute(runSQL, params);
			}else {
				msg = "您已超过本周身份证查询次数，系统限制一周内最多查询" + limtTimes +"次";
			}
	...
	public List<Map> queryYCXX(Map<String, String> params) throws OptimusException{
		...
		if(params!=null&&params.size()>0){
			...
			if(StringUtils.isNotBlank(pripid)){
				...
				sql.append(" and not (t.type = '3' and t.removetype = '0')  order by t.createtime desc");
				list.add(pripid);
			}
		}else{
			return null;
	...
	public List<Map> queryYZWFSX(Map<String, String> params) throws OptimusException {
		...
		if (params != null && params.size() > 0) {
			...
			sql.append(" select * from view_dc_ms_dishonesty_dir t where t.id = ? ");
	 ...
	 * @throws OptimusException 
	 */
	public List<Map> queryCaseXX(String entname,String pripid,String bregno) throws OptimusException{
		IPersistenceDAO dao= this.getPersistenceDAO("dc_dc");												
		StringBuffer sql= new StringBuffer();
	    ...
	    sql.append("select *  from v_case_zhcx  where (1 = 2    ");
	    
	    if(!"".equals(entname)){
	    	sql.append(" or instr(LITIGANTNAME, ?) > 0 or  bname = ? ");
	    	list.add(entname);
	    	list.add(entname);
	    }
	    
	    if(StringUtils.isNotEmpty(pripid)){
	    	sql.append("  or bpripid  = ? ");
	    	list.add(pripid);
	    
	    }
	  
	    
	    if(StringUtils.isNotEmpty(bregno)){
	    	sql.append(" or bregno = ? ");
	    	list.add(bregno);
	    }
	...
	public List<Map> queryXZXX(String priPid) throws OptimusException{
		...
		//IPersistenceDAO dao= this.getPersistenceDAO("dc_dc");												
		StringBuffer sql= new StringBuffer();
		sql.append("SELECT t.*, s.LimitYYCode, s.LimitItemCode from tdXZSDInfo t LEFT JOIN tdXZSDDetail s on t.ID = s.InfoID where LimitState='1' and s.LimitYYCode <> '0017' ");
		//sql.append("select * from v_dc_gqzy_qyxz_xx t where t.xzlb = '1' ");  // xzlb 为状态 status
		List<String> list = new ArrayList<String>();
		if(!"".equals(priPid)){
		...
		if(StringUtil.isNotEmpty(priPid)){
			//sql = new StringBuffer("SELECT t.*, s.LimitYYCode, s.LimitItemCode from dc_ms_ba_entilimit t LEFT JOIN dc_ms_ba_tdxzsddetail s on t.ID = s.InfoID where LimitState='1' and s.LimitYYCode <> '0017' ");
			sql = new StringBuffer("select t.*, t.LimitYYCode, t.LimitItemCode  from v_dc_ms_ba_entilimit t ");
			sql.append(" where  t.entid = ? ");
			params.add(priPid);
	...
	public Object queryXZformXX(String priPid) throws OptimusException{
		...
		//IPersistenceDAO dao= this.getPersistenceDAO("dc_dc");												
		StringBuffer sql= new StringBuffer();
		sql.append("SELECT cast(u.cname as VARCHAR) as limituser, cast(b.cname as VARCHAR) as hzuser, c.deptname as hzunit, t.hztime, t.xzlb, cast(t.memo as VARCHAR(400)) as memo from tdXZSDInfo t LEFT JOIN CodeDB.dbo.tcDept c on t.HZUnit = c.code LEFT JOIN UserDB.dbo.tdUserInfo u on t.limituser = u.cuserid LEFT JOIN UserDB.dbo.tdUserInfo b on t.HZUser = b.cuserid where 1=1 ");
		//sql.append("select * from v_dc_gqzy_xzform_xx t where 1=1 ");
		List<String> list = new ArrayList<String>();
		if(!"".equals(priPid)){
			sql.append(" and t.id = ? ");
			//sql.append(" and t.id = ? ");
		...
		List<String> params = new ArrayList<String>();
		if(StringUtil.isNotEmpty(priPid)){
			sql = new StringBuffer("SELECT u.user_name AS limituser, b.user_name AS hzuser, c.deptname AS hzunit, t.APPRDATE, t.xzlb , t.REMARKS AS memo FROM dc_ms_ba_entilimit t LEFT JOIN DC_CODE.TCDEPT c ON trim(t.APPRDEP) = c.code LEFT JOIN ( SELECT replace(t.user_id, '@SZAIC', '') AS cuserid, t.user_name FROM db_yyjc.jc_public_people t ) u ON upper(t.OPERATOR) = upper(u.cuserid) LEFT JOIN ( SELECT replace(t.user_id, '@SZAIC', '') AS cuserid, t.user_name FROM db_yyjc.jc_public_people t ) b ON upper(t.APPROVER) = upper(b.cuserid) WHERE 1 = 1 ");
			sql.append(" and t.id = ?");
			params.add(priPid);
		...
		//IPersistenceDAO dao= this.getPersistenceDAO("dc_dc");												
		StringBuffer sql= new StringBuffer();
		//20180910之前的//sql.append("select * from v_dc_gqzy_xzform_xx t where 1=1 ");
		//20191028之前的//sql = new StringBuffer("SELECT u.user_name AS limituser, b.user_name AS hzuser, c.deptname AS hzunit, t.APPRDATE, t.xzlb , t.REMARKS AS memo FROM dc_ms_ba_entilimit t LEFT JOIN DC_CODE.TCDEPT c ON trim(t.APPRDEP) = c.code LEFT JOIN ( SELECT replace(t.user_id, '@SZAIC', '') AS cuserid, t.user_name FROM db_yyjc.jc_public_people t ) u ON upper(t.OPERATOR) = upper(u.cuserid) LEFT JOIN ( SELECT replace(t.user_id, '@SZAIC', '') AS cuserid, t.user_name FROM db_yyjc.jc_public_people t ) b ON upper(t.APPROVER) = upper(b.cuserid) WHERE 1 = 1 ");
		
		sql.append(" select * from V_DC_MS_BA_ENTILIMIT t where 1=1 ");
	...
	public List<Map> queryXZlistXX(String priPid) throws OptimusException{
		...
		//IPersistenceDAO dao= this.getPersistenceDAO("dc_dc");												
		StringBuffer sql= new StringBuffer();
		sql.append("SELECT t.limityycode, t.limititemcode, t.jcfs, t.limittimebegin, t.limittimeend, t.limitstate, cast(t.memo as VARCHAR(400)) as memo from tdXZSDDetail t where 1=1 ");
		//sql.append("select * from v_dc_gqzy_xzlist_xx t where 1=1 ");
		List<String> list = new ArrayList<String>();
		if(!"".equals(priPid)){
		...
		List<String> params = new ArrayList<String>();
		if(StringUtil.isNotEmpty(priPid)){
			//sql = new StringBuffer("SELECT t.limityycode, t.limititemcode, t.jcfs, t.limittimebegin, t.limittimeend, t.limitstate, cast(t.memo as VARCHAR(400)) as memo from dc_ms_ba_tdxzsddetail t where 1=1 ");
			sql = new StringBuffer("select * from v_dc_ms_ba_tdxzsddetail t ");
			sql.append(" where  t.infoid = ?");
			params.add(priPid);
			return typechage(dao.queryForList(sql.toString(), params));
		}else{
			return null;
	...
	/**https://www.baidu.com/s?wd=601600%20中国铝业复牌&rsf=4562&rsp=8&f=1&oq=中国铝业&ie=utf-8&usm=3&rsv_idx=1&rsv_pq=afa3ad7600005526&rsv_t=b2ee37chZYbSnlOqX6v572b8EUH35Ne5MZRf1M3gle1O21HFOBCI39Y5LeU&rqlang=cn&rs_src=0
	 * 查询股权质押限制信息
	 * @param priPid
	 * @return
	 * @throws OptimusException 
	 */
	public List<Map> queryGQZYXX(String priPid) throws OptimusException{
		//IPersistenceDAO dao= this.getPersistenceDAO("dc_sqlserver");												
		IPersistenceDAO dao= this.getPersistenceDAO("dc_dc");												
		StringBuffer sql= new StringBuffer();
	 ...
	 * @throws OptimusException 
	 */
	public Object queryGQZYformXX(String priPid) throws OptimusException{
		//IPersistenceDAO dao= this.getPersistenceDAO("dc_sqlserver");												
		IPersistenceDAO dao= this.getPersistenceDAO("dc_dc");												
		StringBuffer sql= new StringBuffer();
	 ...
	 * @throws OptimusException 
	 */
	public List<Map> queryGQZYlistXX(String priPid) throws OptimusException{
		//IPersistenceDAO dao= this.getPersistenceDAO("dc_sqlserver");												
		IPersistenceDAO dao= this.getPersistenceDAO("dc_dc");												
		StringBuffer sql= new StringBuffer();
