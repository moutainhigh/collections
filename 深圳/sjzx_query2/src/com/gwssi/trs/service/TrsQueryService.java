...
public class TrsQueryService extends BaseService {
	...
	public List<Map> querynzwzduibixx(String pripid, String beforeAfter,String altitemcode, String regino) throws OptimusException {
		...
		}else if("C7".equals(altitemcode)){
			//newSql.append("select distinct t.persname as inv ,t.cerno as cerno ,t1.subconam as subconam, func_getcode('C00026',t1.INVTYPE) as INVTYPE, func_getcode('C00001',t1.CERTYPE) as CERTYPE, t1.conprop as conprop  from dc_ra_mer_member_ext a left  join dc_ra_mer_persons_ext t  on t.id = a.id  left join  DC_RA_MER_INVEST_ext t1 on  t1.main_tb_id = a.main_tb_id where  a.main_tb_id = ?  and  t.regino = ?");
			newSql.append("select distinct t.persname as inv ,t.cerno as cerno ,t1.subconam as subconam, func_getcode('C00026',t1.INVTYPE) as INVTYPE, func_getcode('C00001',t1.CERTYPE) as CERTYPE, t1.conprop as conprop  from dc_ra_mer_member_ext a left  join dc_ra_mer_persons_ext t  on t.id = a.id  left join  DC_RA_MER_INVEST_ext t1 on    t1.id = a.id where  a.main_tb_id = ?  and  t.regino = ?");
			if("1".equals(beforeAfter)){
				//newSql.append(" and t1.bgtype='1' ");
				newSql.append(" and a.bgtype='1' ");
			}else if("2".equals(beforeAfter)){
				//newSql.append(" and t1.bgtype='2' ");
				newSql.append(" and a.bgtype='2' ");
			}
		}else{
			newSql.append(oldSql);
			if("1".equals(beforeAfter)){
	 ...
	 */
	public Object queryYZWFSX(String id) throws OptimusException {
		IPersistenceDAO dao = getPersistenceDAO(getDetail_datasourcekey());
		StringBuffer sql = new StringBuffer();
		List<String> params = new ArrayList<String>();
		if (StringUtils.isNotBlank(id)) {
			sql.append("select * from v_ent_ms_dishonesty_directory t where t.entid = ?");
			params.add(id);
