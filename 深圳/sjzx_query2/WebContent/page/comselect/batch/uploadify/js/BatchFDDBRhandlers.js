...
function uploadProgress(file, bytesLoaded, bytesTotal) {
	try {
		...
		progress.setStatus("正在将上传的文件解析并提取数据到数据库中查询，请稍后...");
	} catch (ex) {
		this.debug(ex);
	}
}
...
function uploadSuccess(file, serverData) {
	...
	/*	$.ajax({
			url:"/query/batch/FDDBRList.do?flag=1",
			dataType:"json",
			success:function(data){
				console.log(data);
				var counts = data.data[1];
				var fitcounts = data.data[2];
				$("#lists").css("display","block");
				$("#allItem").html(counts.data);
				$("#fitItem").html(fitcounts.data);
			}
			
		});
		
		//展示比对结果列表数据
		$("#batchFDDBRListGrid").gridpanel("option", "dataurl","/query/batch/FDDBRList.do?flag=1");
		$("#batchFDDBRListGrid").gridpanel("reload");*/
		
		
		$("#batchFDDBRListGrid").gridpanel("option", "dataurl","/query/batch/FDDBRList.do");
		$("#batchFDDBRListGrid").gridpanel("reload");
