...
function isNotEmpty(val) {
	...
	var excelFlag = <%=excelFlag%>;
	 ...
	 $(function() {
		$.ajax({
			 ...
			 success: function(data) {
				 ...
				 if(isNotEmpty(data.industryphy)){
					 _html += "<li>认缴注册资本结束总额(万元)："+data.industryphy+"</li>";
				 }
		...
		$("#gridpanel").gridpanel('option', 'dataurl',rootPath + "/comselectqh/toQuery.do");
		...
		if(!excelFlag){
		 ...
		 $.ajax({
			 type: "POST",
			 url:rootPath+"/comselectqh/toQuery.do?cntFlag=1",
			 data:{"mydata":jsonstr},
			 dataType:"json",
			 success: function(data) {
	...
	function viewSelectItem(excelFlag,count){
		var title="导出参数";
		var frameurl=""+'/page/comselect/excelExport_qianhai.html?data=' + jsonstr+'&excelFlag='+excelFlag+'&count='+count;
		var win = null;
		win = top.jazz.widget({
			vtype: 'window',
			name: 'topWindow',
	...
	function regDetail(id, enttype, opetype, entstatus, entid, entname, regno) {
		...
		var url = urlleft + "/" + urlright + "?flag=" + encode("0")
				...
				+ "&regno=" + encode(regno)+ "&bregno=" + encode(regno); 
		window.open(url);
