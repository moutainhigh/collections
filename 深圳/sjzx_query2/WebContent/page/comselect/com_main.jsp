...
.jazz-state-highlight, .jazz-widget-content .jazz-state-highlight{
...
//此方法暂时不用
function getNbYear2(id){
	//var str= "";
	$.ajax({
		   type: "POST",
		   url:rootPath+"/NBselect/code_value.do?type=ancheyear",
		   dataType: 'json',
		   success: function(data) {
		 		...
		 		for (var i = 0; i < data.length; i++) {
		 			...
		 			$("div[id='year']").comboxfield('addOption', text, vale);
		 		...
		 		/* var firstVak = data[0]["value"];
		 		$("#chooseYear").val(firstVak)
		 		yearArray.push(firstVak); */
		   },
...
function getNbYear(id){
	var date = new Date();
	var year  = date .getFullYear(); //获取完整的年份(4位)
	for (var i = 1; i <10; i++) {
		$("div[id='year']").comboxfield('addOption', year-i, year-i);
	}
}
...
function adddiv(thi){
	...
	$(thi).parents("th").prev().find("select").attr("disabled",false)
...
/*  var str=	JSON.stringify(getFormJson("#formID"));
 alert(str);
	...
	$.ajax({
		...
		dualtree();
 	    $("#formID").attr("action",rootPath+"/comselect/topange.do?estdate_start="+estdate_start+"&estdate_end="+estdate_end+"&apprdate_start="
 	    		+apprdate_start+"&apprdate_end="+apprdate_end);
  	    //$("#formID").submit(); 
  	   //$("#formID").attr("action",rootPath+"/comselect/topange.do");
  	  /*  var count = 0;
  	   var flags = $(".fontwidth input[type='checkbox']").each(function(){
  	  ...
  	  /* var tempSkyArray = unique1(yearArray);
  	  
  	  $("#chooseYear").val(tempSkyArray); */
...
<input type="hidden" id="coditions" name="sky">
					...
					<tr>
						<th>
						<div>
							<span style="display: inline-block;text-align: right;width: 54%">商事主体名称:</span>
							<select name="entname_logic" disabled="disabled" class="inputjuzhong" style="width:28%;float: right;">
						...
						<div>	
								...
								<input type="text" class="inputjuzhong" style="height:25px; line-height:25px;;width:58%" name="entname" value="" />
					...
					<tr>
						<th>
							<div><span class="domArea">地址:</span>
							<select name="dom_logic"  disabled="disabled"  class="inputjuzhong" style="width:28%;float: right;">
										<option selected="selected" value="and">并且</option>
										<option value="or">或者</option>
							</select>
							</div>
						...
						<th>
							<div>
								<select name="dom_term"  class="inputjuzhong" style="height:25px; line-height:25px;width:20%">
										<option selected="selected" value="include">包含</option>
								 	  	<option  value="notinclude">不包含</option>
								</select>
								<input type="text"  class="inputjuzhong" style="height:25px;line-height:25px; width:58%" name="dom" value="" />
								<input type="button"  class="inputjuzhong" value="增加" onclick= "adddiv(this);"/>
								<input type="button"  class="inputjuzhong" value="删除"  onclick= "deldiv(this);"/>
							</div>
						</th>
						...
						<th class="class_td4" >
							<div>
								<span style="display: inline-block;text-align: right;width: 50%;">经营范围:</span>
									<select  disabled="disabled"  name="opscope_logic" class="inputjuzhong" style=" width:28%;float: right;">
											<option selected="selected" value="and">并且</option>
											<option value="or">或者</option>
								</select>
							</div>
						...
						<th class="class_td5 thleft hangjianju" >
							<div>
								<select name="opscope_term"  class="inputjuzhong" style="height:25px;line-height:25px; width:20%">
										<option selected="selected" value="include">包含</option>
								 	  	<option  value="notinclude">不包含</option>
								</select>
								<input type="text"  class="inputjuzhong" style="height:25px;line-height:25px; width:53%" name="opscope" value="" />
								<input type="button"  class="inputjuzhong" value="增加" onclick= "adddiv(this);"/>
								<input type="button"  class="inputjuzhong" value="删除"  onclick= "deldiv(this);"/>
							</div>
						</th> 
...
<!-- 					<tr>	
					...
					<tr>
						...
						<th class="class_td5 thleft" colspan="3"  >
							<div class="hangjianju">
								...
								<label><font class="fontwidth" style="width:110px;"> <input name="regstate"  class="inputjuzhong" type="checkbox" checked="checked" value="1" /> 开业（存续）</font></label>
								<label><font class="fontwidth" style="width:80px;"> <input name="regstate"     class="inputjuzhong" type="checkbox" value="2" /> 吊销</font></label>
								<label><font class="fontwidth" style="width:80px;"> <input name="regstate"     class="inputjuzhong" type="checkbox" value="4" /> 注销</font></label>
								<label><font class="fontwidth" style="width:80px;"> <input name="regstate"     class="inputjuzhong" type="checkbox" value="6" /> 迁出</font></label>
								<label><font class="fontwidth" style="width:100px;"> <input name="regstate"     class="inputjuzhong" type="checkbox" value="8" /> 撤销登记</font></label>
								<label><font class="fontwidth" style="width:130px;"> <input name="regstate"     class="inputjuzhong" type="checkbox" checked="checked" value="7" /> 个体暂时吊销</font></label>
								<label><font class="fontwidth" style="width:80px;"> <input name="regstate"     class="inputjuzhong" type="checkbox" value="11" checked="checked"  /> 停业</font></label>
								<label><font class="fontwidth" style="width:80px;"> <input name="regstate"      class="inputjuzhong" type="checkbox" value="9" /> 其他</font></label>
					...
					<th colspan='3' style='text-align: left;'>
						<label>
							<input type="radio"  class="inputjuzhong"  name="yichangminglu" value="2"  checked="true"/> <font  color="red">全部</font> &nbsp;&nbsp;
						</label>
						<label>
							<input type="radio"  class="inputjuzhong"  name="yichangminglu" value="1"  /> <font  color="#666">是</font> &nbsp;&nbsp;
						</label>
						<label> 
							<input type="radio"  class="inputjuzhong" name="yichangminglu" value="0" /> <font   color="#666">否</font>
						 </label>
					</th>
					</tr>
						
				  <tr>
						<th class="class_td4">年报标识：</th>
						<th class="class_td5 thleft" colspan="3">
							<div>
								<div style="float: left; height: 28px; line-height: 28px;">
									<label> <input
										type="radio" checked="true" class="inputjuzhong" name="nbflag"
										value="全部" /> <font color="#666">全部</font>
									</label>&nbsp;&nbsp;
								</div>
							</div>
							<div
								style="margin-left: 10px; float: left; height: 28px; line-height: 28px;">
								<label> <input type="radio" class="inputjuzhong"
									name="nbflag" value="1" /> <font color="#666">已年报</font>
								</label>&nbsp;&nbsp; <label> <input type="radio"
									class="inputjuzhong" name="nbflag" value="0" /> <font
									color="#666">未年报</font>
								</label>
							</div>
			
							<div
								style="float: left; height: 28px; line-height: 28px; margin-left:20px;color:rgb(102, 102, 102)">
								年报年度:&nbsp;&nbsp;
								<!--  <select  id="year"  style="height:25px; width:80px" disabled="disabled" multiple="multiple">
										 <option value=""> </option>
										 </select> -->
								<div id='year' name="year" style='margin-top: 10px;margin-left:-10px;' height="25px"
									vtype="comboxfield" multiple="ture" label="" labelwidth="120"
									labelalign="left" width="160" valuetip="请选择年报年度……" tooltip=""
									rule=""></div>
							</div> <!-- <input  name="year" type="hidden" id="chooseYear"> -->
							<div
								style="margin-left: -10px; float: left; height: 28px; line-height: 28px; color: #666">（注：蓝色背景为选中的年报年度）</div>
...
.selectYear{
background: #185AAE;color:#fff}

</style>
<script>
	$(function(){
		$("div[name='year']").comboxfield("option","disabled",true);
		
		
		$("input[name='nbflag']").click(function(){
			var _this = $(this);
			var txt = _this.val();
			
			 if(txt=="全部"){
				$/* ("#year").attr("disabled","disabled");
				$("#year").prepend("<option value=''> </option>"); //为Select插入一个Option(第一个位置)
				$("#year option[value='']").attr("selected", true); */
				
				$("div[name='year']").comboxfield("option","disabled",true);
				$("div[name='year']").comboxfield("reset");
				
			}else{
				/* $("#year").removeAttr("disabled");
				$("#year option[value='']").remove();  */
				$("div[name='year']").comboxfield("option","disabled",false);
				
				
				var date = new Date();
				var year  = date .getFullYear(); //获取完整的年份(4位)
				$("div[name='year']").comboxfield('option', 'defaultvalue',year-1);
			} 
		...
		/* $("#year option").click(function(){
			 ...
			 var flag = $("#year option[value='"+yearStr+"']").hasClass("selectYear");
			 
			 if(!flag){
				 $("#year option[value='"+yearStr+"']").addClass("selectYear");
				 
				 
				 yearArray.push(yearStr);
				 
				 $("#chooseYear").val(yearArray);
			 }else{
				 
				 $("#year option[value='"+yearStr+"']").removeClass("selectYear");
				 yearArray.remove(yearStr);
