	...
	$(function(){
		var entityNo=getUrlParam("entityNo");
  		...
  		if(entityNo!=null){
 			...
 			queryHistory(entityNo,priPid,sourceflag,type);
  		}
 	}); 
 	
 	function queryUrl() {
		$("#jbxxGrid1").gridpanel("hideColumn", "approvedate");
			$("#jbxxGrid1").gridpanel("showColumn", "modfydate");
		$('#jbxxGrid1').gridpanel('option', 'dataurl',rootPath+
				'/datacompre/queryZzjgDm.do');
		$('#jbxxGrid1').gridpanel('query', [ 'formpanel']);
		
			$("#jbxxGrid2").gridpanel("hideColumn", "approvedate");
			$("#jbxxGrid2").gridpanel("showColumn", "modfydate");
		$('#jbxxGrid2').gridpanel('option', 'dataurl',rootPath+
				'/datacompre/queryZzjgDm.do');
		$('#jbxxGrid2').gridpanel('query', [ 'formpanel']);
	}
	
 	function queryHistory(entityNo,priPid,sourceflag,type,economicproperty){
		$('#jbxxGrid1').gridpanel('option', 'dataurl',rootPath+
				'/reg/detail.do?flag='+entityNo+'&priPid='+priPid+'&sourceflag='+sourceflag+'&type='+type);
		$('#jbxxGrid1').gridpanel('query', [ 'formpanel']);
		
		$('#jbxxGrid2').gridpanel('option', 'dataurl',rootPath+
				'/reg/detail.do?flag='+(parseInt(entityNo)+1)+'&priPid='+priPid+'&sourceflag='+sourceflag+'&type='+type);
		$('#jbxxGrid2').gridpanel('query', [ 'formpanel']);
	}
