$(function() {
	...
	if (entityNo != null) {
		...
		queryHistory(entityNo, priPid, economicproperty, flag);
	}
});
function queryUrl() {
...
function queryHistory(entityNo, priPid, sourceflag, flag, economicproperty) {
	//alert(entityNo);
	$('#jbxxGrid').gridpanel(
			'option',
			'dataurl',
			rootPath + '/reg/detailTitle.do?flag=' + entityNo + '&priPid=' + priPid
					+ '&sourceflag=' + sourceflag + '&sign=' + flag);
	$('#jbxxGrid').gridpanel('query', [ 'formpanel' ]);
}
