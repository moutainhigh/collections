$(function() {
	...
	var economicproperty = getUrlParam("economicproperty");// 判断是什么企业 个体1 内资2
	// 外资3 集团 4
	...
	if (entityNo != null) {
		...
		// alert(entityNo);
		// alert(priPid);
		// queryHistory(entityNo,priPid,sourceflag);
		queryHistory(entityNo, priPid, economicproperty,alttime,sign,regnio);
	}
});
function queryUrl() {
...
function queryHistory(entityNo, priPid, sourceflag,alttime,sign,regnio) {
	//alert(entityNo);
	$('#jbxxGrid').gridpanel(
			'option',
			'dataurl',
			rootPath + '/reg/detail.do?flag=' + entityNo + '&priPid=' + priPid
					+ '&sourceflag=' + sourceflag+'&alttime='+alttime+'&sign='+sign+"&regnio="+regnio);
	...
	if(url.indexOf("nzwz_biangengxx_two")!=-1){
		$(".jazz-panel-title-font").html(entname+"变更信息");
	}
}
