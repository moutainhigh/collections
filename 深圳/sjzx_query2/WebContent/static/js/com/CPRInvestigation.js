$(function() {
	...
	var infowareid = getUrlParam("infowareid"); // 当前系统主键
	// 获取传递过来的参数，进行初始化请求
	if (entityNo != null) {
		$("#CPRInvestigation").gridpanel("hideColumn", "modfydate");
		$("#CPRInvestigation").gridpanel("showColumn", "approvedate");
		queryHistory(entityNo, infowareid);
	} 
});
function queryUrl() {
	$('#CPRInvestigation').gridpanel('option', 'dataurl',
			rootPath + '/datacompre/queryZzjgDm.do');
	$('#CPRInvestigation').gridpanel('query', [ 'formpanel' ]);
}

function queryHistory(entityNo, infowareid) {
	$('#CPRInvestigation').gridpanel(
			'option',
			'dataurl',
			rootPath + '/CPRShow/CPRInvestigationQueryList.do?flag=' + entityNo + '&infowareid=' + infowareid);
	$('#CPRInvestigation').gridpanel('query', [ 'formpanel' ]);
}

function reset() {
	$("#CPRInvestigation").formpanel('reset');
}
