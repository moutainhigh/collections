...
$(function(){
	if(type==null||type==""){
		...
		$("#maincontent").panel("option", "frameurl",rootPath+'/auth/getViewAddSmRoleHtml.do?systemCode='+systemCode+'&roleCode='+roleCode);
// 		setEnable(pkRole,pkSysIntegration,type);
		setPosiEnable(roleCode,pkSysIntegration,type);
...
function setPosiEnable(roleCode,pksys,type){
	$('#tab_name').tabpanel('enable','1');//设置角色权限分配可用
	$("#maincontent3").panel("option", "frameurl",rootPath+'/auth/getRolePosiHtml.do?roleCode='+roleCode);
	$('#tab_name').tabpanel('enable','2');//设置角色岗位映射可用
}
function closewin(){
...
<body>
	<div name="tab_name" vtype="tabpanel"	showborder="false" overflowtype="2" tabtitlewidth="130"  width="100%" height="100%" orientation="top" id="tab_name">    
	    <ul>    
	        ...
	        <li style="display:none;"><a href="#tab1">角色权限分配</a></li> 
	    ...
	    <div>
	    	...
	    	<div style="width:100%;height:100%;">
		        <div vtype="panel"  width="100%" showtabclose="false" height="100%" titledisplay="false" title="角色岗位映射" name="maincontent3" id="maincontent3" showborder="false">
		   		</div>  	   			   		 
	    	</div>    
	    </div>    
	</div>
