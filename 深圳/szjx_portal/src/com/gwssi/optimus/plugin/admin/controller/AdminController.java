...
public class AdminController extends BaseController {
	...
	public void login(OptimusRequest req, OptimusResponse resp)
		...
		user.setIp(iReceiveHr.getIP(req.getHttpRequest()));
/*		// 根据当前登录的岗位信息查出角色集合
		List<Map> roleList = authService.getRoleIdByPost(
				user.getPostList());
		logger.info("roleList is " + roleList);
		user.setRoleList(roleList);*/
		List<SmSysIntegrationBO> usersyslist = user.getUserSysList();
	//	System.out.println("所具有的所有系统：-----" +usersyslist.size());
		System.out.println("所具有的所有系统：-----" );
		System.out.println("是否超级管理员："+user.getIsSuperAdmin());
		if(usersyslist!=null&&usersyslist.size()>0){
			for(SmSysIntegrationBO bo:usersyslist){
				System.out.println("系统名:"+ bo.getSystemName()+"系统url：" +bo.getIntegratedUrl());
			}
		}

		List<SmSysIntegrationBO> syslist = user.getAdminSysList();
		System.out.println("管理员列表----" );
		if(syslist!=null&&syslist.size()>0){
			for(SmSysIntegrationBO bo:syslist){
				System.out.println("系统名:"+ bo.getSystemName()+"系统url：" +bo.getIntegratedUrl());
			}
		}
		// 将当前登录人的信息存入缓存中
