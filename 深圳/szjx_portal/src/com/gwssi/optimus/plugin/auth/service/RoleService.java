...
import com.gwssi.application.model.JcRoleBO;
...
public class RoleService extends BaseService {
	 ...
	 * @throws OptimusException
	 */
	public List findRoleTree(String systemCode, boolean isSuperAdmin)
			throws OptimusException {
		IPersistenceDAO dao = getPersistenceDAO();
		StringBuffer sql = new StringBuffer();
//		sql.append(
...
//				.append(" and EFFECTIVE_MARKER = ").append("?").append("");
		sql.append("select  r.ROLE_CODE AS ID, r.ROLE_NAME || \'(\' || r.ROLE_CODE || \')\' AS NAME, \'0\' AS pid from SM_SYS_INTEGRATION  i LEFT JOIN JC_ROLE r ON i.SYSTEM_CODE = r.APP_CODE WHERE SYSTEM_CODE =? AND EFFECTIVE_MARKER = ?");
...
//		}
	...
	public List<JcRoleBO> findJcRoleByPKBO(String pkRole) throws OptimusException {
		if (StringUtils.isEmpty(pkRole))
			return null;
		List<String> paramList = new ArrayList<String>();
		IPersistenceDAO dao = getPersistenceDAO();
		StringBuffer sql = new StringBuffer();
		sql.append("select * from JC_ROLE where ROLE_CODE=?");
		paramList.add(pkRole);
		List<JcRoleBO> list =dao.queryForList(JcRoleBO.class, sql.toString(), paramList);
		return list;
	}
	...
	public List<Map> queryJcRoleByAppCode(String appCode,String roleCode) throws OptimusException{
		IPersistenceDAO dao = getPersistenceDAO();
		List<String> paramList = new ArrayList<String>();
		paramList.add(appCode);
		paramList.add(roleCode);
		String str = "select r.ROLE_CODE as roleCode ,r.ROLE_NAME as roleName,r.APP_CODE as appCode,r.TIMESTAMP as timeStamp,i.SYSTEM_NAME as systemName,i.INTEGRATED_URL as  url from jc_role r left join SM_SYS_INTEGRATION i ON r.APP_CODE = i.SYSTEM_CODE where r.APP_CODE = ? and r.role_code=?  and r.FLAG =1";
		List<Map> resultList =dao.queryForList(str, paramList);
		return resultList;
	}
	public List<Map> queryJcRoleByRoleCode(String roleCode) throws OptimusException{
		IPersistenceDAO dao = getPersistenceDAO();
		List<String> paramList = new ArrayList<String>();
		paramList.add(roleCode);
		String str = "select r.ROLE_NAME,p.POST_NAME from JC_ROLE r INNER JOIN jc_post_role pr ON pr.ROLE_CODE = r.ROLE_CODE  INNER JOIN JC_POST p ON p.post_code = pr.post_code where  r.ROLE_CODE = ?";
		List<Map> resultList =dao.queryForList(str, paramList);
		return resultList;
	}
