...
import com.gwssi.application.webservice.service.SysListService;
...
/*		for(int i=0;i<roleList.size();i++){
			...
			if (AppConstants.ROLE_TYPE_SUPER.equals(roleType)) {
				超级管理员
				user.setSuperAdmin(true);
				System.out.println("超级管理员："+roleList.get(i).get("roleCode"));
				supersys=(String) roleList.get(i).get("pkSysIntegration");
				...
				System.out.println("子系统管理员："+roleList.get(i).get("roleCode"));
				子系统管理员
	...
	private  void setUserfunc(User user){
		...
		try {
				...
				}else if(isCurradmin||(user.getAdminSysList()!=null&&user.getAdminSysList().size()>0)){
