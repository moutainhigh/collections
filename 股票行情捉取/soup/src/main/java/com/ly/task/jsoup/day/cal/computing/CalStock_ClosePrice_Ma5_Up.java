...
@Component
public class CalStock_ClosePrice_Ma5_Up {
	...
	public void getStockInfo() {
		...
		List<Stock_ClosePrice_Ma5_Comp> entitys =  null;
		stock_Shou_Yang_One_DayDao.truncateTable(Stock_ClosePrice_Ma5_Comp.class);
		Stock_ClosePrice_Ma5_Comp stock_Shou_Yang_Day_One_Day = null;
		...
		for (int i = 1; i <= step; i++) {
			entitys = new ArrayList<Stock_ClosePrice_Ma5_Comp>(); 
			...
			for (int j = 0; j < list.size(); j++) {
				...
				if(datas!=null&&datas.size()>=3){
					...
					//double isDaYang = (day1Close - day2Close)/day2Close*100;
					double ma5Ma10FuDu  = (day1Ma5 - day1Ma10)/day1Ma10*100;
					
					calday.info(day1IsZhang + " =====  "  + " ===>" + sts.getCode() + "  ==> "+ sts.getName()+" ，涨跌幅度：" + rates);
					
					if(zhangFu>3&&rates>2&&day1IsZhang>0&&ma5Ma10FuDu>1.2){
						
						stock_Shou_Yang_Day_One_Day = new Stock_ClosePrice_Ma5_Comp();
		...
		sendStockDayInfoEmail.taskOneDayEail();
