...
@Component
public class CalStock_Close_Price_Big_Ma30_Up {
	...
	@Autowired
	private Stock_Close_Price_Big_Ma30Dao stock_Shou_Yang_One_DayDao;
	...
	//
	//@Scheduled(cron="*/10 * * * * ?")
	//@Scheduled(cron="30 32 12 ? * *")
	public void getStockInfo() {
		long startTime = System.currentTimeMillis();    //获取开始时间
		List<Stock_Close_Price_Big_Ma30> entitys =  null;
		stock_Shou_Yang_One_DayDao.truncateTable(Stock_Close_Price_Big_Ma30.class);
		Stock_Close_Price_Big_Ma30 stock_Shou_Yang_Day_One_Day = null;
		...
		for (int i = 1; i <= step; i++) {
			entitys = new ArrayList<Stock_Close_Price_Big_Ma30>(); 
			...
			for (int j = 0; j < list.size(); j++) {
				...
				if(datas!=null&&datas.size()>=3){
					...
					double openIsBigMa20 =  day1Open - day1Ma20;
					
					double ma5IsBigMa30 = (day1Ma5 - day1Ma30)/day1Ma30*100;
					double ma5IsBigMa10 = (day1Ma5 - day1Ma30)/day1Ma30*100;
					
					calday.info(day1IsZhang + " =====  "  + " ===>" + sts.getCode() + "  ==> "+ sts.getName()+" ，涨跌幅度：" + rates);
					
					//if(rates>1.5&&currentIsBigMa30>0&&openIsBigMa30>0&&openIsBigMa20>0){
					if(rates>=2.2&&currentIsBigMa30>0&&openIsBigMa30>0&&openIsBigMa20>0&&ma5IsBigMa30>=1&&ma5IsBigMa10>=1.2){
						
						stock_Shou_Yang_Day_One_Day = new Stock_Close_Price_Big_Ma30();
	...
	//	sendStockDayInfoEmail.taskMaEail();
